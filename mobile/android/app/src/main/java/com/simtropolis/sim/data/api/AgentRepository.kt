...
private val Context.agentStore: DataStore<Preferences> by preferencesDataStore(name = "sim_agents")
...
class AgentRepository(private val context: Context) {
    ...
    suspend fun switchToAgent(agent: AgentConfiguration) {
        ...
        // Apply to UserDefaults (the settings that SimApiService reads)
        with(com.simtropolis.sim.SimApplication.instance.settingsRepository) {
    ...
    suspend fun ensureCurrentAgentInList() {
        val repo = com.simtropolis.sim.SimApplication.instance.settingsRepository
