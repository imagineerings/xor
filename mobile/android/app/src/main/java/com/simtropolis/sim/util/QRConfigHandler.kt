package com.simtropolis.sim.util
 ...
 *
 * Expected format:
 *   simchat://configure?data=<url-encoded-json>
...
object QRConfigHandler {
    ...
    fun handleDeepLink(uri: Uri, onResult: ((Boolean, String?) -> Unit)? = null) {
        // Expected: simchat://configure?data=<encoded>
        ...
        val repo = SimApplication.instance.settingsRepository
        scope.launch {
            ...
            if (connected is com.simtropolis.sim.data.api.ApiResult.Success && connected.data) {
                configurationSuccess.value = true
                NoticeManager.showConfigurationSuccess()
                onResult?.invoke(true, null)
            } else {
                val errorMsg = when (connected) {
                    is com.simtropolis.sim.data.api.ApiResult.Error -> connected.message
