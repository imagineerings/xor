...
struct AgentConfiguration: Identifiable, Codable, Equatable {
    ...
    static func defaultName(for url: String) -> String? {
        ...
        // Check for demo-simed pattern
        if lowercaseURL.contains("demo-simed") {
            return "Trial"
        }
...
class AgentStorage: ObservableObject {
    ...
    func ensureCurrentAgentInList() {
        let currentURL = UserDefaults.standard.string(forKey: "sim_base_url") ?? ""
        let currentSecret = UserDefaults.standard.string(forKey: "sim_secret_key") ?? ""
    ...
    func switchToAgent(_ agent: AgentConfiguration) {
        ...
        UserDefaults.standard.set(agent.url, forKey: "sim_base_url")
        UserDefaults.standard.set(agent.secret, forKey: "sim_secret_key")
        UserDefaults.standard.synchronize()
    ...
    func getAgentFromCurrentDefaults() -> AgentConfiguration? {
        let url = UserDefaults.standard.string(forKey: "sim_base_url") ?? ""
        let secret = UserDefaults.standard.string(forKey: "sim_secret_key") ?? ""
...
class ConfigurationHandler: ObservableObject {
    ...
    /// Expected format: simchat://configure?data=<url-encoded-json>
    func handleURL(_ url: URL) -> Bool {
        guard url.scheme == "simchat" else {
    ...
    private func applyConfiguration(_ config: ConfigurationData) {
        ...
        UserDefaults.standard.set(baseURL, forKey: "sim_base_url")
        UserDefaults.standard.set(config.secret, forKey: "sim_secret_key")
        UserDefaults.standard.synchronize()
        
        // Test the connection
        Task {
            let success = await SimAPIService.shared.testConnection()
            ...
            await MainActor.run {
                ...
                if success {
                    ...
                    let originalError = SimAPIService.shared.connectionError ?? "Connection test failed"
    ...
    func switchToAgent(_ agent: AgentConfiguration) {
        ...
        // Test the connection
        Task {
            _ = await SimAPIService.shared.testConnection()
        }
    }
}
