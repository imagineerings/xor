...
class SimAPIService: ObservableObject {
    static let shared = SimAPIService()
    ...
    private var baseURL: String {
        UserDefaults.standard.string(forKey: "sim_base_url") ?? "http://127.0.0.1:62996"
    }

    private var secretKey: String {
        UserDefaults.standard.string(forKey: "sim_secret_key") ?? "test"
    }

    // Trial mode detection
    var isTrialMode: Bool {
        return baseURL.contains("demo-simed.fly.dev")
    }
    ...
    func startAgent(workingDir: String? = nil) async throws -> (
        ...
        do {
            ...
            // For remote simed: use "." (current directory where simed is running)
