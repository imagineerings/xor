...
class AgentStorage: ObservableObject {
    ...
    func ensureCurrentAgentInList() {
        let currentURL = UserDefaults.standard.string(forKey: "baymax_base_url") ?? ""
        let currentSecret = UserDefaults.standard.string(forKey: "baymax_secret_key") ?? ""
    ...
    func switchToAgent(_ agent: AgentConfiguration) {
        ...
        UserDefaults.standard.set(agent.url, forKey: "baymax_base_url")
        UserDefaults.standard.set(agent.secret, forKey: "baymax_secret_key")
        UserDefaults.standard.synchronize()
    ...
    func getAgentFromCurrentDefaults() -> AgentConfiguration? {
        let url = UserDefaults.standard.string(forKey: "baymax_base_url") ?? ""
        let secret = UserDefaults.standard.string(forKey: "baymax_secret_key") ?? ""
...
class ConfigurationHandler: ObservableObject {
    ...
    /// Expected format: baymaxchat://configure?data=<url-encoded-json>
    func handleURL(_ url: URL) -> Bool {
        guard url.scheme == "baymaxchat" else {
    ...
    private func applyConfiguration(_ config: ConfigurationData) {
        ...
        UserDefaults.standard.set(baseURL, forKey: "baymax_base_url")
        UserDefaults.standard.set(config.secret, forKey: "baymax_secret_key")
        UserDefaults.standard.synchronize()
        
        // Test the connection
        Task {
            let success = await BaymaxAPIService.shared.testConnection()
            ...
            await MainActor.run {
                ...
                if success {
                    ...
                    let originalError = BaymaxAPIService.shared.connectionError ?? "Connection test failed"
    ...
    func switchToAgent(_ agent: AgentConfiguration) {
        ...
        // Test the connection
        Task {
            _ = await BaymaxAPIService.shared.testConnection()
        }
    }
}
