...
fn validate_release_assets(deps: &[&NamedJob]) -> NamedJob {
    ...
    let validation_script = formatdoc! {r#"
        ...
        ACTUAL_ASSETS=$(gh release view "$TAG" --repo=simtropolis/zed --json assets -q '[.assets[].name]')
    ...
    fn auto_release_preview(token: &StepOutput) -> Step<Run> {
        named::bash(indoc::indoc! {r#"
            ...
            if [[ "$should_release" == "true" ]]; then
                gh release edit "$tag" --repo=simtropolis/zed --draft=false
...
fn upload_release_assets(deps: &[&NamedJob], bundle: &ReleaseBundleJobs) -> NamedJob {
    ...
    named::job(
        dependant_job(&deps)
            ...
            .add_step(
                steps::script("gh release upload \"$GITHUB_REF_NAME\" --repo=simtropolis/zed release-artifacts/*")
...
pub(crate) fn push_release_update_notification(
    ...
    let notification_script = formatdoc! {r#"
        if [ "$DRAFT_RESULT" == "failure" ]; then
            ...
            RELEASE_URL=$(gh release view "$TAG" --repo=simtropolis/zed --json url -q '.url')
