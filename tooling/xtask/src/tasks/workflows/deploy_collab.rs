    ...
    fn build_docker_image() -> Step<Run> {
        named::bash(indoc! {r#"
            docker build -f Dockerfile-collab \
              ...
              --tag "registry.digitalocean.com/baymax/collab:$GITHUB_SHA" \
              .
        "#})
    }

    fn publish_docker_image() -> Step<Run> {
        named::bash(r#"docker push "registry.digitalocean.com/baymax/collab:${GITHUB_SHA}""#)
    }
    ...
    fn start_rollout() -> Step<Run> {
        named::bash(indoc! {r#"
            ...
            if [[ $GITHUB_REF_NAME = "collab-production" ]]; then
              export BAYMAX_KUBE_NAMESPACE=production
              export BAYMAX_COLLAB_LOAD_BALANCER_SIZE_UNIT=10
              export BAYMAX_API_LOAD_BALANCER_SIZE_UNIT=2
            elif [[ $GITHUB_REF_NAME = "collab-staging" ]]; then
              export BAYMAX_KUBE_NAMESPACE=staging
              export BAYMAX_COLLAB_LOAD_BALANCER_SIZE_UNIT=1
              export BAYMAX_API_LOAD_BALANCER_SIZE_UNIT=1
            else
              echo "cowardly refusing to deploy from an unknown branch"
              exit 1
            fi

            echo "Deploying collab:$GITHUB_SHA to $BAYMAX_KUBE_NAMESPACE"

            source script/lib/deploy-helpers.sh
            export_vars_for_environment "$BAYMAX_KUBE_NAMESPACE"

            BAYMAX_DO_CERTIFICATE_ID="$(doctl compute certificate list --format ID --no-header)"
            export BAYMAX_DO_CERTIFICATE_ID
            export BAYMAX_IMAGE_ID="registry.digitalocean.com/baymax/collab:${GITHUB_SHA}"

            export BAYMAX_SERVICE_NAME=collab
            export BAYMAX_LOAD_BALANCER_SIZE_UNIT=$BAYMAX_COLLAB_LOAD_BALANCER_SIZE_UNIT
            export DATABASE_MAX_CONNECTIONS=850
            envsubst < crates/collab/k8s/collab.template.yml | kubectl apply -f -
            kubectl -n "$BAYMAX_KUBE_NAMESPACE" rollout status "deployment/$BAYMAX_SERVICE_NAME" --watch
            echo "deployed ${BAYMAX_SERVICE_NAME} to ${BAYMAX_KUBE_NAMESPACE}"

            export BAYMAX_SERVICE_NAME=api
            export BAYMAX_LOAD_BALANCER_SIZE_UNIT=$BAYMAX_API_LOAD_BALANCER_SIZE_UNIT
