...
pub fn bump_baymax_version() -> Workflow {
...
fn resolve_versions() -> (steps::NamedJob, ResolvedOutputs) {
    fn extract_versions() -> Step<Run> {
        named::bash(indoc::indoc! {r#"
            version=$(script/get-crate-version baymax)
            major=$(echo "$version" | cut -d. -f1)
            minor=$(echo "$version" | cut -d. -f2)

            channel=$(cat crates/baymax/RELEASE_CHANNEL)
            ...
            pr_branch="bump-baymax-to-v${next_major}.${next_minor}.0"
    ...
    let job = named::job(
        Job::default()
            .cond(Expression::new(
                "github.repository_owner == 'simtropolis'",
            ))
...
fn bump_main(
    ...
    fn bump_version() -> Step<Run> {
        named::bash("cargo set-version -p baymax --bump minor")
    }
    ...
    named::job(
        Job::default()
            ...
            .add_step(steps::CreatePrStep::new(
                format!("Bump Baymax to v{}", outputs.next_version),
                &outputs.pr_branch,
                &token,
            )),
    )
...
fn create_preview_branch(
    ...
    fn promote_to_preview() -> Step<Run> {
        named::bash("echo -n preview > crates/baymax/RELEASE_CHANNEL")
    }
    ...
    .with_files("crates/baymax/RELEASE_CHANNEL")
    .into();
...
fn promote_to_stable(
    ...
    let read_version_step = named::bash(indoc::indoc! {r#"
            stable_version=$(script/get-crate-version baymax)
            {
    ...
    let write_channel = named::bash("echo -n stable > crates/baymax/RELEASE_CHANNEL");
