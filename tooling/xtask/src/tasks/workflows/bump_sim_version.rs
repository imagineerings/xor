...
pub fn bump_sim_version() -> Workflow {
...
fn resolve_versions() -> (steps::NamedJob, ResolvedOutputs) {
    fn extract_versions() -> Step<Run> {
        named::bash(indoc::indoc! {r#"
            ...
            channel=$(cat crates/sim/RELEASE_CHANNEL)
            ...
            pr_branch="bump-sim-to-v${next_major}.${next_minor}.0"
...
fn bump_main(
    ...
    fn bump_version() -> Step<Run> {
        named::bash("cargo set-version -p sim --bump minor")
    }
    ...
    named::job(
        Job::default()
            ...
            .add_step(steps::CreatePrStep::new(
                format!("Bump Sim to v{}", outputs.next_version),
                &outputs.pr_branch,
                &token,
            )),
    )
...
fn create_preview_branch(
    ...
    fn promote_to_preview() -> Step<Run> {
        named::bash("echo -n preview > crates/sim/RELEASE_CHANNEL")
    }
    ...
    .with_files("crates/sim/RELEASE_CHANNEL")
    .into();
...
fn promote_to_stable(
    ...
    let read_version_step = named::bash(indoc::indoc! {r#"
            stable_version=$(script/get-crate-version sim)
            {
    ...
    let write_channel = named::bash("echo -n stable > crates/sim/RELEASE_CHANNEL");
