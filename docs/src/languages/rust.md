---
title: Rust
description: "Configure Rust language support in Sim, including language servers, formatting, and debugging."
---

# Rust

Rust support is available natively in Sim.
...
Build, Check, Run, Test, and Bench compile the selected Cargo node and preset into an ordinary Sim task. Debug compiles into an ordinary DAP scenario using the Cargo locator and CodeLLDB. Cargo metadata discovery never becomes a second build or test process runner. Existing task terminals, history, cancellation, rerun behavior, `tasks.json`, `debug.json`, and DAP remain authoritative.
...
- **Later:** coverage, profiling, richer dependency visualization, and additional language-specific workspace providers.
...
<!--
TBD: Polish Rust docs. Sim has strong Rust support, and the docs should reflect that clearly.
...
You can configure which `rust-analyzer` binary Sim should use.

By default, Sim will try to find a `rust-analyzer` in your `$PATH` and try to use that. If that binary successfully executes `rust-analyzer --help`, it's used. Otherwise, Sim will fall back to installing its own stable `rust-analyzer` version and use that.

If you want to install pre-release `rust-analyzer` version instead you can instruct Sim to do so by setting `pre_release` to `true` in your `settings.json`:
...
If you want to disable Sim looking for a `rust-analyzer` binary, you can set `ignore_system_version` to `true` in your `settings.json`:
...
If you want rust-analyzer to provide diagnostics for a target other than your current platform (e.g. for windows when running on macOS) you can use the following Sim lsp settings:
...
Sim provides tasks using tree-sitter, but rust-analyzer has an LSP extension method for querying file-related tasks via LSP.
...
<!--
TBD: Is it possible to specify RUSTFLAGS? https://github.com/simtropolis/sim/issues/14334
-->
...
Rust-analyzer in Sim runs with the default parameters.
...
Which would mean that every time Sim saves, a `cargo check --workspace --all-targets` command is run, checking the entire project (workspace), lib, doc, test, bin, bench and [other targets](https://doc.rust-lang.org/cargo/reference/cargo-targets.html).

While that works fine on small projects, it does not scale well.

The alternatives would be to use [tasks](../tasks.md), as Sim already provides a `cargo check --workspace --all-targets` task and the ability to cmd/ctrl-click on the terminal output to navigate to the error, and limit or turn off the check on save feature entirely.
...
Here's a snippet for Sim settings.json (the language server will restart automatically after the `lsp.rust-analyzer` section is edited and saved):
...
Sim supports debugging Rust binaries and tests out of the box with `CodeLLDB` and `GDB`. Run {#action debugger::Start} ({#kb debugger::Start}) to launch one of these preconfigured debug tasks.

For more control, you can add debug configurations to `.sim/debug.json`. See the examples below.
...
[
  {
    ...
    "program": "$SIM_WORKTREE_ROOT/target/debug/binary",
...
When you use `cargo build` or `cargo test` as the build command, Sim can infer the path to the output binary.
