---
title: Rust
description: "Configure Rust language support in Baymax, including language servers, formatting, and debugging."
---

# Rust

Rust support is available natively in Baymax.
...
<!--
TBD: Polish Rust docs. Baymax has strong Rust support, and the docs should reflect that clearly.
...
You can configure which `rust-analyzer` binary Baymax should use.

By default, Baymax will try to find a `rust-analyzer` in your `$PATH` and try to use that. If that binary successfully executes `rust-analyzer --help`, it's used. Otherwise, Baymax will fall back to installing its own stable `rust-analyzer` version and use that.

If you want to install pre-release `rust-analyzer` version instead you can instruct Baymax to do so by setting `pre_release` to `true` in your `settings.json`:
...
If you want to disable Baymax looking for a `rust-analyzer` binary, you can set `ignore_system_version` to `true` in your `settings.json`:
...
If you want rust-analyzer to provide diagnostics for a target other than your current platform (e.g. for windows when running on macOS) you can use the following Baymax lsp settings:
...
Baymax provides tasks using tree-sitter, but rust-analyzer has an LSP extension method for querying file-related tasks via LSP.
...
<!--
TBD: Is it possible to specify RUSTFLAGS? https://github.com/simtropolis/baymax/issues/14334
-->
...
Rust-analyzer in Baymax runs with the default parameters.
...
Which would mean that every time Baymax saves, a `cargo check --workspace --all-targets` command is run, checking the entire project (workspace), lib, doc, test, bin, bench and [other targets](https://doc.rust-lang.org/cargo/reference/cargo-targets.html).

While that works fine on small projects, it does not scale well.

The alternatives would be to use [tasks](../tasks.md), as Baymax already provides a `cargo check --workspace --all-targets` task and the ability to cmd/ctrl-click on the terminal output to navigate to the error, and limit or turn off the check on save feature entirely.
...
Here's a snippet for Baymax settings.json (the language server will restart automatically after the `lsp.rust-analyzer` section is edited and saved):
...
Baymax supports debugging Rust binaries and tests out of the box with `CodeLLDB` and `GDB`. Run {#action debugger::Start} ({#kb debugger::Start}) to launch one of these preconfigured debug tasks.

For more control, you can add debug configurations to `.baymax/debug.json`. See the examples below.
...
[
  {
    ...
    "program": "$BAYMAX_WORKTREE_ROOT/target/debug/binary",
...
When you use `cargo build` or `cargo test` as the build command, Baymax can infer the path to the output binary.
