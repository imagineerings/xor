---
title: JavaScript
description: "Configure JavaScript language support in Baymax, including language servers, formatting, and debugging."
---

# JavaScript

JavaScript support is available natively in Baymax.
...
Configure formatting in Settings ({#kb baymax::OpenSettings}) under Languages > JavaScript, or add to your settings file:
...
Baymax supports JSX syntax highlighting out of the box.

In JSX strings, the [`tailwindcss-language-server`](./tailwindcss.md) is used to provide autocompletion for Tailwind CSS classes.

## JSDoc

Baymax supports JSDoc syntax in JavaScript and TypeScript comments that match the JSDoc syntax.
Baymax uses [tree-sitter/tree-sitter-jsdoc](https://github.com/tree-sitter/tree-sitter-jsdoc) for parsing and highlighting JSDoc.

## ESLint

You can configure Baymax to format code using `eslint --fix` by running the ESLint code action when formatting.

Configure code actions on format in Settings ({#kb baymax::OpenSettings}) under Languages > JavaScript, or add to your settings file:
...
Configure in Settings ({#kb baymax::OpenSettings}) under Languages > JavaScript, or add to your settings file:
...
Baymax supports debugging JavaScript code out of the box with `vscode-js-debug`.
...
[
  {
    ...
    "type": "chrome",
    "request": "launch",
    "file": "$BAYMAX_WORKTREE_ROOT/index.html",
