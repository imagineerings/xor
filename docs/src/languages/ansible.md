---
title: Ansible
description: "Configure Ansible language support in Sim, including language servers, formatting, and debugging."
---

# Ansible

Support for Ansible in Sim is provided via a community-maintained [Ansible extension](https://github.com/kartikvashistha/sim-ansible).
...
To change this behavior, you can add a `"file_types"` section to Sim settings inside your project (`.sim/settings.json`) or your Sim user settings (`~/.config/sim/settings.json`) to match your folder/naming conventions. For example:
...
- or, configure the YAML language server settings to set this schema for all your inventory files, that match your inventory pattern, under your Sim settings ([ref](https://sim.dev/docs/languages/yaml)):
...
When desired, any of the above default settings can be overridden under the `"lsp"` section of your Sim settings file. For example:

```json [settings]
{
  "lsp": {
    // The Sim Ansible extension prefixes all settings with `ansible`
