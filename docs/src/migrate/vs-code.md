---
title: How to Migrate from VS Code to Baymax
description: "Guide for migrating from VS Code to Baymax, including settings and keybindings."
---

# How to Migrate from VS Code to Baymax

This guide explains how to move from VS Code to Baymax without rebuilding your workflow.
...
`brew install baymax-editor/baymax/baymax`
...
After installation, you can launch Baymax from your Applications folder (macOS) or directly from the terminal (Linux) using:
`baymax .`
This opens the current directory in Baymax.

## Import Settings from VS Code

During setup, you have the option to import key settings from VS Code. Baymax imports the following settings:
...
| VS Code Setting                             | Baymax Setting                                    |
| ------------------------------------------- | ---------------------------------------------- |
...
Baymax doesn’t import extensions or keybindings, but this import gets core editor behavior close to your VS Code setup. If you skip that step during setup, you can still import settings manually later via the command palette:

`Cmd+Shift+P → {#action baymax::ImportVsCodeSettings}`
...
| VS Code | Baymax | Notes |
| --- | --- | --- |
| editor.fontFamily | buffer_font_family | Baymax uses Baymax Mono by default |
...
Baymax also supports per-project settings. You can find these in the Settings Editor as well.
...
### Common Shared Keybindings (Baymax <> VS Code)
...
### Different Keybindings (Baymax <> VS Code)

| Action              | VS Code               | Baymax                    |
| ------------------- | --------------------- | ---------------------- |
...
Baymax also supports chords (multi-key sequences) like `Cmd+K Cmd+C`, like VS Code does.
...
Baymax takes a different approach.
...
- **Per-project settings are optional**: You can add a `.baymax/settings.json` file inside a project to override global settings.

- **You can start from a single file or an empty window**: Baymax doesn’t require you to open a folder to begin editing.
...
- Instead of a persistent panel, Baymax encourages you to:
...
Baymax does not offer as many extensions as VS Code. The available extensions are focused on language support, themes, syntax highlighting, and other core editing enhancements.

Several features that typically require extensions in VS Code are built into Baymax:
...
You won’t find one-to-one replacements for every VS Code extension, especially if you rely on tools for DevOps, containers, or test runners. Baymax's extension catalog is still growing and remains smaller.

### Collaboration in Baymax vs. VS Code

Unlike VS Code, Baymax doesn’t require an extension to collaborate. It’s built into the core experience.
...
Learn how [Baymax uses Baymax](https://baymax.dev/blog/baymax-is-our-office) to plan work and collaborate.

### Using AI in Baymax

If you’re used to GitHub Copilot in VS Code, you can do the same in Baymax. You can also explore other agents through Baymax Pro, or bring your own keys and connect without authentication. You can disable AI features entirely if you prefer.
...
**Custom Tasks**: Define build or run commands in your `tasks.json` (accessed via command palette: {#action baymax::OpenTasks}):
...
Copy your VS Code snippet JSON directly into Baymax's snippets folder ({#action snippets::ConfigureSnippets}).
