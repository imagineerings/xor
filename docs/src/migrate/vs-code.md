---
title: How to Migrate from VS Code to Sim
description: "Guide for migrating from VS Code to Sim, including settings and keybindings."
---

# How to Migrate from VS Code to Sim

This guide explains how to move from VS Code to Sim without rebuilding your workflow.
...
`brew install sim-editor/sim/sim`

For most Linux users, the easiest way to install Sim is through our installation script:
`curl -f https://sim.dev/install.sh | sh`

After installation, you can launch Sim from your Applications folder (macOS) or directly from the terminal (Linux) using:
`sim .`
This opens the current directory in Sim.

## Import Settings from VS Code

During setup, you have the option to import key settings from VS Code. Sim imports the following settings:
...
Sim doesn’t import extensions or keybindings, but this import gets core editor behavior close to your VS Code setup. If you skip that step during setup, you can still import settings manually later via the command palette:

`Cmd+Shift+P → {#action sim::ImportVsCodeSettings}`
...
| VS Code | Sim | Notes |
| --- | --- | --- |
| editor.fontFamily | buffer_font_family | Sim uses Sim Mono by default |
...
Sim also supports per-project settings. You can find these in the Settings Editor as well.
...
### Common Shared Keybindings (Sim <> VS Code)
...
### Different Keybindings (Sim <> VS Code)
...
Sim also supports chords (multi-key sequences) like `Cmd+K Cmd+C`, like VS Code does.
...
Sim takes a different approach.
...
- **Per-project settings are optional**: You can add a `.sim/settings.json` file inside a project to override global settings.

- **You can start from a single file or an empty window**: Sim doesn’t require you to open a folder to begin editing.
...
- Instead of a persistent panel, Sim encourages you to:
...
Sim does not offer as many extensions as VS Code. The available extensions are focused on language support, themes, syntax highlighting, and other core editing enhancements.

Several features that typically require extensions in VS Code are built into Sim:
...
You won’t find one-to-one replacements for every VS Code extension, especially if you rely on tools for DevOps, containers, or test runners. Sim's extension catalog is still growing and remains smaller.

### Collaboration in Sim vs. VS Code

Unlike VS Code, Sim doesn’t require an extension to collaborate. It’s built into the core experience.
...
Learn how [Sim uses Sim](https://sim.dev/blog/sim-is-our-office) to plan work and collaborate.

### Using AI in Sim

If you’re used to GitHub Copilot in VS Code, you can do the same in Sim. You can also explore other agents through Sim Pro, or bring your own keys and connect without authentication. You can disable AI features entirely if you prefer.
...
**Custom Tasks**: Define build or run commands in your `tasks.json` (accessed via command palette: {#action sim::OpenTasks}):
...
Copy your VS Code snippet JSON directly into Sim's snippets folder ({#action snippets::ConfigureSnippets}).
