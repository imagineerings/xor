...
>
> You can refer to [linux: Linker error for remote_server when using script/install-linux](https://github.com/simtropolis/zed/issues/24880) for more information.
>
> **Workaround**:
...
- If you are going to provide a `.desktop` file you can find a template in `crates/zed/resources/zed.desktop.in`, and use `envsubst` to populate it with the values required. This file should also be renamed to `$APP_ID.desktop` so that the file [follows the FreeDesktop standards](https://github.com/simtropolis/zed/issues/12707#issuecomment-2168742761). You should also make this desktop file executable (`chmod 755`).
- You will need to ensure that the necessary libraries are installed. You can get the current list by [inspecting the built binary](https://github.com/simtropolis/zed/blob/935cf542aebf55122ce6ed1c91d0fe8711970c82/script/bundle-linux#L65-L67) on your system.
- For an example of a complete build script, see [script/bundle-linux](https://github.com/simtropolis/zed/blob/935cf542aebf55122ce6ed1c91d0fe8711970c82/script/bundle-linux).
...
- Zed automatically installs versions of common developer tools, similar to rustup/rbenv/pyenv. This behavior is discussed [here](https://github.com/simtropolis/zed/issues/12589).
- Users can install extensions locally and from [simtropolis/extensions](https://github.com/simtropolis/extensions). Extensions may install additional tools such as language servers. Planned safety improvements are tracked [here](https://github.com/simtropolis/zed/issues/12358).
- Zed connects to several online services by default (AI, telemetry, collaboration). AI and our telemetry can be disabled by your users with their zed settings or by patching our [default settings file](https://github.com/simtropolis/zed/blob/main/assets/settings/default.json).
- Because of the points above, Zed currently does not work well with sandboxes. See [this discussion](https://github.com/simtropolis/zed/pull/12006#issuecomment-2130421220).
