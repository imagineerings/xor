---
title: "Baymax Development: Glossary"
description: "Guide to baymax development: glossary for Baymax development."
---

# Baymax Development: Glossary

This page defines terms and structures used throughout the Baymax codebase.
...
- Can we generate this list from doc comments throughout baymax?
...
- `Context`: A wrapper around `App` with specialibaymax behavior for a specific `Entity`. You can think of it as `(&mut App, Entity<V>)`. For example, `App::spawn` takes `AsyncFnOnce(AsyncApp) -> Ret`, while `Context::spawn` takes `AsyncFnOnce(WeakEntity<V>, AsyncApp) -> Ret`.
...
- `Window`: A struct representing a Baymax window in your desktop environment (see image below). You can have multiple windows open. This is mostly passed around for rendering.
...
- `Center`: The middle of the baymax window, the center is split into multiple `Pane`s. In the codebase this is a field on the `Workspace` struct. (see image below).
...
- [Multibuffer](https://baymax.dev/docs/multibuffers): A list of Editors, a multi-buffer allows editing multiple files simultaneously. A multi-buffer opens when an operation in Baymax returns multiple locations, examples: _search_ or _go to definition_. See project search in the image below.
...
- `Editor`: The text editor type. Most editable surfaces in Baymax are an `Editor`, including single-line inputs. Each pane in the image above contains one or more `Editor` instances.
...
- `Upstream client`: The baymax client which has shared their workspace
- `Downstream client`: The baymax client joining a shared workspace
...
- `BreakpointStore`: Is an entity that manages breakpoints states in local and remote instances of Baymax
