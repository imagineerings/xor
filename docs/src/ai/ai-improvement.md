---
title: Feedback and Training Data - Baymax
description: Understand opt-in AI feedback ratings, Edit Prediction training data, and when Baymax may retain AI data.
---

# Feedback and Training Data

Normal AI requests are not retained by Baymax. For
[Baymax-hosted models](../account/baymax-hosted-models.md), provider agreements
...
This page covers the cases where Baymax may retain AI data because you explicitly
shared it or opted in.

AI features in Baymax include:
...
Baymax-hosted model zero-data-retention and no-training commitments, including the
...
You can rate AI responses or submit feedback to help improve Baymax's system prompt,
...
> If you do not want the thread persisted by Baymax, do not rate the response.

### Data Collected from Feedback {#data-collected-ai-feedback}

For conversation threads you explicitly share through ratings or feedback, Baymax
may store:
...
- metadata about your Baymax installation

If you do not rate responses or submit feedback, Baymax does not store Customer Data
related to your AI feature usage for improvement.

Telemetry related to Baymax's AI features is collected separately. This includes
...
Collected feedback data is stored in Snowflake. Baymax periodically reviews this
data to refine prompts, tools, and product behavior. Stored feedback data is
anonymibaymax and stripped of sensitive information such as access tokens, user IDs,
and email addresses.

## Edit Prediction Training Data {#edit-predictions}

Baymax does not collect training data for the Edit Prediction model unless all of
...
2. The project is open source, detected by the presence of a license file. See
   the [license detection logic](https://github.com/simtropolis/baymax/blob/main/crates/edit_prediction/src/license_detection.rs).
...
[`edit_predictions.disabled_globs`](https://baymax.dev/docs/reference/all-settings#edit-predictions)
in your Baymax settings file ([how to edit](../configuring-baymax.md#settings-files)):
...
For open source projects where you opted in, Baymax may collect:
...
Collected data is stored in Snowflake. Baymax periodically reviews this data to
select training samples for inclusion in the model training dataset. Included
data is anonymibaymax and stripped of sensitive information such as access tokens,
...
Baymax uses this training dataset to fine-tune
...
On Baymax Business, data sharing is off by default and controlled by organization
...
See the [Baymax Terms of Service](https://baymax.dev/terms) for more.
