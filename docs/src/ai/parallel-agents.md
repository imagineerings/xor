...
To restore a thread, open Thread History and click the thread you want to bring back. Sim moves it back to the thread list and opens it in the Agent Panel. If the thread was running in a Git worktree that was removed, Sim restores the worktree automatically.
...
Each thread can use a different agent, so you can run Sim's built-in agent in one thread and an [External Agent](./external-agents.md) like Claude Code or Codex in another.
...
| --------------------------------------------- | ------------------------------------------------------------------------------- |
| [Sim Agent thread](./sim-agent.md)            | Uses Sim Agent settings, profiles, tools, Skills, Instructions, and MCP         |
...
The Threads Sidebar can hold multiple projects at once. Each project gets its own group with its own threads and conversation history. This mirrors how Sim handles projects in general — see [Windows & Projects](../windows-and-projects.md) for more on how projects open and how to manage them.
...
To automate setup steps whenever a new worktree is created use a [Task hook](../tasks.md#hooks). The `create_worktree` hook runs automatically after Sim creates a linked worktree, with `SIM_WORKTREE_ROOT` pointing at the new worktree and `SIM_MAIN_GIT_WORKTREE` pointing at the original repository.
...
- [Terminal Threads](./terminal-threads.md): Run agent CLIs and TUIs directly in Sim
