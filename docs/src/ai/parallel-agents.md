...
To restore a thread, open Thread History and click the thread you want to bring back. Baymax moves it back to the thread list and opens it in the Agent Panel. If the thread was running in a Git worktree that was removed, Baymax restores the worktree automatically.
...
Each thread can use a different agent, so you can run Baymax's built-in agent in one thread and an [External Agent](./external-agents.md) like Claude Code or Codex in another.
...
| --------------------------------------------- | ------------------------------------------------------------------------------- |
| [Baymax Agent thread](./baymax-agent.md)            | Uses Baymax Agent settings, profiles, tools, Skills, Instructions, and MCP         |
...
The Threads Sidebar can hold multiple projects at once. Each project gets its own group with its own threads and conversation history. This mirrors how Baymax handles projects in general — see [Windows & Projects](../windows-and-projects.md) for more on how projects open and how to manage them.
...
To automate setup steps whenever a new worktree is created use a [Task hook](../tasks.md#hooks). The `create_worktree` hook runs automatically after Baymax creates a linked worktree, with `BAYMAX_WORKTREE_ROOT` pointing at the new worktree and `BAYMAX_MAIN_GIT_WORKTREE` pointing at the original repository.
...
- [Terminal Threads](./terminal-threads.md): Run agent CLIs and TUIs directly in Baymax
