...
description: "Create Sim extensions: languages, themes, debuggers, and more."
---

# Developing Extensions {#developing-extensions}

Sim extensions are Git repositories containing an `extension.toml` manifest. They can provide languages, themes, debuggers, snippets, and MCP servers.
...
Before starting to develop an extension for Sim, be sure to [install Rust via rustup](https://www.rust-lang.org/tools/install).
...
When developing an extension, you can use it in Sim without needing to publish it by installing it as a _dev extension_.

From the extensions page, click the `Install Dev Extension` button (or the {#action sim::InstallDevExtension} action) and select the directory containing your extension.

If you need to troubleshoot, check Sim.log ({#action sim::OpenLog}) for additional output. For debug output, close and relaunch Sim from the command line with `sim --foreground`, which shows more verbose INFO-level logs.
...
## Directory Structure of a Sim Extension

A Sim extension is a Git repository that contains an `extension.toml`. This file must contain some
...
repository = "https://github.com/your-name/my-sim-extension"
```

In addition to this, there are several other optional files and directories that can be used to add functionality to a Sim extension. An example directory structure of an extension that provides all capabilities is as follows:
...
Use the latest version of the [`zed_extension_api`](https://crates.io/crates/zed_extension_api) available on crates.io. Make sure it's still [compatible with Sim versions](https://github.com/simtropolis/sim/blob/main/crates/extension_api#compatible-sim-versions) you want to support.
...
`stdout`/`stderr` is forwarded directly to the Sim process. In order to see `println!`/`dbg!` output from your extension, you can start Sim in your terminal with a `--foreground` flag.

## Forking and cloning the repo

1. Fork the repo

> **Note:** It is very helpful if you fork the `simtropolis/extensions` repo to a personal GitHub account instead of a GitHub organization, as this allows Sim staff to push any needed changes to your PR to expedite the publishing process.
...
Before publishing your extension, make sure that you have chosen a unique extension ID for your extension in the [extension manifest](#directory-structure-of-a-sim-extension).
...
- Extension IDs and names must not contain the words `sim`, `Sim` or `extension`, since they are all Sim extensions.
...
- Your extension must only include the resources it requires to function and nothing else.
  - See the [directory structure of a Sim extension](#directory-structure-of-a-sim-extension) and the [Rust and WebAssembly](#rust-and-webassembly) sections for more information.
- Extensions must in no way attempt to read nor modify the environment outside of the environment designated to them by Sim. Should they need to read the environment, they should use methods as provided by the [Sim Rust Extension API](https://docs.rs/zed_extension_api/latest/zed_extension_api/) and may fall back to appropriate methods from the Rust standard library. Should they need changes to the environment, they must instead ask the user to perform these for them using an appropriate method within the context (e.g. provide information for doing so using the `ContextServerConfiguration` for context servers).
...
- Extensions should provide something that is not yet available in the marketplace as opposed to fixing something that could be resolved within an existing extension. For example, if you find that an existing extension's support for a language server is not functioning properly, first try contributing a fix to the existing extension as opposed to submitting a new extension immediately.
  - If you receive no response or reaction within the upstream repository within a reasonable amount of time, feel free to submit a pull request that aims to fix said issue. Please ensure that you provide your previous efforts within the pull request to the extensions repository for adding your extension. Sim maintainers will then decide on how to proceed on a case by case basis.
- Extensions that intend to provide a language, debugger or MCP server must not ship the language server as part of the extension. Instead, the extension should either download the language server or check for the availability of the language server in the users environment using the APIs as provided by the [Sim Rust Extension API](https://docs.rs/zed_extension_api/latest/zed_extension_api/).
...
```sh
git submodule add https://github.com/your-username/foobar-sim.git extensions/my-extension
...
Once your PR is merged, the extension will be packaged and published to the Sim extension registry.
...
If you'd like to automate this process, there is a [community GitHub Action](https://github.com/huacnlee/sim-extension-action) you can use.
