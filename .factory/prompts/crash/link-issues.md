...
Search **only** issues in `simtropolis/sim` (prefer `gh issue list` / `gh issue view` / GraphQL if available) by:
...
Check both open and recently closed issues in `simtropolis/sim`.
...
## High Confidence
- [#12345](https://github.com/simtropolis/sim/issues/12345) — <title>
...
- Do not include issues from any repository other than `simtropolis/sim`.
