...
1. **3.1** WHEN the Phase 1 Rust flavor is built, THEN the `zed` package SHALL be built with exactly the catalog-selected product features including `agentic` and `rust-tools`, and its remote server SHALL receive the corresponding Rust tooling feature.
