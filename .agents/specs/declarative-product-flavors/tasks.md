...
- [x] 4. Route every platform bundle through the resolved Rust plan
  ...
  - [x] 4.3. Consume the product plan in the Windows installer
    ...
    - _Evidence: Windows uses catalog-derived Inno definitions, product executable/icon/GUID/AppUserModelID/mutex/scheme/output names, robust Visual Studio discovery, and the x86_64 dry plan. Xtask removes Windows' `\\?\` verbatim prefix before passing canonical paths to external tools, and the bundler verifies and passes the catalog icon's absolute source path directly to Inno. The generated `windows-2022` CI job tests this path conversion and compiles the installer's actual `AppId` and `SetupIconFile` directives with the catalog GUID and icon, including the literal-brace escaping required by Inno Setup._
