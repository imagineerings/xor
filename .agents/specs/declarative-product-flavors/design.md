...
- Integration:
  ...
  - Windows passes catalog-derived Inno defines for application GUID, name, executable, icon, AppUserModelID, registry prefix, mutex, Appx identity, and output filename. The installer template escapes the GUID's opening brace after preprocessor expansion so Inno treats it as a literal application ID. Before crossing into PowerShell and Inno, xtask simplifies canonical paths to normal Win32 paths instead of exposing the `\\?\` verbatim prefix. The bundler verifies the catalog icon immediately before invoking Inno and passes its absolute source path directly to `SetupIconFile`; generated Windows CI tests the Windows path conversion and compiles both resource directives with the catalog GUID and icon. Windows executable resources also consume the resolved display name, icon set, and release version. The updater helper receives the resolved executable rather than hard-coded `Zed.exe` jobs; reusable rollback closures borrow captured paths.
