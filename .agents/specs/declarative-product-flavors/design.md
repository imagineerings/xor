...
| --- | --- | --- | --- | --- |
| `rust` | enabled | Copper / `copper` | `dev.ideflavors.rust`, `ide-rust`, `ide-rust` data, `rust` updates | `agentic`, `rust-tools`, built-in Rust and rust-analyzer, rustup onboarding, Rust engineer profile |
...
- Integration:
  ...
  - macOS may retain internal Cargo package/binary name `zed`; after `cargo bundle` creates the base application, the script updates exact plist keys (`CFBundleName`, `CFBundleDisplayName`, `CFBundleIdentifier`, `CFBundleExecutable`, and `CFBundleURLTypes`), installs the configured icon, renames the application/DMG, and only then signs. Cargo metadata remains a neutral internal base instead of one marketing product per table.
  - Windows passes catalog-derived Inno defines for application GUID, name, executable, icon, AppUserModelID, registry prefix, mutex, Appx identity, and output filename. The updater helper receives the resolved executable rather than hard-coded `Zed.exe` jobs.
...
- Snapshot `cargo xtask bundle --product rust --dry-run` for every supported target and assert `agentic`, `rust-tools`, remote `rust-tools`, product-specific target directories, output names, and signing policy.
