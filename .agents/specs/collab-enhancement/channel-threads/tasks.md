...
- [ ] 10. Write tests
  ...
  - _implemented direct DB coverage for the existing `Database` thread query layer in `crates/collab/tests/integration/db_tests/channel_tests.rs`; initial client/server integration coverage in `crates/collab/tests/integration/channel_chat_tests.rs`; full-flow, thread compose, and realtime open-thread UI coverage in `crates/collab/tests/integration/channel_chat_ui_tests.rs`; unread/read UI tests remain_
