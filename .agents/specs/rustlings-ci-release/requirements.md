...
12. **2.12** THE release workflow SHALL generate one commit-bound license artifact, compile the application and remote server in independent concurrent platform matrices, validate every intermediate artifact's commit, target, and catalog feature metadata, and package the matching inputs in three final native jobs before publication.
