...
11. **2.11** THE generated native release matrix SHALL use GitHub-managed object-level compiler caching isolated by runner operating system, target triple, Rust toolchain, and an explicit cache schema, and its cache daemon SHALL remain available across long single-crate release compilations while preserving cold-build fallback, strict statistics, the release profile, exact product features, native smoke tests, and the complete publish barrier.
12. **2.12** THE release workflow SHALL generate one commit-bound license artifact and commit-bound bundle plans, compile the application and remote server in independent concurrent platform matrices, validate every intermediate artifact's commit, target, and catalog feature metadata, cache the exact macOS `cargo-bundle` tool separately from build outputs, and package the matching inputs in three final native jobs before publication without recompiling `xtask`.
...
5. **3.5** THE macOS plist, Windows executable resources, packaged CLI, and application version SHALL use the selected release version and resolved product identity; native jobs SHALL reject metadata mismatches and failed installed CLI/editor-help smoke checks before upload. Windows foreground startup SHALL remain available without Comfy and SHALL pass a system-specs smoke check with temporary user data.
