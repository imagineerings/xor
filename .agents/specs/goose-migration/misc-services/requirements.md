...
**User Story:** As a zed user, I want a Q&A bot that can answer questions about using the agent, so that I can get help without reading all documentation.
...
2. **1.2** THE service SHALL ingest only approved canonical Zed documentation with defined freshness, authentication, privacy, prompt-injection, abuse-control, rate-limit, and retention policies
...
**User Story:** As a zed user, I want to import sessions from other formats, so that I can migrate conversations from other tools.

#### Acceptance Criteria

1. **2.1** THE existing thread import pipeline SHALL support the approved Claude Code, Codex, and Pi JSONL transcript formats in addition to Zed-native import
...
**User Story:** As a zed user, I want to share agent sessions via Nostr, so that I can publish and discover sessions on a decentralized network.
...
**User Story:** As a zed developer, I want example integrations, so that I can see how to use and extend the agent.

#### Acceptance Criteria

1. **4.1** THE migration SHALL inventory Goose's MCP wiki example and port an equivalent only if the corresponding Zed extension surface is public and approved
2. **4.2** THE migration SHALL inventory plugin examples and add only examples for the approved Zed plugin/extension format
3. **4.3** THE migration SHALL inventory `frontend_tools.py` and map it to an approved Zed tool/UI protocol rather than copying a Goose-specific endpoint
...
**User Story:** As a zed developer, I want scripts for common development tasks, so that development workflows are automated.

#### Acceptance Criteria

1. **5.1** THE migration SHALL map Goose's Windows build script to Zed's existing Windows CI/release path and add nothing when behavior is already covered
...
3. **5.3** DIAGNOSTICS viewing SHALL extend Zed's existing diagnostics/log tooling with redaction and size limits instead of copying a parallel viewer unless a confirmed workflow gap remains
4. **5.4** DATABASE inspection SHALL extend or document Zed's existing thread/database tooling with read-only defaults, backup, version checks, and explicit destructive confirmation
5. **5.5** MCP test workflows SHALL reuse Zed context-server/agent-server test harnesses
...
7. **5.7** RELEASE automation SHALL remain owned by Zed's release pipeline; only approved migration packaging inputs may be added
...
**User Story:** As a zed developer, I want a proxy for intercepting and debugging provider API errors, so that I can diagnose provider integration issues.

#### Acceptance Criteria

1. **6.1** WHERE a standalone provider error proxy is still needed after reviewing Zed HTTP diagnostics, IT SHALL be an explicit opt-in developer tool and SHALL never be selected in production configuration by default
