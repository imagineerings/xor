...
- **Migration-plan coverage estimate:** **87.5%** across in-scope `C1`-`C5` capabilities. Formula: `(C1 + C2 + C3 + 0.5 × C4) / (C1 + C2 + C3 + C4 + C5) = (21 + 34 + 54 + 17) / 144`.
- **Fully reusable, extendable, or fully specified:** 109 of 144 in-scope capabilities (75.7%).
- **Estimated implementation coverage:** **26.4%**, using full credit for `C1` and half credit for `C2`; `C3`/`C4`/`C5` receive no implementation credit. This is intentionally conservative because this audit did not implement product code.
- **Explicitly excluded/internal:** eight capabilities (`C6=3`, `C7=5`).

| Classification | Count | Meaning |
| --- | ---: | --- |
| C1 | 21 | Reusable in Sim without changes |
| C2 | 34 | Existing Sim behavior should be extended |
| C3 | 54 | Fully covered by a migration specification, not evidenced as implemented |
| C4 | 34 | Partially covered by a migration specification |
...
| DUI — desktop UI | 17 | 4 | 8 | 3 | 2 | 0 | 0 | 0 |
...
- **Desktop:** DUI-010 prompt/skill management scope and live-session semantics; DUI-012 display modes and remaining native-Goose-app versus MCP-App product scope.
...
- Authoritative audit artifacts: `coverage-catalog.md`, this summary, and `master-migration-plan.md`.
