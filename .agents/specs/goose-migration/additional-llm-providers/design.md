...
- The curated declarative catalog contains dozens of JSON definitions. Compatible services should be presets over Sim's existing OpenAI/Ollama-compatible providers; dedicated provider code requires evidence of protocol deviation.
- Current local inference uses llama.cpp and MLX-oriented paths, not Candle. Reuse Sim's `llama_cpp` owner and make MLX/product/platform scope explicit.
...
Reuse Sim's local inference, credentials, HTTP, cache, and model-configuration integration points for search/download/evict/delete lifecycle. A new shared downloader is justified only after existing owners cannot meet the multi-file/cancellation contract.
