...
**User Story:** As a zed user, I want protection against prompt injection and adversarial inputs, so that malicious instructions cannot hijack the agent.
...
**User Story:** As a zed user, I want protection against sensitive data leakage in agent outputs, so that confidential information is not accidentally sent to external services.
...
**User Story:** As a zed user, I want content classification for safety, so that harmful content can be identified and blocked.
...
**User Story:** As a zed user, I want a unified security scanner that coordinates all security checks, so that security is consistently applied.
...
**User Story:** As a zed developer, I want a pattern system for threat detection, so that I can define and update security patterns without code changes.
...
**User Story:** As a zed user, I want to confirm sensitive actions before the agent performs them, so that I maintain control over critical operations.
...
**User Story:** As a zed user, I want the system to examine tool calls against permission policies, so that actions are automatically classified by risk level.
...
**User Story:** As a zed user, I want an optional conservative read-only detector, so that strictly read-only operations can avoid unnecessary confirmation without granting write access.
...
**User Story:** As a zed user, I want permission decisions to be remembered, so that I am not repeatedly asked about the same action.
...
- Existing zed: `crates/agent/src/tool_permissions.rs`, `crates/sandbox/`
