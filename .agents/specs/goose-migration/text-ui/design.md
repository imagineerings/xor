# Design Document: Text UI / Terminal UI (GPUI/CLI Equivalents)

## 1. Overview

Migrate goose's text-based terminal UI features into sim's existing CLI (`crates/cli/`) and terminal infrastructure (`crates/terminal/`, `crates/terminal_view/`). Goose's React-based TUI becomes a new interactive CLI mode with rich terminal output — markdown rendering, configuration wizard, extension management, onboarding, and slash commands.

### Key Architectural Decisions

- **Interactive CLI mode**: A new `goose interactive` subcommand (or `goose tui`) that opens an interactive session in the terminal.
- **Terminal rendering via `crossterm` + `ratatui`**: Lightweight TUI rendering without requiring a full GPUI context. Sim's GPUI is for the desktop app; a terminal UI should use standard terminal libraries.
- **CLI crate extension**: Add interactive mode to `crates/cli/` rather than creating a new crate. The CLI crate already handles subcommands.
- **Markdown via `crates/markdown/`**: Use sim's existing markdown parser, adding ANSI terminal rendering on top.
- **Configuration wizard uses existing settings**: The wizard reads/writes the same settings files as the desktop UI and GPUI settings panels.

## 2. Architecture

```mermaid
graph TD
    subgraph "CLI Interactive Mode (crates/cli/)"
        Interactive[Interactive Session]
        Parser[Input Parser]
        Renderer[Terminal Renderer]
        History[Command History]
    end

    subgraph "Input Processing"
        SlashParser[Slash Command Parser]
        Multiline[Multiline Editor]
        Autocomplete[Autocomplete Provider]
    end

    subgraph "Rendering"
        MarkdownTerm[Markdown → ANSI]
        CodeHighlight[Syntax Highlighting]
        Progress[Progress / Spinner]
        ToolDisplay[Tool Call Display]
    end

    subgraph "Subcommands"
        Configure[configure wizard]
        ExtList[extension commands]
        Onboard[onboarding flow]
    end

    subgraph "Backend"
        Agent[crates/agent/]
        Settings[crates/settings/]
        Extensions[crates/extension/]
        Recipes[crates/recipe/]
    end

    Interactive --> Parser
    Interactive --> Renderer
    Interactive --> History
    Parser --> SlashParser
    Parser --> Multiline
    Multiline --> Autocomplete
    Renderer --> MarkdownTerm
    Renderer --> CodeHighlight
    Renderer --> Progress
    Renderer --> ToolDisplay
    Interactive --> Agent
    Configure --> Settings
    ExtList --> Extensions
    Onboard --> Agent
    Interactive --> Recipes
```

## 3. Components and Interfaces

### Component: Interactive Session

```rust
pub struct InteractiveSession {
    conversation: Vec<Message>,
    current_input: String,
    input_history: Vec<String>,
    history_index: isize,
}

impl InteractiveSession {
    pub async fn start(&mut self, cx: &mut App) -> Result<()>;
    pub async fn process_input(&mut self, input: &str, cx: &mut App) -> Result<()>;
    pub fn render_conversation(&self) -> String;
    pub fn handle_slash_command(&mut self, cmd: &str, args: &str) -> Result<()>;
}
```

### Component: Terminal Renderer

```rust
pub struct TerminalRenderer {
    width: u16,
    height: u16,
}

impl TerminalRenderer {
    pub fn render_message(&self, msg: &Message) -> Vec<String>; // lines of output
    pub fn render_markdown(&self, md: &str) -> Vec<String>;
    pub fn render_code_block(&self, code: &str, language: &str) -> Vec<String>;
    pub fn render_spinner(&self, label: &str) -> String;
    pub fn render_tool_call(&self, tool: &ToolCall, state: ToolState) -> Vec<String>;
    pub fn clear(&self);
    pub fn set_status(&self, status: &str);
}
```

### Component: Configuration Wizard

```rust
pub struct ConfigWizard {
    steps: Vec<WizardStep>,
    current_step: usize,
    answers: HashMap<String, String>,
}

impl ConfigWizard {
    pub async fn run(&mut self) -> Result<()>;
    pub fn current_prompt(&self) -> &str;
    pub fn process_answer(&mut self, answer: &str) -> Result<()>;
    pub fn is_complete(&self) -> bool;
}

struct WizardStep {
    prompt: String,
    input_type: InputType,
    validator: Option<Box<dyn Fn(&str) -> Result<()>>>,
    default: Option<String>,
}

enum InputType {
    Text { secret: bool },
    Confirm,
    Select { options: Vec<String> },
    File { must_exist: bool },
}
```

### Component: Extension Commands (CLI)

```rust
pub fn cmd_ext_list(extensions: &ExtensionStore) -> Result<()>;
pub fn cmd_ext_add(path: &str) -> Result<()>;
pub fn cmd_ext_remove(name: &str) -> Result<()>;
pub fn cmd_ext_status(name: &str) -> Result<()>;
```

### Component: Onboarding Flow

```rust
pub struct CliOnboarding {
    steps: Vec<OnboardingStep>,
}

impl CliOnboarding {
    pub fn is_first_run() -> bool;
    pub async fn run(&mut self) -> Result<()>;
}

enum OnboardingStep {
    Welcome,
    ProviderSetup,
    FirstMessage,
    ExtensionIntro,
    Complete,
}
```

### Component: Slash Commands (CLI)

```rust
pub struct CliSlashCommands {
    commands: HashMap<String, CliSlashHandler>,
}

type CliSlashHandler = Box<dyn Fn(&str, &mut InteractiveSession) -> Result<()>>;

impl CliSlashCommands {
    pub fn default() -> Self;
    pub fn handle(&self, cmd: &str, args: &str, session: &mut InteractiveSession) -> Result<()>;
    pub fn help_text(&self) -> String;
}

// Built-in commands:
// /help        → Show available commands
// /recipe      → Run a recipe
// /skill       → Load a skill
// /clear       → Clear conversation
// /save        → Save session
// /load        → Load session
// /model       → Switch model
// /mode        → Change agent mode
// /exit        → Quit
```

## 4. Data Models

```rust
pub struct Message {
    pub role: Role,
    pub content: String,
    pub timestamp: DateTime<Utc>,
}

pub enum Role {
    User,
    Assistant,
    System,
    Tool,
}

pub enum ToolState {
    Running,
    Completed { result: String },
    Failed { error: String },
}
```

## 5. Correctness Properties

### Property 1: Markdown Fidelity

_For any_ markdown content [rendered in the terminal], ALL structural elements (headings, lists, code blocks, links, tables) SHALL be visually distinguishable.

**Validates: Requirement 4.1**

### Property 2: Configuration Persistence

_For any_ setting [changed via the configuration wizard], [after the wizard completes], THE setting SHALL be persisted to the same configuration files used by the desktop UI.

**Validates: Requirement 2.4**

### Property 3: Command History

_For any_ input [submitted in the interactive session], [when the user presses up-arrow], THE previous input SHALL be recalled.

**Validates: Requirement 1.2**

## 6. Error Handling

| Error Scenario | Handling |
|---|---|
| Terminal too narrow (< 80 cols) | Warn and recommend wider terminal |
| Unicode rendering issues | Fall back to ASCII alternatives |
| Provider config invalid in wizard | Re-prompt with error message |
| Extension load fails | Show error, do not crash session |
| Agent takes too long | Show progress indicator with cancel option |

## 7. Testing Strategy

- **Unit tests**: Markdown → ANSI rendering, slash command parsing
- **Integration tests**: Full interactive session with mock agent
- **Wizard tests**: All wizard paths (success, validation error, cancel)
- **Performance tests**: Rendering throughput for long conversations

## References

- Source: `projects/goose/ui/text/` — React TUI (design reference)
- Sim: `crates/cli/` — CLI framework
- Sim: `crates/terminal/`, `crates/terminal_view/` — terminal emulator
- Sim: `crates/markdown/` — markdown parsing
- Sim: `crates/agent_ui/src/terminal_inline_assistant.rs` — terminal inline agent

## Requirements traceability

| Requirement | Design element | Verification |
| --- | --- | --- |
| 1.1 | Interactive TUI Session audit design | Observable scenario and failure-path test for 1.1 |
| 1.2 | Interactive TUI Session audit design | Observable scenario and failure-path test for 1.2 |
| 1.3 | Interactive TUI Session audit design | Observable scenario and failure-path test for 1.3 |
| 1.4 | Interactive TUI Session audit design | Observable scenario and failure-path test for 1.4 |
| 1.5 | Interactive TUI Session audit design | Observable scenario and failure-path test for 1.5 |
| 2.1 | Configuration Wizard audit design | Observable scenario and failure-path test for 2.1 |
| 2.2 | Configuration Wizard audit design | Observable scenario and failure-path test for 2.2 |
| 2.3 | Configuration Wizard audit design | Observable scenario and failure-path test for 2.3 |
| 2.4 | Configuration Wizard audit design | Observable scenario and failure-path test for 2.4 |
| 3.1 | Extensions Management in Terminal audit design | Observable scenario and failure-path test for 3.1 |
| 3.2 | Extensions Management in Terminal audit design | Observable scenario and failure-path test for 3.2 |
| 3.3 | Extensions Management in Terminal audit design | Observable scenario and failure-path test for 3.3 |
| 3.4 | Extensions Management in Terminal audit design | Observable scenario and failure-path test for 3.4 |
| 4.1 | Markdown Rendering in Terminal audit design | Observable scenario and failure-path test for 4.1 |
| 4.2 | Markdown Rendering in Terminal audit design | Observable scenario and failure-path test for 4.2 |
| 4.3 | Markdown Rendering in Terminal audit design | Observable scenario and failure-path test for 4.3 |
| 4.4 | Markdown Rendering in Terminal audit design | Observable scenario and failure-path test for 4.4 |
| 5.1 | Terminal Onboarding audit design | Observable scenario and failure-path test for 5.1 |
| 5.2 | Terminal Onboarding audit design | Observable scenario and failure-path test for 5.2 |
| 5.3 | Terminal Onboarding audit design | Observable scenario and failure-path test for 5.3 |
| 6.1 | Slash Commands in TUI audit design | Observable scenario and failure-path test for 6.1 |
| 6.2 | Slash Commands in TUI audit design | Observable scenario and failure-path test for 6.2 |
| 6.3 | Slash Commands in TUI audit design | Observable scenario and failure-path test for 6.3 |
| 7.1 | Tool Call Display audit design | Observable scenario and failure-path test for 7.1 |
| 7.2 | Tool Call Display audit design | Observable scenario and failure-path test for 7.2 |
| 7.3 | Tool Call Display audit design | Observable scenario and failure-path test for 7.3 |
| 8.1, 8.2, 8.3, 8.4 | D-CLI-OUTPUT | Contract snapshots, stdout/stderr separation, failure, cancellation, and broken-pipe tests |
| 9.1, 9.2, 9.3 | D-CLI-GENERATED-DOCS | Generated completion/manpage drift and packaging tests |
| 10.1, 10.2, 10.3 | D-CLI-REVIEW | Diff selection, dry-run, repository preservation, permission, and failure tests |
| 11.1, 11.2, 11.3 | D-CLI-TERM | Shell quoting, privacy, nested-hook, opt-out, and unsupported-shell tests |
| 12.1, 12.2, 12.3 | D-CLI-OWNERSHIP | Command inventory review and domain-service integration tests |

## Audit design corrections

- **D-CLI-OUTPUT:** Version the JSON and stream-event schema. Treat stdout as an API in machine modes, route diagnostics to stderr, and emit one terminal event even on cancellation or partial failure.
- **D-CLI-GENERATED-DOCS:** Generate completions and manual pages from the canonical command tree during packaging so generated files cannot drift.
- **D-CLI-REVIEW:** If approved, keep review orchestration as a thin command over existing repository and agent services; dry-run cannot mutate repository or session state.
- **D-CLI-TERM:** Shell integration is opt-in generated text. It never edits startup files itself and redacts or excludes secret-bearing commands by default.
- **D-CLI-OWNERSHIP:** The CLI dispatch layer owns argument parsing, output, and exit codes only. Each product domain owns its behavior and persistence.
