# Goose-to-Sim Migration Audit Index

The authoritative migration inventory is [coverage-catalog.md](coverage-catalog.md). It records 152 stable capability IDs, observable behavior, source evidence, existing Sim evidence, requirement/design/task traceability, one exclusive classification, ownership, remaining verification, confidence, and open questions.
...
2. Executable Sim paths are required for `C1` or `C2`; stubs, declarations, dependencies, docs, and tests without a runtime path do not count.
3. Existing Sim owners are extended before proposing another crate, registry, persistence store, event stream, renderer, scheduler, or service.
