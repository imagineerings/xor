...
- **Shared sessions via deeplinks**: Use zed's existing `parse_zed_link` deeplink mechanism for sharing sessions.

## 2. Architecture

```mermaid
graph TD
    subgraph "Existing Zed Desktop (crates/zed/)"
        GPUI[GPUI Framework]
    ...
    Workspace --> RecipeUI
    Notifications --> Agent
    AutoUpdateUI -->|update check zed| Agent
```
...
// Deeplink handler in crates/zed/
fn handle_shared_session_link(url: &Url, cx: &mut App) {
    // Parses zed://session/<encoded-data>
    // Imports session
...
- **Visual tests**: Use zed's visual test framework for component screenshots
...
- Zed: `crates/zed/` — main desktop binary
- Zed: `crates/agent_ui/` — agent panel, conversation, configuration
- Zed: `crates/settings_ui/` — settings panel
- Zed: `crates/workspace/` — workspace/panel management
- Zed: `crates/notifications/` — notification system
- Zed: `crates/auto_update/`, `crates/auto_update_ui/` — update system
- Zed: `crates/onboarding/` — onboarding
