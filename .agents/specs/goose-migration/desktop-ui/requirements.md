...
Migrate the user-facing features of goose's Electron desktop app by building GPUI-based equivalents within baymax's existing desktop application. Rather than porting React/Electron code, we map each goose UI feature to the appropriate GPUI component or build new GPUI views where gaps exist.

Baymax already provides: GPUI rendering framework, workspace/panel system, agent panel, conversation view, inline assistant, configuration UI, theme system, notifications, settings UI, and onboarding. Goose's desktop app adds: recipe execution UI, scheduling UI, startup diagnostics, shared sessions, mesh networking, dedicated session manager UI, and ACP client connection management.

## Glossary

- **GPUI**: Baymax's native Rust UI framework
...
- **Workspace**: Baymax's window/panel management system (`crates/workspace/`)
- **ACP Client**: Agent-Client Protocol connection integrated in `crates/acp_thread/`
- **Mesh**: Peer-to-peer networking (if applicable to baymax's architecture)

## Requirements

### Requirement 1: Agent Session Management UI

**User Story:** As a baymax user, I want to manage agent sessions (list, create, delete, rename) through the GPUI interface, so that I can organize my agent conversations.
...
**User Story:** As a baymax user, I want to discover, view, and run recipes from the GPUI interface, so that I can execute multi-step workflows without the CLI.
...
**User Story:** As a baymax user, I want to schedule agent tasks from the GPUI interface, so that I can set up recurring operations.
...
**Mapping:** New GPUI settings view or panel, integrating with baymax's existing `crates/scheduler/` crate.

### Requirement 4: Startup Diagnostics

**User Story:** As a baymax user, I want the application to check for configuration and connectivity issues on startup, so that I can resolve problems early.
...
**User Story:** As a baymax user, I want to share agent sessions with other users via links, so that I can collaborate on agent interactions.
...
**Mapping:** GPUI integration using baymax's existing deeplink mechanism (`parse_baymax_link`) and session serialization.

### Requirement 6: ACP Connection Management

**User Story:** As a baymax user, I want visibility into the ACP connection state, so that I know when the agent is connected, disconnected, or reconnecting.
...
**User Story:** As a baymax user, I want to configure provider extensions and agent settings through the GPUI settings UI, so that I can manage all configuration visually.
...
**User Story:** As a baymax user, I want agent-related notifications (task complete, action required, errors) via the GPUI notification system, so that I stay informed.

#### Acceptance Criteria

1. THE agent SHALL use baymax's existing notification system (`crates/notifications/`) for agent events
...
**Mapping:** Integrate agent events with baymax's `Toast` and `NotificationId` system from `crates/workspace/`.

### Requirement 9: Internationalization (i18n)

**User Story:** As a baymax user, I want the UI in my language, so that I can use the application comfortably.

#### Acceptance Criteria

1. THE GPUI application SHALL support i18n following baymax's approach (if any exists) or introduce a lightweight i18n system
...
**User Story:** As a baymax user, I want to be notified of and manage application updates from the UI, so that I stay up to date.
...
All goose desktop UI features will be implemented as **GPUI components/views** within baymax's existing architecture:

```
goose Electron/React Feature          →  baymax GPUI Equivalent
──────────────────────────────────────────────────────────────
...
mesh.ts                               →  Evaluate; may not be needed in baymax
```
...
- Existing baymax: `crates/ui/` — UI components
- Existing baymax: `crates/settings_ui/` — settings UI
- Existing baymax: `crates/workspace/` — workspace/panel management
- Existing baymax: `crates/notifications/` — notification system
- Existing baymax: `crates/auto_update/`, `crates/auto_update_ui/` — update system
- Existing baymax: `crates/onboarding/` — onboarding views
- Existing baymax: `crates/theme/` — theming system
