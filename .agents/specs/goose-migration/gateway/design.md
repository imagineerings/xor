...
- **Telegram via `teligram-rs` or raw HTTP**: Use the Telegram Bot API directly over HTTP rather than a heavy framework, consistent with zed's dependency philosophy.
- **Gateway manager as an Entity**: Following zed's GPUI patterns, the gateway manager is an `Entity<GatewayManager>` that can be observed.
- **Optional dependency**: The gateway crate is an optional feature of the zed application, not a core dependency.
...
impl PairingService {
    pub fn pair_platform_user(&mut self, platform_id: &str, zed_user: &str) -> Result<()>;
    pub fn lookup_zed_user(&self, platform_id: &str) -> Option<String>;
...
| --- | --- | --- |
| 1.1 | Gateway Manager audit design | Observable scenario and failure-path test for 1.1 |
| 1.2 | Gateway Manager audit design | Observable scenario and failure-path test for 1.2 |
| 1.3 | Gateway Manager audit design | Observable scenario and failure-path test for 1.3 |
| 1.4 | Gateway Manager audit design | Observable scenario and failure-path test for 1.4 |
| 2.1 | Telegram Gateway audit design | Observable scenario and failure-path test for 2.1 |
| 2.2 | Telegram Gateway audit design | Observable scenario and failure-path test for 2.2 |
| 2.3 | Telegram Gateway audit design | Observable scenario and failure-path test for 2.3 |
| 2.4 | Telegram Gateway audit design | Observable scenario and failure-path test for 2.4 |
| 3.1 | Gateway Pairing audit design | Observable scenario and failure-path test for 3.1 |
| 3.2 | Gateway Pairing audit design | Observable scenario and failure-path test for 3.2 |
| 3.3 | Gateway Pairing audit design | Observable scenario and failure-path test for 3.3 |
| 4.1 | Telegram Message Formatting audit design | Observable scenario and failure-path test for 4.1 |
| 4.2 | Telegram Message Formatting audit design | Observable scenario and failure-path test for 4.2 |
| 4.3 | Telegram Message Formatting audit design | Observable scenario and failure-path test for 4.3 |
