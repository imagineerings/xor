...
Implement an HTTP REST API server as a new `crates/sim-server/` crate using Axum, providing endpoints for agent interaction, session management, recipes, scheduling, configuration, and system monitoring. Include OpenAPI documentation and optional tunnel support.
...
- [x] 1. Create server crate with core infrastructure
  ...
  - _Requirements: 1, 13_
  - _writes: crates/sim-server/src/lib.rs, crates/sim-server/src/auth.rs, crates/sim-server/src/error.rs, crates/sim-server/src/configuration.rs_
...
- [x] 2. Implement agent routes
  ...
  - _Requirements: 2_
  - _writes: crates/sim-server/src/routes/agent.rs_
...
- [x] 3. Implement session routes
  ...
  - _Requirements: 3_
  - _writes: crates/sim-server/src/routes/sessions.rs_
...
- [x] 4. Implement session event bus
  ...
  - _Requirements: 4_
  - _writes: crates/sim-server/src/event_bus.rs, crates/sim-server/src/routes/session_events.rs_
...
- [x] 5. Implement recipe routes
  ...
  - _Requirements: 5_
  - _writes: crates/sim-server/src/routes/recipes.rs_
...
- [x] 6. Implement configuration routes
  ...
  - _Requirements: 6_
  - _writes: crates/sim-server/src/routes/config.rs_
...
- [x] 7. Implement schedule routes
  ...
  - _Requirements: 7_
  - _writes: crates/sim-server/src/routes/schedules.rs_
