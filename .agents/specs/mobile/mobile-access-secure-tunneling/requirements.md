...
Baymax users who work on remote machines (accessed via SSH) currently connect their desktop client through Baymax's remote server. However, there is no way for the **Baymax mobile app** to establish a secure connection to these remote development environments.

This feature **migrates** the tunnel management functionality (currently in the `goose-server` crate) into Baymax, and adds a settings panel where users can start/stop a secure tunnel and display a QR code that the mobile app scans to configure and connect. This enables developers to access their remote workspaces from a mobile device seamlessly, with no external server dependency.
...
| **Mobile App** | The Baymax companion mobile application that scans the QR code to establish a secure connection to the remote environment. |
| **Tunnel Manager** | The Baymax-native module (migrated from `goose-server`) responsible for starting, stopping, and monitoring secure tunnels. |
| **Remote Server** | Baymax's existing SSH-based remote server infrastructure (`remote_server` crate) that provides remote development capabilities. |

## Requirements

### Requirement 1: Tunnel Management Module (Migrated from goose-server)

**User Story:** As a developer, I want the tunnel start/stop/status logic to live inside Baymax, so that mobile access works without depending on an external goose-server.

#### Acceptance Criteria

1.1 The tunnel start, stop, and status query logic currently in `goose-server`'s `/tunnel/start`, `/tunnel/stop`, and `/tunnel/status` SHALL be migrated into Baymax as a native module (crate or submodule).
...
**User Story:** As a Baymax user, I want a dedicated settings page where I can start and stop a secure tunnel natively, so that I can expose my workspace to the mobile app on demand.

#### Acceptance Criteria

2.1 WHEN the user opens the Baymax settings window THEN THE system SHALL display a "Mobile Access" settings page under an appropriate section (e.g., "Remote Development" or "Server").
...
3.5 WHERE the Mobile Access settings page is displayed THE system SHALL show human-readable connection instructions below the QR code (e.g., "Open the Baymax mobile app, tap 'Add Connection', and scan this QR code.").
...
4.1 WHERE the user has an active SSH remote connection in Baymax THEN THE system SHALL use the remote server's SSH identity and host information to pre-configure the tunnel parameters.
...
4.3 THE system SHALL remember the last-used tunnel configuration across Baymax restarts (persist in settings).
...
5.2 IF Baymax exits while a tunnel is running THEN THE system SHALL attempt to clean up the tunnel on next startup (or the tunnel SHALL self-terminate after a timeout).
...
**User Story:** As a Baymax user, I want the Mobile Access page to look and behave consistently with the existing settings UI.
...
6.3 WHEN there is an error or status change THEN THE system SHALL use standard Baymax notification/messaging patterns (inline banner).
