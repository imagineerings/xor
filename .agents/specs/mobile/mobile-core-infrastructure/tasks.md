...
**Key constraints:**
- iOS tasks write to `mobile/ios/Sim/`
- Android tasks write to `mobile/android/app/src/main/java/com/simtropolis/sim/`
...
- [ ] 2. Implement TunnelDetector (pure function, identical on both platforms)
  ...
  - Test with URL corpus covering: Tailscale (100.x.x.x, *.ts.net), Cloudflare, SSH (127.0.0.1, localhost), Trial (demo-simed.fly.dev), Direct LAN
...
- [ ] 4. Implement CredentialManager with secure storage
  ...
  - QR code URL parsing (`simchat://configure?data=...` format)
...
- [ ] 5. Implement AgentAPIService with HTTP client and retry
  ...
  - _writes: Android: `data/api/SimApiService.kt` (extend existing), `.../SimApiServiceTest.kt`_
...
- [ ] 10. Wire up connection lifecycle in app entry points
  ...
  - _writes: iOS: `SimApp.swift` (modify), `AppLifecycleManager.swift`_
  - _writes: Android: `SimApplication.kt` (modify), `data/repository/AppLifecycleManager.kt`_

## Notes

- iOS files with the same name as existing files (e.g., `SimAPIService.swift`, `ConfigurationHandler.swift`) extend/modify the existing implementation rather than replacing it entirely
