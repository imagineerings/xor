...
This specification defines the typography, iconography, spacing, shapes, and editor UI behavior for a Spectrum 2-inspired visual language in Zed. While the companion [Spectrum 2 Theme](../spectrum-2-theme/requirements.md) spec covers **color**, this spec covers everything else that makes the editor feel like a cohesive, approachable, modern tool.

Spectrum 2's design principles center on being **approachable, bright, rounded-feeling, and clear**. This spec translates those principles into concrete Zed settings, theme extensions, and code changes across typography, icons, spacing, shapes, and editor chrome.

## Glossary

- **Typography Scale**: The system of font sizes, weights, line heights, and font families used across the UI and editor buffers
- **Icon Theme**: A Zed extension or built-in theme that maps file types and UI states to SVG icon files
- **Dynamic Spacing**: Zed's spacing system using `DynamicSpacing::BaseNN` rems-based spacing tokens
...
Spectrum 2 uses [Adobe Clean](https://fonts.adobe.com/fonts/adobe-clean) as its primary typeface — a humanist sans-serif with approachable, warm curves. For Zed:
...
For Zed, this translates to:
- A new Spectrum 2-inspired icon theme as a Zed extension (or bundled theme)
...
Spectrum 2 uses an 8px grid with a 4px sub-unit for fine adjustments. Zed already uses a `DynamicSpacing` system. The recommendation is:
...
| Element | S2-Inspired Radius | Zed Current (approx) |
|---|---|---|
...
| **Minimap** | Show as a block overview, not character-precise | No Zed setting change — minimap rendering is built-in |
...
| **Gutter** | Show line numbers only (no breakpoint gutter when no breakpoints) | No change needed — Zed already hides breakpoint gutter when empty |
...
**User Story:** As a Zed user selecting the Spectrum 2 Inspired theme, I want the typography to feel approachable and modern like Spectrum 2, so that the text in both UI and editor buffers is comfortable to read.
...
3. IF Inter is not installed on the system THEN THE system SHALL fall back to the default Zed UI font stack
4. IF SF Mono is not installed THEN THE system SHALL fall back to JetBrains Mono, then to the default Zed buffer font
...
7. THE icon theme SHALL be distributed as a Zed extension that users can install from the Extensions page
...
**User Story:** As an existing Zed user, I want the Spectrum 2 UI language changes to not break my existing setup.
...
3. IF a user uninstalls the Spectrum 2 Inspired icon theme THEN the system SHALL fall back to the default Zed icon theme
...
- Redesigning core Zed SVG icons (search, settings, close, etc.) — these are shared across all themes and controlled by the app
- Changing the Zed DynamicSpacing tokens themselves (the values are fine; what changes is how elements use them)
