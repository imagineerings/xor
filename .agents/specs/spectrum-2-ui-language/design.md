...
| Icon theme distribution | Zed extension (not bundled) | Keeps core binary size small; aligns with how other icon themes work |
...
- **Rust** — Zed's UI framework (GPUI) for all code changes
- **SVG** — Icons are SVG files served from the asset source
- **JSON** — Theme extensions use the Zed theme schema; icon themes use the icon theme schema
...
The icon theme is a Zed extension with this structure:
...
version = "0.1.0"
schema_version = 1
description = "Spectrum 2-inspired file and folder icons for Zed"
authors = ["Ahmad Vegah"]
...
<!-- impl: crates/theme/src/fallback_themes.rs#zed_default_dark -->
...
| Icon extension not installed | Default Zed icon theme used | No error — Icon Theme Selector only shows installed themes |
...
| Test | What it verifies |
|---|---|
| `test_icon_theme_valid_json` | Icon theme JSON is valid per Zed schema |
