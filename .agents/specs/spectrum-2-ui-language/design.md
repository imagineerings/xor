...
| Icon theme distribution | Sim extension (not bundled) | Keeps core binary size small; aligns with how other icon themes work |
...
- **Rust** — Sim's UI framework (GPUI) for all code changes
- **SVG** — Icons are SVG files served from the asset source
- **JSON** — Theme extensions use the Sim theme schema; icon themes use the icon theme schema
...
The icon theme is a Sim extension with this structure:
...
version = "0.1.0"
schema_version = 1
description = "Spectrum 2-inspired file and folder icons for Sim"
authors = ["Ahmad Vegah"]
...
<!-- impl: crates/theme/src/fallback_themes.rs#sim_default_dark -->
...
| Icon extension not installed | Default Sim icon theme used | No error — Icon Theme Selector only shows installed themes |
...
| Test | What it verifies |
|---|---|
| `test_icon_theme_valid_json` | Icon theme JSON is valid per Sim schema |
