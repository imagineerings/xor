# Requirements: Port Zed v1.11.3 changes into Sim

## Problem

Sim identifies as version 1.10.2 and retains extensive Sim-specific behavior,
...
therefore requires explicit commit/file accounting and Sim-aware adaptation
...
- In scope: preservation of Sim branding, `sim`/`sim_actions` shell paths,
...
- Out of scope: Zed organization community boards/ranking automation and Zed
  ...
  external tracker changes, or any remote Sim repository.

## Constraints

- The local filesystem is the sole Sim authority. Missing Git metadata SHALL
...
1. WHEN an applicable upstream dependency, runtime, filesystem, network, task-scheduler, rendering, or GPUI change is approved THEN Sim SHALL carry its endpoint behavior and compatible dependency state without discarding local runtime integrations.
...
1. WHEN an approved editor, language, grammar, Vim, Markdown, picker, search, terminal, command-palette, theme, or interaction change is exercised THEN Sim SHALL expose the upstream v1.11.3 behavior through its existing UI and action architecture.
2. WHEN the upstream change includes a regression, GPUI, grammar, fixture, or settings test THEN Sim SHALL port or adapt the test and preserve existing action names, settings compatibility, and accessibility behavior.

### Requirement 3: Git, project, worktree, protocol, and workspace parity

#### Acceptance criteria

1. WHEN a project is a normal, bare, or linked worktree THEN Sim SHALL preserve upstream v1.11.3 repository identity and protocol semantics, including compatible generated protocol output.
2. WHEN users inspect, stage, unstage, partially stage, search, diff, or navigate Git/project content THEN Sim SHALL provide the approved upstream behavior and regression fixes without corrupting index, buffer, or diff state.
3. WHEN upstream workspace, pane, project-panel, or Git UI changes are ported THEN Sim SHALL preserve its multi-workspace model, thread sidebar, persistence, panel registration, and Comfy panel integration.

### Requirement 4: Agent, provider, context-server, account, and collaboration parity

#### Acceptance criteria

1. WHEN approved agent, ACP, model-provider, Bedrock, OpenAI, OpenCode, context-server, or tool-schema behavior is used THEN Sim SHALL match upstream v1.11.3 request, validation, UI, and error behavior.
2. WHEN approved account, OAuth, cloud API, LiveKit, collaboration, or protocol behavior is used THEN Sim SHALL carry the correctness/compatibility change through retained Sim services and endpoints.
3. WHEN these changes overlap local agent UI, credentials, subscription logic, or thread navigation THEN the port SHALL preserve Sim credentials, hosted URLs, sidebar semantics, stateless behavior, and local provider customizations.

### Requirement 5: Product shell, settings, assets, documentation, and delivery integrity

#### Acceptance criteria

1. WHEN approved actions, menus, open-listener behavior, settings, keymaps, icons, updater UI, documentation, or app-shell behavior is ported THEN Sim SHALL expose it with Sim names, URLs, branding, release channels, and product boundaries.
...
3. WHEN the port is complete THEN the Sim package and lockfile SHALL identify version 1.11.3, excluded Zed community/merch behavior SHALL remain absent, and patch-equivalent/reverted upstream entries SHALL not be duplicated.
