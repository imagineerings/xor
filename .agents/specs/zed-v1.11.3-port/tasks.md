...
- [x] 1. Port runtime, dependency, GPUI, filesystem, network, and platform changes
  ...
  - Outcome: Applicable runtime and platform rows expose v1.11.3 behavior with coherent dependencies and preserved Sim integrations.
...
- [x] 2. Port editor, language, search, Markdown, terminal, Vim, picker, and interaction changes
  ...
  - Outcome: Approved editor/language/search/terminal behavior and tests match v1.11.3 through Sim-native actions and settings.
...
- [x] 3. Port Git, project, worktree, protocol, workspace, pane, and project-panel changes
  ...
  - _reads: .agents/specs/zed-v1.11.3-port/upstream-inventory.md, .agents/specs/zed-v1.11.3-port/port-matrix.md, crates/sidebar, crates/sim/src/sim.rs_
  ...
  - Outcome: Linked/bare worktrees, protocols, partial staging, Git UI, project behavior, and workspace fixes match v1.11.3 without regressing Sim multi-workspace/sidebar/Comfy behavior.
  - Design: D2, D3, D4, D5, D6 / Git-project-workspace port.
  - Done when: Protocol generation and focused integration/GPUI/persistence tests pass and all Sim-specific workspace seams remain covered.
...
- [x] 4. Port agent, ACP, provider, context-server, account, cloud, collaboration, and LiveKit changes
  ...
  - _reads: .agents/specs/zed-v1.11.3-port/upstream-inventory.md, .agents/specs/zed-v1.11.3-port/port-matrix.md, crates/sim_credentials_provider, crates/sidebar_
  ...
  - Outcome: Applicable agent/provider/service behavior matches v1.11.3 while Sim credentials, endpoints, subscriptions, stateless behavior, and thread sidebar remain authoritative.
...
- [x] 5. Reconcile the Sim app shell, actions, settings, assets, documentation, tooling, CI, exclusions, and final version
  ...
  - _writes: Cargo.toml, Cargo.lock, .github, assets, crates/sim, crates/sim_actions, crates/client/src/sim_urls.rs, crates/icons, crates/onboarding, crates/title_bar, crates/ui, docs/src, extensions/workflows, script, tooling/xtask_
  - _validation: cargo check -p sim_
  - _Requirements: 5.1, 5.2, 5.3, 6.4_
  - Outcome: Sim-native shell integration, generated pairs, docs, delivery tooling, exclusions, and version 1.11.3 are coherent.
  - Design: D4, D5, D7, D8 / delivery-shell port.
  - Done when: Sim builds, schema/keymap/docs/tooling checks pass, version metadata is 1.11.3, excluded Zed product automation remains absent, and equivalent/prerequisite entries are verified without duplicate implementation.
  - Evidence: `cargo xtask workflows` regenerated generator-owned workflow pairs and `cargo xtask check-workflows` passed; hand-authored workflows have explicit least-privilege permissions. Focused UI/settings/auto-update/xtask checks and tests passed, version metadata is 1.11.3, and negative product-boundary checks remain clean. `cargo check -p sim --offline` is blocked only by the `webrtc-sys` build script's prohibited non-upstream binary download; the docs tree has no Node/Prettier manifest, so its planned `npx` command is not locally defined.
