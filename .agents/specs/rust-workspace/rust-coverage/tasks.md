...
- [x] 1. Add bounded language-neutral coverage annotations
  - [x] 1.1. Implement generic coverage state and protocol
    ...
    - Evidence: `cargo test -p project --features test-support,cargo-workspace,structured-execution,rust-coverage source_coverage -- --nocapture` and `cargo test -p proto source_coverage -- --nocapture` pass bounded state, hidden-worktree filtering, malformed/stale generation and protobuf round-trip coverage.
  - [x] 1.2. Add generic gutter annotations and compact summary
    ...
    - Evidence: `cargo test -p language_tools source_coverage_summary -- --nocapture` passes summary filtering/navigation/status plus a GPUI editor gutter apply/replace/clear test; the editor module also contains the 100,000-range visible-window budget test.

## Milestone 2: Rust collector adapter

- [x] 2. Integrate an explicit supported collector through Tasks
  - [x] 2.1. Add the authoritative Rust coverage artifact provider
    ...
    - Evidence: `cargo test -p project --features test-support,cargo-workspace,structured-execution,rust-coverage rust_coverage_provider -- --nocapture` passes supported, malformed, unsupported, oversized, visible-path and stale-cancellation cases without tool or network access.
  - [x] 2.2. Register the Rust coverage provider on enabled desktop/headless hosts
    ...
    - Evidence: Enabled `remote_server`/`zed` rust-tools checks, disabled `remote_server`/`zed` checks and `./script/check-rust-tools-feature-boundary` pass with coverage registered only on matching authoritative hosts.
  - [x] 2.3. Add the explicit Cargo Run with Coverage action
    ...
    - Evidence: `cargo test -p cargo_ui cargo_coverage -- --nocapture` passes exact collector argv/artifact planning and bounded setup/failure behavior; workspace lifecycle handling publishes Loading, Error, Stale or Current typed coverage state.

## Milestone 3: Coverage acceptance

- [x] 3. Validate cross-provider, remote and scale behavior
  - [x] 3.1. Add deterministic coverage acceptance fixtures
    ...
    - Evidence: The focused project coverage command passes `source_coverage_acceptance_covers_rust_non_rust_privacy_and_failures`; the generic summary and editor-module budget tests cover bounded visible projection and stale marker replacement.
