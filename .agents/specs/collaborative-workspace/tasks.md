...
- [ ] 32. Enrich the semantic activity feed
  ...
  - [x] 32.2. Map message, presence and job events to ActivityItem
    ...
    - _Discovered contradiction (2026-08-24): canonical `Message` intentionally does not own reply ancestry or resolved actor labels, while a fused `PresenceSnapshot` intentionally has neither an activity projection sequence nor an agent-specific waiting-for-user flag. Fabricating those values in the mapper or adding them to the authoritative aggregates would reverse the approved ownership boundaries. The narrow correction accepts those already-authorized presentation inputs alongside the canonical records, verifies every supplied principal against the aggregate and registers the Multiplayer-only module in the crate root. The original Nostr event, community/principal and community/job identities remain the sole row keys; no transcript, presence, job or reconciliation store is added._
    - _Evidence: 2026-08-24 — added a Multiplayer-only collaboration projector for canonical messages, fused presence snapshots and delegated jobs. Human and owner-attested-agent authors remain explicit; message edits and deletion advance the original lowercase Nostr-event row, current signed provenance stays in the detail handle and replies add a target link without another transcript item. Versioned principal presence maps online, away, offline and waiting-for-user states and permits an in-place wait/resume. Canonical job versions advance one participants-visible delegated-work row from requested/accepted/running to completed, cancelled or failed with actor IDs checked against requester, executor or transition authority. Four focused fixtures pass same-version deduplication, edit/reply reconciliation, human/agent authorship, wait/resume and all three terminal job outcomes. The repository-native release test target passed 4/4 with 445 unrelated tests filtered; a final-source isolated harness also passed 4/4 and warning-denied Clippy. Multiplayer release checking and repository-standard release/all-target warning-denied Clippy passed; Rust formatting, diff hygiene, dependency/inventory/specification gates are recorded in the enclosing checkpoint commit._

  - [x] 32.3. Map Git, workflow, moderation and system events
    ...
    - _Discovered contradiction (2026-08-24): the canonical workflow and moderation aggregates are intentionally introduced only by later Milestones 34 and 36, so this earlier UI leaf cannot import, duplicate or prematurely define their state machines. The narrow correction delegates every already-canonical Git/review/CI input to Task 27.4 and accepts a bounded, non-payload, versioned presentation envelope for the exact Buzz workflow/moderation status vocabulary plus system lifecycle states. It also registers the Multiplayer-only module in the crate root. Future kinds and schema versions remain generic raw references until their authoritative adapter exists; no workflow, moderation, audit, policy or persistence owner is added._
    - _Evidence: 2026-08-24 — added a closed 15-kind platform catalog covering pending, running, approval-waiting, completed, failed and cancelled workflows; opened, resolved and dismissed moderation reports; applied and lifted restrictions; and connected, disconnected, timed-out and failed system states. Consequence weighting raises approval/report intervention to Permission and disconnection/timeout/failure to Error while routine work remains PlatformOperation/Lifecycle. Workflow and moderation events require non-nil community scope, registered source families must match, workflow runs require stable object identities and all known records retain typed links or bounded detail handles. Unknown nonzero schema versions and future kinds map once to Generic with Unknown outcome and the exact raw-source identity, without accepting payload bytes. The catalog fixture passes all 15 registered kinds with insert-plus-duplicate reduction, the consequence/fallback/delegation suite passes 4/4, and direct delegation equals Task 27.4 byte-for-byte. Final-source harness and repository-standard release/all-target warning-denied Clippy, Multiplayer/Standard release checking, Rust formatting, diff hygiene, dependency/inventory and canonical specification gates are recorded in the enclosing checkpoint commit._

  - [x] 32.4. Reconcile cross-source activity updates
    ...
    - _Discovered contradiction (2026-08-24): Task 8.4 correctly rejects a terminal lifecycle or outcome changing for one source identity, but a compatibility timeout and a later authoritative completion are distinct sources whose relationship cannot be discovered safely from labels, actor/session similarity or timestamps. The narrow correction requires a caller-owned correlation ID plus canonical row ID, registers the Multiplayer-only module in the crate root and retains Task 8.4 as the sole per-source ordering authority. No transport, aggregate, persistence, correlation-discovery or renderer owner is added._
    - _Evidence: 2026-08-24 — added an explicit cross-source reconciler with immutable source/canonical correlation fences, exact actor-kind/identity validation, conflict-checked context enrichment and visibility that can only narrow. Each source advances through the canonical reducer; deterministic provenance selection retains timeout and disconnect intervention states while allowing a late authoritative terminal outcome to supersede compatibility inference and a later authoritative running record to resume a disconnected row. Similar actions remain separate unless the caller explicitly correlates them, and every contributing source identity/class remains inspectable. Four final-source release fixtures passed all six three-source reorderings, duplicate and stale frames, timeout, disconnect/resume, late authoritative completion, privacy narrowing and source/canonical collision rejection. Multiplayer release checking and repository-standard release/all-target warning-denied Clippy, Rust formatting, diff hygiene, dependency/inventory and canonical specification gates are recorded in the enclosing checkpoint commit._

  - [x] 32.5. Render complete semantic cards and interventions
...
- [ ] 33. Merge remote-agent providers with Zed remote execution
  ...
  - [x] 33.2. Parse and bound hostile provider output
    ...
    - _Discovered contradiction (2026-08-24): the planned single implementation file cannot compile or be consumed without registering it in `crates/remote/src/remote.rs`. The repository also forbids Smol timers because they escape GPUI's tracked scheduler, while a real provider subprocess legitimately waits on external OS I/O in tests. The narrow correction registers the module, injects `gpui::BackgroundExecutor` for production polling/deadlines and explicitly permits parking only in the three real-process fixtures; no lifecycle state, staging policy or provider configuration is introduced._
    - _Evidence: 2026-08-24 — added a one-process-per-operation boundary with one-MiB request/stdout and 64-KiB stderr caps, GPUI-tracked ten-second info and ten-minute deploy deadlines, a two-second pipe-drain ceiling, cancellable bounded readers and process-group/Windows-job termination. Nonzero exits override parseable stdout; successful responses must be one exact strict info or deploy object with bounded strings and explicit protocol version one. All returned strings and diagnostics are recursively scrubbed of request environment values and recognizable Nostr/support tokens before return. Five focused release tests pass a malformed-response corpus, strict valid info, recursive secret redaction, bounded success, oversized output, nonzero partial JSON and a killed hanging provider. Repository-standard all-target/all-feature warning-denied Clippy also passed; final release, formatting, dependency, specification and inventory gates are recorded in the enclosing checkpoint commit._

  - [x] 33.3. Implement provider deploy/inspect/terminate lifecycle
    ...
    - _Discovered contradiction (2026-08-24): Buzz provider protocol v1 has only `info` and `deploy`; its normative no-management-channel rule forbids inventing provider `inspect` or `terminate` calls. The approved threat model instead requires canonical inspection with explicit staleness/unsupported capabilities and owner-authorized `!shutdown` followed by external confirmation. Implementing cancellation-safe same-bytes negotiation also requires threading a cancellation token through Task 33.2's invoker, registering the new module and adding SHA-256 plus Windows file-lock dependencies, so the narrow correction includes `agent_provider_protocol.rs`, `remote.rs`, `crates/remote/Cargo.toml` and `Cargo.lock`. No job/session/presence binding, secret resolution or undocumented provider operation is added._
    - _Evidence: 2026-08-24 — added current-discovery and explicit-untrusted-approval admission; private non-writable exact-byte staging with SHA-256 and a replacement-denying execution guard; info-before-deploy negotiation on one staged artifact; and an identity-scoped, generation-fenced lifecycle. Repeated deployment returns the existing instance without another invocation. Cancelled, timed-out or failed deploy operations remain explicitly uncertain until reconciliation rather than fabricating rollback. Inspection reports canonical-only state and unsupported provider management, while termination emits one injected authorized shutdown request and requires exact agent-ID confirmation before terminal state. Four focused release scenarios pass staged deploy/digest, inspect and duplicate convergence, in-flight process-tree cancellation, timeout uncertainty and request/duplicate/confirmation termination; all 15 discovery/protocol/lifecycle regressions and repository-standard all-target/all-feature warning-denied Clippy pass. Final release, formatting, dependency, specification and inventory gates are recorded in the enclosing checkpoint commit._

  - [x] 33.4. Bind provider secrets and project configuration
    ...
    - _Discovered contradiction (2026-08-24): the planned standalone Agent module cannot compile or expose its boundary without crate-root registration and direct canonical credential, remote lifecycle, NSEC/secp256k1 and zeroization dependencies, which also update `Cargo.lock`. Task 33.2's redactor recognized NSEC-shaped identities and environment values but not arbitrary raw identity or authorization values, so the narrow correction extends its existing request-derived secret set. No credential value is added to settings, provider configuration, diagnostics or a serialization surface._
    - _Evidence: 2026-08-24 — added a durable deploy template containing only typed scalar provider configuration plus redacted protected-credential/process-environment references. Configuration is bounded to 20 fields/64 KiB, rejects secret-like keys and nested/null values by construction, and preserves numeric values as JSON numbers. Execution-time resolution reads only named credentials, zeroizes owned source buffers, requires a valid canonical 32-byte key whose derived x-only public key matches the selected agent, NSEC-encodes it, and bounds at most 256 environment references to 64 KiB each/512 KiB total. Locked storage, missing/empty/mismatched identity, non-UTF-8 or oversized values fail before a provider call with static redacted diagnostics. Four focused Agent release tests passed locked keyring, missing/empty/mismatched identity, ephemeral identity/environment resolution and typed config/secret separation; all 15 provider regressions passed with raw identity and authorization echo redaction. Warning-denied release Clippy passed for the Agent library and all Remote targets; final formatting, release, dependency, specification and inventory gates are recorded in the enclosing checkpoint commit._

  - [x] 33.5. Bind remote instances to jobs, sessions and presence
    ...
    - _Discovered contradiction (2026-08-24): the planned standalone file cannot expose or compile the adapter without crate-root registration, and it cannot validate that a caller-supplied remote binding matches the canonical execution request because `JobExecutionRequest` previously exposed no read-only identity/scope accessors. The narrow correction registers the module and adds those accessors in `jobs.rs`; it does not change job admission, leasing, transition or session semantics. The runtime retains Task 33.4's reference-only template and resolves its credentials only after the existing coordinator acquires the canonical lease and session claim._
    - _Evidence: 2026-08-24 — added a remote execution adapter that binds one community/job, assigned executor, job-scoped ACP session, canonical signed-presence subject/key and provider lifecycle identity before launch. It delegates lease acquisition, session create/resume and terminal publication to the existing job coordinator; provider deployment occurs only after those claims, repeated terminal execution is a no-op, and disconnect retains the lease for canonical recovery. Cancellation stops the exact session, requests owner-authorized provider shutdown and requires the exact provider agent ID before lifecycle termination is confirmed. Inspection exposes lifecycle/session/deployment state, execution phase, canonical presence and unsupported provider inspection/termination capabilities; signed heartbeat expiry becomes explicit stale/offline status and has no control path. Four focused Agent release tests passed launch/result/exactly-once replay, exact 180-second heartbeat staleness, cancellation/shutdown confirmation and disconnect recovery. Warning-denied release all-target/all-feature Agent Clippy passed; final formatting, release, dependency, inventory and specification gates are recorded in the enclosing checkpoint commit._

  - [x] 33.6. Add provider L1–L3 conformance and cleanup tests
    ...
    - _Discovered contradiction (2026-08-24): `agent` has a normal dependency on `remote`, so a Remote integration target cannot import the Task 33.5 adapter without introducing a cyclic package dependency. The narrow correction keeps job/session/result/cancellation/disconnect execution evidence in Task 33.5's owning Agent tests and drives the lower provider boundary through Remote's real discovery, protocol and lifecycle APIs. The supplied Buzz checkout is an external planning source rather than a CI dependency, so the one test target embeds only the bounded canonical Kubernetes v1 info/full-launch fixture values needed for hermetic conformance; it neither imports Kubernetes code nor claims a live apiserver test._
    - _Evidence: 2026-08-24 — added a Unix hermetic executable-provider target derived from the canonical Buzz Kubernetes wire fixtures and L1–L3 contract. The launch case drives discovery, direct info, same-bytes staged info and deploy, verifies no secret enters either preflight, checks the exact full launch/config shape (owner, enabled presence, digest-pinned image and positive inactivity policy), observes canonical deployment and proves replay invokes no fourth process. Hostile fixtures reject an absent protocol version before deploy, redact NSEC/auth/environment echoes and retain `DeploymentUncertain` after a deploy-stage rejection. The cleanup fixture cancels a hanging deploy, proves both provider and descendant PIDs exit, proves the private staged executable directory is removed, preserves the discovered source and leaves explicit uncertain state. The exact `cargo test -p remote agent_provider_conformance -- --nocapture` command passed 3/3, as did the release form and warning-denied release all-target/all-feature Remote Clippy. Final formatting, diff, dependency, inventory and specification gates are recorded in the enclosing checkpoint commit._
...
- [ ] 34. Port and complete workflows and approvals
  ...
  - [x] 34.2. Add workflow definition, run and step schema
    ...
    - _Discovered contradiction (2026-08-24): Collab's SQLx migration source accepts versioned reversible `.up.sql` and `.down.sql` pairs, not the planned unversioned `collaboration_workflows.sql`, and the task's validation requires executable migration discovery plus rollback. The narrow correction uses one timestamped reversible pair and adds the established Collab integration-test target; no runtime repository or approval table is pulled forward from Tasks 34.9 or 34.6._
    - _Evidence: 2026-08-24 — added six tenant-leading, forced-RLS tables separating stable workflow identities, immutable content-addressed definition versions, optimistic current-version heads, definition-pinned runs, ordered idempotent steps and bounded retry attempts. Community/project scope binds one exact canonical project-group version; composite foreign keys prevent tenant, run, step or definition-version drift; trigger, step and retry operation IDs provide durable replay keys; closed lifecycle/failure classes, temporal checks and byte ceilings bound execution records; source provenance remains explicit throughout. The reverse migration drops dependents in exact order without cascade. Five focused release tests passed for schema/version keys, relations, byte bounds, tenant-policy source, SQLx reversible checksums and the opt-in live PostgreSQL harness; `COLLAB_WORKFLOW_MIGRATION_TEST_DATABASE_URL` was unset, so live database RLS/rollback execution was explicitly skipped. Warning-denied release Clippy passed for the workflow migration target. Collab-wide Clippy separately reached an unrelated pre-existing redundant clone in `buzz_object_git_metadata_import.rs`. Final formatting, diff, dependency, inventory and specification gates are recorded in the enclosing checkpoint commit._

  - [x] 34.3. Implement cron and event trigger admission
    ...
    - _Discovered contradiction (2026-08-24): the Task 4.5 security handoff requires numeric catch-up windows/runs from Task 4.4, but the completed operational-limit registry assigns workflow evaluator, timeout and queue numbers without a schedule catch-up value. Unlimited catch-up would violate WF-03, so this leaf installs a conservative 24-hour scan horizon and sixteen-run retained catch-up ceiling with explicit skipped/pre-window backlog; changing either remains subject to the registry's security-review rule. The planned single-file write also cannot validate the public admission boundary or close the repository's old-definition claim race without the focused integration target, evaluator/cron dependencies and a current-head check, so those narrow supporting writes are included._
    - _Evidence: 2026-08-24 — added tenant- and channel-bound collaboration-event admission with exact common-policy source and current workflow-owner authority, closed message/reaction/diff matching, 4 KiB trigger contexts and a fail-fast 2–32-worker condition pool with a 100 ms wall deadline. False conditions and emoji mismatches produce no run; evaluator errors remain typed failures. Database time exclusively drives normalized cron and epoch-aligned interval fires, while worker skew is surfaced without changing results. Durable cursors initialize without retroactive work, scan at most 24 hours, retain the newest sixteen fires and report skipped/pre-window backlog. Tenant/workflow/source-derived UUIDv5 trigger, run and ordered-step identities make event replays and schedule replicas converge on one exact repository claim; claims now reject superseded definition versions. Five focused release tests pass skew, missed/bounded interval, duplicate event, false condition, revoked source and duplicate cron-replica behavior; repository regressions pass 8/8 including the superseded-version fence and parser regressions pass 8/8. Warning-denied all-feature release Clippy passes for the trigger target. Dependency, canonical inventory and specification gates pass; final formatting and diff-hygiene gates are recorded in the enclosing checkpoint commit._

  - [x] 34.4. Implement secure webhook trigger admission
    ...
    - _Discovered contradiction (2026-08-24): the task line combines inbound webhook admission with DNS/private-range/redirect controls, while the accepted Task 4.5 handoff assigns those network controls to outbound boundary WF-08 and assigns inbound authentication/body/replay to WF-04; the named `agent-workflow.md` explicitly excludes redesigning workflow/webhook policy, whose canonical detail is in `security/workflow.md`. The narrow correction implements both reusable boundaries in the planned webhook module without applying destination-DNS semantics to an inbound caller. Task 4.4 supplies the 1 MiB/10-second transport values but no replay/key/schema numbers or webhook rate/concurrency budget, so this leaf uses a conservative five-minute freshness window, 128-byte idempotency key, 64 scalar fields and 4 KiB values; deployment-owned rate/concurrency admission remains with Tasks 44.3/45.2 rather than borrowing an unrelated service's limits. Module registration, focused integration tests, HMAC/bytes dependencies and the shared stable-run request helper are the minimum supporting writes needed to compile and prove the public boundary._
    - _Evidence: 2026-08-24 — added a header-only HMAC-SHA-256 protocol bound to trusted tenant, immutable workflow ID, timestamp, idempotency key and declared body digest. Credential references resolve late, secret bytes are redacted/zeroed, invalid signatures and revoked owners reject before body polling, signed timestamps have a five-minute database-time window and the ten-second pre-claim timeout cannot leave a partial run. Bodies stream through a 1 MiB ceiling, must match the signed digest and parse as at most 64 bounded scalar fields under a collision-free webhook namespace. Stable source-derived UUIDv5 claims make exact delivery replay return one durable run. The reusable outbound policy requires HTTPS/443, rejects userinfo/fragments, bounds DNS, rejects any mixed private/special-use answer, pins the validated address and builds a hostname-verifying no-proxy/no-redirect client. Seven focused release tests pass signature ordering, body tampering, owner revocation, oversize/reserved fields, timeout, replay and mixed-answer SSRF/redirect policy; trigger regressions pass 5/5, repository regressions pass 8/8 and parser regressions pass 8/8. Warning-denied all-feature release Clippy passes for the focused target. The repository-standard Collab wrapper separately reaches an unrelated pre-existing needless lifetime in `collaboration_event_migration.rs`. Dependency, canonical inventory and specification gates pass; final formatting and diff hygiene are recorded in the enclosing checkpoint commit._

  - [x] 34.5. Implement workflow action execution
    ...
    - _Discovered contradiction (2026-08-24): the task line names an agent-job action, but Buzz and the accepted Task 34.1 definition contract expose exactly seven actions and no agent-job action name, fields, prompt-custody contract or definition-version migration. Inventing an eighth executable YAML shape here would bypass the immutable parser/version boundary and the Task 31.5 job/session authority. The narrow correction therefore includes a typed canonical `RequestAgentJob` command form for the eventual approved adapter but admits execution only for actions actually pinned in the immutable definition; unsupported out-of-band job injection remains impossible and truthful. The planned standalone file also needs module registration, one focused integration target and promotion of the already-locked `zeroize` dependency from test-only to runtime for late-bound outbound secrets._
    - _Evidence: 2026-08-24 — added a tenant/version/run/step validation boundary whose injected authority receives the active lease fence, immutable creator and exact rendered target immediately before every effect. Message, formerly stubbed DM and formerly stubbed topic, and reaction actions submit to one canonical command interface with the durable step operation ID; exact replay returns a duplicate receipt and a mismatched receipt fails. Bounded scalar-only templates fail closed before authority, command outputs are capped at 64 KiB and permission denial produces no mutation. Outbound webhooks reuse the Task 34.4 HTTPS/443, DNS pinning, private-address, TLS-hostname, no-proxy and no-redirect policy; reserve the provider idempotency header; bound body and response to 1 MiB; reject redirects/non-success/oversize/timeout; late-resolve referenced credentials after authorization and destination validation; zero secret material; and persist only bounded structured response metadata. Approval remains a typed suspension input and delays remain bounded/cancellable. Eight focused release tests pass DM/topic completion, reaction dispatch without loopback, stable replay, permission-before-command/DNS/secret ordering, stale-run and unresolved-template rejection, and non-retryable permission/ambiguous failure classification. Focused warning-denied all-feature release Clippy passes. The repository-standard Collab wrapper separately reaches an unrelated pre-existing redundant clone in `buzz_object_git_metadata_import.rs`. Parser 8/8, webhook 7/7, trigger 5/5 and repository 8/8 release regressions pass; dependency, inventory and canonical specification gates pass, with final formatting and diff hygiene recorded in the enclosing checkpoint commit._

  - [x] 34.6. Implement durable approval suspend and resume
    ...
    - _Discovered contradiction (2026-08-24): the planned single-file write cannot make request/wait/outbox or decision/checkpoint/outbox commits durable because Task 34.9 intentionally left approval records and intents to this owner; the minimum supporting writes therefore add tenant-fenced approval/outbox migrations, module registration and a focused repository test. The short validation line names grant/deny only, while accepted security boundary WF-09 explicitly assigns expiry races, self-approval and version mismatch here; those fail-closed paths are included. Buzz's advertised arbitrary role/mention syntax has no complete authorization semantics, so the canonical closed policy admits only any active non-bot member, owner, owner-or-administrator and exact canonical principal forms and rejects every unknown spelling._
    - _Evidence: 2026-08-24 — added a tenant-scoped approval repository that atomically persists one immutable request, waiting step/run checkpoint, fenced lease release and notification outbox intent. A random 256-bit capability is SHA-256-only at rest and zeroed after use; request/note/expiry bounds fail closed. Decisions require `workflows:approve`, resolve scoped-token subjects, lock current active membership, enforce the closed eligibility policy, reject bots and creator self-approval, and compare-and-set the exact waiting run/version/step. Grant completes the approval step and durably queues resume; deny or expiry cancels the step/run and durably queues cancellation. Stable request/outbox IDs and decision operation IDs make exact retries duplicates while racing outcomes conflict. Seven focused tests pass durable restart recovery, grant/deny single-winner behavior, revoked membership, duplicate response, expiry-vs-late-grant, self-approval and waiting-version mismatch; the same checked-in test source ran through a lean harness after the full Collab test-support graph exhausted local disk on unrelated UI/WebRTC dependencies. Focused compilation and migration/RLS assertions pass. The warning-denied repository Clippy wrapper compiles the Collab library before reaching the unrelated pre-existing redundant clone in `buzz_object_git_metadata_import.rs`. Dependency, external-Buzz inventory, specification, formatting and diff-hygiene gates pass; final commit identity is recorded by the enclosing checkpoint._

  - [x] 34.7. Render native workflow and approval state
    ...
    - _Discovered contradiction (2026-08-24): the planned standalone source-file write cannot compile or remain inside the approved Multiplayer-only classification without crate-root registration, an internal `tasks_ui/multiplayer-tools` feature and forwarding from Zed's sole public feature. The minimum supporting manifest and registration writes add no dependency and leave Standard `tasks_ui` unchanged. The service repository types are deliberately not linked into the shared UI crate; the native surface accepts a bounded presentation checkpoint and emits version-fenced intents for the existing authorization owner._
    - _Evidence: 2026-08-24 — added a native GPUI workflow task surface that validates exact definition/run versions, contiguous steps, approval-to-step references and waiting-state consistency before rendering definition metadata, run checkpoints, steps, scheduled retries and historical approval outcomes. Pending grant/deny buttons appear only for a caller-authorized viewer and carry community/workflow/run/approval identity plus expected run version; the service must still reauthorize them. Raw service messages are discarded at the projection boundary, while bounded public failure codes, affected scope and the last trustworthy checkpoint remain visible; unavailable service state exposes a projection retry without minting execution authority. Seven focused `#[gpui::test]` cases pass running, waiting approval, grant, deny, retry, redacted scoped failure and unavailable service. Standard and flagged `tasks_ui` checks, the flagged focused test suite, warning-denied repository Clippy, exact Cargo-feature metadata assertions, collaboration dependency checks, specification validation, formatting and diff hygiene pass. The global Standard-Zed tree-only harness still reports the pre-existing `agent -> collaboration_domain` and `acp_thread -> nostr_compat` paths introduced outside this leaf; this empty-feature UI adapter adds neither dependency and passes both crate configurations. Final commit identity is recorded by the enclosing checkpoint._

  - [x] 34.8. Add workflow replay and crash-recovery scenarios
    ...
    - _Evidence: 2026-08-24 — added six deterministic integration scenarios over the production workflow boundaries. A durable claimer proves that an injected database outage creates no event run and that reconstructed event, missed-cron and signed-webhook admission owners converge on one stable run per source. A dependency that commits a canonical command before losing its receipt proves that executor restart reuses the durable step operation ID, observes one duplicate effect and rechecks current authority before every attempt; revoked authority and a cancelled checkpoint admit no later effect. Timeout failure schedules an exact repository retry that reconstructs as a duplicate, a stale pre-recovery lease cannot checkpoint, and ambiguous delivery remains repair-only. Injected approval-outbox failure rolls back the decision transaction; a grant/deny race leaves one grant and one recovered resume intent, while an independently winning denial fences a late grant and recovers one cancel intent. The same suite asserts bounded public failure text and debug-redacted credentials. All six checked-in scenarios pass through the lean production-Collab harness, and warning-denied Clippy passes. The full Collab test-support target reached its unrelated LiveKit WebRTC download and could not resolve the external host in the restricted environment; the lean harness excludes that dev-only graph while compiling the same production crate and canonical test source. Specification, formatting and diff-hygiene gates pass; final commit identity is recorded by the enclosing checkpoint._
...
- [ ] 35. Port audit chains and usage accounting
  ...
  - [x] 35.2. Add audit-chain persistence schema
    ...
    - _Discovered contradiction (2026-08-24): SQLx discovers reversible migrations only as versioned `.up.sql`/`.down.sql` pairs, not the planned unversioned single file, and the planned write set has no executable owner for the required migration tests. The narrow correction adds the next versioned reversible pair and one focused integration target; it adds no repository, event writer, export service or runtime registration._
    - _Evidence: 2026-08-24 — added an append-only forced-RLS audit schema with exact community/sequence/hash predecessor foreign keys, mutually exclusive genesis/native/imported-Buzz shapes, unique community-local operation IDs and bounded typed-field JSON. One community-keyed head references an existing entry and its database guard permits only a single next-sequence/native-predecessor advance; versioned exporter cursors reference exact entries and cannot regress, change identity or be deleted. Reversible down SQL removes cursors, heads, entries and their guards in dependency order without `CASCADE`. A lean harness compiled the canonical checked-in integration target directly: all five tests passed append-only/link/bridge constraints, one-head/export monotonicity, restrictive tenant-policy structure, exact down order and SQLx-discovered up/down SHA-384 checksums; its optional isolated-PostgreSQL path was skipped because `COLLAB_AUDIT_MIGRATION_TEST_DATABASE_URL` is unset. Warning-denied focused Clippy and the collaboration dependency check passed. The normal `collab` target was blocked before this test by the unrelated LiveKit WebRTC artifact's unavailable network download. Formatting, diff hygiene, inventory and canonical specification validation are recorded in the enclosing checkpoint commit._

  - [x] 35.3. Record security and administrative operations
...
- [x] 36. Port moderation and administration
  ...
  - [x] 36.2. Persist moderation and archive records
    ...
    - _Discovered contradiction (2026-08-24): SQLx requires a versioned reversible `.up.sql`/`.down.sql` pair and the requested behaviors cannot be evidenced without a registered integration test, while Buzz identity archive state is defined in `0001_initial_schema.sql` and community archive state in `0016_community_archival.sql`, outside the planned `0006-*` read. The narrow correction adds migration version `20260824000500`, one focused migration test, and synchronized design/task evidence. It also adds Buzz's closed `nudity` and `malware` report categories to the Task 36.1 domain enum so compatibility rows are not collapsed into `other`; no repository, enforcement, API, notice or archive command behavior lands here._
    - _Evidence: 2026-08-24 — added seven forced-RLS, community-leading tables for immutable reports, audit-attributed actions and one-time resolutions plus version histories for restrictions, personal mutes, identity archives and community archives. Exact target-shape checks, bounded private context, closed action/reason/resolution vocabularies, same-community membership foreign keys and a mandatory action-to-audit-operation foreign key retain privacy and complete attribution. Every row carries bounded source system/record/version/observation provenance with paired integrity evidence. Partial unique indexes allow one current/active state while a retirement-only trigger preserves all prior versions and rejects deletion or mutation. The exact checked-in integration target passed 6/6 in a lean SQLx harness; its live PostgreSQL 14 case rejected a second current restriction, preserved the retired and current versions, rejected historical mutation, exposed zero rows to a foreign tenant and applied the down migration cleanly. Warning-denied all-target Clippy passed for the same target, Cargo metadata confirms native `collab` test discovery, and the focused domain tests passed 5/5 after the compatibility vocabulary correction. The disposable loopback-only PostgreSQL container and database were auto-removed. Formatting and diff hygiene passed; dependency, inventory and canonical specification gates are recorded in the enclosing checkpoint commit. A direct `collab` no-run build was stopped before reaching this test when unrelated GPUI/media dependencies consumed the remaining disk, so no result is claimed for that broader target._

  - [x] 36.3. Enforce moderation at common authorization boundaries
    ...
    - _Discovered contradiction (2026-08-24): the planned standalone policy file cannot compile or expose its decision types without crate-root registration, and neither `Option` nor the existing moderation aggregate can distinguish an authoritative absence from an unavailable or duplicate-current archive/restriction lookup. The narrow correction registers and re-exports the module and adds explicit absent/current/unavailable/ambiguous policy snapshots, including minimal identity/community archive read models matching Task 36.2. It adds no repository, transport adapter, API, notification, archive command or alternate membership policy._
    - _Evidence: 2026-08-24 — added a pure common moderation wrapper that first requires the existing tenant/scope/membership/role/resource decision, then validates every authoritative snapshot against the admitted community and effective principal (including scoped-token subjects). Active bans deny reads and mutations, active timeouts deny mutations only until strict expiry, identity archive removes active authority, and community archive preserves read-only access. Historical reads issue an opaque decision retaining the archived author's exact principal without granting that author active access. Unavailable, ambiguous, malformed and cross-tenant snapshots fail closed behind stable content-free diagnostic codes. Five focused policy tests cover permanent ban, timeout/read/expiry boundaries, identity and community archive behavior, historical attribution, all ambiguity seams and foreign state; the complete domain suite passed 168/168 and warning-denied release Clippy passed. Rust formatting and diff hygiene passed; dependency, inventory and canonical specification gates are recorded in the enclosing checkpoint commit._

  - [x] 36.4. Implement operator moderation APIs
    ...
    - _Writes: crates/collab/src/admin/moderation.rs, crates/collab/src/audit/events.rs_
    - _Validation: API tests cover role matrix, tenant mismatch, stale action and redacted partial failure_
    - _Discovered contradiction (2026-08-24): the planned nested service file cannot compile or be exposed without registering `admin` in the Collab crate, the canonical audit vocabulary had no queue-read operation, and this crate disables library unit-test harnesses. The narrow correction adds the module registrations, one closed `moderation.view_queue` audit operation and a dedicated integration-test target. Because the canonical moderation repository and transport wiring are later owners, this leaf injects one backend contract that receives the authorized command and audit event together instead of introducing a speculative HTTP boundary, transaction owner or second store._
    - _Evidence: 2026-08-24 — added a transport-neutral operator API for bounded queue reads, member report filing, owner/admin report resolution, bans, timeouts and identity archive, plus owner-only community archive. Each endpoint requires its exact scope/action/resource shape and the common moderation decision; aggregate mutations preserve canonical version, target-role and operation guards, archive commands use explicit current-version fences, and every accepted command binds the same actor and operation ID into a canonical moderation audit event before backend execution. Backend responses are tenant/operation/version checked and conflict, unavailable, invalid and partial outcomes expose only stable content-free diagnostics. Four native integration tests cover the least-privilege role matrix, exact audit attribution, cross-tenant rejection, stale actions and redacted partial failure; native test, library check and warning-denied library/test Clippy passed. The repository all-target/all-feature Collab wrapper reached the crate but stopped on the pre-existing `needless_lifetimes` lint in `crates/collab/tests/collaboration_event_migration.rs:16`, outside this leaf. Rust formatting, diff hygiene, dependency, inventory and canonical specification gates are recorded in the enclosing checkpoint commit._

  - [x] 36.5. Add native moderation queue UI
    ...
    - _Writes: crates/collab_ui/src/moderation.rs, crates/collab_ui/src/collab_ui.rs_
    - _Validation: GPUI tests cover empty queue, resolve, denied action, stale report and partial service failure_
    - _Discovered contradiction (2026-08-24): the planned feature file cannot compile or be exposed without multiplayer-gated crate-root registration, while directly importing Task 36.4's Collab service would create a cycle because `collab` already uses `collab_ui` for test support. The narrow correction registers the module and emits a typed transport-neutral action request carrying the canonical report and exact version; the approved adapter remains responsible for constructing and invoking the operator API command. It adds no service dependency, authorization decision, operation-ID generator, persistence or network task._
    - _Evidence: 2026-08-24 — added an owner/admin-only native queue projection with a bounded 200-report snapshot, exact tenant/presentation joins, open-first stable ordering, bounded redacted evidence summaries, target-applicable actions and visible resolution state. Theme-token rows use semantic list/list-item/status/alert/group roles, screen-reader labels, wrapping standard buttons and a scrollable minimum-width-safe layout. Destructive work requires explicit confirm/cancel, emits one version-fenced request and disables concurrent actions; exact completion preserves immutable report fields and advances one matching resolution version, while refresh races, denied actions, invalid responses and partial/unavailable failures retain the queue behind stable operator-safe feedback and an explicit refresh path. Five deterministic timer-free GPUI fixtures passed for empty/role denial, confirmed resolution, inapplicable-action denial, stale refresh and redacted partial failure; warning-denied feature-gated library/test Clippy passed. Repository formatting, diff hygiene, dependency, inventory and canonical specification gates are recorded in the enclosing checkpoint commit._

  - [x] 36.6. Port moderation commands to Zed CLI
    ...
    - _Writes: crates/cli/src/collaboration_moderation.rs, crates/cli/src/cli.rs, crates/cli/Cargo.toml, Cargo.lock_
    - _Validation: golden CLI tests cover output, denied/stale errors, redaction and exit codes_
    - _Discovered contradiction (2026-08-24): the planned feature file cannot compile or remain dependency-isolated without crate-root registration and explicit optional dependencies. The narrow correction registers the module only under a new opt-in `cli/multiplayer-tools` feature and records its Collab, domain and JSON dependencies in the manifest/lockfile; the default CLI graph remains free of Collab. Task 42.8 still owns public command-line parsing and integration into the common collaboration command/output contract._
    - _Evidence: 2026-08-24 — added a redacted `ModerationCliCommand` wrapper over every canonical Task 36.4 report/list/resolve/ban/timeout/identity-archive/community-archive command and an injected synchronous executor boundary suitable for the existing CLI process. Success emits deterministic newline-delimited JSON receipts or bounded authorized report records with private context structurally redacted; stable content-free failures preserve Buzz-compatible usage=1, service=2, authorization=3, unexpected=4 and stale-conflict=5 exit classes, and only definitely unavailable reads/writes are retryable. Five golden unit tests passed for all command verbs/write output, exact list output and evidence redaction, denied/stale diagnostics, the complete error/exit/retry matrix and debug redaction; warning-denied feature-enabled Clippy passed. A no-default-features library check and dependency-tree denial proved ordinary CLI builds contain neither `collab` nor `collaboration_domain`. Repository formatting, diff hygiene, dependency, inventory and canonical specification gates are recorded in the enclosing checkpoint commit._

  - [x] 36.7. Add moderation and administration security scenarios
    ...
    - _Writes: crates/cli/tests/moderation_administration.rs, crates/cli/Cargo.toml, Cargo.lock_
    - _Validation: E2E suite reports no cross-tenant evidence, role bypass or unaudited action_
    - _Discovered contradiction (2026-08-24): placing this integration target under `collab` pulls that crate's complete unrelated development graph, while a direct `collab_ui` development dependency from `cli` pulls the full production UI graph; both exhaust the bounded validation volume before reaching the security scenarios. The narrow correction owns the test beside the feature-gated CLI adapter and compiles the exact production `collab_ui/src/moderation.rs` source module by path with only its direct test dependencies. This follows the existing CLI-to-Collab feature direction, adds development dependencies only, and creates no production dependency edge or duplicate UI implementation._
    - _Evidence: 2026-08-24 — added three end-to-end security scenarios spanning the canonical personal-mute aggregate and common moderation policy, native queue request flow, focused CLI adapter and real operator API. A member-owned mute rejects administrator, foreign-tenant and archived-identity mutation without changing version or history. Member-role, archived-owner and foreign-archive requests fail before backend work; foreign report results and partial writes remain content-free and redact private evidence. The successful owner flow confirms one version-fenced native resolution, executes the exact canonical command through CLI and operator boundaries, captures one same-tenant audit event with the original actor, operation, report and subject attribution, and applies the authoritative response without exposing evidence. The exact target passed 8/8 tests, including the production UI module's five GPUI cases, and passed warning-denied feature-enabled Clippy using a single-job non-incremental build after the ordinary incremental build exhausted disk. Formatting, diff hygiene, dependency, inventory and canonical specification gates are recorded in the enclosing checkpoint commit._
...
- [ ] 37. Port retention, deletion and recovery state machines
  ...
  - [x] 37.2. Implement authoritative retention worker
    ...
    - _Writes: crates/collab/src/retention/worker.rs, crates/collab/src/retention.rs, crates/collab/src/lib.rs, crates/collab/tests/retention_worker.rs_
    - _Validation: worker tests cover interruption, retry, hold, partial batch and idempotent resume_
    - _Discovered contradiction (2026-08-24): the planned nested worker cannot compile or expose its API without Collab module registration, and this crate disables inline library tests, so the narrow write set adds the module roots and a focused integration target. No canonical concrete retention/checkpoint repository is specified by this leaf; the worker therefore injects one tenant-bound backend whose required transaction atomically applies authority actions and advances the checkpoint. Search, cache, push and media cleanup remain owned by Tasks 37.3, 37.8 and 37.9._
    - _Evidence: 2026-08-24 — added a tenant-scoped retention worker with a 1–1000 record bound, strict ordered opaque source positions, validated durable checkpoint hydration, monotonic checkpoint versions, sweep generations and exact cumulative live/archive/delete accounting. Each record is evaluated through the canonical Task 37.1 policy; unavailable authority commits only the preceding safe prefix, active legal hold preserves archive-only visibility and completed sweeps reset their cursor so later deadlines remain eligible. The injected backend contract atomically applies every authority action with the checkpoint advance and distinguishes pre-commit failure from an unknown atomic outcome, allowing restart to reload the committed cursor without duplicate action. Five exact-source integration scenarios passed for pre-commit interruption and exact resume, unknown-outcome recovery, legal-hold preservation, partial-batch authority repair and invalid bound/tenant/checkpoint rejection; warning-denied Clippy passed on the same production and checked-in test sources in a direct dependency harness because the complete unrelated Collab development graph exceeded the bounded validation volume. Rust formatting and the collaboration dependency boundary passed; inventory and canonical specification validation are recorded in the enclosing checkpoint commit._

  - [x] 37.3. Converge search projections after retention
    ...
    - _Writes: crates/collab/src/retention/search_cleanup.rs, crates/collab/src/retention.rs, crates/collab/src/search/indexer.rs, crates/collab/tests/retention_search_cleanup.rs_
    - _Validation: search cleanup tests converge after delayed, duplicate and interrupted delivery_
    - _Discovered contradiction (2026-08-24): the generic Task 22.1 indexer can validate an operation's outcome only after applying it, so a cleanup adapter that checked the returned outcome would reject a non-retention outbox row too late. The narrow correction adds a retention-specific indexer entry point that requires the content-free `RetentionExpired` mutation before document I/O, plus module registration and a focused integration target. It preserves the existing search table, outbox contract and projection checkpoint rather than creating a second index or content store._
    - _Evidence: 2026-08-24 — added a bounded tenant cleanup consumer whose validated durable checkpoint binds an exact retention source position, strictly increasing authoritative outbox sequence, monotonic version and converged count. Each delivery invokes only the retention-specific search indexer path; the indexer rejects every non-retention mutation before document write, commits an empty `RetentionExpired` tombstone with its existing search checkpoint and treats an already-applied or superseded sequence as converged replay. The cleanup checkpoint advances by exact compare-and-set only after projection success, so failure between the two safely replays the fenced outbox row, while an unknown checkpoint outcome reloads without repeating a committed delivery. Five focused cleanup/adapter tests and all four existing search-indexer scenarios passed together (9/9), covering delayed bounded delivery, duplicate replay, pre/post-checkpoint interruption, projection unavailability, tenant/bound/checkpoint rejection and pre-write mutation enforcement. Warning-denied Clippy passed on the exact production and checked-in test sources, as did Rust formatting, diff hygiene and the collaboration dependency boundary; inventory and canonical specification validation are recorded in the enclosing checkpoint commit._

  - [x] 37.4. Define durable whole-community deletion transitions
    ...
    - _Writes: crates/collaboration_domain/src/community_deletion.rs, crates/collaboration_domain/src/collaboration_domain.rs_
    - _Validation: state tests reject partial tenant reuse, stale authority and invalid rollback transitions_
    - _Discovered contradiction (2026-08-24): the standalone aggregate cannot compile or expose its canonical evidence types without crate-root registration and living-design traceability, so the narrow write set includes those files. Buzz's concrete submitted/inventoried/approved/fenced/drained/storage/database/cache/verification stages mix domain safety with executor ownership; this leaf retains their essential requested, verified, reversible, irreversible, completed and failed boundaries while leaving phase checkpoints, leases, retries, storage I/O and community-row mutation to Task 37.5._
    - _Evidence: 2026-08-24 — added a tenant-scoped event-backed deletion aggregate whose bounded immutable transition stream carries archived-community authority version, attributed principal, unique operation and observation time at every state change. Inventory, irreversible-boundary, rollback and logical-absence evidence is represented by nonzero redacted digests; reversible and irreversible evidence binds one exact nonzero fence generation. The sole successful delete path is requested → verified → reversible → irreversible → completed, rollback completes only before the irreversible boundary, failures retain their exact active phase and only closed transient reasons resume. Hydration replays all evidence and rejects cross-tenant fragments, partial/skipped histories, stale archive authority, duplicate operations, malformed versions and invalid rollback without mutation. Five focused release tests passed these paths and failure/recovery behavior; the complete collaboration-domain suite passed 178/178 and repository-standard release/all-target/all-feature warning-denied Clippy passed. Rust formatting, diff hygiene and the collaboration dependency boundary passed; inventory and canonical specification validation are recorded in the enclosing checkpoint commit._

  - [x] 37.5. Implement checkpointed community deletion executor
    ...
    - _Discovered contradiction (2026-08-25): the planned nested-only source cannot compile or expose the executor without Collab module registration and cannot run validation because the library test harness is disabled. Canonical database, search, cache/push, object and Git owners also do not share one atomic transaction or deletion API that this leaf could safely synthesize. The narrow correction adds module registration, one checked-in integration target and an injected backend contract requiring exact fence/phase/checkpoint idempotency plus durable receipts. It adds no second store, concrete destructive adapter, lease authority, recovery command or operator API._
    - _Evidence: 2026-08-25 — added a one-step checkpointed executor that atomically records the aggregate's irreversible transition and initial checkpoint before admitting the fixed database → search → cache → push → object-storage → Git sequence. Checkpoints bind the exact tenant, deletion, aggregate boundary version, fence generation and opaque boundary/evidence digests; every phase advances one version and one ordinal only, and completion requires all six receipts and appends the canonical deleted transition with the final evidence. Backend effects are keyed by the exact deletion/fence/phase/checkpoint attempt and unknown outcomes must remain reloadable, so retries cannot skip or repeat a committed irreversible effect. Four focused integration tests passed pre-commit failure at each phase, post-effect unknown outcome at each phase, unknown terminal completion and rejection of irreversible state without its atomic checkpoint; all runs retained one ordered effect per phase. `cargo check -p collab --lib` and the actual focused Collab test target passed. The repository Clippy wrapper passed warning-denied release/all-target validation of the exact production and checked-in test sources in a disposable minimal harness; the full Collab release invocation exhausted disk while compiling unrelated audio/WASI dependencies before reaching a leaf lint result, so no full-target Clippy result is claimed. All 28 frozen client contracts, complete inventory, dependency, Rust formatting, diff and canonical specification gates passed._

  - [x] 37.6. Implement pre-irreversible recovery and operator status
    ...
    - _Discovered contradiction (2026-08-25): the planned standalone recovery file cannot compile or run focused validation without parent registration and a checked-in integration target, while no concrete owner in the read set can atomically restore pre-quiesce state, release the reversible fence and append the canonical rollback transition. The narrow correction registers the module and injects that exact durable backend operation. It reuses the common authorization request and `communities:manage` policy shape, and adds no repository, HTTP route, UI, new role policy, irreversible undo or second deletion state machine._
    - _Evidence: 2026-08-25 — added an operator API that authorizes before lookup, permits owners/admins to read status and owners alone to restore with the exact current aggregate version and actor-bound authority evidence. Verified, reversible, failed-verified and failed-reversible aggregates restore through one backend operation that must recover pre-quiesce state, release any fence and append the canonical rolled-back transition atomically. Any irreversible checkpoint refuses restore, including checkpoints after each of the six destructive phases. The redacted status exposes only current/last-trustworthy closed stages, completed/total counts, next phase, checkpoint version, closed halt reason and `restore`/`resume`/`none`; tenant/deletion IDs, fence values, digests, principals and operations are absent. Four focused tests passed all reversible states, every at/beyond-boundary checkpoint, irreversible and failed progress/halt mapping, owner/admin/member separation and authorization-before-lookup. Both deletion integration suites passed together (8/8), `cargo check -p collab --lib` passed, and the repository Clippy wrapper passed warning-denied release/all-target validation of the exact production and checked-in recovery test sources in a disposable minimal harness. All 28 frozen client contracts, complete inventory, dependency, Rust formatting, diff and canonical specification gates passed._

  - [x] 37.7. Add mixed-version retention and deletion fault suite
    ...
    - _Discovered contradiction (2026-08-25): the frozen migration directory contains sanitized `global-v0` and `relay-owner-scoped-v1` retention records and version expectations, but no executable legacy worker or deletion checkpoint bytes. The narrow correction validates those exact fixture generations and admits their records through the canonical retention boundary, then composes the public retention worker, deletion executor and recovery API with injected failure owners. It does not recreate retired worker logic, fabricate serialized deletion fixtures, add a production adapter or change any state machine._
    - _Evidence: 2026-08-25 — added an isolated three-scenario fault suite over the actual production seams. Both frozen retention generations were checked for their shared source identity, exact version, two sanitized records and expected migration contract; `global-v0` then survived a pre-commit failure and `relay-owner-scoped-v1` an outcome-unknown commit, with both converging to one archive-policy deletion and one TTL deletion, one effect each and a completed two-delete sweep. A recovery commit returning unknown reloaded as exactly one rolled-back terminal state with no destructive phase effect. Outcome uncertainty injected independently after each database, search, cache, push, object-storage and Git effect reached exactly one canonical deleted state with one ordered effect per phase, no restore and one completion. The new suite passed 3/3 and all three focused retention/deletion suites passed together (11/11). The repository Clippy wrapper passed warning-denied release/all-target validation of the exact suite and composed production sources in a disposable minimal harness. All 30 SQL and 32 desktop-version migration fixtures, all 28 frozen client contracts, complete inventory, dependency, Rust formatting, diff and canonical specification gates passed._
  ...
  - [x] 37.9. Clean retained media references and verified orphans
    ...
    - _Writes: crates/collab/src/retention/media_cleanup.rs, crates/collab/src/retention.rs, crates/collab/tests/retention_media_cleanup.rs_
    - _Validation: tests preserve shared content, remove true orphans and resume safely after object-store failure_
    - _Discovered contradiction (2026-08-24): the planned standalone source requires retention module registration and an integration target. Collab has canonical attachment types and the Task 38.4 object-provider/deletion lease contract but no concrete attachment-reference repository in this leaf's write set. The cleanup therefore defines one injected durable backend contract that must atomically detach the exact tenant/message links, verify the all-tenant reference graph, reserve only true orphan generations and compare-and-set the retention-media checkpoint; object keys, provider credentials and prefix-derived liveness remain outside the boundary._
    - _Evidence: 2026-08-24 — added a bounded tenant cleanup consumer over strictly ordered retention source positions. One stable operation groups up to 32 unique same-message attachment links; the durable backend atomically removes them and returns only generation-fenced Task 38.4 deletion leases for hashes with no remaining tenant reference. Conditional object deletes treat absent bytes as converged, preserve concurrently changed generations for the later global sweep, halt on version artifacts and finalize every lease before advancing the exact checkpoint. Five focused tests pass shared-content preservation, true-orphan removal, reference-commit/object-store-failure retry with the same lease, changed-generation preservation, foreign/regressing pre-mutation rejection and malformed attachment batches. The combined exact-source media/retention harness passes 17/17 with warning-denied all-target Clippy. Repository-standard all-target/all-feature Clippy reaches Collab and stops on the pre-existing `needless_lifetimes` lint in `collaboration_event_migration.rs`, outside this leaf. Formatting, diff hygiene, dependency, inventory and canonical specification gates are recorded in the enclosing checkpoint commit._
...
- [ ] 38. Merge media storage and Blossom compatibility
  ...
  - [x] 38.2. Implement authenticated upload admission
    ...
    - _Writes: crates/collab/src/media/upload_admission.rs, crates/collab/src/media.rs, crates/collab/src/lib.rs, crates/collab/tests/media_upload_admission.rs_
    - _Validation: tests cover unauthorized, wrong tenant, oversize, replay and expired admission_
    - _Discovered contradiction (2026-08-24): the planned single production file requires Collab module-root registration and a focused integration test target. Buzz's HTTP route interleaves trusted-host authentication, request streaming and storage, while this task precedes byte validation and object storage. The canonical boundary therefore accepts an already-established tenant/principal authorization request, reserves only a content-bound operation through an injected durable backend and exposes no upload URL, object key or storage credential; Tasks 38.3 and 38.4 retain byte and storage ownership._
    - _Evidence: 2026-08-24 — added exact `media:write`/tenant/principal/resource authorization before size disclosure or backend access, configurable positive byte and maximum one-hour lifetime bounds, checked admission expiry, and atomic backend reservation semantics. Exact retries recover the original durable admission; changed hash or size, expired/future-dated state, malformed identity and cross-tenant processing fail closed. Five exact-source integration tests cover authorized issuance, unauthorized oversized input, wrong tenant, oversize, exact/conflicting replay and expiration, and pass with warning-denied all-target Clippy. Diagnostics redact the expected content digest and the admission type contains no storage credential or location. Repository formatting, diff hygiene, dependency boundaries, inventory and canonical specification validation are recorded in the enclosing checkpoint commit._

  - [x] 38.3. Implement MIME, magic-byte and content validation
    ...
    - _Reads: projects/buzz/crates/buzz-media/**, crates/media/**_
    - _Writes: Cargo.lock, crates/collab/Cargo.toml, crates/collab/src/media.rs, crates/collab/src/media/validation.rs, crates/collab/tests/media_validation.rs_
    - _Validation: corpus tests cover polyglot, truncated, oversized, hash mismatch and supported files_
    - _Discovered contradiction (2026-08-24): the planned standalone source needs Collab module registration, existing image decoding plus an MP4 parser dependency and a focused corpus target. The normal Collab integration target also activates unrelated LiveKit dev dependencies and cannot reach this focused test in a network-isolated first build, so the same production sources and corpus additionally run in a lean exact-source harness. Storage publication, HTTP deadlines/concurrency and derived variants remain with Tasks 38.4, 38.7 and 45.2._
    - _Evidence: 2026-08-24 — added an admission-revalidated streaming session that writes to an anonymous automatically removed spool, counts bytes independently, hashes every chunk and emits a consume-once validated reader only after exact size/hash/type agreement. Closed magic-byte and structural validators reject trailing/polyglot data and metadata channels for JPEG, PNG, GIF, WebP and ISO MP4; bounded decoders enforce 25-million-pixel images, 1,000 GIF frames and one H.264 video plus at most one AAC track under 600 seconds, 3840×2160, 1,024 top-level boxes, 100,000 total boxes and depth 32. Five corpus tests pass in both the canonical Collab target and an exact-source harness for supported generated files in all five formats, three-byte chunking, readable byte preservation, redacted diagnostics, trailing PNG/ZIP polyglot, truncation, admitted and streamed byte overflow, decoded-pixel overflow, hash/MIME mismatch and corrupt decoder input; warning-denied all-target exact-source Clippy passes. The whole Collab library compiles offline; the canonical target passed after its unrelated prebuilt LiveKit dependency was downloaded on the approved network retry. Repository-standard release/all-target/all-feature Clippy reached Collab and then stopped on a pre-existing `needless_lifetimes` error in `collaboration_event_migration.rs`, outside this leaf. Repository formatting, diff hygiene, dependency boundaries, inventory and canonical specification validation are recorded in the enclosing checkpoint commit._

  - [x] 38.4. Implement tenant-scoped object storage and cleanup
    ...
    - _Writes: crates/collab/src/media/object_store.rs, crates/collab/src/media.rs, crates/collab/tests/media_object_store.rs_
    - _Validation: tests cover range, duplicate hash, tenant fence, missing object and safe orphan cleanup_
    - _Discovered contradiction (2026-08-24): the planned standalone source requires Collab module registration and a focused integration target. Canonical media metadata and the task's write set provide no concrete persistence schema for publication bindings, all-tenant reference proofs or cleanup leases; inventing an object-key/sidecar authority would contradict MED-05 and MED-10. The storage service therefore defines one injected durable authority contract whose implementations must atomically publish tenant bindings, fence publishers during fresh orphan reservations and compare-and-set cleanup checkpoints. Provider-specific S3 credentials, key parsing, deadlines and SQL persistence remain adapter/configuration work, while Task 37.9 removes expired canonical attachment references._
    - _Evidence: 2026-08-24 — added a provider-neutral object store that consumes only validated readers, conditionally writes shared content-addressed bytes, verifies exact existing hash/size/MIME/generation evidence and publishes a tenant binding only afterward using the admission's stable operation and timestamp. Exact retries converge and equal hashes across two communities share bytes without sharing authority. Authorized reads require exact `media:read`, tenant/resource membership and canonical binding resolution before provider access or range validation; one checked inclusive range is capped and exact-length verified, foreign paths never reach storage and a missing published object fails without fabricated bytes. Cleanup uses strict ordered pages, a durable version/checkpoint cursor, a grace period longer than admission lifetime and atomic fresh all-tenant reference leases; shared and young bytes survive, only a generation-matched verified orphan is removed, and unknown taxonomy, incomplete/versioned state or indeterminate authority halts. Five focused integration tests and two source unit tests pass duplicate/replay, range, tenant fence, missing object, shared/young/orphan, unknown-taxonomy and interrupted-delete resume cases; the complete Collab library passes 23/23 and the combined exact-source media harness passes 12/12 with warning-denied all-target Clippy. Repository-standard all-target/all-feature Clippy reaches Collab but stops on pre-existing `needless_lifetimes` and `redundant_clone` lints in unrelated integration targets. Formatting, diff hygiene, dependency, inventory and canonical specification gates are recorded in the enclosing checkpoint commit._

  - [x] 38.5. Implement thumbnail and native rendering integration
    ...
    - _Discovered contradiction (2026-08-24): `media` is already a dependency of GPUI, so the planned low-level crate cannot import GPUI elements or platform views without a cycle. An unconditional collaboration-domain dependency would also leak multiplayer code into Standard Zed through that same edge. The narrow correction exposes a typed, platform-neutral presentation contract behind a non-default `media/collaboration` feature whose image formats map directly to the existing native renderer, and adds only the crate-root, manifest, lockfile and focused low-level FFI safety writes required to compile and close MED-08. Link fetching, renderer concurrency/lifecycle limits and final GPUI composition remain Tasks 45.2 and 38.7 rather than granting this adapter network or UI authority._
    - _Evidence: 2026-08-24 — added a non-default collaboration adapter with tenant-fenced native presentation plans for canonical JPEG, PNG, WebP, GIF, MP4 and bounded legacy audio, with supported thumbnail/poster selection, explicit missing/unsupported-variant fallback, user-controlled animation, disabled audio/video autoplay, inert unsupported/active downloads and bounded control/bidirectional-safe accessibility labels. HTTPS link cards reject credentials, expose the normalized origin and never auto-navigate. Thumbnail derivation rechecks exact size/hash/type, bounds input bytes/pixels/decoder allocation and output dimensions/bytes, applies orientation and re-encodes a metadata-free JPEG before returning an unpublished canonical descriptor; failure cannot mutate metadata. CoreMedia/CoreVideo helpers now return errors for failed status or null framework objects rather than asserting or constructing invalid references. Six focused feature-enabled renderer tests pass thumbnail success/failure and stored-variant selection, active unsupported media, accessibility labels, missing thumbnail/poster fallback, user-controlled audio/video and inert links; warning-denied all-target/all-feature Media Clippy passes, the default Media crate remains collaboration-free, and the downstream macOS crate compiles with runtime shaders. Dependency, inventory and canonical specification gates are recorded in the enclosing checkpoint commit._

  - [x] 38.6. Implement the Blossom compatibility adapter
    ...
    - _Discovered contradiction (2026-08-24): `nostr_compat` is an approved protocol-compatibility layer whose dependency boundary forbids storage and transport, so the planned adapter cannot call Collab's concrete admission/object-store services without collapsing that boundary. The narrow correction verifies and normalizes Blossom wire authority into typed canonical intents, paths, ranges, descriptors and closed errors; Task 38.7 owns the service wiring and differential proof. The standalone planned file also needs module registration plus the direct URL parser dependency/allowlist and lock update required for credential-free public-origin validation. This worktree omits `projects/buzz`, while an existing protocol unit test uses a compile-time Buzz fixture; full validation temporarily exposes the user-supplied source checkout at that expected path without retaining a worktree change._
    - _Evidence: 2026-08-24 — added bounded standard/base64url `Nostr` envelope decoding and cryptographic BUD-11 kind-24242 verification with exact upload/get verbs, expiration/freshness, tenant-host server tags and blob/server scopes. Both `/upload` and `/media/upload` produce the same stable event-derived canonical operation. Bare-hash, canonical-extension and `.thumb.jpg` aliases resolve only against same-tenant canonical metadata; missing variants and mismatched extensions return compatible not-found errors. Single absolute/open/suffix ranges retain Buzz semantics while capping output to the injected Task 38.4 limit, multipart falls back to a full response, and invalid ranges map to 416. Descriptor projection uses a validated HTTPS tenant origin (HTTP only for loopback), omits absent legacy metadata, and preserves private immutable caching, restrictive CSP, `nosniff` and inert disposition for non-inline content. Six focused tests pass signed and tampered/expired/wrong-scope authorization, both upload aliases, original/bare/thumbnail/download resolution, capped ranges, canonical descriptor JSON and closed protocol errors. The complete protocol crate passes 81 unit plus four integration tests and repository-standard warning-denied Clippy. Dependency, inventory and canonical specification gates are recorded in the enclosing checkpoint commit._

  - [x] 38.7. Add old/new media compatibility conformance
    ...
    - _Discovered contradiction (2026-08-24): the planned Collab integration test must call Task 38.5's feature-gated native presentation contract, but Collab has no production dependency on `media` and adding one would invert the established low-level boundary. The narrow correction adds only a test-only `media/collaboration` dependency and its lockfile edge; the production dependency graph remains unchanged. This worktree also omits `projects/buzz`, so the inventory gate temporarily exposes the user-supplied Buzz checkout at the expected path without retaining a symlink. The repository Clippy wrapper mandates a fresh release/all-target closure and exhausted the host volume before reaching the target; a warning-denied exact debug target completed after removing only reproducible partial release artifacts._
    - _Evidence: 2026-08-24 — added an independent projected-observation checker and a live consolidated trace over signed kind-24242 upload/get authorization, both `/upload` aliases, canonical member admission, seven-byte-chunk validation, exact SHA-256/size/MIME and 4×2 decoder metadata, content-addressed publication, tenant-authorized inclusive range reads, canonical/bare/thumbnail URL resolution, Blossom descriptor projection and native thumbnail/accessibility presentation. The Buzz oracle and consolidated path match exactly for original and variant identities, returned range bytes, rendering metadata, wrong-server authorization denial and closed 401/404/415/416/503 errors. Coverage requires upload, download, render, authorization-failure and protocol-failure observations; mutation checks reject a missing download seam and changed upload hash. The focused integration target passes 2/2 and warning-denied exact-target Clippy passes. The repository release/all-target wrapper reached the media-enabled graph without a lint but stopped when its fresh closure exhausted disk. Formatting, diff hygiene, dependency boundaries, source inventory and canonical specification validation are recorded in the enclosing checkpoint commit._
...
- [ ] 39. Consolidate huddles, voice, TTS and transcription
  ...
  - [x] 39.2. Implement the approved native huddle transport adapter
    ...
    - _Discovered contradiction (2026-08-24): the planned wording implies that `audio` can import the concrete LiveKit client, but the established graph is `livekit_client -> audio`; reversing it would introduce a cycle. The narrow correction adds the canonical-domain dependency and a LiveKit-facing connect/callback/resource contract to `audio`, plus crate-root registration and its generated lockfile edge. Existing LiveKit callers consume that contract without changing transport ownership or adding a second room implementation._
    - _Evidence: 2026-08-24 — added a native huddle transport adapter that deterministically derives one LiveKit room and generation-bound participant identity, issues nonzero attempt-scoped connection requests and rejects wrong-room, stale-attempt, stale-generation, unknown-participant and duplicate-roster callbacks. Connect and reconnect reconcile only the current canonical roster, retain the authenticated local principal, report missing remote participants and cap native admission at the 25-participant compatibility ceiling. Reconnect state retains the same room/identity for at most 90 seconds and exact deadline expiry releases resources before permitting a fresh attempt. A bounded registry owns room, capture, playback, track, subscription, timer and credential resources; cancellation, reconnect expiry, canonical terminal state and adapter drop close every resource in reverse order, continue after individual failures and surface structured cleanup counts/failure kinds. Five focused tests cover connect identity/roster fencing, reconnect and exact expiry, idempotent participant callbacks, authorized terminal cleanup with a failing resource and cancel/drop release-once behavior. The complete audio suite passes 14/14 and repository warning-denied release/all-target/all-feature Clippy passes; formatting, diff, dependency, inventory and specification gates are recorded in the enclosing checkpoint commit._

  - [x] 39.3. Implement the Buzz audio compatibility adapter
    ...
    - _Discovered contradiction (2026-08-24): the planned write surface names only the leaf adapter, but `collab` had no huddle module and disables unit-test harness generation for its library. The narrow correction registers `crates/collab/src/huddle.rs` from the crate root and places the required contradiction suite in `crates/collab/tests/buzz_audio.rs`; no production dependency or second room implementation is added._
    - _Evidence: 2026-08-24 — added a generation-bound Buzz v1/v2 audio gateway over the canonical huddle and an injected native bridge. Missing versions default to v1, the first successful admission pins the room version, unsupported or mixed versions fail before admission, and the adapter caps compatibility peers at 25 with reusable indices and monotonic roster revisions. V1 forwards bounded Opus payloads; v2 validates the fixed eight-byte big-endian header and nonempty payload, clamps invalid dBov to -127, preserves flags and never bridges invalid or oversized frames. Join, leave and end use canonical lifecycle commands; attach failure retains canonical state for exact retry, detach/close failures remain visible, and terminal cleanup drains every peer before closing the native generation. A monotonic migration/drain-only/retired window rejects new legacy creation during drain while preserving existing retries and media, and forbids retirement until compatibility peers are empty. Five independent integration tests pass for lifecycle equivalence, v2 framing/invalid input, default/pinned/unsupported versions, compatibility-window draining and bridge-failure recovery. The focused warning-denied release/all-feature Clippy target passes. The repository all-target wrapper reaches `collab` but is blocked by a pre-existing `needless_lifetimes` warning in `crates/collab/tests/collaboration_event_migration.rs`; formatting, diff, dependency, inventory and specification gates are recorded in the enclosing checkpoint commit._

  - [x] 39.4. Implement microphone, speaker, mute and push-to-talk controls
    ...
    - _Discovered contradiction (2026-08-24): the planned write surface omits the feature-gated Collab UI module registration and the direct dependency needed to bind controls to Task 39.2's exact native callback scope. The narrow correction registers the module, adds `audio` only to the existing `multiplayer-tools` feature and records the generated lockfile edge; Standard Zed remains unchanged. The canonical Collab UI test target currently stops before this module because its unrelated test-support graph exposes `RemoteConnectionOptions::Mock` to a non-exhaustive production match in `remote_connection.rs`, so focused GPUI behavior validation uses the checked-in source through a minimal harness._
    - _Evidence: 2026-08-24 — added a GPUI huddle-control view bound to one canonical huddle identity and Task 39.2 native room/attempt scope through an injected transport contract. Transactional microphone mute, speaker mute, push-to-talk enable/hold and device switches update visible confirmed state only after native success; permission, missing-device, unavailable-connection and stale-attempt failures retain both huddle and prior controls while exposing a typed retry and device-default fallback. Bounded input/output identities and labels reject malformed or duplicate inventory, absent selections fail before transport access, selected-device loss attempts the system default without ending or rebinding the huddle, and PTT opens capture only while held. Five checked-in `gpui::test` cases pass through an exact-source harness for mute/speaker/PTT, failed switch plus safe retry, device-loss fallback, permission-denied retry and wrong-native-room rejection. The real feature-enabled Collab UI library type-check and warning-denied Clippy pass against the actual audio/native-scope crate; warning-denied all-target exact-source Clippy also passes. Formatting, diff, dependency, inventory and canonical specification gates are recorded in the enclosing checkpoint commit._

  - [x] 39.5. Integrate local TTS voice models
    ...
    - _Discovered contradiction (2026-08-24): the planned service file must be registered by the audio crate root to be usable, while importing Buzz's ONNX runtime would invert the approved native audio/model boundary and make deterministic cancellation tests depend on a 165 MiB model. The narrow correction exports the service and exposes a verified, cancellable request/completion contract for an injected local inference worker; it adds no runtime, network or storage authority._
    - _Evidence: 2026-08-24 — added a generation-bound native TTS service that admits only the exact pinned April Pocket model manifest and Buzz's content-identified bundled/imported voices. Imported metadata is fenced to the canonical hash-derived key/file plus 25 MiB, 2–30 second and 8–96 kHz bounds. Present canonical human/agent speakers can enqueue eight ≤50-token, ≤4,096-character chunks; requests carry only verified identities, attribution and an atomic cancellation handle. A single in-flight request, nonzero attempt fences, cancellation/drop draining and stale-completion rejection prevent old work from publishing. Returned 24 kHz mono PCM is nonempty, finite, full-scale and capped at 30 seconds. Missing model, invalid/corrupt model or voice, queue/output overflow and backend failure are typed and visible without ending the huddle; failed synthesis retries under a fresh fence and preserves attribution. Five exact-source tests cover the named missing-model, invalid-voice, cancellation, output-bound and recovery scenarios, including corrupt manifest/path and imported-voice identity/bound checks; warning-denied all-target Clippy passes in the isolated exact-source harness. Formatting, diff, dependency, inventory and canonical specification gates are recorded in the enclosing checkpoint commit. The canonical audio test build downloaded its pinned WebRTC input but exhausted the host's remaining 13 GiB while compiling the unrelated full GPUI/Wasmtime graph, before reaching the audio crate; generated artifacts were cleaned and the deterministic harness provides executable evidence for the checked-in module._

  - [x] 39.6. Implement transcription-to-channel projection
    ...
    - _Discovered contradiction (2026-08-24): the planned nested file requires registration in the existing non-`mod.rs` huddle root, and independently mutating the canonical message and huddle aggregate would permit a final transcript to diverge from its `HuddleTranscriptReference`. The narrow correction exports the module and gives the injected canonical store one compare-and-set operation containing both the channel projection and optional final reference; it adds no second message, huddle or persistence authority._
    - _Evidence: 2026-08-24 — added a huddle-generation-bound transcript projector that validates bounded text, nonzero source revision/generation, model revision, content-free track digest, exact segment/message/participant identity and time ordering before policy or storage. A caller-injected current consent/channel authorizer runs before store reads and supplies a retention ceiling that can tighten but never extend. Contiguous partial revisions update one stable projected channel message; the final transition is terminal and atomically carries the exact domain-validated huddle transcript reference. Independent nonzero projection versions, stable operation IDs and injected compare-and-set outcomes support idempotent replay, visible pre-commit/unknown failures and safe retry without duplicate links. Text is bounded to 16 KiB/4,096 characters, custom diagnostics expose only its byte count, and redaction or exact-deadline expiry removes content while preserving bounded provenance. Five exact-source tests pass multi-partial/final projection, unavailable-store failure plus exact recovery, redaction/privacy, retention boundary/idempotency and unauthorized/stale/post-final rejection; warning-denied all-target Clippy passes on the same production module. Focused Rust formatting, diff, collaboration dependency, Buzz inventory and canonical specification gates are recorded in the enclosing checkpoint commit. The isolated harness avoids Collab's unrelated full development graph, which had already exceeded the bounded validation volume during the immediately preceding audio task._

  - [x] 39.7. Add the native huddle workspace UI
    ...
    - _Discovered contradiction (2026-08-24): the planned nested file also requires feature-gated registration in the Collab UI crate root, while directly depending on `collab` would invert the existing UI/service dependency direction. The narrow correction registers the module and consumes canonical huddle state plus lifecycle operations through injected snapshot/service contracts, preserving one domain authority and the existing dependency boundary._
    - _Evidence: 2026-08-24 — added a GPUI native huddle workspace bound to one canonical identity and confirmed snapshot, with exact canonical participant inventory, roles, presence, moderation, bounded reactions, lifecycle state and bounded transcript rows. Partial transcript rows may precede linkage, while final/redacted/expired rows must reproduce a canonical `HuddleTranscriptReference`; duplicate segments, foreign participants and mismatched links fail closed, and custom diagnostics omit transcript text. The view composes Task 39.4's audio controls, renders accessible lifecycle/participants/reactions/transcript surfaces and keeps network, voice-model and transcript availability independently retryable. Start, join, leave, end and exact scoped retry replace visible state only after the injected service returns a matching canonical postcondition, retaining the last coherent snapshot on failures. Five source-identical `gpui::test` cases pass start/join/leave/end, device failure, voice-model failure/retry, network failure/retry and partial/final/redacted transcript display; warning-denied all-target Clippy passes in the same harness using the production huddle-controls module and a lightweight native-scope stub. The focused harness avoids the previously recorded unrelated Collab UI test-support failure around the non-exhaustive `RemoteConnectionOptions::Mock` match. Formatting, diff, dependency, inventory and canonical specification gates are recorded in the enclosing checkpoint commit._

  - [x] 39.8. Add native and Buzz huddle interoperability scenarios
    ...
    - _Discovered contradiction (2026-08-24): the prescribed Collab UI integration-test location cannot name Task 39.3's real Buzz gateway without a test-only dependency on the `collab` crate. The narrow correction adds only that dev dependency; production dependencies and the native/UI/service direction remain unchanged._
    - _Evidence: 2026-08-24 — added an independent E2E observation trace that drives the actual native transport adapter and Buzz v1 compatibility gateway against fixed canonical identities, operations and timestamps, then requires byte-for-byte-equivalent canonical huddle event streams. Both paths produce exactly start, speaker join, transcript link, speaker leave and explicit end, with the Buzz path additionally proving bounded Opus forwarding through its injected native bridge and terminal generation close. The same trace composes the production GPUI audio controls and workspace to prove device failure retains mute, exact retry commits, a final linked transcript is displayable, network recovery does not mutate canonical huddle state, and a failed leave retains the prior snapshot before exact retry commits the transport-produced leave. A separate checker-bite test rejects missing/reordered observations and incomplete canonical lifecycle output. Both tests pass, and warning-denied all-target Clippy passes, in a source-identical harness containing the production native adapter, Buzz adapter, huddle controls, workspace and integration test; the harness avoids rebuilding Collab's unrelated server dependency graph. Focused formatting, diff, dependency, inventory and canonical specification gates are recorded in the enclosing checkpoint commit._

- [x] 40. Port pairing into canonical credential storage
  ...
  - [x] 40.2. Implement the ephemeral pairing relay
    ...
    - _Discovered contradiction (2026-08-24): the planned write names a source file in a service package that does not yet exist, so it cannot compile or ship without a package manifest, root workspace membership/dependency registration and lockfile entry. The narrow correction creates that single executable package and its mechanical workspace metadata alongside the prescribed main source._
    - _Evidence: 2026-08-24 — added a loopback-default Axum WebSocket relay mounted only at `/pair`, preserving Nostr `REQ`/kind-24134 `EVENT`/`CLOSE` compatibility while providing immediate EOSE and no history. Ingress is capped at 96 KiB and accepts one exact `#p` filter, canonical kind, one exact recipient tag, valid event ID/Schnorr signature, ±120/5-second freshness and a bounded base64 NIP-44 v2 Task 40.1 frame. The in-memory core admits at most 128 connections for exactly 120 seconds, 32 messages/eight event attempts per connection, eight queued recipient frames, twelve deliveries per recipient and 1,024 replay IDs. It routes only to one live recipient; absent/ambiguous targets, invalid input, exhausted quotas and full queues fail without reserving replay state. Socket termination, explicit disconnect and expiry remove subscriptions/ciphertext queues, while state contains no private key, decrypted payload, durable record or storage interface. Six focused tests pass expiry cleanup, connection capacity, full-queue no-reservation, replay suppression, disconnect cleanup and zero durable/private-key state; warning-denied all-target `pair_relay` Clippy passes. Formatting, diff, dependency, inventory and canonical specification gates are recorded in the enclosing checkpoint commit._

  - [x] 40.3. Import paired identities into Zed credentials
    ...
    - _Discovered contradiction (2026-08-24): the planned standalone write cannot consume Task 40.1's typed payload or become reachable without Multiplayer-gated module registration, a direct optional `nostr_compat` dependency and its lockfile edge. The narrow correction adds only that feature-gated wiring. Zed's canonical local signer can take custody only of an `nsec`; the other extensible NIP-AB payload types fail closed instead of being reinterpreted as local signing keys._
    - _Evidence: 2026-08-24 — added a Multiplayer-only pairing credential adapter that borrows a dual-confirmed Task 40.1 payload, admits only an exact whitespace-free bounded `nsec`, derives its actual x-only identity and selects the existing community/account/profile/public-key credential namespace. New keys flow through the shared protected-store kernel for canonical raw-secret write, cryptographic read-back and cleanup; an existing matching destination is idempotent, while conflicting, malformed, unsupported, locked, interrupted and unverified outcomes return redacted closed errors without touching any prior credential or consuming the source payload. Four focused tests pass verified canonical import/read-back, a write that commits then reports interruption with destination reconciliation, a locked keyring with zero writes, and source preservation across success/idempotence/unsupported failure. The complete Multiplayer credential suite passes 22/22 and focused warning-denied library Clippy passes. The repository-standard release/all-target Clippy attempt was blocked only by local disk exhaustion while compiling unrelated GPUI dependencies; cleaning regenerable artifacts and rerunning the scoped warning-denied graph passed. Formatting, diff, dependency, inventory and canonical specification gates are recorded in the enclosing checkpoint commit._

  - [x] 40.4. Port the pairing CLI
    ...
    - _Discovered contradiction (2026-08-24): the authoritative Buzz interop binary exposes only `source`, `target` and `test-vectors`, whereas this approved task explicitly requires `create`, `receive`, `cancel` and `status`; it also accepts raw `--nsec` input and optional `--show-secret` output that conflict with canonical Zed credential custody and the leaf's no-secret-logging gate. The narrow correction implements the approved four-verb machine contract, retains `source`/`target` only as aliases, takes receive QR material on standard input and leaves identity secrets with the injected credential owner. A reachable typed module also requires Multiplayer-gated registration plus direct optional `nostr_compat`/`thiserror` dependencies and lock edges._
    - _Evidence: 2026-08-24 — added strict Clap parsing for `pair create --relay ...`, QR-on-stdin `pair receive`, `pair cancel <session-id>` and `pair status <session-id>`, with canonical `PairingRelayUrl`/`PairingQr` admission, four-relay and 128-byte opaque-session bounds, and discoverable legacy `source`/`target` aliases. The injected executor boundary returns canonical session states, an explicit sensitive create QR, verified import receipts or closed errors without owning entropy, WebSockets, SAS decisions, credentials or session persistence. Deterministic newline-delimited JSON output exposes no received identity secret; custom command/execution diagnostics redact QR and output bodies. Six golden tests pass create/alias, receive/alias verified import, exact expiry conflict output/exit 5, cancel, status and QR/identity-secret non-disclosure. Focused feature-enabled tests and warning-denied all-target Clippy pass; the Standard library check passes with the new dependencies disabled. Formatting, diff, dependency, inventory and canonical specification gates are recorded in the enclosing checkpoint commit._

  - [x] 40.5. Add desktop/mobile/CLI pairing interoperability
    ...
    - _Discovered contradiction (2026-08-25): the prescribed integration test cannot execute Task 40.2's binary-only relay or Task 40.3's feature-gated credential importer from Collab without a callable library target and narrow test-only dependency edges. The correction exposes one router factory, compiles the exact existing relay source as its descriptive library target, and adds only the direct Collab development dependencies needed by the independent test; the deployed binary, protocol/storage authority and Standard production graph are unchanged. Task 3.2 freezes Buzz mobile and CLI versions but intentionally has no Zed desktop row, so the matrix binds desktop to the checked-in Zed package version while retaining the frozen Buzz versions and mobile version-one negotiation contract._
    - _Evidence: 2026-08-25 — added a table-driven six-direction desktop/mobile/CLI interoperability matrix over current Zed desktop plus frozen Buzz mobile/CLI versions. Every path parses the canonical QR through the frozen mobile version-one boundary, establishes both live subscriptions against the production ephemeral WebSocket relay, transports signed kind-24134 offer/SAS/payload/complete events, verifies equal six-digit SAS values and transcript binding, transfers an exact nsec payload and reaches dual terminal completion with the derived imported public key. Separate cases prove relay-level duplicate-event rejection without a second delivery, encrypted cancellation delivered exactly once, exact 120-second expiry and corrupt/version-two QR failure without state advance. The real canonical credential importer additionally survives a store write that commits before losing its receipt: it removes the unverified destination, preserves an unrelated prior credential and succeeds on exact retry with cryptographic read-back. All four checked-in tests pass in 0.11 seconds with loopback access; all twelve relay library/binary tests pass, relay warning-denied all-target Clippy passes, and warning-denied Clippy passes for the exact production/test sources in a lean harness. The full Collab Clippy graph was attempted first and exhausted the remaining disk while materializing unrelated editor/agent metadata after reaching this target; only the regenerable worktree target cache was then cleaned. Rust formatting and diff hygiene pass; dependency, inventory and canonical specification gates are recorded in the enclosing checkpoint commit._
...
- [x] 41. Port relay mesh and shared-compute scheduling
  ...
  - [x] 41.2. Implement compute advertisements and expiry
    ...
    - _Reads: projects/buzz/desktop/src/features/mesh-compute/**, projects/buzz/desktop/src-tauri/src/mesh_llm/{mod,discovery,identity,coordinator}.rs, crates/remote/src/mesh/protocol.rs_
    - _Writes: crates/remote/src/mesh/{advertisement,protocol}.rs, crates/remote/src/mesh.rs, .agents/specs/collaborative-workspace/{design,tasks}.md_
    - _Validation: tests cover spoofed resource, expiry, revoke, duplicate peer and bounded advertisement size_
    - _Discovered contradiction (2026-08-25): the planned Buzz frontend directory exposes only toggle/presentation state and a model string, while the signed owner/endpoint binding, member intersection, target deduplication and freshness behavior that this compatibility leaf must preserve live in the adjacent frozen Tauri mesh runtime. The narrow read correction includes those factual sources. The planned standalone advertisement file also cannot compile or require proof of Task 41.1 admission without parent-module registration and a protocol-issued authenticated-peer receipt; the narrow write correction adds only that opaque receipt/accessor and registration. It does not add a listener, compute runtime, eligibility decision, resource lease or scheduler._
    - _Evidence: 2026-08-25 — added endpoint-signed, versioned compute advertisements bound to Task 41.1's authenticated deployment/community/owner/membership/endpoint/runtime receipt. Available and draining records require one to 16 bounded immutable model identities with exact approved artifact digests and capability subsets plus complete finite nonzero CPU, memory, accelerator, cache, network, context, token, wall/idle, concurrency and queue claims within local ceilings. Validation requires all deployment/community/user/device opt-ins and permits only initial self-hosted trust classes; third-party policy cannot be constructed. Records expire within 60 seconds. The bounded registry treats them only as active hints, excludes draining state, removes at the exact expiry boundary, prevents one endpoint from inflating capacity through multiple devices and retains bounded sharing-generation revocation floors so later records in a revoked generation stay unavailable. Six focused tests passed spoofed resource/artifact rejection, exact expiry, revocation fencing, duplicate endpoint, encoded/model bounds and independent opt-in failure; the full 53-test `remote` library suite passed. Repository-prescribed release/all-target/all-feature Clippy for `remote` passed with warnings denied. Canonical spec, full inventory, collaboration dependency-boundary, Rust formatting and diff-hygiene gates passed._

  - [x] 41.3. Integrate approved mesh scheduling with remote jobs
    ...
    - _Reads: crates/agent/src/{jobs,remote_execution}.rs, crates/remote/src/mesh/advertisement.rs, .agents/specs/collaborative-workspace/{decisions/adr-006-shared-compute.md,security/mesh.md}_
    - _Writes: crates/remote/src/mesh/scheduler.rs, crates/remote/src/mesh.rs, .agents/specs/collaborative-workspace/{design,tasks}.md_
    - _Validation: scheduler tests cover eligibility, fairness, capacity, revoked peer and no silent fallback_
    - _Discovered contradiction (2026-08-25): `agent` already depends on `remote`, so importing Task 33.5's desktop `JobExecutionAuthority` and lease types into `remote::mesh` would create a dependency cycle and would couple shared-compute admission to a UI/runtime-owned adapter. The narrow correction defines an injected, state-free mesh scheduling authority whose single acquisition operation must atomically compose the canonical job-version/executor and locally enforced resource leases; service/RPC composition remains the canonical implementation owner. The standalone scheduler also requires parent-module registration and living design/task traceability. It does not add a listener, serving runtime, prompt transport, job repository, local/cloud fallback or production activation._
    - _Evidence: 2026-08-25 — added a community-scoped content-free scheduler whose bounded requests fix canonical job/version/attempt/executor identity, requester, immutable model/capability, explicit context class, complete resource demand and lease nonce/timing before selection. A versioned audit-attributed policy bounds weights, requester/community queues, concurrency, third-party exclusion, aging and owner-reserved slots. Fresh advertisements are filtered by tenant, trust, model, capability, context, digest, expiry and resource policy, then an injected canonical authority rechecks current consent/membership/delegation/trust/model/sandbox/local capacity before one exact atomic executor/resource lease. Selection permanently binds the attempt before acquisition, so capacity races retry only that node and a revoked node remains visibly unavailable even while another eligible advertisement exists. Five focused scheduler tests passed eligibility denial, deterministic weighted fairness, authoritative capacity recovery, local capacity/owner reservation, revoked peer and no-silent-fallback behavior; the full 58-test `remote` library suite passed. Repository-prescribed release/all-target/all-feature Clippy for `remote` passed with warnings denied. Canonical spec, full inventory, collaboration dependency-boundary, Rust formatting and diff-hygiene gates passed._

  - [x] 41.4. Add shared-compute availability and failure UI
    ...
    - _Depends on: 41.3_
    - _Reads: crates/remote/src/mesh/{advertisement,scheduler}.rs, crates/collab_ui/src/**_
    - _Writes: Cargo.lock, crates/collab_ui/Cargo.toml, crates/collab_ui/src/{collab_ui,mesh_compute}.rs, .agents/specs/collaborative-workspace/{design,tasks}.md_
    - _Validation: GPUI tests cover available, stale, revoked, no capacity and provider failure states_
    - _Discovered contradiction (2026-08-25): the planned standalone UI file cannot compile against or expose Task 41.3's scheduler types, or remain absent from the Standard dependency graph, without Multiplayer-gated crate-root registration plus an optional `remote` dependency and generated lock edge. Deterministic endpoint identities in focused tests also require a test-only `iroh-base` edge. The narrow correction adds only this mechanical reachability and a reusable unmounted read-only view; it does not activate mesh compute, mount a product surface, mutate trust/policy, retry, reroute or own scheduler state._
    - _Evidence: 2026-08-25 — added a Multiplayer-only GPUI shared-compute status view that accepts exact scheduler-selected provider identity plus a bounded, nonempty, control-free execution-location label. Available state renders eligible slots, location, immutable model and approved trust class; queued and running states retain the selected provider. Stale, revoked, no-capacity, unapproved-provider, policy-denied and provider-failure states render distinct semantic status/alert copy, explicitly preserving no-fallback, no-reroute and unknown-outcome no-retry behavior. Provider/display mismatches and invalid availability fail closed, and the view exposes no action or policy control. Six focused GPUI tests passed eligible capacity/location, stale, revoked, no capacity, unapproved provider and unknown provider failure. The warning-denied Multiplayer feature library Clippy, default-feature release library check, Standard depth-one dependency exclusion, Rust formatting and diff hygiene passed; focused GPUI execution required temporary removal of nine stale downstream test-support cfg attributes caused by the pre-existing non-propagating `RemoteConnectionOptions::Mock` feature graph, all restored before validation and absent from this leaf._

  - [x] 41.5. Add mesh partition, security and load scenarios
    ...
    - _Reads: projects/buzz/perf/**, crates/remote/src/mesh/**, .agents/specs/collaborative-workspace/security/{mesh,operational-limits}.md_
    - _Writes: crates/remote/tests/mesh_compute.rs, .agents/specs/collaborative-workspace/{design,tasks}.md_
    - _Validation: approved mesh test command meets partition-recovery, resource and fairness budgets_
    - _Discovered contradiction (2026-08-25): the planned integration test file can exercise the complete public mesh boundary without production changes, but Task 41.5's operational budgets and threat assignments are normative only in the mesh/operational-limit records and the living-documentation gate requires the resulting conformance contract and evidence to remain traceable. The narrow read/write correction includes those records. It does not add a load service, synthetic production metric, listener, scheduler policy, execution runtime or deployment activation._
    - _Evidence: 2026-08-25 — added five composed integration scenarios across the public protocol, advertisement, scheduler and executor/resource-lease APIs. A canonical-authority partition rejects gossip without burning its nonce, exact recovery succeeds, replay then fails and membership revocation blocks the next frame. A 1,024-frame authenticated gossip load remains within the bounded replay window and rejects a duplicate. Endpoint-advertised capacity cannot widen a lower local resource ceiling; an atomic capacity race remains bound to one provider and recovers there, while revoking a bound provider with another active candidate proves no fallback. The fairness load fills the approved 256-entry community queue with sixteen equal-weight requesters at sixteen requests each and a 100 ms admission cadence: every service prefix differs by at most one, p95 logical wait is 24.3 seconds against the 30-second budget, the final normalized max/min service ratio is 1.0 against the 1.25 limit, and all 256 leases are released with no retained authority resource. The focused five-test command and full 66-test `remote` package suite passed; repository-prescribed release/all-target/all-feature Clippy passed with warnings denied._
...
- [ ] 42. Merge agent-first collaboration commands into Zed CLI
  ...
  - [x] 42.4. Port project and repository CLI commands
    ...
    - _Discovered contradiction (2026-08-25): the planned legacy/contract read scope cannot type or revalidate project groups, hosted repositories, permissions or ref manifests without the canonical project-group, repository-registry and object-store contracts, and the new adapter is unreachable without parent-module registration. The narrow correction adds only those canonical reads, the `collaboration::git` declaration and living evidence; it adds no parser, signer, authorization reducer, repository host, object transfer, clone implementation, persistence or Standard-build edge._
    - _Evidence: 2026-08-25 — added fourteen Multiplayer-only project, repository, permission, ref and clone request variants carrying canonical identifiers, validated project values, hosted coordinates/authority, exact principals/permissions, operation IDs and optimistic versions through one injected executor. Project results are revalidated through the canonical signed aggregate before stable JSON output; hosted versus external authority, ref manifests and bounded clone coordinates retain explicit shapes, and every mutation returns one resource/version receipt. Five focused golden tests passed project grouping/identity, clone/ref output, Zed/external hosting, exact permission forwarding and the complete stable error/mismatched-result matrix. Repository-prescribed release/all-target/all-feature CLI Clippy passed with warnings denied._

  - [x] 42.5. Port agent, persona, memory and job CLI commands
  ...
  - [x] 42.10. Port channel CLI commands
    ...
    - _Discovered contradiction (2026-08-25): the planned legacy/contract read scope cannot express canonical channel types and lifecycle, optimistic mutations, membership roles/status, channel-target invites, templates or attributed topic/canvas metadata without the completed collaboration domain, and the new adapter is unreachable without parent-module registration. The narrow correction adds only those canonical reads, the `collaboration::channels` declaration and living evidence. It adds no parser, relay client, authorization reducer, persistence, template resolver, membership transaction, invite token generator or Standard-build edge._
    - _Evidence: 2026-08-25 — added typed channel get/list/create/update/archive/restore, member list/add/role/remove, channel invite create/redeem/revoke, template create and topic/canvas get/set variants through one injected executor. Commands preserve canonical type/visibility/lifecycle, role/status/scope, tenant identity, operation attribution and optimistic versions; update clear semantics fail closed and invite credentials remain redacted except for explicit creation output. Five focused golden tests passed all six channel types, all five roles, canonical channel invite targeting, template/topic/canvas values, archive receipts, credential redaction, invalid updates, mismatched outcomes and the complete stable error/exit matrix. The full 45-test collaboration CLI suite and repository-prescribed release/all-target/all-feature CLI Clippy passed with warnings denied._

  - [x] 42.11. Port DM, read-state and social CLI commands
    ...
    - _Discovered contradiction (2026-08-25): the planned legacy/contract read scope cannot represent validated encrypted DM transport, owner-only read/reminder replicas, canonical forum/emoji projection or body-redacted feedback state without the completed domain types and NIP-44 compatibility codec, and the new adapter is unreachable without parent-module registration. The narrow correction adds only those canonical reads, the `collaboration::social` declaration and living evidence. It adds no plaintext encryption/decryption, key handling, relay client, authorization reducer, reminder scheduler, search indexing, persistence, forum projector, emoji resolver, feedback workflow or Standard-build edge._
    - _Evidence: 2026-08-25 — added DM list/open/participant/leave and validated ciphertext send, read status/mark read/mark unread, reminder list/create/update/dismiss, forum list/vote, emoji list/set/remove and feedback submit/status/update variants through one injected executor. Commands retain canonical tenant/resource identity, versions, limits, replacement heads, cursors/votes, emoji and feedback transitions while debug output redacts every payload. Ciphertext is never emitted; decrypted reminder content appears only for explicit successful owner reads; feedback status omits bodies; privacy and authorization denials share one non-leaking class. Six focused golden tests passed encrypted DM forwarding/redaction, DM lifecycle, owner-scoped manual unread state, reminder disclosure, forum/emoji projection, feedback body exclusion, mismatched outcomes and the complete stable privacy/error/exit matrix. The full 51-test collaboration CLI suite and repository-prescribed release/all-target/all-feature CLI Clippy passed with warnings denied._

  - [x] 42.12. Port patch, pull-request, issue and review CLI commands
    ...
    - _Discovered contradiction (2026-08-25): the planned legacy/contract read scope cannot express canonical NIP-34 patch/PR/issue/status records, versioned review decisions or bounded CI suites without the completed collaboration domain and NIP-34 compatibility codecs, and the new adapter is unreachable without parent-module registration. The narrow correction adds only those canonical reads, the `collaboration::review` declaration and living evidence. It adds no Git transport, event signer, relay client, repository mutation, authorization reducer, review persistence, merge executor, CI runner or Standard-build edge._
    - _Evidence: 2026-08-25 — added patch create/get/list, PR open/update/get/list, issue create/get/list, repository-status publication, review get/open/revision/comment/decision/readiness and CI status get/publish variants through one injected executor. Commands retain canonical repository/event/commit and review/branch identities, optimistic version/revision/head fences, operation attribution, bounded list limits and sanitized CI fields while command and outcome debug output redacts payloads. Reads emit stable kind-checked NIP-34, review, approval, readiness and CI projections; mismatched record kinds fail closed, stale state maps to conflict and denied approval maps to authorization. Six focused golden tests passed patch/PR/issue/status output, review comments/approvals, CI runs, stale/denied handling, mismatched kinds and the complete stable error/exit matrix. The full 57-test collaboration CLI suite and repository-prescribed release/all-target/all-feature CLI Clippy passed with warnings denied._
...
- [ ] 43. Repoint web, mobile and admin clients with version negotiation
  ...
  - [x] 43.2. Implement common client feature negotiation endpoints
    ...
    - _Discovered contradiction (2026-08-25): the planned single-file write cannot expose a library module, mount the pre-tenant HTTP route in the Collab executable, or provide executable endpoint coverage because the Collab library disables its unit-test harness. The narrow correction adds only crate-root registration, the process-level route mount, one integration test target and living design evidence; it does not add tenant, resource, authorization, persistence or alternate protocol authority._
    - _Evidence: 2026-08-25 — added one bounded policy-version-one negotiation owner for all six published clients, the exact Collab 0.44.0/schema 20260824000500 service window, six client-facing protocol versions and sixteen closed feature IDs. The process mounts `POST /v1/collaboration/compatibility` before database setup; requests are capped at 16 KiB and contain only bounded client/version/protocol/feature data. Supported writes alone return a private admission value; read-only features, unknown clients/features/protocols, incompatible client/service/schema versions and malformed duplicates cannot produce one. HTTP returns the published status-426 `upgrade_required` envelope, while Nostr returns `OK` or terminal `CLOSED` with `upgrade-required:`/`read-only:` reasons, and invalid client identities are not reflected. Six endpoint tests passed supported HTTP route and admission, read-only denial, incompatible-version 426, unknown-feature denial, Nostr supported/upgrade frames, runtime-schema rejection, malformed duplicate rejection and oversized-identity redaction. The Collab executable check, focused warning-denied release Clippy, formatting, dependency boundary, 28 frozen client contracts, complete inventory, diff check and canonical specification validation passed. Repository-prescribed all-target Clippy also passed the changed library and compatibility target before stopping on the pre-existing redundant clone at `crates/collab/tests/buzz_object_git_metadata_import.rs:116`, which this focused leaf does not modify._

  - [x] 43.3. Repoint web invite and authentication flows
    ...
    - _Discovered contradiction (2026-08-25): the planned target `clients/web` does not yet exist, while dependency 18.5 intentionally stops at canonical invite-domain behavior and exposes no HTTP handler to import. This client leaf therefore defines and tests only the dependency-free browser adapter and exact canonical endpoint contract under its planned path, using injected transport responses; it does not invent server persistence, authorization, NIP-98 verification or redemption authority. The browser-compatible test remains beside the adapter because no consolidated web package/test harness exists until the later client compatibility leaves._
    - _Evidence: 2026-08-25 — added a dependency-free browser authentication boundary for Buzz web 0.1.0 that preserves `/invite/$code` and the frozen byte-exact `buzz://join` relay/code/policy-receipt handoff. The adapter performs the common compatibility preflight before canonical invite resolution, community join-policy lookup and redemption, accepts only HTTPS or loopback service origins, caps request time and response size and projects only bounded community/policy/result fields. Redemption invokes NIP-07 only after a supported write result, rejects missing, denied, mutated or structurally invalid signer output, normalizes the signed event and binds kind 27235 to the exact URL, POST method, SHA-256 payload and fresh nonce. Expired, exhausted, revoked and invalid invites have closed user guidance, while HTTP 426 exposes the declared minimum/maximum version and stops before signer or invite access. Four browser-Web-API contract tests passed preserved route/deep-link, canonical resolve/policy/redeem and exact payload authentication; expired/exhausted responses; signer denial without a redeem request; and minimum-version rejection before signer/resource access. Exact frozen TypeScript 6.0.3 strict type-check, Node syntax checks, all 28 frozen client contracts, inventory, dependency, formatting, diff and canonical specification gates passed._

  - [x] 43.4. Repoint web repository browsing and downloads
    ...
    - _Discovered contradiction (2026-08-25): the planned client target does not yet have a consolidated web harness, and dependency 25.5 intentionally exposes a transport-independent smart-HTTP read service rather than global browser metadata, tree or blob routes. This leaf therefore defines and tests only the dependency-free browser adapter and exact mocked canonical endpoint contract, plus the shared NIP-98 helper's required GET form. It does not invent route resolution, hosted-object storage, ref selection, server authorization or a new HTTP authority; the later service transport owner must bind these paths to the canonical Git service._
    - _Evidence: 2026-08-25 — added a dependency-free repository browser client that preserves `/repos`, repository detail, nested blob and download URLs while carrying canonical hosted-repository UUIDs behind the route discriminator. Every operation performs the common `repository-browse` read preflight; public reads omit identity and private reads use kind-27235 NIP-98 bound to the exact canonical GET URL and method. The adapter caps JSON, list, tree, preview and download sizes; validates UUIDs, NIP-34-compatible route IDs, refs, paths, tree entries, object IDs, content lengths and ETags; requires exact requested `Content-Range` values for bounded range downloads; and maps denied and missing repositories or objects to the same closed errors. Five browser-Web-API tests passed public/private browsing and exact private authorization, blob identity, range download and mismatched-range rejection, generic 403/404 absence, and every frozen repository/detail/blob/download URL. The prior four invite/authentication browser tests remained green. Exact frozen TypeScript 6.0.3 strict type-check, Biome 2.4.16 format/check, all 28 frozen client contracts, complete inventory, dependency, Rust formatting, diff and canonical specification gates passed._

  - [x] 43.5. Add web client compatibility E2E
    ...
    - _Discovered contradiction (2026-08-25): the consolidated `clients/web` tree has no package-level browser runner or deployable canonical invite/repository handlers, while the published and frozen matrices contain exactly one supported web version, Buzz 0.1.0. The narrow test-only correction composes the real dependency-free migrated adapters with the frozen 0.1.0 route/deep-link contract through an injected canonical service boundary. It does not claim live server, persistence, authorization, browser-rendering or release-environment coverage; those remain with their service and release owners._
    - _Evidence: 2026-08-25 — added a browser-Web-API compatibility matrix for the single closed supported version, proving Buzz 0.1.0 and the migrated adapter agree on the client ID/version, invite and repository routes, consent fields, exact `buzz://join` handoff and canonical preflight contract. The supported E2E resolves an invite and join policy, redeems it with the four frozen NIP-98 tags, lists a canonical hosted repository and reads its validated blob while retaining repository detail/blob/download URLs. Separate matrix cases prove HTTP 426 rejects an incompatible write before signer, tenant/resource access or mutation; read-only rejects before signing or mutation; and incompatible repository download stops before object lookup. All four compatibility cases and the prior nine focused auth/repository browser cases passed. Exact frozen TypeScript 6.0.3 strict type-check and Biome 2.4.16 format/check passed across all migrated web source and test files; all 28 frozen client contracts, complete inventory, dependency, Rust formatting, diff and canonical specification gates also passed._

  - [x] 43.6. Repoint mobile auth, community and collaboration APIs
    ...
    - _Discovered contradiction (2026-08-25): the planned `clients/mobile` target and deployable canonical mobile HTTP routes do not yet exist, while frozen secure storage identifies communities with a generated local UUID/relay origin and may retain an `nsec`; it contains no canonical Zed account, community or profile identity that can be safely inferred. The narrow correction defines and tests only the pure-Dart mobile adapter, injected bootstrap/authorization contract and versioned local binding store under the planned path. Legacy import requires an already-resolved canonical binding, stores only an opaque credential reference and never copies the raw signing key. It adds no server route, credential resolver, authorization, domain reducer, Flutter provider or release packaging authority._
    - _Evidence: 2026-08-25 — added a canonical mobile binding keyed by exact decimal service-account ID, community UUID and profile UUID, retaining the verified Nostr public key, HTTPS/loopback service origin, WSS/loopback relay and opaque device credential reference. Activation negotiates the frozen Buzz mobile 0.0.0+1 protocols and invite/community/collaboration features before authorizer or tenant access, signs the exact canonical account/community bootstrap URL through an injected credential owner and accepts only an exact matching bounded bootstrap. One serialized schema-v1 local document write retains bindings, active selection and bounded channel/message/DM/media identity-revision-digest records; switching changes only the active canonical tuple, exact binding/record retries deduplicate, newer revisions replace in place, stale records cannot overwrite and same-revision conflicts fail closed. Three focused tests passed minimum-version rejection before authorization/storage, two-account/two-community switching without cross-scope record loss and repeated legacy/binding/bootstrap application without duplicated records. Exact Dart 3.11.4 formatting and static analysis passed with no issues; all 28 frozen client contracts, complete inventory, dependency, Rust formatting, diff and canonical specification gates also passed._

  - [x] 43.7. Migrate mobile background reconnect, push and pairing
    ...
    - _Discovered contradiction (2026-08-25): no consolidated Flutter host yet binds the planned platform directory, and ADR-005's exact APNs body contains only the fixed reconnect alert—not lease generations, entity IDs or content that a client could independently authorize. The narrow correction implements and tests pure-Dart lifecycle admission around injected connection, current-lease, compatibility and NIP-AB ports. It does not invent push content or client-side server authority, change the two approved APNs profiles, duplicate pairing cryptography, import credentials, open sockets or register a platform provider._
    - _Evidence: 2026-08-25 — added a serialized lifecycle coordinator retaining Buzz's five-second background grace, no-op short pause, disconnect after a sustained background transition and immediate reconnect plus canonical refetch on foreground or foreground network restoration. Fixed APNs parsing accepts only `buzz-ios-production`/`buzz-ios-sandbox` lease profiles and the exact reconnect-only body, rejects content-bearing variants, refuses revoked or expired local lease views and disconnects again after a background wake fetch. Added bounded NIP-AB version-one offer admission with lowercase source/secret checks, nonzero secret, one-to-four unique WSS/loopback relays, retained recovery mode, exact six-digit SAS, injected crypto/socket/protected-store operations, receive-identity matching and the canonical 120-second expiry fence. Five focused tests passed short/long background transitions, network reconnect, fixed wake plus revoked lease, successful and expired pairing and upgrade rejection before pairing transport. The prior three mobile data/storage regressions remained green, and exact Dart 3.11.4 formatting plus full migrated-mobile static analysis passed with no issues. All 28 frozen client contracts, complete inventory, dependency, Rust formatting, diff and canonical specification gates also passed._

  - [x] 43.8. Add mobile collaboration compatibility E2E
    ...
    - _Discovered contradiction (2026-08-25): the planned test-only write scope could not exercise read-state or search because the closed mobile entity contract exposed only channels, messages, DMs and media, and the migrated compatibility request did not advertise the already-required push or pairing surfaces. The narrow correction adds only those two projection identity kinds and the two feature names to the existing pure-Dart client contract. It adds no content fields, server API, transport, authorization, storage schema, Flutter host or device integration authority._
    - _Evidence: 2026-08-25 — added a dependency-free mobile compatibility harness over the migrated data, storage, lifecycle and pairing boundaries plus the frozen Buzz 0.0.0+1 version. The supported path negotiates channels, messages, DMs, read state, search, media, push and pairing; persists only canonical entity identity/revision/digest state; reloads all six projections offline; and confirms stored state excludes private keys, message bodies, ciphertext, search queries and media bytes. Both approved production-device and sandbox-simulator profiles passed five-second background disconnect, foreground reconnect/refetch, revision replacement without duplication and fixed wake-only reconnect/fetch/disconnect. A frozen version-one NIP-AB offer completed with the exact 120-second fence, while version two failed parsing and upgrade-required activation stopped before identity authorization, resource lookup, storage and pairing transport. Exact Dart 3.11.4 formatting/static analysis and all four matrix scenarios passed; the prior three mobile data/storage and five lifecycle/pairing regressions remained green. All 28 frozen client contracts, complete inventory, dependency, Rust formatting, diff and canonical specification gates also passed._

  - [x] 43.9. Repoint admin web resources to canonical APIs
    ...
    - _Discovered contradiction (2026-08-25): Collab has transport-neutral moderation and community-deletion operator APIs plus one deployment-wide Prometheus endpoint, but it does not yet bind the complete community/member/invite/moderation/archive/deletion/tenant-metrics resource set to canonical HTTP handlers. The narrow correction defines and tests a client-owned injected transport contract and does not claim those routes are deployable, recreate role or tenant authorization, mint credentials, execute domain transitions or repurpose the unscoped service metrics endpoint._
    - _Evidence: 2026-08-25 — added a dependency-free Buzz admin 0.1.0 data adapter for canonical community, member, invite, moderation report, archive, deletion recovery and bounded tenant-metrics resources. Every operation negotiates exactly `moderation` or `admin-lifecycle` through the common compatibility endpoint before operator authorization or resource lookup; lifecycle calls carry `communities:manage`, moderation queue/resolution calls carry `moderation:manage`, and writes preserve canonical operation IDs and optimistic versions. Three focused resource tests passed the complete least-privilege request sequence, stable stale-write mapping with the exact version/operation body, redacted deletion progress and a 426 minimum-version failure before any scoped request. Node 24.13 and Bun 1.3.6 executed the suite, TypeScript 5.9.3 strict checking passed for the production contracts and client, and Prettier 3.7.4 reported exact formatting. All 28 frozen client contracts, complete inventory, dependency, Rust formatting, diff and canonical specification gates also passed._

  - [x] 43.10. Migrate admin authentication and failure UX
    ...
    - _Discovered contradiction (2026-08-25): the retained Buzz admin application relies only on implicit same-origin browser credentials and exposes no canonical operator login, session metadata or credential-resolution contract, while task 43.9 deliberately leaves the service HTTP binding injected. The narrow correction composes an injected session provider and opaque credentialed transport plus framework-neutral browser failure state; it does not mint or persist a session, expose a bearer, trust client metadata as server authority, change the frozen React host or invent a login endpoint._
    - _Evidence: 2026-08-25 — added a canonical operator-session transport that leaves compatibility preflight unauthenticated, then admits resource access only for a current non-expired canonical community/principal session with an owner/admin role, the exact requested `communities:manage` or `moderation:manage` scope and a tenant-matching resource path. Only an opaque credential reference reaches its injected resolving transport. Added fixed-copy failure views for access denial, expiry/sign-in, stale reload, unknown outcome, upgrade and service retry plus a resource controller that retains last trustworthy data during partial failure. Four browser-contract tests passed role denial and expiry before credential use, partial-service retention, successful retry and two tenant/resource canaries absent from rendered state; the three prior resource tests remained green. TypeScript 5.9.3 strict checking and Prettier 3.7.4 formatting passed for the production session and failure contracts. All 28 frozen client contracts, complete inventory, dependency, Rust formatting, diff and canonical specification gates also passed._

  - [x] 43.11. Add admin web compatibility E2E
    ...
    - _Discovered contradiction (2026-08-25): the frozen admin manifest covers only the retained reports/feedback routes and same-origin failure UX, while tasks 43.9–43.10 expose injected client contracts because the full canonical HTTP/session host remains unbound. The narrow correction composes those production client boundaries with a test-owned canonical service implementing only client-visible behavior and consumes all four frozen Buzz admin contracts; it does not claim live endpoint, database, role-policy, credential, Playwright-host or release-environment coverage._
    - _Evidence: 2026-08-25 — added a frozen-manifest-driven Buzz admin 0.1.0 compatibility harness over the canonical resource and operator-session clients and the sole published Collab 0.44.0 service version. The supported matrix provisions community/member state, creates/lists/revokes an invite, lists/resolves a report, enforces owner-only archive, restores a reversible deletion and reads post-transition metrics with one matching compatibility preflight per resource request. Five E2E scenarios passed the full supported lifecycle, server-side admin-role denial, cross-tenant rejection before credential access, recovery projection without authority evidence, unsupported-version rejection before session/resource access and all four retained reports/feedback route and UX fixtures. Node 24.13 and Bun 1.3.6 passed the five scenarios and all seven prior admin regressions; TypeScript 5.9.3 strict checking and Prettier 3.7.4 formatting passed the composed harness. All 28 frozen client contracts, complete inventory, dependency, Rust formatting, diff and canonical specification gates also passed._

- [x] 44. Consolidate deployment, configuration and release pipelines
  ...
  - [x] 44.2. Consolidate local and Compose environments
    ...
    - _Evidence: 2026-08-25 — the current and prior immutable image configurations rendered successfully, with equal canonical/runtime dependency bindings, required health ordering, stable named volumes, a loopback-only object endpoint and rollback limited to recreating Collab with `--no-deps`; the contract checker also rejected drift from the documented digest gate and readiness verification. A real isolated Docker Compose smoke run brought all BusyBox dependency stand-ins healthy, completed object-store initialization, brought the collaboration stand-in healthy, passed the final readiness probe and removed every scoped container, volume and network. The first two live attempts exposed health-document and shell trap-lifetime defects; both were corrected before the passing run. The complete inventory, all 28 frozen client contracts, dependency, Rust formatting, shell syntax, diff and canonical specification gates passed._

  - [x] 44.3. Consolidate Helm and Kubernetes resources
    ...
    - _Discovered contradiction (2026-08-25): the existing Collab executable advertises `migrate` in its usage diagnostic but does not dispatch that command, while Task 44.4 explicitly owns the ordered migration jobs, checksums, compatibility ceiling and halt behavior. This leaf therefore establishes and renders the bounded pre-install/pre-upgrade hook with a separate DDL Secret and current schema floor, but does not claim that a production migration can run until Task 44.4 supplies and validates the executable contract._
    - _Evidence: 2026-08-25 — added a disabled-by-default Zed collaboration chart with immutable production/rollback image selection, equal canonical/runtime configuration bindings, existing runtime and separate DDL Secrets, TLS Ingress and Gateway API routes, HPA CPU/WebSocket bounds, disruption budget, topology spread, RWX/existing Git storage, private monitoring, and scoped runtime/migration NetworkPolicies for database, Redis, object and optional push/pair/mesh traffic. The focused Helm suite linted default, active, production and rollback values; rendered the production-like Gateway, separate TLS Ingress and optional push/pair/mesh profiles; verified fixed probe/resource/security bounds, a public-route allowlist that excludes metrics, schema hook annotations, credential separation, storage preservation and migration suppression during rollback; and rejected missing secrets, mutable production images, unattached production ingress, missing Redis, unsafe multi-replica RWO storage, absent/incompatible rollback digests and unknown values. Shell syntax, Ruby parsing, all 28 frozen client contracts, complete inventory, dependency, Rust formatting, diff and canonical specification gates passed._

  - [x] 44.4. Add canonical schema migration jobs and contracts
    ...
    - _Writes: deploy/collaboration/migrations/*, deploy/collaboration/charts/collaboration/{README.md,values.yaml,values-production.yaml,values.schema.json,templates/_helpers.tpl,templates/migration-job.yaml,tests/render.sh}_
    - _Validation: dry run applies, resumes, detects checksum drift and rolls back before compatibility boundary_
    - _Discovered contradiction (2026-08-25): Task 44.3 could only render a placeholder hook against the Collab binary's advertised but undispatched `migrate` command, while this leaf's planned migration-only write could not make that chart handoff executable. The narrow correction replaces only that hook's image/argument contract with the separately pinned DDL artifact, retains the distinct migration Secret and leaves runtime ownership unchanged. Buzz's one-off pre-1321 cutover is not folded into the canonical manifest because it has no down path and explicitly makes the mandatory source snapshot its rollback authority._
    - _Evidence: 2026-08-25 — packaged all 20 canonical Collab SQL pairs in strict order with exact forward/backward SHA-256 values and ceiling `20260824000500`; added an unprivileged PostgreSQL runner with closed URL options, operator-only control/history state, a session advisory lock, per-migration transactions, committed-prefix resume, idempotent re-entry, durable checksum/history/execution halts and a monotonic service-activation rollback floor. The Helm hook now requires a separately published migration digest in production and rejects a chart/artifact ceiling mismatch before database access. A disposable PostgreSQL 17 lifecycle applied six migrations, resumed the remaining fourteen, performed a no-op replay, rolled moderation down and back up, sealed the ceiling and rejected rollback across it; a second database detected modified source bytes, recorded `checksum_drift` and rejected resume. Static manifest mutation, chart render/lint, database URL, required-ceiling, shell, formatting, dependency, inventory, frozen-client, diff and canonical specification gates passed, and all disposable Docker resources were removed._
  ...
  - [x] 44.6. Consolidate build, signing and release workflows
    ...
    - _Reads: projects/buzz/.github/workflows/**, .github/workflows/**, script/**, docs/collaboration/compatibility.md, .agents/specs/collaborative-workspace/fixtures/clients/manifest.json, crates/{zed,collab}/Cargo.toml, services/{pair_relay,push_gateway}/Cargo.toml, tools/buzz_compat/Cargo.toml, deploy/collaboration/**_
    - _Writes: .github/workflows/collaboration-release.yml, script/collaboration-release-contract, .agents/specs/collaborative-workspace/{design,tasks}.md_
    - _Validation: release-contract dry run verifies packages, signatures, versions, notices and no production publish_
    - _Discovered contradiction (2026-08-25): the task wording could imply duplicating Zed's established platform signing workflow or publishing a runnable push-gateway image, but native Zed packaging already has a canonical release owner and `push_gateway` currently exposes a library/deployment contract rather than a binary. The narrow correction retains native platform release ownership, packages only existing executables as binaries, packages push and migrated clients as source contracts, and emits ephemeral release candidates without production publication or deployment authority._
    - _Evidence: 2026-08-25 — added a manual release-candidate workflow and deterministic contract for eleven exact artifacts: the Multiplayer build profile; Collab, pair-relay and standalone Buzz shim binaries; push-gateway, web, mobile and administration source contracts; canonical migrations; and both collaboration charts. The manifest binds the source commit, full compatibility matrix and digest, exact Cargo/chart/schema/client versions, normalized package metadata, canonical root licenses, generated third-party notices and every archive checksum. A least-privilege second job re-verifies the downloaded candidate and uses GitHub OIDC provenance for the manifest plus all eleven packages. The dry run assembled the candidate twice with identical digests and passed manifest/package/signature/version/notice/workflow checks; independent mutations of an archive, artifact version, workflow write permission and notices all failed closed. Ruby parsed the workflow YAML, and source formatting and diff checks passed. The workflow has no production trigger, environment, secrets, publication command or deployment path._

  - [x] 44.7. Document operational rollout and rollback
    ...
    - _Reads: .agents/specs/collaborative-workspace/{migration-plan.md,security/operational-limits.md,decisions/adr-{001-service-topology,003-git-authority,006-shared-compute}.md}, deploy/collaboration/**, .github/workflows/collaboration-release.yml, script/collaboration-release-contract, crates/collab/src/{main.rs,migration/buzz/checkpoint.rs}, docs/collaboration/compatibility.md_
    - _Writes: docs/collaboration/operations.md, .agents/specs/collaborative-workspace/{design,tasks}.md_
    - _Validation: tabletop review follows each rollback path and identifies last reversible checkpoint_
    - _Discovered contradiction (2026-08-25): the task requests an operational rollout procedure, while the current `/healthz` proves only process/database reachability and Task 44.5 deliberately generated deployment consumers without claiming that every readiness, kill-switch or stop-signal emitter is production-bound. The narrow correction publishes an executable pre-production runbook that identifies those missing runtime proofs as hard promotion gates; it does not deploy, migrate, cut over, restore data, invent an admission controller or authorize production use._
    - _Evidence: 2026-08-25 — added `docs/collaboration/operations.md` with separated release/deployment/migration/data/incident/rollback/tenant authority, immutable change records, configuration and signed-candidate preflight, DDL-secret migration and sealing boundaries, unrouted/read-only/write canary stages, all 16 closed alert responses, seven temporary-bridge precedence/removal rules, recovery objectives and eleven independently selected rollback paths. Every path identifies its last reversible checkpoint, authorized actor, action, completion evidence and prohibition boundary, including pre-floor down migration versus post-activation coordinated restore and irreversible deletion/source-retirement cases. A deterministic Markdown contract passed exact heading/link/schema/stop-signal/rollback inventory checks (`rollback_paths=11`, `stop_signals=16`, ceiling `20260824000500`), and the source-backed tabletop resolved RB-01–RB-11 against the real Compose, Helm, migration, ADR, lifecycle and retirement contracts. The release dry run, Compose checker/shell syntax, migration package checker, observability checker, Helm render suite, Prettier 3.5.0 check and diff hygiene passed; final dependency, frozen-client, inventory, formatting and specification gates are recorded in the enclosing checkpoint commit. No environment was mutated._
