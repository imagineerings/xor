...
1. **9.1** Supported workspace, editor, preview, inspector, task, artifact, persistence, cancellation, recovery, and lifecycle behavior SHALL be owned by existing Sim workspace/project/editor/media/task components.
...
3. **9.3** Godot-compatible project, scene, resource, command, preview, run, and export concepts MAY appear at explicit interoperability boundaries, but UI state and execution SHALL remain Sim-owned.
