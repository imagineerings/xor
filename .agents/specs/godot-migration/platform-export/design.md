...
Use existing Zed task, project, settings, platform, and bundling integration points for approved native packaging and deployment behavior. Godot export presets are parsed as compatibility input only; Zed owns requests, execution, artifacts, diagnostics, cancellation, cleanup, and dependency review. Unsupported targets remain explicit and are never implemented by launching Godot.

## Components

- Existing project/settings persistence for imported preset data.
- Existing task lifecycle for Zed-owned packaging and deployment work.
- Existing platform bundling/signing owners selected by an approved platform-tier decision.

## Correctness Properties

### Property 1: Native Zed Export Task

_For any_ supported export request, Zed SHALL create and execute a native packaging request through existing Zed owners and SHALL NOT migrate or invoke Godot platform templates.

**Validates: Requirement 1.1, 1.2, 2.1**

### D-NATIVE: Native export path

The compatibility boundary ends after preset parsing. Zed-owned task/platform services perform packaging, signing, deployment, cancellation, artifact collection, and cleanup. Hermetic validation runs the exported artifact without Godot and inspects its contents and dependencies.
