...
1.2 IF a sampler, scheduler, or guidance mode is unsupported by Sim THEN THE system SHALL reject execution with an unsupported-sampling diagnostic before model work starts.
...
1.4 WHILE a sampling run is executing THE system SHALL report current step, total steps, preview availability, and cancellation state through the parent Sim job.
...
4.4 IF a model family is present in Comfy but unsupported in Sim THEN THE system SHALL expose a family-specific unsupported diagnostic instead of silently falling back to another runner.
...
5.1 WHEN local execution requires Python, PyTorch, GPU APIs, custom kernels, or native packages THEN THE system SHALL use the `model-serving-packaging/` worker boundary instead of loading those dependencies into Sim UI code.
...
5.4 WHEN execution completes, fails, or is cancelled THEN THE system SHALL update Sim job state, output artifacts, previews, diagnostics, and provenance consistently.
...
6.3 IF Sim intentionally diverges from a Comfy execution behavior THEN THE system SHALL record the safety, security, dependency, platform, or product reason in a machine-readable divergence catalog.
