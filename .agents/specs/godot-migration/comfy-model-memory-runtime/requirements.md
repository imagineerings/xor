...
2. **1.2** WHEN extra model path config is supplied THEN THE system SHALL merge additional roots without replacing Sim project asset roots.
...
3. **3.3** IF a model family is unsupported by Sim THEN THE system SHALL report an unsupported model diagnostic with the missing capability.
...
3. **5.3** WHEN a memory-free or unload-models command is issued THEN THE system SHALL release model resources through Sim worker APIs and report the result.
