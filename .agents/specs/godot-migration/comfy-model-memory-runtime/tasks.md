...
- Start gate: G0 spec consistency, G3 shared world-model foundations, G4 worker safety, and G8 Comfy harness alignment, G9 Zed coverage are satisfied.
...
- [ ] 1. Implement Comfy model folder registry
  ...
  - _Depends on: none_
  - _Reads: /Users/ahmad.vegah/repos/zed/.agents/specs/godot-migration/comfy-model-memory-runtime/requirements.md, /Users/ahmad.vegah/repos/zed/.agents/specs/godot-migration/comfy-model-memory-runtime/design.md, Cargo.toml, projects/comfy_
  - _Validation: python3 .agents/skills/feature-spec/scripts/validate_spec.py /Users/ahmad.vegah/repos/zed/.agents/specs/godot-migration/comfy-model-memory-runtime_
...
- [ ] 3. Implement model preview and safetensors metadata reads
  - Add native Zed adjacent preview lookup and bounded safetensors header metadata extraction without loading model weights or passing metadata reads through to ComfyUI.
...
- [ ] 4. Implement model family capability detection
  - Add native Zed model-family and capability records for Comfy-supported image, video, audio, 3D, adapter, segmentation, depth, and detection families.
...
- [ ] 5. Implement precision, quantization, device, and memory policy resolver
  - Parse native Zed runtime settings, quantization metadata, backend support, dynamic VRAM/offload options, and compatibility diagnostics without passing policy resolution through to ComfyUI.
...
- [ ] 6. Implement model resource release bridge
  - Wire native Zed unload-models and free-memory intents to worker APIs and report success or diagnostic failure without passing resource management through to ComfyUI.
  ...
  - _Reads: /Users/ahmad.vegah/repos/zed/.agents/specs/godot-migration/comfy-model-memory-runtime/requirements.md, /Users/ahmad.vegah/repos/zed/.agents/specs/godot-migration/comfy-model-memory-runtime/design.md, Cargo.toml, projects/comfy_
  - _Validation: python3 .agents/skills/feature-spec/scripts/validate_spec.py /Users/ahmad.vegah/repos/zed/.agents/specs/godot-migration/comfy-model-memory-runtime_
- [ ] 7. Materialize remaining model and memory coverage backlog
  - Convert 120 planned coverage records in model-memory-runtime into native Zed implementation, delegation, unsupported, or divergent outcomes without ComfyUI pass-through.
  ...
  - Expected native Zed writes: crates/world_model/src/comfy_model_folders.rs, crates/world_model/src/comfy_model_family.rs, crates/world_model/src/world_model.rs, crates/world_model/src/comfy_model_family_tests.rs, crates/world_model/tests/comfy_compat_suite.rs, crates/world_model/fixtures/comfy/*.json.
