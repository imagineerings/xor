...
2.2 WHEN a workflow is saved THEN THE system SHALL preserve graph structure, UI metadata, source references, and Sim provenance fields.
...
3.3 WHEN custom node templates exist THEN THE system SHALL expose template names and static template assets through Sim's template service.
...
6.1 WHEN a workflow declares app-mode controls THEN THE system SHALL expose them through the unified authoring app using Sim UI components.
