...
- Start gate: G0 spec consistency, G5 graph safety for validation hooks, G6 provenance for generated metadata links, and G8 Comfy harness alignment, G9 Zed coverage are satisfied.
...
- [ ] 1. Import shipped Comfy blueprints
  ...
  - Preserve shipped names, source paths, graph JSON, categories, attribution, node inventory, dependency diagnostics, and unsupported-node diagnostics as native Zed catalog records without ComfyUI pass-through.
  ...
  - _Depends on: none_
  - _Reads: /Users/ahmad.vegah/repos/zed/.agents/specs/godot-migration/comfy-workflows-blueprints/requirements.md, /Users/ahmad.vegah/repos/zed/.agents/specs/godot-migration/comfy-workflows-blueprints/design.md, Cargo.toml, projects/comfy_
  - _Validation: python3 .agents/skills/feature-spec/scripts/validate_spec.py /Users/ahmad.vegah/repos/zed/.agents/specs/godot-migration/comfy-workflows-blueprints_
- [ ] 2. Implement workflow store and API export
  ...
  - Preserve workflow source, versions, default view, provenance links, graph JSON, and export diagnostics as native Zed records without ComfyUI pass-through.
...
- [ ] 3. Implement global subgraph index
  ...
  - Preserve blueprint/custom-node source metadata, node-pack metadata, graph JSON, and listing diagnostics as native Zed records without ComfyUI registry pass-through.
...
- [ ] 4. Implement workflow template adapter
  - Expose custom node workflow templates and static template assets through Zed template services.
  - Preserve template names, node-pack metadata, safe static asset refs, graph JSON, sanitized listings, and diagnostics as native Zed template records without ComfyUI directory pass-through.
  - _Requirements: 3.3_
...
- [ ] 5. Implement node replacement catalog
  ...
  - Preserve replacement source metadata, input/output mappings, duplicate/conflict diagnostics, and graph-rewrite integration as native Zed records without ComfyUI metadata pass-through.
...
- [ ] 6. Implement embedded workflow metadata extraction
  ...
  - Preserve recovered prompt/workflow JSON, source artifact links, provenance updates, supported-format diagnostics, and non-fatal failures as native Zed metadata records without ComfyUI metadata-reader pass-through.
...
- [ ] 7. Add app-mode metadata bridge
  ...
  - Preserve control kinds, labels, defaults, choices, target node/input bindings, graph fallback, UI ownership, and diagnostics as native Zed app-mode records without ComfyUI frontend pass-through.
  ...
  - _Reads: /Users/ahmad.vegah/repos/zed/.agents/specs/godot-migration/comfy-workflows-blueprints/requirements.md, /Users/ahmad.vegah/repos/zed/.agents/specs/godot-migration/comfy-workflows-blueprints/design.md, Cargo.toml, projects/comfy_
  - _Validation: python3 .agents/skills/feature-spec/scripts/validate_spec.py /Users/ahmad.vegah/repos/zed/.agents/specs/godot-migration/comfy-workflows-blueprints_
- [ ] 8. Materialize remaining workflow and blueprint coverage backlog
  - Convert 89 planned coverage records in workflows-blueprints into native Zed implementation, delegation, unsupported, or divergent outcomes without ComfyUI pass-through.
  ...
  - Native Zed writes: crates/world_model/src/comfy_blueprints.rs, crates/world_model/src/comfy_blueprints_tests.rs, crates/world_model/src/world_model.rs, crates/world_model/tests/comfy_compat_suite.rs, crates/world_model/fixtures/comfy/coverage_ledger.json, crates/world_model/fixtures/comfy/workflows_blueprints_backlog.json.
  - Validation: `cargo test -p world_model comfy_blueprints`.
  - Parity evidence: Mark records implemented only with native Zed workflow, blueprint, subgraph, replacement, or embedded metadata evidence.
