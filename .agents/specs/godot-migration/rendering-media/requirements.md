...
1. **2.1** WHEN a supported texture, image, shader, generated frame, or video is selected THEN THE system SHALL route it to existing Sim preview infrastructure.
...
3. **9.3** Godot-compatible media, texture, shader, material, and scene data MAY cross import/export boundaries, but decoded resources and execution state SHALL be Sim-native.
