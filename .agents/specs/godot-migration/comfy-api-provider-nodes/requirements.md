...
- **Provider Connector**: Baymax service wrapper for one provider's authentication, request schema, task polling, uploads, downloads, and error handling.
...
**User Story:** As a workflow author, I want Baymax to expose Comfy API provider nodes as typed capabilities.
...
1.3 IF a provider capability is unsupported in Baymax THEN THE system SHALL expose an unsupported-provider diagnostic instead of a half-registered node.
...
2.1 WHEN a provider requires credentials THEN THE system SHALL resolve them through Baymax secret infrastructure, not workflow JSON plaintext.
...
3.1 WHEN a provider node starts THEN THE system SHALL validate inputs, upload required source media, create the provider request, and attach the remote task id to the Baymax job node state.
...
4.4 WHEN provider nodes perform text/image/multiview-to-3D, texture, rig, animate, retarget, convert, topology, or model import THEN THE system SHALL register 3D outputs through Baymax 3D artifact routing.
