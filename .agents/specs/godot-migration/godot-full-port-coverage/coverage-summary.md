...
- `GODOT-PROJ-003` — Partially implemented in Zed and should be extended: scan, sort, filter, favorite, rename, remove, and reopen projects in the project manager
- `GODOT-PROJ-004` — Partially implemented in Zed and should be extended: persist recent projects, favorites, tags, sort mode, and missing-project state
...
- `GODOT-EDITOR-001` — Partially implemented in Zed and should be extended: restore open scenes, selected objects, bottom panels, docks, and workspace layout per project
...
- `GODOT-EDITOR-007` — Partially implemented in Zed and should be extended: configure and resolve user, project, feature-tag, and platform-specific editor settings
- `GODOT-EDITOR-009` — Partially implemented in Zed and should be extended: perform undo, redo, history navigation, inspector pinning, and multi-object edits without reentrant updates
- `GODOT-EDITOR-010` — Partially implemented in Zed and should be extended: save, save-as, save-all, autosave, recover, and warn before closing unsaved resources
...
- `GODOT-EDITOR-014` — Partially implemented in Zed and should be extended: search help and class reference by class, method, property, signal, constant, and theme item
...
- `GODOT-UI-001` — Partially implemented in Zed and should be extended: lay out Controls using anchors, offsets, grow directions, minimum sizes, containers, aspect ratios, and RTL mirroring
- `GODOT-UI-002` — Partially implemented in Zed and should be extended: route mouse, touch, keyboard, controller, shortcut, focus, tooltip, and drag/drop events through Control hierarchy
- `GODOT-UI-003` — Partially implemented in Zed and should be extended: provide buttons, ranges, lists, trees, tabs, menus, dialogs, color/file pickers, splitters, and scroll containers
- `GODOT-UI-004` — Partially implemented in Zed and should be extended: edit plain and rich text with selection, undo, syntax, bidi, shaping, images, tables, meta links, and IME
- `GODOT-UI-005` — Partially implemented in Zed and should be extended: resolve theme inheritance, type variations, icons, fonts, sizes, colors, style boxes, and live overrides
- `GODOT-UI-006` — Partially implemented in Zed and should be extended: manage popups, modal dialogs, embedded windows, exclusive state, and safe cancellation
- `GODOT-UI-007` — Partially implemented in Zed and should be extended: expose accessible roles, names, values, actions, focus, and tree updates to platform assistive technology
...
- `GODOT-INPUT-004` — Partially implemented in Zed and should be extended: create and manage multiple windows, screens, modes, flags, focus, DPI, scale, vsync, orientation, and safe areas
- `GODOT-INPUT-005` — Partially implemented in Zed and should be extended: support clipboard, cursor, mouse modes, virtual keyboard, IME composition, and text input
- `GODOT-INPUT-006` — Partially implemented in Zed and should be extended: expose accessibility activation, semantic trees, actions, bounds, focus, announcements, and deactivation
...
- `GODOT-ZED-007` — Missing from the migration specs: author and play Animation, AnimationPlayer, AnimationTree, Tween, tracks, blends, state machines, method/audio tracks, and root motion
- `GODOT-ZED-008` — Missing from the migration specs: route sample playback through buses, sends, effects, capture, device switching, spatial emitters, polyphony, and interactive music
...
- `GODOT-EXT-008` — Partially implemented in Zed and should be extended: reuse Zed extension trust, capability, installation, and UI boundaries instead of creating a second plugin manager
...
- `GODOT-EXPORT-010` — Partially covered by an existing migration spec: launch, stop, remote-deploy, and collect logs from an exported or editor-run project through existing Zed tasks
...
- `GODOT-NET-004` — Partially implemented in Zed and should be extended: perform HTTP requests, redirects, proxies, cookies/headers, body streaming, downloads, timeouts, cancellation, TLS, and size limits
- `GODOT-DEBUG-001` — Partially implemented in Zed and should be extended: format, route, filter, timestamp, persist, and flush stdout/stderr, print, warning, error, and structured engine log messages
...
- `GODOT-DEBUG-007` — Partially implemented in Zed and should be extended: capture errors and crashes with backtraces, symbols, platform handlers, suppression rules, and safe shutdown/reporting
...
- `GODOT-CLI-008` — Partially implemented in Zed and should be extended: print stable help, version, path, verbose, benchmark, and build-feature diagnostics without starting a project
...
- `GODOT-SEC-002` — Partially implemented in Zed and should be extended: establish TLS trust from system/bundled/custom certificates and expose hostname, chain, expiry, and protocol failures
...
- `GODOT-SEC-004` — Partially implemented in Zed and should be extended: store export signing keys, passwords, tokens, and remote credentials through Zed secret facilities with redaction
