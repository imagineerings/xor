...
The native Zed ownership constraint is settled: Godot delegation, runtime linkage, wrappers, hidden instances, and unreviewed source copying are not available alternatives. No direction among the remaining native product/architecture alternatives below was selected by this audit.

| ID | Decision | Material alternatives | Capability impact |
| --- | --- | --- | --- |
| DEC-GODOT-001 | Native runtime product scope and owner composition | Full Zed-native game runtime at existing owners; phased native runtime profiles; import/edit/preview only with executable areas explicitly excluded | Scene lifecycle, rendering, UI runtime, physics, navigation, animation, audio, particles, input, multiplayer, XR, headless, export |
...
| DEC-GODOT-006 | Native extension and plugin trust | Refuse; translate a safe subset into Zed extension models; provide a separately reviewed Zed-owned compatibility host without Godot libraries | `GODOT-EXT-*`, @tool/post-import scripts, security, editor recovery |
| DEC-GODOT-007 | Native import/export ownership | Reproduce a selected importer/exporter subset through existing Zed owners; full native pipeline; explicit unsupported formats/targets | `GODOT-IMPORT-*`, `GODOT-EXPORT-*`, caches, dependencies, formats, platform toolchains |
...
| DEC-GODOT-010 | Resource limits and performance targets | Match selected Godot limits natively; define Zed-specific bounded profiles; explicitly exclude workloads outside approved bounds | Parsing, imports, rendering, networking, worker isolation, mobile/web/headless behavior |
