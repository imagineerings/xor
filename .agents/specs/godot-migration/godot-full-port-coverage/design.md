...
### D-ZED: Physics, navigation, animation, audio, and particles ownership

- Responsibility: Own the `ZED` catalog rows for simulation services, including success, failure, persistence, lifecycle, mode, and platform outcomes.
...
| 9.1 | D-ZED | Catalog scenarios for every ZED row |
| 9.2 | D-ZED | Catalog scenarios for every ZED row |
| 9.3 | D-ZED | Catalog scenarios for every ZED row |
| 9.4 | D-ZED | cargo test -p project -p task -p audio godot_simulation |
