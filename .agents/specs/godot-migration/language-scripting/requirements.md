...
2. **3.2** WHEN Sim game API docs are indexed THEN THE system SHALL make class and capability docs available to language features.
...
1. **9.1** Supported parsing, translation, language services, script execution, debugging, persistence, trust, and lifecycle SHALL be owned by existing Sim `language`, `languages`, `lsp`, `dap`, `extension_host`, and task components as applicable.
...
3. **9.3** `.gd`, C# project, Godot API, and protocol compatibility MAY be preserved at explicit source/file/API boundaries, but successful translations and execution SHALL produce Sim-native source, diagnostics, state, and runtime behavior.
