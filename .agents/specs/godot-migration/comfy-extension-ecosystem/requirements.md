...
1. **1.1** WHEN custom nodes are enabled THEN THE system SHALL discover Python files and directories under configured custom node roots.
2. **1.2** WHEN custom nodes are disabled THEN THE system SHALL skip all custom node loading except explicitly whitelisted packs.
3. **1.3** WHEN a node pack is blocked by policy THEN THE system SHALL record a blocked diagnostic and omit its nodes and web assets.
4. **1.4** IF an extension fails to import THEN THE system SHALL continue startup and expose an import diagnostic.
...
1. **4.1** WHEN locale bundles exist THEN THE system SHALL merge `main.json`, `nodeDefs.json`, `commands.json`, and `settings.json` by language.
2. **4.2** WHEN example workflow folders exist THEN THE system SHALL expose workflow template names and static template assets.
3. **4.3** WHEN subgraph folders exist THEN THE system SHALL expose reusable subgraphs through the shared subgraph index.
