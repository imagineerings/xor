...
Keep Godot-origin networking as native Zed boundary metadata and optional debug metadata. Runtime networking remains excluded unless represented by native Zed task/debug metadata that directly supports the target generative game engine.
...
- Completion gate: no Godot-specific network runtime or protocol adapter is added; native gameplay protocol work requires G7 review and an explicit architecture decision at existing Zed owners.
...
- [ ] 1. Add networking boundary and debug metadata support
  - Encode non-migration decisions and model optional native Zed debug metadata for task/debug workflows.
  ...
  - _Depends on: none_
  - _Reads: /Users/ahmad.vegah/repos/zed/.agents/specs/godot-migration/networking-collaboration/requirements.md, /Users/ahmad.vegah/repos/zed/.agents/specs/godot-migration/networking-collaboration/design.md, Cargo.toml, projects/godot_
  - _Validation: python3 .agents/skills/feature-spec/scripts/validate_spec.py /Users/ahmad.vegah/repos/zed/.agents/specs/godot-migration/networking-collaboration; run supported network/debug scenarios without Godot and inspect processes, endpoints, packages, and dependencies_
