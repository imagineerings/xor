...
| Term | Definition |
|---|---|
| **Zed Theme** | A JSON file at `assets/themes/` or `~/.config/zed/themes/` containing light and dark appearance variants with semantic color tokens (UI, editor, syntax, terminal, diagnostics). |
...
**User Story:** As a new Zed user, I want the default theme to feel modern, clean, and approachable, with layered surfaces and controlled accent colors, so that my first impression of the editor is polished and professional.

#### Acceptance Criteria

1. THE `spectrum-2-inspired.json` file SHALL be placed at `~/.config/zed/themes/spectrum-2-inspired.json` for local usage.
...
3. THE file SHALL reference `https://zed.dev/schema/themes/v0.2.0.json` as its `$schema`.
4. THE file SHALL include `"author": "Ahmad Vegah"` in the metadata.
5. THE file SHALL validate against the Zed theme schema without errors.
...
**User Story:** As a theme developer, I want all Zed theme keys to have explicit values in the new theme, so that no UI element falls back to potentially mismatched defaults.
...
- Touch/pointer detection, RTL support, or other Spectrum 2 concepts not applicable to Zed's Rust-native UI framework.
