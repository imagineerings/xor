...
This implementation plan covers the creation of a `spectrum-2-inspired.json` theme file for Zed and integration into the onboarding theme picker. The plan includes:
...
- [x] 1. Build the theme JSON file
  - Created `~/.config/zed/themes/spectrum-2-inspired.json` with the full structure
  ...
  - _writes: ~/.config/zed/themes/spectrum-2-inspired.json_
...
- [x] 4. Install theme and test in Zed
  - File copied to `~/.config/zed/themes/spectrum-2-inspired.json`
...
- The theme is a user-installed local theme (`~/.config/zed/themes/`), not bundled as a built-in asset.
