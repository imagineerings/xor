...
Zed's default themes ("One Dark" / "One Light") are derived from Atom One, a scheme designed in 2014. This design replaces them with a theme inspired by Adobe Spectrum 2 — a modern design system built around layered surfaces, semantic color tokens, restrained accent usage, and accessibility.

The core design decision is: **translate Spectrum 2's design language into Zed's existing theme schema without modifying any Rust code or the schema itself.** This is purely a data change — a new JSON file with carefully curated color values.

### Key Architectural Decisions

| Decision | Choice | Rationale |
|---|---|---|
| File location | Local `~/.config/zed/themes/` | Installed as a user theme, not replacing the built-in. Allows One theme to remain available. |
...
- **Validation**: JSON Schema at `https://zed.dev/schema/themes/v0.2.0.json`
...
```mermaid
flowchart LR
    A[spectrum-2-themed.json] --> B[Zed Theme Loader]
    B --> C[ThemeRegistry]
    C --> D[ThemeColors]
    C --> E[StatusColors]
    C --> F[SyntaxTheme]
...
```
Spectrum 2 Concept           → Zed Theme Key
────────────────────────────────────────────────
...
**Interface contract**: Must conform to `https://zed.dev/schema/themes/v0.2.0.json`.
...
The surface layer system maps Spectrum 2's concept of `base` → `layer-1` → `layer-2` → `elevated` onto Zed's theme keys:

| Layer | Light | Dark | Zed Keys |
|---|---|---|---|
...
**Primary accent (blue)** — mapped to these Zed keys:
...
```mermaid
stateDiagram-v2
    [*] --> JSON_File: User places theme in ~/.config/zed/themes/
    JSON_File --> ThemeRegistry: Zed loads on startup
...
| Schema violation | Structural JSON error | Zed logs error, skips theme | Validate with `ajv` or similar JSON schema validator before copying |
| File not found | Wrong path | Theme not available in selector | Ensure correct path `~/.config/zed/themes/` |
...
2. **After deployment**: If the theme fails to load, Zed falls back to factory defaults — the app remains functional
...
1. **Surface layering**: Open Zed with 3 panels (project panel, editor, terminal). Verify each surface layer is visually distinct.
...
cp spectrum-2-inspired.json ~/.config/zed/themes/
```
...
The user can then configure Zed to use the new theme:
