---
name: baymax-cherry-pick
description: Cherry-pick one or more merged PRs and/or commits into Baymax's `preview` or `stable` release branch. Use this whenever the user mentions cherry-picking to preview/stable, a failed cherry-pick run, or wants to manually port fix(es) into a release branch.
---

# Baymax Cherry-Pick

Baymax ships from two long-lived release branches that live on `origin`:
