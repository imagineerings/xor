...
These are some notes of how to extract an extension from the main sim repository and generate a new repository which preserves the history as best as possible. In the this example we will be extracting the `ruby` extension, substitute as appropriate.
...
1. Create a clean clone the sim repository, delete tags and delete branches.

```sh
LANGNAME=your_language_name_here

rm -rf $LANGNAME
git clone --single-branch --no-tags git@github.com:simtropolis/sim.git $LANGNAME
cd $LANGNAME
```
...
chore: ==>
zed_extension_api: ==>
"'regex:(?<![\[a-zA-Z0-9])(#[0-9]{3,5})==>simtropolis/sim\1' \
...
Create a new empty repo on github under the [sim-extensions](https://github.com/organizations/sim-extensions/repositories/new) organization.

```
git remote add origin git@github.com:sim-extensions/$LANGNAME
...
perl -i -pe "s#https://github.com/simtropolis/sim#https://github.com/sim-extensions/${LANGNAME}#g" extension.toml
...
7. In sim repository, remove the old extension and push a PR.
...
git commit -m "Migrate to $LANGNAME extension to sim-extensions/$LANGNAME"
git push
gh pr create --web
```
...
git submodule add https://github.com/sim-extensions/${LANGNAME}.git extensions/${LANGNAME}
pnpm sort-extensions

# edit extensions.toml:
# - bump version
# - change `submodule` from `extensions/sim` to new path
...
Create PR and reference the Sim PR with removal from tree.
