...
These are some notes of how to extract an extension from the main baymax repository and generate a new repository which preserves the history as best as possible. In the this example we will be extracting the `ruby` extension, substitute as appropriate.
...
1. Create a clean clone the baymax repository, delete tags and delete branches.

```sh
LANGNAME=your_language_name_here

rm -rf $LANGNAME
git clone --single-branch --no-tags git@github.com:simtropolis/baymax.git $LANGNAME
cd $LANGNAME
```
...
chore: ==>
zed_extension_api: ==>
"'regex:(?<![\[a-zA-Z0-9])(#[0-9]{3,5})==>simtropolis/baymax\1' \
...
Create a new empty repo on github under the [baymax-extensions](https://github.com/organizations/baymax-extensions/repositories/new) organization.

```
git remote add origin git@github.com:baymax-extensions/$LANGNAME
...
perl -i -pe "s#https://github.com/simtropolis/baymax#https://github.com/baymax-extensions/${LANGNAME}#g" extension.toml
...
7. In baymax repository, remove the old extension and push a PR.
...
git commit -m "Migrate to $LANGNAME extension to baymax-extensions/$LANGNAME"
git push
gh pr create --web
```
...
git submodule add https://github.com/baymax-extensions/${LANGNAME}.git extensions/${LANGNAME}
pnpm sort-extensions

# edit extensions.toml:
# - bump version
# - change `submodule` from `extensions/baymax` to new path
...
Create PR and reference the Baymax PR with removal from tree.
