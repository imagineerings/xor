"""Harbor agent wrapper for Baymax's eval-cli binary.
...
class BaymaxAgent(BaseInstalledAgent):
    """Runs Baymax's headless AI agent (eval-cli) to solve tasks.

    The eval-cli binary boots a headless GPUI application and uses the same
    NativeAgent + AcpThread pipeline as the production Baymax editor, driving
    ...
    async def _install_lsps(self, environment: BaseEnvironment) -> None:
        """Pre-install language servers so Baymax doesn't download them at runtime.
        ...
        for name, cmd in lsp_installs:
            try:
                await self.exec_as_agent(
                    environment,
                    command=(
                        'BAYMAX_DATA_DIR="${XDG_DATA_HOME:-$HOME/.local/share}/baymax"; '
                        + cmd
                    ),
                )
            except Exception as exc:
        ...
        try:
            await self.exec_as_agent(
                ...
                command=(
                    "set -euo pipefail; "
                    'BAYMAX_DATA_DIR="${XDG_DATA_HOME:-$HOME/.local/share}/baymax"; '
                    'ESLINT_DIR="$BAYMAX_DATA_DIR/languages/eslint/vscode-eslint-2.4.4"; '
                    'mkdir -p "$ESLINT_DIR"; '
