...
fn main() {
    ...
    app.run(move |cx| {
        ...
        if let Ok(raw_flags) = std::env::var("SIM_EVAL_ENABLE_FLAGS") {
...
/// the same shape as Sim's `openai_compatible` settings key (provider id ->
/// `{ "api_url": ..., "available_models": [...] }`). Lets sim-eval route the
/// agent itself through an OpenAI-compatible endpoint (e.g. Baseten) that isn't
/// one of Sim's built-in providers, without hardcoding it into eval-cli.
const OPENAI_COMPATIBLE_PROVIDERS_ENV: &str = "SIM_OPENAI_COMPATIBLE_PROVIDERS";
...
/// discovery, in the same shape as Sim's `anthropic.available_models` settings
/// key (a list of `{ "name": ..., "max_tokens": ..., ... }` entries). Lets
/// sim-eval run models that exist on the Anthropic API for the configured
...
const ANTHROPIC_AVAILABLE_MODELS_ENV: &str = "SIM_ANTHROPIC_AVAILABLE_MODELS";
...
async fn run_agent(
    ...
    let setup_result: Result<()> = cx.update(|cx| {
        ...
        let profile_field = {
            let raw = std::env::var("SIM_EVAL_DISABLE_TOOLS").unwrap_or_default();
            let disabled = raw
                .split(',')
