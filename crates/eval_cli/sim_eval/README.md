...
Python CLI and installed-agent package for building Sim's headless `eval-cli`
binary, launching benchmark runs on Modal/Harbor/Pier, and fetching results.

This README is for `crates/eval_cli/sim_eval/`. The Rust `eval-cli` binary is
documented in [`../README.md`](../README.md). Commands below assume they are run
from the repository root unless otherwise noted.

Most benchmark users should start with `sim-eval`.
...
This wraps `uv tool install --editable crates/eval_cli/sim_eval --force`, so the
installed `sim-eval` command tracks checkout changes without needing
`PYTHONPATH`. If you prefer not to install globally, run from the checkout with:

```sh
crates/eval_cli/script/sim-eval doctor
```
...
```sh
sim-eval runs                 # recent runs launched from this machine
sim-eval status               # status for the most recent run
sim-eval logs <run-id>        # controller log
sim-eval report <run-id> --fetch
```
...
```sh
uv run --project crates/eval_cli/sim_eval python -m unittest discover -s crates/eval_cli/sim_eval/tests
```
...
Use `sim-eval run` for non-interactive launches:

```sh
# One SWE-Atlas part
sim-eval run rf --from local --n-tasks 5

# Multiple benchmarks share one build when possible
sim-eval run swe-atlas terminal-bench-2.1 --from v0.210.0 --n-tasks 5 --yes

# DeepSWE runs under Pier automatically
sim-eval run deepswe --from local --n-tasks 10

# Inspect what would happen without launching
sim-eval run swe-atlas --from v0.210.0 --plan
sim-eval run deepswe --from local --dry-run
```
...
```sh
sim-eval swe-atlas --interactive
```
...
```sh
sim-eval build --from local
sim-eval run rf --build bld-abc123 --n-tasks 2
sim-eval builds --details
```

Untracked files are not included in builds. If they are irrelevant, opt in:

```sh
sim-eval run rf --from local --allow-untracked --n-tasks 2
```

For reproducible runs, use a clean ref/tag/SHA or require a clean tracked tree:

```sh
sim-eval run rf --from v0.210.0 --n-tasks 2
sim-eval run rf --from local --require-clean --n-tasks 2
```
...
```sh
sim-eval run rf --model sonnet-4.6 --n-tasks 2
sim-eval run rf --model opus-4.5 --n-tasks 2
sim-eval run rf --model baseten:kimi-k2.7-code --n-tasks 2
sim-eval run rf --model baseten:deepseek-v4-pro --n-tasks 2
```
...
```sh
sim-eval run rf --judge leaderboard --n-tasks 2
sim-eval run rf --judge deepseek-v4-pro --judge-model deepseek-ai/DeepSeek-V4-Pro
```
...
```sh
sim-eval runs                         # local, fast recent-run list
sim-eval list --details               # query runs on the Modal volume
sim-eval list -e swe-atlas-rf --details
sim-eval status <run-id>
sim-eval logs <run-id>
sim-eval fetch <run-id>
sim-eval report <run-id> --fetch
```
...
```sh
sim-eval run qna rf terminal-bench-2.1 --from local --n-tasks 2 --yes
```

Suite commands operate on all runs in that group:

```sh
sim-eval suite status <suite-id>
sim-eval suite logs <suite-id>
sim-eval suite fetch <suite-id>
```
...
```sh
sim-eval rejudge <parent-run-id> --judge deepseek-v4-pro
sim-eval rejudge <parent-run-id> --judge kimi-k2.7-code --dry-run
sim-eval report <derived-run-id> --fetch
```
...
```sh
sim-eval baseline record <run-id> --experiment-name swe-atlas-rf
sim-eval baseline list
sim-eval baseline show swe-atlas-rf --model anthropic/claude-sonnet-4-6
```
...
- `sim_eval.agent:SimAgent` for Harbor
- `sim_eval.pier_agent:SimPierAgent` for Pier

For manual Harbor experiments with a locally built Linux binary:

```sh
pip install -e crates/eval_cli/sim_eval/
crates/eval_cli/script/build-linux
