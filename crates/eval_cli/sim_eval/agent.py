"""Harbor agent wrapper for Sim's eval-cli binary.
...
class SimAgent(BaseInstalledAgent):
    """Runs Sim's headless AI agent (eval-cli) to solve tasks.

    The eval-cli binary boots a headless GPUI application and uses the same
    NativeAgent + AcpThread pipeline as the production Sim editor, driving
