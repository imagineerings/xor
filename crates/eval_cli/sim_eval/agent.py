"""Harbor installed-agent adapter for Sim's eval-cli binary."""
...
class SimAgent(BaseInstalledAgent):
    ...
    async def _install_lsps(self, environment: BaseEnvironment) -> None:
        """Pre-install language servers so Sim doesn't download them at runtime.
        ...
        for name, cmd in lsp_installs:
            try:
                await self.exec_as_agent(
                    environment,
                    command=(
                        'SIM_DATA_DIR="${XDG_DATA_HOME:-$HOME/.local/share}/sim"; '
                        + cmd
                    ),
                )
            except Exception as exc:
        ...
        try:
            await self.exec_as_agent(
                ...
                command=(
                    "set -euo pipefail; "
                    'SIM_DATA_DIR="${XDG_DATA_HOME:-$HOME/.local/share}/sim"; '
                    'ESLINT_DIR="$SIM_DATA_DIR/languages/eslint/vscode-eslint-2.4.4"; '
                    'mkdir -p "$ESLINT_DIR"; '
    ...
    def _get_api_env(self) -> dict[str, str]:
        ...
        add_openai_compatible_provider_env(
            env, self._extra_env.get("SIM_OPENAI_COMPATIBLE_PROVIDERS")
        )
        add_anthropic_available_models_env(
            env, self._extra_env.get("SIM_ANTHROPIC_AVAILABLE_MODELS")
        )
        return env
    ...
    def _harbor_agent_budget_sec(self) -> float | None:
        ...
        try:
            ...
            print(
                f"[sim-eval] agent budget: could not read {config_path}: {exc!r}",
                flush=True,
            )
            return None
    ...
    def _task_declared_agent_timeout_sec(self, task_cfg: dict) -> float | None:
        ...
        try:
            ...
            print(
                f"[sim-eval] agent budget: could not parse {task_toml}: {exc!r}",
                flush=True,
            )
            return None
    ...
    def _resolve_task_dir(self, task_cfg: dict) -> Path | None:
        ...
        if name and "/" in name:
            try:
                ...
                print(
                    f"[sim-eval] agent budget: harbor PACKAGE_CACHE_DIR "
                    f"unavailable: {exc!r}",
                    flush=True,
                )
                return None
    ...
    async def run(
        ...
        add_sim_eval_env(
            env, self._extra_env, exclude={"SIM_EVAL_INSTRUCTION_SUFFIX_FILE"}
        )
        ...
        instruction_suffix_file = self._extra_env.get(
            "SIM_EVAL_INSTRUCTION_SUFFIX_FILE"
        )
        ...
        if configured_timeout_raw:
            try:
                ...
                print(
                    "[sim-eval] ignoring non-integer EVAL_CLI_TIMEOUT="
        ...
        if budget_sec is not None:
            ...
            print(
                f"[sim-eval] eval-cli --timeout {timeout_arg}s (harbor agent "
        ...
        await self.exec_as_agent(
            ...
            command=eval_cli_with_log_command(
                ...
                timeout_message=(
                    "[sim-eval] eval-cli timed out (exit 2); continuing to "
