...
def proxy_environment(judge: config.JudgeConfig) -> dict[str, str]:
    ...
    env = {
        "SIM_JUDGE_UPSTREAM": judge.upstream,
        "SIM_JUDGE_AUTH_ENV": judge.auth_env,
    }
    if judge.max_tokens is not None:
        env["SIM_JUDGE_MAX_TOKENS"] = str(judge.max_tokens)
    return env
...
class JudgeProxy:
    ...
    def __init__(self, judge: config.JudgeConfig, port: int = 8089) -> None:
        ...
        self._script_path = Path("/tmp/sim_judge_proxy.py")
    ...
    def __enter__(self) -> JudgeProxy:
        ...
        env["SIM_JUDGE_PROXY_PORT"] = str(self._port)
