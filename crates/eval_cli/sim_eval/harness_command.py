...
def build_harness_command(run_request: dict[str, Any], jobs_dir: str) -> list[str]:
    ...
    if not build_id:
        raise ValueError("sim benchmarks require build_id")
    ...
    command = [
        ...
        *dataset_args(benchmark),
        ...
        "--agent-import-path",
        (
            "sim_eval.pier_agent:SimPierAgent"
            if harness_binary(benchmark) == benchmarks.HARNESS_PIER
            else "sim_eval.agent:SimAgent"
        ),
        "--ae",
    ...
    if openai_compatible_provider_json:
        command.extend(
            [
                "--ae",
                f"SIM_OPENAI_COMPATIBLE_PROVIDERS={openai_compatible_provider_json}",
            ]
        )
    ...
    if anthropic_available_models_json:
        command.extend(
            [
                "--ae",
                f"SIM_ANTHROPIC_AVAILABLE_MODELS={anthropic_available_models_json}",
            ]
        )
