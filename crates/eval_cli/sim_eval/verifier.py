...
class SimJudgeProxyVerifier(Verifier):
    ...
    async def _ensure_judge_proxy(self) -> None:
        ...
        if not merged_env.get("SIM_JUDGE_UPSTREAM"):
            return
        ...
        result = await self.environment.exec(
            command=(
                ...
                "cat > /usr/local/lib/sim_judge_proxy.py <<'SIM_PROXY_EOF'\n"
                f"{JUDGE_PROXY_SCRIPT}"
                "SIM_PROXY_EOF\n"
                "cat > /usr/local/bin/ensure-judge-proxy <<'SIM_ENSURE_EOF'\n"
                f"{JUDGE_PROXY_ENSURE_SCRIPT}"
                "SIM_ENSURE_EOF\n"
                "chmod +x /usr/local/bin/ensure-judge-proxy\n"
                "ensure-judge-proxy\n"
                "python3 - <<'SIM_PROXY_CHECK_EOF'\n"
                "import os, socket\n"
                "port = int(os.environ.get('SIM_JUDGE_PROXY_PORT', '8089'))\n"
                "with socket.create_connection(('127.0.0.1', port), timeout=5):\n"
                "    pass\n"
                "SIM_PROXY_CHECK_EOF\n"
                "if [ -f /tmp/sim-judge-proxy.log ]; then "
                "tail -2 /tmp/sim-judge-proxy.log; fi\n"
            ),
            user="root",
            env=env,
        )
