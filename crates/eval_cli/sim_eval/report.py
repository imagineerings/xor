...
The per-trial record has two sources, one canonical each, with no fallbacks:
  ...
  - Resource usage (tokens / steps / tool calls) comes from the Sim agent's
...
SIM_RESULT_KEYS = {"status", "duration_secs"}
...
def load_sim_result(trial_dir: Path) -> dict | None:
    for path in sorted(trial_dir.rglob("result.json")):
        if path.parent == trial_dir:
            continue
        data = load_json(path)
        if isinstance(data, dict) and SIM_RESULT_KEYS & data.keys():
            return data
    return None
...
def extract_trial(
    ...
    sim_result = load_sim_result(trial_dir)
    if sim_result:
        record.has_result = True
        record.status = sim_result.get("status")
        record.model = sim_result.get("model")
        record.input_tokens = int(sim_result.get("input_tokens") or 0)
        record.output_tokens = int(sim_result.get("output_tokens") or 0)
        record.cache_read_input_tokens = int(
            sim_result.get("cache_read_input_tokens") or 0
        )
        record.cache_creation_input_tokens = int(
            sim_result.get("cache_creation_input_tokens") or 0
        )
        duration = sim_result.get("duration_secs")
        if isinstance(duration, (int, float)):
            record.duration_secs = float(duration)
        # The canonical metrics source: counts eval-cli writes into result.json.
        if isinstance(sim_result.get("step_count"), int):
            record.step_count = sim_result["step_count"]
        if isinstance(sim_result.get("tool_call_count"), int):
            record.total_tool_calls = sim_result["tool_call_count"]
        if isinstance(sim_result.get("tool_calls"), dict):
            record.tool_calls = {
                str(name): int(count)
                for name, count in sim_result["tool_calls"].items()
...
def locate_job_dir(args: Any) -> Path:
    ...
    if not candidate.exists():
        raise ValueError(
            f"no fetched job at {candidate}. Run `sim-eval fetch {run_id} "
