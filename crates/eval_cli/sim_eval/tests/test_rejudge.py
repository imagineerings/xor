...
from sim_eval import config, rejudge
...
class JudgeEnvironmentTests(unittest.TestCase):
    def test_proxy_environment_from_judge_config(self) -> None:
        ...
        self.assertEqual(env["SIM_JUDGE_UPSTREAM"], judge.upstream)
        self.assertEqual(env["SIM_JUDGE_AUTH_ENV"], judge.auth_env)
        self.assertEqual(env["SIM_JUDGE_MAX_TOKENS"], str(judge.max_tokens))

    def test_proxy_environment_omits_unset_max_tokens(self) -> None:
        judge = config.get_judge("leaderboard")
        env = rejudge.proxy_environment(judge)
        self.assertNotIn("SIM_JUDGE_MAX_TOKENS", env)
