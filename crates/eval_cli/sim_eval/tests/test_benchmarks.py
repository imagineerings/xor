...
from sim_eval import benchmarks, config, harness_command
...
class HarnessCommandTests(unittest.TestCase):
    ...
    def test_swe_atlas_rf_wires_judge(self) -> None:
        ...
        self.assertTrue(
            any(arg.startswith("SIM_JUDGE_UPSTREAM=") for arg in verifier_env)
        )
        self.assertTrue(
            any(arg.startswith("SIM_JUDGE_AUTH_ENV=") for arg in verifier_env)
        )
        self.assertIn("SIM_JUDGE_MAX_TOKENS=8192", verifier_env)
        self.assertFalse(any(arg.startswith("SIM_JUDGE_") for arg in agent_env))
    ...
    def test_deepswe_uses_pier_path_without_cli_allowlist(self) -> None:
        ...
        self.assertIn("sim_eval.pier_agent:SimPierAgent", command)
    ...
    def test_harbor_benchmarks_use_harbor_agent(self) -> None:
        ...
        self.assertIn("sim_eval.agent:SimAgent", command)
    ...
    def test_extra_env_is_forwarded_to_harness_agent(self) -> None:
        command = harness_command.build_harness_command(
            make_run_request(
                "swe-atlas-rf",
                extra_env={
                    "SIM_EVAL_INSTRUCTION_SUFFIX_FILE": "/data/prompts/checklist.md"
                },
            ),
            "/tmp/jobs",
        )
