...
REPO_URL = os.environ.get(
    "AGENT_EVALS_REPO_URL", "https://github.com/simtropolis/sim.git"
)
ZIG_URL = (
...
def build_eval_cli(build_request: dict[str, Any]) -> dict[str, Any]:
    ...
    workdir = pathlib.Path("/build/sim")
    if workdir.exists():
...
def cleanup_artifacts(request: dict[str, Any]) -> dict[str, Any]:
    """On-demand prune (the `sim-eval cleanup` command), supporting --dry-run
...
def run_controller(run_request: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        package_file = sim_eval.__file__
        if package_file is None:
            raise RuntimeError("could not resolve sim_eval package location")
