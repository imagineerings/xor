...
def resolve_source(
    ...
    Precedence: `--from` wins over `--sim-version` / `--base-sha`.
    ...
    sim_version = getattr(args, "sim_version", None)
    if sim_version:
        resolved = source.resolve_remote_ref(repo_url, sim_version)
        return None, True, sim_version, resolved
