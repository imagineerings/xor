...
def command_rejudge(args: argparse.Namespace) -> int:
    ...
    print(f"  sim-eval status {rejudge_request['run_id']}")
    print(f"  sim-eval report {rejudge_request['run_id']} --fetch")
    return 0
...
def launch_prepared_run(
    ...
    print(f"  sim-eval status {run_id}")
    print(f"  sim-eval logs {run_id}")
    print(f"  sim-eval report {run_id} --fetch")
