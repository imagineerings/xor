...
def add_build_source_options(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--clean-source",
        action="store_true",
        help="Build exactly --base-sha/--sim-version with no local patch",
    )
    parser.add_argument(
        "--sim-version",
        help="Git ref/tag/SHA of Sim to build as a clean source snapshot",
    )
...
def add_model_options(parser: argparse.ArgumentParser) -> None:
    ...
    advanced.add_argument(
        ...
        choices=("sim", "baseten"),
        default="sim",
        help="Use 'sim' for built-in provider/model ids, or 'baseten' for Baseten Model APIs",
    )
    advanced.add_argument(
        "--baseten-model",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Common workflows:\n"
            "  sim-eval run swe-atlas -m sonnet-4.6       # launch the SWE-Atlas suite\n"
            "  sim-eval run rf -m sonnet-4.6              # launch one benchmark\n"
            "  sim-eval runs                              # what did I launch recently?\n"
            "  sim-eval status                            # check my most recent run\n"
            "  sim-eval status <run-id>                   # check a specific run\n"
            "  sim-eval logs <run-id>                     # print controller logs\n"
            "  sim-eval report <run-id> --fetch           # fetch + score a run\n"
            "\n"
