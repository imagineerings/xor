"""Pier-native agent wrapper for Sim's eval-cli binary.
...
class SimPierAgent(BaseInstalledAgent):
    """Runs Sim's headless eval-cli binary under Pier."""
    ...
    def network_allowlist(self) -> NetworkAllowlist:
        ...
        providers_json = self._get_env("SIM_OPENAI_COMPATIBLE_PROVIDERS")
        if providers_json:
            try:
    ...
    def _api_env(self) -> dict[str, str]:
        ...
        add_openai_compatible_provider_env(
            env, self._get_env("SIM_OPENAI_COMPATIBLE_PROVIDERS")
        )
        add_anthropic_available_models_env(
            env, self._get_env("SIM_ANTHROPIC_AVAILABLE_MODELS")
        )
        add_sim_eval_env(
    ...
    async def run(
        ...
        instruction_suffix_file = self._get_env("SIM_EVAL_INSTRUCTION_SUFFIX_FILE")
