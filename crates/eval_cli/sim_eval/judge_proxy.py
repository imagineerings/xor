...
the `SIM_JUDGE_MAX_TOKENS` floor) rather than a copy per call site.
"""
...
# The shim listens on 127.0.0.1:PORT and forwards to SIM_JUDGE_UPSTREAM, applying
...
#     `max_tokens`) and raised to the SIM_JUDGE_MAX_TOKENS floor so the runtime
#     judge matches the offline-calibrated token budget.
# When SIM_JUDGE_AUTH_ENV is set, it replaces verifier auth with the named
# sandbox env var; otherwise it passes auth headers through unchanged.
JUDGE_PROXY_SCRIPT = """\
import json, os, urllib.request, urllib.error
from http.server import BaseHTTPRequestHandler, ThreadingHTTPServer

UPSTREAM = os.environ.get("SIM_JUDGE_UPSTREAM", "https://api.anthropic.com/v1").rstrip("/")
PORT = int(os.environ.get("SIM_JUDGE_PROXY_PORT", "8089"))
AUTH_ENV = os.environ.get("SIM_JUDGE_AUTH_ENV")
MAX_TOKENS_FLOOR = int(os.environ.get("SIM_JUDGE_MAX_TOKENS", "0"))
...
# the port; the daemon inherits SIM_JUDGE_UPSTREAM from the verifier setup exec
# env. stdout/stderr/stdin are fully detached so the spawning exec doesn't hang
# on open pipes.
JUDGE_PROXY_ENSURE_SCRIPT = """\
#!/bin/bash
if ! (exec 3<>/dev/tcp/127.0.0.1/${SIM_JUDGE_PROXY_PORT:-8089}) 2>/dev/null; then
    nohup python3 /usr/local/lib/sim_judge_proxy.py \\
        >>/tmp/sim-judge-proxy.log 2>&1 </dev/null &
    sleep 1
fi
"""
