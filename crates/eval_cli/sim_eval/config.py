...
JUDGE_PROXY_VERIFIER_IMPORT_PATH = "sim_eval.verifier:SimJudgeProxyVerifier"
...
class JudgeConfig:
    ...
    # judge shim (SIM_JUDGE_MAX_TOKENS). Set for Baseten judges so the runtime
...
def judge_verifier_args(judge: JudgeConfig, judge_model: str) -> list[str]:
    args = [
        ...
        "--ve",
        f"SIM_JUDGE_UPSTREAM={judge.upstream}",
        "--ve",
        f"SIM_JUDGE_AUTH_ENV={judge.auth_env}",
    ]
    if judge.max_tokens is not None:
        args.extend(["--ve", f"SIM_JUDGE_MAX_TOKENS={judge.max_tokens}"])
    return args
