...
def add_openai_compatible_provider_env(
    ...
    if providers_json:
        env["SIM_OPENAI_COMPATIBLE_PROVIDERS"] = providers_json
...
def add_anthropic_available_models_env(
    ...
    if models_json:
        env["SIM_ANTHROPIC_AVAILABLE_MODELS"] = models_json


def add_sim_eval_env(
    env: dict[str, str], extra_env: dict[str, str], *, exclude: set[str] | None = None
) -> None:
    exclude = exclude or set()
    for key, value in extra_env.items():
        if key.startswith("SIM_EVAL_") and key not in exclude:
            env[key] = value
...
def eval_cli_with_log_command(
    ...
    *,
    ...
    """
    status_file = "/tmp/sim-eval-eval-cli-status"
