...
mod tests {
    ...
    async fn test_urls(cx: &mut gpui::TestAppContext) {
        ...
        cx.set_state(indoc! {"
            Let's test a [complex](https://sim.dev/channel/had-(oops)) caseˇ.
        "});

        let screen_coord = cx.pixel_position(indoc! {"
            Let's test a [complex](https://sim.dev/channel/had-(ˇoops)) case.
            "});
        ...
        cx.assert_editor_text_highlights(
            HighlightKey::HoveredLinkState,
            indoc! {"
            Let's test a [complex](«https://sim.dev/channel/had-(oops)ˇ») case.
        "},
        );
        ...
        assert_eq!(
            cx.opened_url(),
            Some("https://sim.dev/channel/had-(oops)".into())
        );
    }
    ...
    async fn test_hover_preconditions(cx: &mut gpui::TestAppContext) {
        ...
        // No link
        cx.set_state(indoc! {"
            Let's test a [complex](https://sim.dev/channel/) caseˇ.
        "});
        assert_no_highlight!(cx);

        // No modifier
        let screen_coord = cx.pixel_position(indoc! {"
            Let's test a [complex](https://sim.dev/channel/ˇ) case.
            "});
        ...
        // Modifier active
        let screen_coord = cx.pixel_position(indoc! {"
            Let's test a [complex](https://sim.dev/channeˇl/) case.
            "});
        cx.simulate_mouse_move(screen_coord, None, Modifiers::secondary_key());
        cx.assert_editor_text_highlights(
            HighlightKey::HoveredLinkState,
    ...
    async fn test_urls_at_beginning_of_buffer(cx: &mut gpui::TestAppContext) {
        ...
        cx.set_state(indoc! {"https://sim.dev/releases is a cool ˇwebpage."});

        let screen_coord =
            cx.pixel_position(indoc! {"https://sim.dev/relˇeases is a cool webpage."});

        cx.simulate_mouse_move(screen_coord, None, Modifiers::secondary_key());
        cx.assert_editor_text_highlights(
            HighlightKey::HoveredLinkState,
            indoc! {"«https://sim.dev/releasesˇ» is a cool webpage."},
        );

        cx.simulate_click(screen_coord, Modifiers::secondary_key());
        assert_eq!(cx.opened_url(), Some("https://sim.dev/releases".into()));
    }
    ...
    async fn test_urls_at_end_of_buffer(cx: &mut gpui::TestAppContext) {
        ...
        cx.set_state(indoc! {"A cool ˇwebpage is https://sim.dev/releases"});

        let screen_coord =
            cx.pixel_position(indoc! {"A cool webpage is https://sim.dev/releˇases"});

        cx.simulate_mouse_move(screen_coord, None, Modifiers::secondary_key());
        cx.assert_editor_text_highlights(
            HighlightKey::HoveredLinkState,
            indoc! {"A cool webpage is «https://sim.dev/releasesˇ»"},
        );
