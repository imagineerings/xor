    ...
    fn get_language_server_command(
        ...
        async move {
            let ret = maybe!(async move {
                ...
                // TODO: This should now be done via the `sim::make_file_executable` function in
                // Sim extension API, but we're leaving these existing usages in place temporarily
                // to avoid any compatibility issues between Sim and the extension versions.
                //
