...
const PROFILE: &str = "00000000-0000-0000-0000-000000042500";
const KEY_ID: &str = "task425.publisher";
const KEY: &[u8; 32] = b"0123456789abcdef0123456789abcdef";
const NODE_ID: &str = "OpenRouterLLMNode";
    ...
    smol::block_on(async {
        ...
        let actuator = smol::spawn(async move {
            let run = async move {
                ...
                assert!(matches!(
                    route.receive_call(Duration::from_millis(20)).await,
...
struct ProviderBridgeRunEvidence {
...
struct ProviderBridgeHarness {
    profile_id: ProfileId,
    host: ComponentHost,
    executor: Arc<PrivateWorkerPluginExecutor>,
...
impl ProviderBridgeHarness {
    async fn new(plugin_worker_arguments: Vec<String>) -> Result<Self, Box<dyn Error>> {
        let profile_id = ProfileId(Uuid::parse_str(PROFILE)?);
        let base_registry = comfy_runtime::generated_native_node_registry_projection(None)?;
        let contract_sha256 = base_registry
            .provider_binding_contract_sha256(
...
async fn drive_provider_v2_actuator(
    actuator_receiver: async_channel::Receiver<NativeProviderWorkerV2ActuatorRoute>,
    provider_policy: ProviderPolicy,
    invocation_timeout: Duration,
...
impl ControllerFixture {
    fn new(profile_id: ProfileId) -> Result<Self, Box<dyn Error>> {
...
fn provider_inputs() -> Result<InvocationInputs, Box<dyn Error>> {
...
fn provider_policy() -> Result<ProviderPolicy, Box<dyn Error>> {
...
fn provider_manifest(
    ...
    let mut manifest = PluginManifest {
        ...
        provenance: ManifestProvenance {
            source: "fixture://task425-provider".to_owned(),
...
fn provider_component() -> Result<Vec<u8>, Box<dyn Error>> {
    decode_base64(
        include_str!("../../comfy_plugin_host/tests/fixtures/provider_streaming_component").trim(),
    )
}
...
fn worker_policy() -> SupervisorPolicy {
    SupervisorPolicy {
