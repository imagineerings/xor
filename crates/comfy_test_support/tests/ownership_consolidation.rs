...
#[test]
fn val_ownership_native_video_codec_h264_mp4_thread_bridge_001()
-> Result<(), Box<dyn std::error::Error>> {
    let root = repository_root()?;
    let service =
        fs::read_to_string(root.join("crates/comfy_runtime/src/native_video_codec_service.rs"))?;
    for required in [
        ...
        assert!(
            service.contains(required),
            "H.264 MP4 thread bridge lacks {required}"
        );
    }
    for forbidden in [
        ...
        assert!(
            !service.contains(forbidden),
            "H.264 MP4 thread bridge exposes forbidden {forbidden}"
        );
    }
    ...
    for required in [
        "synthetic-retained-h264-mp4-owned-tensor-thread-bridge",
        "one_deliberate_post_encode_copy_observed",
        "native_or_workspace_owner_crosses_thread_response",
        "public_node_service_backing_slice_effect_path_or_output_committer_reachable",
        "media_payload_or_handle_published",
    ] {
        assert!(
            fixture.contains(required),
            "H.264 MP4 thread fixture lacks {required}"
        );
    }
    ...
    let concern = concerns
        ...
        .find(|concern| {
            concern.get("concern").and_then(serde_json::Value::as_str)
                == Some("native_video_reviewed_codec_registry_h264_mp4_thread_bridge")
        })
        .ok_or("missing H.264 MP4 thread bridge ownership concern")?;
    assert_eq!(
        concern
            .get("canonical_owner")
            .and_then(serde_json::Value::as_str),
        Some(
            "comfy_runtime::native_video_codec_service::NativeLtxvCodecRequestProxy::encode_h264_mp4_batch"
        )
    );
    let task_id = "comfy-parity-native-video-codec-h264-mp4-thread-bridge-foundation";
    let mapped_concerns = concerns
        .iter()
        .filter(|concern| {
            concern
...
#[test]
fn val_ownership_native_video_backing_representation_001() -> Result<(), Box<dyn std::error::Error>>
{
    let root = repository_root()?;
    let media = fs::read_to_string(root.join("crates/comfy_media/src/native_node_payload.rs"))?;
    for required in [
        ...
        assert!(
            media.contains(required),
            "VIDEO backing media owner lacks {required}"
        );
    }
    for forbidden in [
        "file_path:",
        "PathBuf",
        "NativeVideoTrimWindow",
        "unsafe impl Send",
    ] {
        assert!(
            !media.contains(forbidden),
            "VIDEO backing media owner exposes deferred {forbidden}"
        );
    }

    let planner = fs::read_to_string(root.join("crates/comfy_media/src/video.rs"))?;
    assert!(
        planner.contains(".components()") && planner.contains("InvalidVideo"),
        "video planner does not reject encoded backing"
    );
    let nodes = fs::read_to_string(root.join("crates/comfy_nodes/src/families/video_01.rs"))?;
    assert!(
        nodes.contains("cannot decode an encoded VIDEO backing yet"),
        "GetVideoComponents does not fail closed on encoded backing"
    );
    let provider =
        fs::read_to_string(root.join("crates/comfy_runtime/src/provider_materialization.rs"))?;
    assert!(
        provider.contains(".components()")
            && provider.contains("ProviderMaterializationError::InvalidNativePayload"),
        "provider materialization does not reject encoded backing"
    );
    let plugin = fs::read_to_string(root.join("crates/comfy_plugin_host/src/registry_adapter.rs"))?;
    assert!(
        plugin.contains("stored") && plugin.contains(".components()"),
        "plugin projection does not require component backing"
    );
    let runtime =
        fs::read_to_string(root.join("crates/comfy_runtime/src/native_video_codec_service.rs"))?;
    assert!(
        runtime.contains("NativeVideoPayload::checked_h264_mp4_from_component"),
        "H.264 backing service does not return canonical encoded VIDEO"
    );
    let fixture = fs::read_to_string(root.join(
        "crates/comfy_test_support/fixtures/video/encoded-video-backing-payload/manifest.json",
    ))?;
    for required in [
        "synthetic-encoded-video-backing-payload",
        "actor_owned_tensor_moved_without_second_copy",
        "component_consumers_reject_encoded_without_decode",
        "trim_window_or_video_slice_reachable",
    ] {
        assert!(
            fixture.contains(required),
            "encoded VIDEO fixture lacks {required}"
        );
    }
    ...
    let concern = concerns
        ...
        .ok_or("missing canonical VIDEO ownership concern")?;
    assert_eq!(
        concern
            .get("canonical_owner")
            .and_then(serde_json::Value::as_str),
        Some("comfy_media::NativeVideoPayload")
    );
    let task_id = "comfy-parity-native-video-backing-representation-foundation";
    assert_eq!(
        concerns
            .iter()
            .filter(|concern| {
...
#[test]
fn val_ownership_native_video_codec_ltxv_h264_mp4_demux_001()
-> Result<(), Box<dyn std::error::Error>> {
    let root = repository_root()?;
    let ffi = fs::read_to_string(root.join("crates/comfy_runtime/src/native_video_codec_ffi.rs"))?;
    for required in [
        "pub(crate) struct NativeLtxvH264DemuxLimits",
        "pub(crate) struct NativeLtxvH264Demux",
        "pub(crate) fn open_first_h264_video_stream",
        "open_bounded_avio_borrowed_input",
        "open_first_ltxv_h264_stream_with_check",
        "avformat_open_input",
        "av_find_best_stream",
        "retained_ltxv_mp4_demux_borrows_bytes_and_selects_first_h264_video_stream",
        "retained_ltxv_mp4_demux_rejects_stream_drift_and_cancellation_atomically",
    ] {
        assert!(
            ffi.contains(required),
            "LTXV H.264 MP4 demux lacks {required}"
        );
    }
    for forbidden in [
        "pub fn format_context_ptr",
        "pub fn input_context_ptr",
        "NativeStoredPayload::Video",
        "OutputCommitter",
    ] {
        assert!(
            !ffi.contains(forbidden),
            "LTXV H.264 MP4 demux exposes forbidden {forbidden}"
        );
    }
    ...
    for required in [
        "synthetic-retained-ltxv-h264-mp4-demux",
        "native_demux_call_choreography_executed",
        "whole_input_copy",
        "packet_decode_executed",
        "decoded_rgb_result_produced",
        "media_payload_or_handle_published",
    ] {
        assert!(
            fixture.contains(required),
            "LTXV demux fixture lacks {required}"
        );
    }
    ...
    let concern = policy
        ...
        .and_then(|concerns| {
            concerns.iter().find(|concern| {
                concern.get("concern").and_then(serde_json::Value::as_str)
                    == Some("native_video_reviewed_codec_registry_ltxv_h264_mp4_reopen")
            })
        })
        .ok_or("missing LTXV H.264 MP4 demux ownership concern")?;
    assert!(
        concern
            .get("consolidation_tasks")
            .and_then(serde_json::Value::as_array)
            .is_some_and(|tasks| tasks.iter().any(|task| {
                task.as_str()
                    == Some("comfy-parity-native-video-codec-ltxv-h264-mp4-demux-foundation")
            }))
    );
    Ok(())
}
...
#[test]
fn val_ownership_native_video_codec_ltxv_h264_mp4_decode_001()
-> Result<(), Box<dyn std::error::Error>> {
    let root = repository_root()?;
    let ffi = fs::read_to_string(root.join("crates/comfy_runtime/src/native_video_codec_ffi.rs"))?;
    for required in [
        ...
        "av_read_frame",
        "sws_scale",
        "retained_ltxv_h264_decode_returns_first_rgb8_frame_and_drains_packets",
        "retained_ltxv_h264_decode_failure_cancellation_and_retry_are_atomic",
    ] {
        assert!(ffi.contains(required), "LTXV H.264 decode lacks {required}");
    }
    for forbidden in [
        "pub fn frame_ptr",
        "pub fn packet_ptr",
        ...
        assert!(
            !ffi.contains(forbidden),
            "LTXV H.264 decode exposes forbidden {forbidden}"
        );
    }
    ...
    for required in [
        "synthetic-retained-ltxv-h264-first-frame-decode",
        "native_decode_call_choreography_executed",
        "decoded_rgb_tensor_constructed",
        "valid_mp4_h264_numeric_oracle",
        "media_payload_or_handle_published",
    ] {
        assert!(
            fixture.contains(required),
            "LTXV decode fixture lacks {required}"
        );
    }
    ...
    let concern = policy
        ...
        .and_then(|concerns| {
            concerns.iter().find(|concern| {
                concern.get("concern").and_then(serde_json::Value::as_str)
                    == Some(
                        "native_video_reviewed_codec_registry_ltxv_h264_mp4_retained_first_frame_decode",
                    )
            })
        })
        .ok_or("missing LTXV H.264 MP4 first-frame decode ownership concern")?;
    assert!(
        concern
            .get("consolidation_tasks")
            .and_then(serde_json::Value::as_array)
            .is_some_and(|tasks| tasks.iter().any(|task| {
                task.as_str()
                    == Some("comfy-parity-native-video-codec-ltxv-h264-mp4-decode-foundation")
            }))
    );
    Ok(())
}
...
#[test]
fn val_ownership_native_video_codec_ltxv_node_service_001() -> Result<(), Box<dyn std::error::Error>>
{
    let root = repository_root()?;
    let nodes = fs::read_to_string(root.join("crates/comfy_nodes/src/execution.rs"))?;
    for required in [
        ...
        "ltxv_preprocess_service_requires_checked_identity_and_preserves_context_authority",
    ] {
        assert!(
            nodes.contains(required),
            "LTXV node service lacks {required}"
        );
    }
    ...
    for required in [
        ...
        assert!(
            codec_service.contains(required),
            "LTXV codec proxy adapter lacks {required}"
        );
    }
    ...
    for required in [
        ...
        assert!(
            executor.contains(required),
            "execution engine omits LTXV service propagation {required}"
        );
    }
    ...
    for required in [
        ...
        assert!(
            native_executor.contains(required),
            "native image executor omits LTXV service identity or propagation {required}"
        );
    }
    ...
    for forbidden in [
        ...
        "unsafe impl Sync",
        "Arc<NativeLtxvH264Codec>",
        "NativeStoredPayload::Ltxv",
    ] {
        assert!(!nodes.contains(forbidden));
        assert!(!codec_service.contains(forbidden));
        assert!(!executor.contains(forbidden));
        assert!(!native_executor.contains(forbidden));
    }

    let fixture = fs::read_to_string(
        root.join("crates/comfy_test_support/fixtures/video/codec-ltxv-node-service/manifest.json"),
    )?;
    for required in [
        "synthetic-ltxv-node-service-injection",
        "execution_engine_exact_proxy_propagation",
        "configuration_identity_partitions_cache",
        "codec_package_discovery_or_provisioning",
        "ltxv_preprocess_node_implemented",
    ] {
    ...
    let concern = policy
        ...
        .and_then(|concerns| {
            concerns.iter().find(|concern| {
                concern.get("concern").and_then(serde_json::Value::as_str)
                    == Some("native_video_reviewed_codec_service_ltxv_node_injection")
            })
        })
        .ok_or("missing LTXV node-service injection ownership concern")?;
    assert!(
        concern
            .get("consolidation_tasks")
            .and_then(serde_json::Value::as_array)
            .is_some_and(|tasks| tasks.iter().any(|task| {
                task.as_str()
                    == Some("comfy-parity-native-video-codec-ltxv-node-service-foundation")
            }))
    );
    Ok(())
}
...
#[test]
fn val_ownership_native_tensor_grid_sample_foundation_001() -> Result<(), Box<dyn std::error::Error>>
{
    let root = repository_root()?;
    ...
    for required in [
        ...
        assert!(
            source.contains(required),
            "tensor grid sample lacks {required}"
        );
    }
    let tests = fs::read_to_string(
        root.join("crates/comfy_tensor/tests/ops/spatial_functional_kernel_01.rs"),
    )?;
    assert!(tests.contains("grid_sample_tensor_is_bounded_dtype_preserving_and_failure_atomic"));
                ...
                concern.get("concern").and_then(serde_json::Value::as_str)
                    ...
                    && concern
                        ...
                        .is_some_and(|tasks| {
                            tasks.iter().any(|task| {
                                task.as_str()
                                    == Some("comfy-parity-native-tensor-grid-sample-foundation")
                            })
                        })
            }))
    );
...
#[test]
fn val_ownership_native_film_warp_foundation_001() -> Result<(), Box<dyn std::error::Error>> {
    let root = repository_root()?;
    let model = fs::read_to_string(root.join("crates/comfy_model/src/frame_interpolation.rs"))?;
    for required in [
        "pub fn film_warp_with_context_exact_native(",
        "fn film_base_grid(",
        "Scalar::Float(2.0 / width as f64)",
        "Scalar::Float(2.0 / height as f64)",
        "GridPaddingMode::Border",
        "align_corners: false",
        "film_warp_uses_pixel_centers_and_is_failure_atomic",
    ] {
        assert!(
            model.contains(required),
            "bounded FILM warp lacks {required}"
        );
    }
    let fixture = fs::read_to_string(root.join(
    ...
    assert!(fixture.contains("film_net.py"));
    assert!(fixture.contains("two_by_two_half_pixel_horizontal_flow"));
    assert!(fixture.contains("\"align_corners\": false"));
    assert!(fixture.contains("\"rife_grid_semantics\": false"));
    assert!(fixture.contains("\"codec_execution\": false"));
    let policy: serde_json::Value = serde_json::from_str(&fs::read_to_string(
        root.join(".agents/specs/comfy-parity/ownership-policy.json"),
    )?)?;
    assert!(
        policy
            ...
            .is_some_and(|concerns| concerns.iter().any(|concern| {
                concern.get("concern").and_then(serde_json::Value::as_str)
                    == Some("native_rife_frame_interpolation_film_pixel_center_warp_execution")
                    && concern
                        .get("consolidation_tasks")
                        .and_then(serde_json::Value::as_array)
                        .is_some_and(|tasks| {
                            tasks.iter().any(|task| {
                                task.as_str() == Some("comfy-parity-native-film-warp-foundation")
                            })
                        })
            }))
    );
...
#[test]
fn val_ownership_native_film_flow_pyramid_synthesis_foundation_001()
-> Result<(), Box<dyn std::error::Error>> {
    let root = repository_root()?;
    let model = fs::read_to_string(root.join("crates/comfy_model/src/frame_interpolation.rs"))?;
    for required in [
        ...
        "film_flow_pyramid_synthesis_upsamples_doubles_adds_and_reverses",
    ] {
        assert!(
            model.contains(required),
            "FILM flow synthesis lacks {required}"
        );
    }
    let fixture = fs::read_to_string(root.join(
        "crates/comfy_test_support/fixtures/models/frame-interpolation/film-flow-pyramid-synthesis/manifest.json",
    ))?;
    assert!(fixture.contains("flow_pyramid_synthesis"));
    assert!(fixture.contains("[124.0, 12.0, 1.0]"));
    assert!(fixture.contains("\"coarsest_output_aliases_source\": true"));
    assert!(fixture.contains("\"flow_predictor_execution\": false"));
    let policy: serde_json::Value = serde_json::from_str(&fs::read_to_string(
        root.join(".agents/specs/comfy-parity/ownership-policy.json"),
    )?)?;
    assert!(
        policy
            ...
            .is_some_and(|concerns| concerns.iter().any(|concern| {
                concern.get("concern").and_then(serde_json::Value::as_str)
                    == Some("native_rife_frame_interpolation_film_flow_pyramid_synthesis")
                    && concern
                        .get("consolidation_tasks")
                        .and_then(serde_json::Value::as_array)
                        .is_some_and(|tasks| {
...
#[test]
fn val_ownership_native_film_fusion_foundation_001() -> Result<(), Box<dyn std::error::Error>> {
    let root = repository_root()?;
    let model = fs::read_to_string(root.join("crates/comfy_model/src/frame_interpolation.rs"))?;
    for required in [
        ...
        "fuse.output_conv",
        "film_fusion_executes_nearest_coarse_to_fine_convolution_schedule",
    ] {
        assert!(
            model.contains(required),
            "retained FILM fusion graph lacks {required}"
        );
    }
    let fixture = fs::read_to_string(root.join(
        "crates/comfy_test_support/fixtures/models/frame-interpolation/film-fusion/manifest.json",
    ))?;
    assert!(fixture.contains("Fusion.forward"));
    assert!(fixture.contains("groups_coarse_to_fine"));
    assert!(fixture.contains("[15.0, 30.0, 45.0]"));
    assert!(fixture.contains("\"production_dense_weight_numeric_parity\": false"));
    assert!(fixture.contains("\"multi_timestep_or_sequence_execution\": false"));
    ...
    assert!(
        policy
            ...
            .is_some_and(|concerns| concerns.iter().any(|concern| {
                concern.get("concern").and_then(serde_json::Value::as_str)
                    == Some("native_rife_frame_interpolation_film_fusion_execution")
                    && concern
                        .get("consolidation_tasks")
                        .and_then(serde_json::Value::as_array)
                        .is_some_and(|tasks| {
                            tasks.iter().any(|task| {
                                task.as_str() == Some("comfy-parity-native-film-fusion-foundation")
                            })
                        })
            }))
    );
...
fn val_ownership_native_film_image_pyramid_foundation_001() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    for required in [
        ...
        "film_image_pyramid_repeats_exact_pooling_and_is_failure_atomic",
    ] {
        assert!(
            model.contains(required),
            "bounded FILM image pyramid lacks {required}"
        );
    }
    let fixture = fs::read_to_string(root.join(
        "crates/comfy_test_support/fixtures/models/frame-interpolation/film-image-pyramid/manifest.json",
    ))?;
    assert!(fixture.contains("build_image_pyramid"));
    assert!(fixture.contains("three_level_four_by_four_image_pyramid"));
    assert!(fixture.contains("\"storage\": \"aliases_input\""));
    assert!(fixture.contains("\"film_feature_extractor_execution\": false"));
    assert!(fixture.contains("\"codec_execution\": false"));
    let policy: serde_json::Value = serde_json::from_str(&fs::read_to_string(
        root.join(".agents/specs/comfy-parity/ownership-policy.json"),
    )?)?;
    assert!(
        policy
            ...
            .is_some_and(|concerns| concerns.iter().any(|concern| {
                concern.get("concern").and_then(serde_json::Value::as_str)
                    == Some("native_rife_frame_interpolation_film_image_pyramid_execution")
                    && concern
                        .get("consolidation_tasks")
                        .and_then(serde_json::Value::as_array)
                        .is_some_and(|tasks| {
                            tasks.iter().any(|task| {
                                task.as_str()
                                    == Some("comfy-parity-native-film-image-pyramid-foundation")
                            })
                        })
            }))
    );
