//! A persistent registry of git worktrees that Sim itself created.
//!
//! Thread archival deletes a worktree's directory from disk, so it must be
//! certain the worktree was created by Sim rather than by the user. This
//! module records each Sim-created worktree in the local database, keyed by
//! its path (and remote host, for remote projects), along with the creation
//! time of the worktree's git metadata directory at the time Sim created it.
//!
...
//! recreated outside Sim, so deletion must be skipped. Every failure mode
...
//! different Sim install (e.g. another release channel, or another machine
...
/// Records that Sim created the worktree at `worktree_path`, along with the
...
/// Returns the recorded creation time for a worktree Sim created, or `None`
/// if Sim has no record of creating it.
...
/// Removes the record for a worktree, either because Sim deleted it or
/// because the directory on disk turned out not to be the one Sim created.
