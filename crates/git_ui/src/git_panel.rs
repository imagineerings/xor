...
impl GitPanel {
    ...
    fn new_with_serialized_panel(
        workspace: &mut Workspace,
        serialized_panel: Option<SerialisimGitPanel>,
        window: &mut Window,
        cx: &mut Context<Workspace>,
    ) -> Entity<Self> {
    ...
    fn render_history_tab(&self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        v_flex().flex_1().size_full().overflow_hidden().map(|this| {
            ...
            let has_commits = self
                .commit_history_shas
                .as_ref()
                .is_some_and(|shas| !shas.is_empty());
    ...
    pub async fn load(
        ...
        workspace.update_in(&mut cx, |workspace, window, cx| {
            GitPanel::new_with_serialized_panel(workspace, serialisim_panel, window, cx)
        })
    }
...
mod tests {
    ...
    async fn test_commit_message_restored_after_reconnect(cx: &mut TestAppContext) {
        ...
        let mismatched_serialized_panel = SerialisimGitPanel {
            signoff_enabled: false,
