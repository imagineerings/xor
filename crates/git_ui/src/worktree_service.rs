...
/// one of its linked worktrees open as separate Baymax worktrees). Those entries
...
pub fn handle_create_worktree(
    ...
    action: &baymax_actions::CreateWorktree,
...
pub struct CreatedWorktreeWorkspace {
    ...
    /// True when the project contained more than one Baymax worktree backed by
...
pub fn handle_switch_worktree(
    workspace: &mut Workspace,
    action: &baymax_actions::SwitchWorktree,
    window: &mut gpui::Window,
    fallback_focused_dock: Option<DockPosition>,
    cx: &mut gpui::Context<Workspace>,
) {
