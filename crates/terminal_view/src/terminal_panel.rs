...
mod tests {
    ...
    #[gpui::test]
    async fn structured_task_terminal_navigation_uses_existing_panel_item(cx: &mut TestAppContext) {
        cx.executor().allow_parking();
        init_test(cx);
        ...
        let terminal = window_handle
            ...
            .expect("terminal panel window should remain available");
        let terminal_id = task::StructuredTerminalId(terminal.entity_id().as_u64());
        let handle = StructuredTaskHandle::new(TaskId("navigation".to_string()));
        window_handle
            .update(cx, |_, window, cx| {
                handle.mark_running(Some(terminal_id), cx);
                let terminal_panel = terminal_panel.clone();
                handle.set_reveal(move |window, cx| {
                    TerminalPanel::reveal_structured_terminal_handle(
                        &terminal_panel,
                        terminal_id,
                        window,
                ...
                assert!(handle.reveal_terminal(window, cx));
            })
            .expect("terminal panel window should remain available");
    }
