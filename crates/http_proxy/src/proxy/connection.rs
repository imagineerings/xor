...
fn is_forbidden_ip(ip: IpAddr) -> bool {
    ...
    // building `bwrap_test_helper`), so in a real Sim build the env var has no
    // effect and cannot disable DNS-rebinding/SSRF protection.
    #[cfg(feature = "nixos-integration-tests")]
    if std::env::var_os("SIM_SANDBOX_PROXY_ALLOW_LOCAL_IPS").is_some() {
        return false;
    }
    match ip {
...
fn deny_request(
    ...
    let body = format!(
        "Request blocked by the Sim sandbox network policy.\n\n  \
         Reason: {}\n\n  \
    ...
    let response = format!(
        "HTTP/1.1 511 Network Authentication Required\r\n\
         Via: 1.1 sim-sandbox-proxy\r\n\
         Proxy-Status: sim-sandbox-proxy; error={}; details=\"{}\"\r\n\
