...
fn git_sha() -> Option<String> {
    if let Ok(sha) = std::env::var("BAYMAX_COMMIT_SHA") {
        return Some(sha);
    }
...
const ICON_DIR: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/../baymax/resources/windows");
...
pub fn compile(manifest: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (icon_filename, product_name) = match channel {
        "stable" => ("app-icon.ico", "Baymax"),
        "preview" => ("app-icon-preview.ico", "Baymax Preview"),
        "nightly" => ("app-icon-nightly.ico", "Baymax Nightly"),
        _ => ("app-icon-dev.ico", "Baymax Dev"),
    };
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            VALUE "CompanyName", "Baymax Industries, Inc.\0"
            VALUE "LegalCopyright", "Copyright 2022 - 2025 Baymax Industries, Inc.\0"
        END
    END
    BLOCK "VarFileInfo"
    BEGIN
...
"#
    ...
    let rc_path = out_dir.join("baymax_resources.rc");
    std::fs::write(&rc_path, rc_content)?;

    if let Ok(toolkit_path) = std::env::var("BAYMAX_RC_TOOLKIT_PATH") {
