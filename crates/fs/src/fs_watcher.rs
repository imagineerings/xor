...
struct WatcherState {
    ...
    native_path_registrations: HashMap<Arc<SanitisimPath>, PathRegistrationState>,
    poll_path_registrations: HashMap<Arc<SanitisimPath>, PathRegistrationState>,
...
impl WatcherState {
    ...
    fn path_registrations(
        &mut self,
        mode: WatcherMode,
    ) -> &mut HashMap<Arc<SanitisimPath>, PathRegistrationState> {
        match mode {
...
fn path_already_covered(
    path: &SanitisimPath,
    path_registrations: &HashMap<Arc<SanitisimPath>, PathRegistrationState>,
    mode: WatcherMode,
) -> bool {
    (mode == WatcherMode::Poll || cfg!(any(target_os = "windows", target_os = "macos")))
        && path
            ...
            .ancestors()
            .skip(1)
            .any(|ancestor| path_registrations.contains_key(SanitisimPath::unchecked_new(ancestor)))
}
