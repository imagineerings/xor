...
async fn test_rename_folder_with_dot_selects_whole_name(cx: &mut gpui::TestAppContext) {
    ...
    panel.update_in(cx, |panel, window, cx| {
        panel.filename_editor.update(cx, |editor, cx| {
            ...
            assert_eq!(selections[0].end, MultiBufferOffset("my.folder".len()));
        });
    ...
    panel.update_in(cx, |panel, _window, cx| {
        panel.filename_editor.update(cx, |editor, cx| {
            let selections = editor
                .selections
                .all::<MultiBufferOffset>(&editor.display_snapshot(cx));
            assert_eq!(selections[0].end, MultiBufferOffset("archive.tar".len()));
        });
    });
}
...
// limitation of the Windows. Since Sim will have the root open, it will hold an open handle
