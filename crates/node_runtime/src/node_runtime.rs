        ...
        let instance = if options.allow_binary_download {
            ...
            match ManagedNodeRuntime::install_if_needed(&state.http).await {
                Ok(instance) => {
                    log::log!(
                        log_level,
                        "using Baymax managed Node.js at {} since {}",
                ...
                Err(err) => {
                    ...
                    Box::new(UnavailableNodeRuntime {
                        error_message: format!(
                            "failure while downloading and/or installing Baymax managed Node.js, \
                            restart Baymax to retry: {}",
                            err
                        )
                        .into(),
                    }) as Box<dyn NodeRuntimeTrait>
            ...
            match result {
                Ok(output) => {
                    if output.status.success() {
                        ...
                        log::warn!(
                            "Baymax managed Node.js binary at {} failed check with output: {:?}",
                            node_binary.display(),
                            output
                        );
                        false
                ...
                Err(err) => {
                    log::warn!(
                        "Baymax managed Node.js binary at {} failed check, so re-downloading it. \
                        Error: {}",
                        node_binary.display(),
                        err
                    );
...
mod tests {
    ...
    fn test_build_npm_command_args_inserts_prefix_before_subcommand() {
        let args = build_npm_command_args(
            None,
            Some(Path::new("/tmp/baymax-prefix")),
            Path::new("/tmp/cache"),
            None,
            None,
            None,
