...
use client::{Client, ProxySettings, RefreshLlmTokenListener, UserStore, parse_sim_link};
...
use crate::sim::{CrashHandler, OpenRequestKind, eager_load_active_theme_and_icon_theme};
...
fn files_not_created_on_launch(errors: HashMap<io::ErrorKind, Vec<&Path>>) {
    let message = "Sim failed to launch";
...
fn fail_to_open_window(e: anyhow::Error, _cx: &mut App) {
    eprintln!(
        "Sim failed to open a window: {e:?}. See https://sim.dev/docs/linux for troubleshooting steps."
    );
    ...
    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
    {
        ...
        _cx.spawn(async move |_cx| {
            ...
            let notification_id = "dev.sim.Oops";
            proxy
                .add_notification(
                    notification_id,
                    Notification::new("Sim failed to launch")
                        .body(Some(
                            format!(
                                "{e:?}. See https://sim.dev/docs/linux for troubleshooting steps."
                            )
                            .as_str(),
                        ))
...
fn main() {
    ...
    // `sim --crash-handler` Makes sim operate in minidump crash handler mode
    ...
    if args.record_etw_trace {
        ...
        if let Err(error) =
            etw_tracing::record_etw_trace(sim_pid, &output_path, etw_socket.as_str())
        {
    ...
    #[cfg(target_os = "windows")]
    match util::get_sim_cli_path() {
        Ok(path) => askpass::set_askpass_program(path),
        Err(err) => {
            eprintln!("Error: {}", err);
            if std::option_env!("SIM_BUNDLE").is_some() {
                process::exit(1);
            }
        }
    }
    ...
    if args.system_specs {
        ...
        println!("Sim System Specs (from CLI):\n{}", system_specs);
        return;
    }
    ...
    let failed_single_instance_check = if *sim_env_vars::SIM_STATELESS
        || *release_channel::RELEASE_CHANNEL == ReleaseChannel::Dev
    {
        ...
        #[cfg(any(target_os = "linux", target_os = "freebsd"))]
        {
            crate::sim::listen_for_cli_connections(open_listener.clone()).is_err()
        }
        ...
        #[cfg(target_os = "macos")]
        {
            use sim::mac_only_instance::*;
            ensure_only_instance() != IsOnlyInstance::Yes
        }
    };
    if failed_single_instance_check {
        println!("sim is already running");
        return;
    }
        ...
        Some(
            app.background_executor().spawn(crashes::init(
                InitCrashHandler {
                    ...
                    sim_version: semver::Version::new(
                        app_version.major,
                        app_version.minor,
                        app_version.patch,
                    )
                    .to_string(),
                    binary: "sim".to_string(),
                ...
                |pid| paths::temp_dir().join(format!("sim-crash-handler-{pid}")),
    ...
    app.run(move |cx| {
        ...
        sim::watch_settings_files(fs.clone(), cx);
        ...
        sim::init(cx);
        #[cfg(target_os = "macos")]
        sim::move_to_applications::init(cx);
        ...
        acp_tools::init(cx);
        sim::telemetry_log::init(cx);
        sim::remote_debug::init(cx);
        ...
        sim::watch_user_agents_md(app_state.fs.clone(), cx);
    ...
    if let Some(kind) = request.kind {
        match kind {
                    ...
                    multi_workspace.update(cx, |multi_workspace, window, cx| {
                        multi_workspace.workspace().update(cx, |workspace, cx| {
                            if let Some(panel) = workspace.focus_panel::<AgentPanel>(window, cx) {
                                ...
                                log::warn!(
                                    "sim://agent received but the AgentPanel is not registered \
                ...
                workspace::with_active_or_new_workspace(cx, |_workspace, window, cx| {
                    cx.spawn_in(window, async move |workspace, cx| {
                        let res = async move {
                            ...
                            let uri = format!("sim://schemas/{}", schema_path);
            ...
            OpenRequestKind::Setting { setting_path } => {
                // sim://settings/languages/$(language)/tab_size  - DONT SUPPORT
                // sim://settings/languages/Rust/tab_size  - SUPPORT
                ...
                cx.spawn(async move |cx| {
                    ...
                    workspace.update(cx, |_, window, cx| match setting_path {
                        None => window.dispatch_action(Box::new(sim_actions::OpenSettings), cx),
...
struct Args {
    ...
    ///
    /// URLs can either be `file://` or `sim://` scheme, or relative to <https://sim.dev>.
    ...
    /// On macOS, the default is `~/Library/Application Support/Sim`.
    /// On Linux/FreeBSD, the default is `$XDG_DATA_HOME/sim`.
    /// On Windows, the default is `%LOCALAPPDATA%\Sim`.
    ...
    /// Instructs sim to run as a dev server on this machine. (not implemented)
    #[arg(long)]
    ...
    /// prevents Sim from starting, so you can't run `sim: copy system specs to
    /// clipboard`
    #[arg(long)]
    system_specs: bool,

    /// Used for recording minidumps on crashes by having Sim run a separate
    ...
    /// The PID of the Sim process to trace for heap analysis.
    #[cfg(target_os = "windows")]
    #[arg(long, hide = true, allow_hyphen_values = true)]
    etw_sim_pid: Option<i64>,
    ...
    /// Unix socket path for IPC with the parent Sim process.
    #[cfg(target_os = "windows")]
    #[arg(long, hide = true)]
    ...
    match std::fs::canonicalize(Path::new(&arg)) {
        ...
        Err(_) => {
            if arg.starts_with("file://")
                || arg.starts_with("sim://")
                || arg.starts_with("sim-cli://")
                || arg.starts_with("ssh://")
                || parse_sim_link(arg, cx).is_some()
            {
                arg.into()
            } else {
                format!("file://{arg}")
