...
impl CliError {
    fn invalid(message: impl Into<String>) -> Self {
        Self {
            ...
            hint: Some("run `sim comfy --help` to inspect the native command surface".into()),
        }
    }
}
        ...
        CliAction::Invoke(invocation) => match invocation.disposition {
            CommandDisposition::Migration => {
                render_explicit_disposition(
                    ...
                    "This source command manages Python, ComfyUI, or Python custom nodes. Sim preserves its inputs as migration evidence and uses native runtime profiles and Rust/WASM plugins instead; no source process was started.",
                );
                MIGRATION_EXIT
            }
...
fn native_environment_report() -> Result<Value, CliError> {
    ...
    Ok(json!({
        "schema": "sim.comfy.environment/1",
        "runtime": "native-rust",
        "variables": variables,
...
fn native_profile_report() -> Value {
    ...
    json!({
        "schema": "sim.comfy.profile-selection/1",
        "runtime": "native-rust",
...
fn native_operation(invocation: &ParsedInvocation) -> Result<NativeExecutionPlan, CliError> {
    ...
    if has_option(&invocation.arguments, "--host") || has_option(&invocation.arguments, "--port") {
        return Ok(headless(NativeCliOperation::Migration {
            replacement: "socketless native runtime service or `sim comfy serve`".into(),
            detail: "source host/port options identify an external Comfy server and are never contacted by production Sim".into(),
        }));
    }
    ...
    match invocation.command_path.as_str() {
        ...
        "comfy jobs cancel" => Ok(headless(NativeCliOperation::CancelJobs {
            ...
            operation_id: format!("sim-comfy-cancel-{}", Uuid::new_v4()),
        })),
...
fn serve_configuration(
    ...
    let certificate_identity = certificate_path
        .as_ref()
        .map(|path| format!("sim-comfy-cli:{}", Path::new(path).display()));
    let tls = match (
...
fn parse_action(arguments: &[String]) -> Result<CliAction, CliError> {
    ...
    if arguments.get(command_start).map(String::as_str) == Some("serve") {
        ...
        return Ok(CliAction::Invoke(ParsedInvocation {
            feature_id: "SIM-COMFY-NATIVE-SERVE".into(),
...
fn validate_synthetic_serve_arguments(arguments: &[String]) -> Result<(), CliError> {
    ...
    while let Some(argument) = arguments.get(index) {
        ...
        return Err(CliError::invalid(format!(
            "unknown `sim comfy serve` option `{argument}`"
        )));
    }
    Ok(())
}
...
fn help_value(command_path: Option<&str>) -> Result<Value, CliError> {
    ...
    let commands = catalog
        ...
        .chain(std::iter::once(json!({
            "feature_id": "SIM-COMFY-NATIVE-SERVE",
            "path": "comfy serve",
            "hidden": false,
...
fn contract_registry() -> Result<Value, CliError> {
    ...
    for spec in CONTRACT_CATALOGS {
        ...
        catalogs.insert(
            spec.name.into(),
            json!({
                "schema": "sim.comfy.contract-catalog/1",
                "row_count": rows.len(),
                "sha256": spec.sha256,
                "rows": rows,
            }),
    ...
    Ok(json!({
        "schema": "sim.comfy.contract-registry/1",
        "catalogs": catalogs,
    }))
}
...
fn render_explicit_disposition(
    ...
    let value = json!({
        ...
        "error": {
            "code": code,
            "message": message,
            "replacement": "Sim native runtime profiles and versioned Rust/WASM plugins",
        }
    });
    match invocation.output_mode {
        OutputMode::Pretty => eprintln!("{}: {message}", invocation.command_path),
...
fn catalog() -> Result<&'static Catalog, CliError> {
    CATALOG
        ...
        .map_err(|error| CliError {
            code: "catalog_invalid".into(),
            message: error.clone(),
            hint: Some("the compiled Sim binary contains an invalid CLI catalog".into()),
        })
}
...
mod tests {
    ...
    #[derive(Parser, Debug)]
    #[command(name = "sim", subcommand_precedence_over_arg = true)]
    struct TestArgs {
        #[command(subcommand)]
    ...
    fn comfy_cli_contract_parses_every_command_parameter_and_disposition() -> anyhow::Result<()> {
        let parsed = TestArgs::try_parse_from(["sim", "comfy", "jobs", "ls", "--json"])?;
        ...
        assert_eq!(registry["schema"], "sim.comfy.contract-registry/1");
        for spec in CONTRACT_CATALOGS {
    ...
    fn comfy_cli_private_runtime_state_is_not_an_asset_namespace() -> anyhow::Result<()> {
        let temporary = std::env::temp_dir().join(format!("sim-comfy-cli-{}", Uuid::new_v4()));
