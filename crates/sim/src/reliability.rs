...
fn start_memory_usage_logging(cx: &App) {
    ...
    cx.background_spawn(async move {
        let Some(process_id) = sysinfo::get_current_pid().log_err() else {
            return;
        };
        let refresh_kind = ProcessRefreshKind::nothing().with_memory();
        let mut system = System::new();
        let mut last_logged_resident = None;
        ...
        loop {
            ...
            if refreshed == 1
                && let Some(process) = system.process(process_id)
            {
...
async fn upload_minidump(
    ...
    ::telemetry::event!(
        ...
        crashed_version = metadata.init.sim_version.clone(),
    ...
    for (index, gpu) in metadata.gpus.iter().cloned().enumerate() {
        ...
        form = form
            .text(
                ...
                "A GPU found on the users system. May or may not be the GPU Sim is running on",
            )
...
// users - except - we know internal users use Sim.
