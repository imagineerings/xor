...
//! Visual Test Runner
//!
//! This binary runs visual regression tests for Sim's UI. It captures screenshots
//! of real Sim windows and compares them against baseline images.
//!
...
#[cfg(target_os = "macos")]
fn main() {
    // Set SIM_STATELESS early to prevent file system access to real config directories
    // This must be done before any code accesses sim_env_vars::SIM_STATELESS
    // SAFETY: We're at the start of main(), before any threads are spawned
    unsafe {
        std::env::set_var("SIM_STATELESS", "1");
    }
...
mod constants {
    ...
    pub const BASELINE_DIR: &str = "crates/sim/test_fixtures/visual_tests";

    /// Embedded test image (Sim app icon) for visual tests.
...
fn run_visual_tests(project_path: PathBuf, update_baseline: bool) -> Result<()> {
    ...
    // Test settings use Courier font, but we want the real Sim fonts for visual tests
    cx.update(|cx| {
        settings::init(cx);
    });
...
This is a test project for visual testing of Sim.
...
fn run_agent_thread_view_test(
    ...
    let send_future = thread.update(cx, |thread, cx| {
        thread.send(vec!["Show me the Sim logo".into()], cx)
    });
...
fn run_multi_workspace_sidebar_visual_tests(
    ...
    let workspace2_dir = canonical_temp.join("sim");
    ...
    // Add worktree to workspace 2 (index 1) so it shows as "sim"
                ...
                container().child(
                    ThreadItem::new("ti-linked-no-branch", "Review worktree cleanup")
                        ...
                        .worktrees(vec![ThreadItemWorktreeInfo {
                            ...
                            full_path: "/worktrees/focal-arrow/sim".into(),
                            highlight_positions: Vec::new(),
                            kind: WorktreeKind::Linked,
                            branch_name: None,
                        }]),
                ...
                container().child(
                    ThreadItem::new("ti-same-name", "Implement feature")
                        ...
                        .worktrees(vec![ThreadItemWorktreeInfo {
                            ...
                            full_path: "/worktrees/stoic-reed/sim".into(),
                            highlight_positions: Vec::new(),
                            kind: WorktreeKind::Linked,
...
fn run_sidebar_duplicate_project_names_visual_tests(
    ...
    // Create directory structure where every leaf directory is named "sim" but
    ...
    //
    //   code/sim/       — project1 (single worktree)
    //   code/foo/sim/   — project2 (single worktree)
    //   code/bar/sim/   — project3, first worktree
    //   code/baz/sim/   — project3, second worktree
    //
    ...
    // Three projects, third has two worktrees (all leaf names "sim")
    //
    // project1: code/sim
    // project2: code/foo/sim
    // project3: code/bar/sim + code/baz/sim
    //
