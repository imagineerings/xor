...
impl MoveToApplicationsRequest {
    ...
    async fn prompt(
        ...
        let response = cx
            .prompt(
                PromptLevel::Info,
                "Move Sim to Applications?",
                Some(
                    "Sim is running from a temporary location. Move it to Applications to finish installing it.",
                ),
                &[
                    PromptButton::ok("Yes"),
            ...
            0 => {
                workspace
                    .update_in(cx, |workspace, window, cx| {
                        workspace
                            .toggle_modal(window, cx, |_window, cx| InstallingSimModal::new(cx));
                    })
                    .ok();
                if let Err(error) = move_to_applications(&self.app_path, cx).await {
                    ...
                    cx.prompt(
                        ...
                        "Failed to move Sim to Applications",
...
pub struct InstallingSimModal {
    focus_handle: FocusHandle,
    finished: bool,
}

impl InstallingSimModal {
...
impl EventEmitter<DismissEvent> for InstallingSimModal {}

impl ModalView for InstallingSimModal {
...
impl Focusable for InstallingSimModal {
    fn focus_handle(&self, _cx: &App) -> FocusHandle {
        self.focus_handle.clone()
    }
}

impl Render for InstallingSimModal {
        ...
        v_flex()
            ...
            .child(
                div()
                    ...
                    .child(Label::new("Installing Sim…")),
            )
            .child(
                h_flex()
                    .w_full()
                    ...
                    .child(
                        v_flex()
                            .gap_1()
                            .child(Label::new("Moving Sim to Applications"))
                            .child(
                                Label::new("Sim will reopen when installation is complete.")
