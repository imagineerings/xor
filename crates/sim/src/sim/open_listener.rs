...
use client::{SimLink, parse_sim_link};
...
impl OpenRequest {
    ...
    pub fn parse(request: RawOpenRequest, cx: &App) -> Result<Self> {
        ...
        for url in request.urls {
            if let Some(server_name) = url.strip_prefix("sim-cli://") {
                this.kind = Some(OpenRequestKind::CliConnection(connect_to_cli(server_name)?));
            } else if let Some(action_index) = url.strip_prefix("sim-dock-action://") {
                this.kind = Some(OpenRequestKind::DockMenuAction {
                    index: action_index.parse()?,
                });
            } else if let Some(file) = url.strip_prefix("file://") {
                this.parse_file_path(file)
            } else if let Some(file) = url.strip_prefix("sim://file") {
                this.parse_file_path(file)
            } else if let Some(file) = url.strip_prefix("sim://ssh") {
                let ssh_url = "ssh:/".to_string() + file;
                this.parse_ssh_file_path(&ssh_url, cx)?
            } else if let Some(extension_id) = url.strip_prefix("sim://extension/") {
            ...
            } else if url == "sim://" || url == "sim://open" || url == "sim://open/" {
                this.kind = Some(OpenRequestKind::FocusApp);
            } else if let Some(schema_path) = url.strip_prefix("sim://schemas/") {
                this.kind = Some(OpenRequestKind::BuiltinJsonSchema {
                    schema_path: schema_path.to_string(),
                });
            } else if url == "sim://settings" || url == "sim://settings/" {
                this.kind = Some(OpenRequestKind::Setting { setting_path: None });
            } else if let Some(setting_path) = url.strip_prefix("sim://settings/") {
                this.kind = Some(OpenRequestKind::Setting {
                    setting_path: Some(setting_path.to_string()),
                });
            } else if let Some(clone_path) = url.strip_prefix("sim://git/clone") {
                this.parse_git_clone_url(clone_path)?
            } else if let Some(commit_path) = url.strip_prefix("sim://git/commit/") {
                this.parse_git_commit_url(commit_path)?
            } else if url.starts_with("ssh://") {
                this.parse_ssh_file_path(&url, cx)?
            } else if let Some(sim_link) = parse_sim_link(&url, cx) {
                match sim_link {
    ...
    fn parse_skill_install_url(&mut self, url: &str) -> Result<()> {
        // Format: sim://skill?data=<base64url of SKILL.md contents>
...
pub fn listen_for_cli_connections(opener: OpenListener) -> Result<()> {
    ...
    let sock_path = paths::data_dir().join(format!("sim-{}.sock", *RELEASE_CHANNEL_NAME));
...
async fn open_local_workspace(
    ...
    // `std::env::current_dir()`, since the Sim app process's cwd is typically
...
mod tests {
    use super::*;
    use crate::sim::{open_listener::open_local_workspace, tests::init_test};
    use cli::CliResponse;
    use editor::Editor;
    use futures::poll;
    ...
    fn test_parse_malformed_skill_install_url_errors(cx: &mut TestAppContext) {
        ...
        let result = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["sim://skill?data=!!!notbase64".into()],
                    ..Default::default()
                },
                cx,
            )
    ...
    fn agent_url_with_prompt(prompt: &str) -> String {
        let mut serializer = url::form_urlencoded::Serializer::new("sim://agent?".to_string());
    ...
    fn test_parse_agent_url_with_trailing_slash(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["sim://agent/?prompt=hello".into()],
                    ..Default::default()
                },
                cx,
            )
    ...
    fn test_parse_focus_app_url(cx: &mut TestAppContext) {
        ...
        for url in ["sim://", "sim://open", "sim://open/"] {
            let request = cx.update(|cx| {
                OpenRequest::parse(
                    RawOpenRequest {
                        urls: vec![url.into()],
    ...
    fn test_parse_git_commit_url(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["sim://git/commit/abc123?repo=path/to/repo".into()],
                    ..Default::default()
                },
                cx,
            )
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["sim://git/commit/def456?repo=path%20with%20spaces".into()],
                    ..Default::default()
                },
                cx,
            )
        ...
        cx.update(|cx| {
            assert!(
                OpenRequest::parse(
                    RawOpenRequest {
                        urls: vec!["sim://git/commit/abc123?repo=".into()],
                        ..Default::default()
                    },
                    cx,
                )
        ...
        let result = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec!["sim://git/commit/abc123?foo=bar".into()],
                    ..Default::default()
                },
                cx,
            )
    ...
    async fn test_open_workspace_with_nonexistent_files(cx: &mut TestAppContext) {
        ...
        // but tell Sim to add it to the current workspace
        open_workspace_file(
            path!("/root/file6.txt"),
            workspace::OpenOptions {
        ...
        // but tell Sim to NOT add it to the current workspace
        open_workspace_file(
            path!("/root/file7.txt"),
            workspace::OpenOptions {
                workspace_matching: workspace::WorkspaceMatching::None,
    ...
    fn test_parse_git_clone_url(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec![
                        "sim://git/clone/?repo=https://github.com/simtropolis/sim.git".into(),
                    ],
                    ..Default::default()
                },
                cx,
    ...
    fn test_parse_git_clone_url_with_encoding(cx: &mut TestAppContext) {
        ...
        let request = cx.update(|cx| {
            OpenRequest::parse(
                RawOpenRequest {
                    urls: vec![
                        "sim://git/clone/?repo=https%3A%2F%2Fgithub.com%2Fsimtropolis%2Fsim.git"
                            .into(),
                    ],
                    ..Default::default()
                },
    ...
    async fn test_e2e_no_flags_no_windows_no_prompt(cx: &mut TestAppContext) {
        ...
        let (status, prompt_shown) = run_cli_with_sim_handler(
            cx,
            app_state,
