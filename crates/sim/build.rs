...
fn main() {
    #[cfg(target_os = "linux")]
    {
        ...
        for lib in &dlopened_libs {
            if let Some(libdir) = pkg_config::get_variable(lib, "libdir").ok() {
                ...
                eprintln!("sim build.rs: {lib} not found in pkg-config's path");
            }
        }
    ...
    if cfg!(target_os = "macos") {
        ...
        // Weakly link ReplayKit to ensure Sim can be used on macOS 10.15+.
    ...
    let git_sha = match std::env::var("SIM_COMMIT_SHA").ok() {
        Some(git_sha) => {
    ...
    if let Some(git_sha) = git_sha {
            ...
            && build_profile == "release"
        {
            ...
            println!("cargo::warning=Info: using '{git_sha}' hash for SIM_COMMIT_SHA env var");
        }
    }
