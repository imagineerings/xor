...
use mcp_memory::{MemoryServer, SqliteMemoryStore};
...
fn main() -> Result<()> {
    let database_path = std::env::args()
        .nth(1)
        .or_else(|| std::env::var("SIM_MEMORY_MCP_DB").ok())
        .ok_or_else(|| anyhow!("usage: mcp-memory <memory-sqlite-path>"))?;
    let store = SqliteMemoryStore::open(database_path)?;
...
fn handle_json_rpc_request(server: &MemoryServer, line: &str) -> Value {
