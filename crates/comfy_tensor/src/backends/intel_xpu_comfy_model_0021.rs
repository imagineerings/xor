...
use comfy_backend_xpu::{
    XpuAllocation, XpuElementType, XpuEvent, XpuExecutionError, XpuExecutionSession,
};
use comfy_types::DeviceKind;
use std::{fmt, sync::Arc};

const MAX_XPU_STREAMS: usize = 1_024;
const MAX_XPU_PENDING_EVENTS: usize = 4_096;

struct RuntimeAdapter(XpuExecutionSession);

#[derive(Clone)]
enum XpuTrackedEvent {
    Native(XpuEvent),
    Synchronized,
}
...
impl RuntimeAdapter {
    ...
    ) -> Result<XpuAllocation, TensorError> {
        let element_type = xpu_element_type(dtype)?;
    ...
    fn add(
        &self,
        _stream: &(),
        _element_type: XpuElementType,
        left: &XpuAllocation,
        right: &XpuAllocation,
        output: &XpuAllocation,
        _elements: u64,
        cancellation: &CancellationToken,
    ) -> Result<XpuTrackedEvent, TensorError> {
        self.0
...
struct XpuStorageInner {
    allocation: Option<XpuAllocation>,
    byte_length: u64,
...
struct XpuStorage {
    backend_id: u64,
    device: DeviceId,
    inner: Arc<XpuStorageInner>,
}

enum PreparedCopySource {
    Cpu(Vec<u8>),
    Xpu {
        storage: Arc<XpuStorageInner>,
        byte_offset: u64,
    },
}

impl fmt::Debug for XpuStorage {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("XpuStorage")
...
impl BackendStorage for XpuStorage {
...
pub struct XpuTensorBackend {
    ...
    events: BackendEventTracker<XpuTrackedEvent>,
}

impl XpuTensorBackend {
    pub fn from_certified_session(
        session: XpuExecutionSession,
        ...
        let device = DeviceId::new(DeviceKind::Xpu, ordinal);
        let (major, minor) = vendor
            .level_zero_api_versions()
            .iter()
            .copied()
            .min()
            .ok_or_else(|| TensorError::Faulted {
                reason: "certified XPU session has no Level Zero API version".to_owned(),
            })?;
        let one_dnn = vendor.onednn_version();
        let properties = NativeDeviceProperties::new_with_allocation_limit(
            ...
            u32::from(minor),
            Some(format!(
                "Intel {:#06x}:{:#06x}; oneDNN {}.{}.{}",
                vendor.vendor_id(), vendor.device_id(), one_dnn.0, one_dnn.1, one_dnn.2
            )),
            true,
        )?;
        Self::from_runtime(
    ...
    fn from_runtime(
        ...
        if effective_memory_limit_bytes == 0 {
            return Err(TensorError::AllocationFailed {
                ...
                reason: "XPU effective memory limit must be nonzero".to_owned(),
            });
        }
        let capabilities = xpu_capability_matrix(device, properties)?;
        ...
        Ok((
            Self {
                ...
                backend_id,
                logical_memory,
                streams: BackendResourceRegistry::new("XPU semantic streams", MAX_XPU_STREAMS),
                events: BackendEventTracker::new(
                    "XPU pending events",
                    MAX_XPU_PENDING_EVENTS,
                ),
            },
            authority,
        ))
    ...
    pub fn upload_bytes(
        ...
        let allocation = if storage_bytes == 0 {
            ...
            if destination_offset != 0 {
                ...
                let zeros = try_zeroed_bytes(storage_length, "XPU upload gap initialization")?;
    ...
    pub fn download_bytes(
        ...
        let mut bytes = try_zeroed_bytes(byte_length, "XPU download staging")?;
    ...
    fn download_storage_bytes(
        ...
        let byte_length = usize::try_from(storage.byte_length)
            .map_err(|_| TensorError::ShapeOverflow)?;
        let mut bytes = try_zeroed_bytes(byte_length, "XPU test full-storage download")?;
        if let Some(allocation) = &storage.allocation {
            let stream = self.stream(context.stream, context.cancellation)?;
            self.runtime
    ...
    fn storage(&self, tensor: &Tensor) -> Result<Arc<XpuStorageInner>, TensorError> {
        tensor
            .backend_storage::<XpuStorage>()
    ...
    fn require_descriptor(
            ...
            || descriptor.layout() != Layout::Contiguous
        {
            return Err(self.unsupported(
                operation,
                "the reviewed XPU baseline accepts contiguous f16/f32 tensors only",
            ));
        }
    ...
    fn track_event(
        &self,
        context: &ExecutionContext<'_>,
        create: impl FnOnce() -> Result<XpuTrackedEvent, TensorError>,
    ...
    fn unsupported_result<T>(
        ...
        self.check_context(context)?;
        Err(self.unsupported(operation, "no reviewed XPU kernel is registered"))
    }
}

impl CachedAllocationOwner for XpuTensorBackend {
...
impl TensorBackend for XpuTensorBackend {
    fn device(&self) -> DeviceId {
        self.device
    }
    ...
    fn reserve_workspace(
        ...
        XpuTensorBackend::reserve_workspace(self, context, requested)
    }
    ...
    fn allocate(
        ...
        let allocation = if byte_length == 0 {
            ...
            let bytes = try_zeroed_bytes(byte_length, "XPU zero initialization")?;
            self.runtime
    ...
    fn copy(
        ...
        let allocation = if storage_bytes == 0 {
            ...
            let storage_length =
                usize::try_from(storage_bytes).map_err(|_| TensorError::ShapeOverflow)?;
            let zeros = try_zeroed_bytes(storage_length, "XPU copy gap initialization")?;
            self.runtime
                .copy_from_host(&stream, &allocation, 0, &zeros, context.cancellation)?;
            Some(allocation)
        };
        match source {
            PreparedCopySource::Xpu {
                storage,
                byte_offset,
            } => {
                if let (Some(destination), Some(source)) = (&allocation, &storage.allocation) {
                    ...
                    let byte_length =
                        usize::try_from(byte_length).map_err(|_| TensorError::ShapeOverflow)?;
                    let mut bytes = try_zeroed_bytes(byte_length, "XPU copy staging")?;
                    self.runtime.copy_to_host(
                        &stream,
                        source,
                        byte_offset,
    ...
    fn binary(
        ...
        {
            return Err(TensorError::Faulted {
                reason: "XPU add requires identical input and output shapes".to_owned(),
            });
        }
        ...
        let left_allocation = left
            ...
            .as_ref()
            .ok_or_else(|| TensorError::Faulted {
                reason: "nonempty XPU add left input has no allocation".to_owned(),
            })?;
        let right_allocation = right
            .allocation
            .as_ref()
            .ok_or_else(|| TensorError::Faulted {
                reason: "nonempty XPU add right input has no allocation".to_owned(),
            })?;
        let element_type = xpu_element_type(output.dtype())?;
        let stream = self.stream(context.stream, context.cancellation)?;
...
fn xpu_capability_matrix(
    device: DeviceId,
    properties: NativeDeviceProperties,
) -> Result<BackendCapabilityMatrix, TensorError> {
    if device.kind() != DeviceKind::Xpu {
        return Err(TensorError::DeviceMismatch {
            expected: DeviceId::new(DeviceKind::Xpu, device.ordinal()),
            actual: device,
        });
    }
...
fn xpu_element_type(dtype: DType) -> Result<XpuElementType, TensorError> {
    match dtype {
...
fn map_execution_error(operation: &str, error: XpuExecutionError) -> TensorError {
    let device = DeviceId::new(DeviceKind::Xpu, 0);
    match error {
        XpuExecutionError::Load(error) => TensorError::UnsupportedCapability {
            operation: operation.to_owned(),
            device,
            reason: format!("a registry-certified XPU execution session is unavailable: {error}"),
        },
        XpuExecutionError::InvalidCertifiedInputs { reason } => {
            TensorError::UnsupportedCapability {
                operation: operation.to_owned(),
                device,
                reason: format!("certified XPU session inputs are invalid: {reason}"),
            }
        }
        XpuExecutionError::Cancelled => TensorError::Cancelled,
        XpuExecutionError::OutOfMemory { requested, limit } => TensorError::AllocationFailed {
            requested: u64::try_from(requested).unwrap_or(u64::MAX),
            reason: format!("{operation}: XPU allocation limit is {limit} bytes"),
        },
        XpuExecutionError::DeviceLost {
            ...
            reason: format!(
                "{operation}: XPU device {device} was lost during {native_operation}"
            ),
        },
        XpuExecutionError::IdentifierOverflow => TensorError::IdentifierOverflow,
...
#[cfg(test)]
#[path = "../../tests/backends/intel_xpu_comfy_model_0021.rs"]
mod tests;
