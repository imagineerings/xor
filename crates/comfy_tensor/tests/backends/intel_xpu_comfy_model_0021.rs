...
fn test_backend_with_limits(
...
) -> Result<(XpuTensorBackend, ScratchReservation), TensorError> {
    let session = XpuExecutionSession::for_test_harness(
        0,
        "Injected Intel XPU adapter",
        0x8086,
        0x56a0,
        vec![(1, 6), (1, 9)],
        (3, 5, 2),
    ...
    .map_err(|error| map_execution_error("sim.xpu.test-harness", error))?;
    let cancellation = CancellationToken::default();
    let (backend, authority) = XpuTensorBackend::from_certified_session(
        session,
...
fn test_backend() -> Result<(XpuTensorBackend, ScratchReservation), TensorError> {
    ...
    let strides = match shape.as_slice() {
        ...
        _ => {
            return Err(TensorError::Faulted {
                reason: "XPU offset fixture supports rank zero or one".to_owned(),
            });
        }
    };
...
fn dtype_bytes(dtype: DType, values: &[f32]) -> Result<Vec<u8>, TensorError> {
    match dtype {
        ...
        _ => Err(TensorError::Faulted {
            reason: "XPU fixture requires f16 or f32".to_owned(),
        }),
    }
}
...
-> Result<(), TensorError> {
    ...
    assert_eq!(backend.device(), DeviceId::new(DeviceKind::Xpu, 0));
    ...
    let properties = backend
        ...
        .ok_or_else(|| TensorError::Faulted {
            reason: "XPU test device properties are absent".to_owned(),
        })?;
    assert_eq!(properties.name(), "Injected Intel XPU adapter");
    ...
    assert_eq!(properties.minor(), 6);
    assert_eq!(
        properties.architecture(),
        Some("Intel 0x8086:0x56a0; oneDNN 3.5.2")
    );
...
-> Result<(), TensorError> {
    let (backend, scratch) = test_backend()?;
    let cancellation = CancellationToken::default();
    for stream in 0..MAX_XPU_STREAMS {
        backend.stream(StreamId::new(stream as u64), &cancellation)?;
    }
    assert!(matches!(
        backend.stream(StreamId::new(MAX_XPU_STREAMS as u64), &cancellation),
        Err(TensorError::ResourceLimitExceeded {
            resource: "XPU semantic streams",
            limit: MAX_XPU_STREAMS,
        })
    ));

    let context = context(scratch, &cancellation);
    let mut events = Vec::with_capacity(MAX_XPU_PENDING_EVENTS);
    for _ in 0..MAX_XPU_PENDING_EVENTS {
        events.push(backend.record_event(&context)?);
    }
    assert!(matches!(
        backend.record_event(&context),
        Err(TensorError::ResourceLimitExceeded {
            resource: "XPU pending events",
            limit: MAX_XPU_PENDING_EVENTS,
        })
    ));
    for event in events {
        backend.wait_event(event, &context)?;
...
fn execution_errors_map_to_stable_tensor_error_categories() {
    assert!(matches!(
        map_execution_error("allocate", XpuExecutionError::Cancelled),
    ...
    assert!(matches!(
        map_execution_error(
            ...
            XpuExecutionError::DeviceLost {
                device: 2,
                operation: "onednn_graph_execute",
            },
        ),
        TensorError::DeviceLost { .. }
    ));
    assert!(matches!(
        map_execution_error("identifier", XpuExecutionError::IdentifierOverflow),
        TensorError::IdentifierOverflow
    ));
    assert!(matches!(
        map_execution_error("bounds", XpuExecutionError::ResourceBounds {
            offset: 3,
            length: 4,
            available: 6,
        }),
