...
fn test_backend_with_runtime(
...
) -> Result<(MluTensorBackend, ScratchReservation), TensorError> {
    let device = DeviceId::new(DeviceKind::Mlu, 0);
    let properties = NativeDeviceProperties::new(
        ...
        1,
        20,
        Some("test-mlu370".to_owned()),
        true,
    )?;
    let cancellation = CancellationToken::default();
    let memory_limit_bytes = runtime.memory_limit_bytes;
    let (backend, authority) = MluTensorBackend::from_runtime(
...
fn test_backend() -> Result<(MluTensorBackend, ScratchReservation), TensorError> {
...
fn dtype_bytes(dtype: DType, values: &[f32]) -> Result<Vec<u8>, TensorError> {
    match dtype {
        ...
        _ => Err(TensorError::Faulted {
            reason: "MLU edge fixture requires f16 or f32".to_owned(),
        }),
    }
}
...
fn instance_matrix_is_derived_from_the_certified_kernel_surface() -> Result<(), TensorError> {
    ...
    assert_eq!(backend.device(), DeviceId::new(DeviceKind::Mlu, 0));
    ...
    let properties =
        backend
            ...
            .ok_or_else(|| TensorError::Faulted {
                reason: "MLU test device properties are absent".to_owned(),
            })?;
    assert_eq!(properties.name(), "test Cambricon MLU");
    assert_eq!(properties.total_memory_bytes(), 1024 * 1024);
    assert_eq!(properties.architecture(), Some("test-mlu370"));
...
fn f16_odd_scalar_empty_cancellation_and_dtype_boundaries_are_exact() -> Result<(), TensorError> {
    ...
    assert!(matches!(
        ...
        Err(TensorError::Faulted { reason })
            if reason == "MLU add left shape [3] does not match output shape [2]"
    ));
    assert!(matches!(
        backend.binary(
            BinaryOperation::Add,
        ...
        Err(TensorError::Faulted { reason })
            if reason == "MLU add right shape [3] does not match output shape [2]"
    ));
    assert_eq!(
...
-> Result<(), TensorError> {
    ...
    let strided_mlu_descriptor = TensorDescriptor::new_strided(
        vec![2],
        vec![2],
        0,
        DType::F32,
    ...
    let strided_mlu = base.view(strided_mlu_descriptor, ViewAccess::ReadOnly)?;
    ...
    let (foreign_mlu, _) = other.upload_bytes(
        descriptor(vec![2])?,
        &f32_bytes(&[1.0, 2.0]),
        &other_context,
    )?;
...
fn stream_and_event_registries_are_bounded() -> Result<(), TensorError> {
    let (backend, scratch) = test_backend()?;
    let cancellation = CancellationToken::default();
    for stream in 0..MAX_MLU_STREAMS {
        backend.stream(StreamId::new(stream as u64), &cancellation)?;
    }
    assert!(matches!(
        backend.stream(StreamId::new(MAX_MLU_STREAMS as u64), &cancellation),
        Err(TensorError::ResourceLimitExceeded {
            resource: "MLU streams",
            limit: MAX_MLU_STREAMS,
        })
    ));

    let context = context(scratch, &cancellation);
    let mut events = Vec::with_capacity(MAX_MLU_PENDING_EVENTS);
    for _ in 0..MAX_MLU_PENDING_EVENTS {
        events.push(backend.record_event(&context)?);
    }
    assert!(matches!(
        backend.record_event(&context),
        Err(TensorError::ResourceLimitExceeded {
            resource: "MLU pending events",
            limit: MAX_MLU_PENDING_EVENTS,
        })
    ));
    let latest = events.last().copied().ok_or_else(|| TensorError::Faulted {
        reason: "MLU event fixture did not record events".to_owned(),
    })?;
