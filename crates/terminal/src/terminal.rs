...
/// Inserts Sim-specific environment variables for terminal sessions.
/// Used by both local terminals and remote terminals (via SSH).
pub fn insert_sim_terminal_env(
    env: &mut HashMap<String, String>,
    version: &impl std::fmt::Display,
) {
    env.insert("SIM_TERM".to_string(), "true".to_string());
    env.insert("TERM_PROGRAM".to_string(), "sim".to_string());
...
impl TerminalBuilder {
    ...
    pub fn new(
        ...
        let fut = async move {
            ...
            insert_sim_terminal_env(&mut env, &version);
...
impl Terminal {
    ...
    /// remote host, in case Sim is connected to a remote host.
    ...
    fn register_task_finished(
        ...
        if !lines_to_show.is_empty() {
            ...
            // when Sim task finishes and no more output is made.
...
mod tests {
    ...
    async fn test_hyperlink_ctrl_click_drag_outside_bounds(cx: &mut TestAppContext) {
        let terminal = init_ctrl_click_hyperlink_test(
            cx,
            b"Visit https://sim.dev/ for more\r\nThis is another line\r\n",
        );
