...
pub(super) type AlacrittyTerm = Term<BaymaxListener>;
...
#[derive(Clone)]
pub(super) struct BaymaxListener(UnboundedSender<PtyEvent>);
...
pub(super) fn new_term(
    ...
    let mut term = Term::new(config.clone(), &bounds, BaymaxListener(events_tx));
...
pub(super) fn spawn_event_loop(
    ...
    let event_loop = EventLoop::new(term, BaymaxListener(events_tx), pty, drain_on_exit, false)
...
impl EventListener for BaymaxListener {
...
pub(super) fn clear_saved_screen(term: &mut Term<BaymaxListener>) {
...
pub(super) fn make_content(term: &Term<BaymaxListener>, last_content: &Content) -> Content {
...
pub(super) fn content_text(term: &Term<BaymaxListener>) -> String {
    let start = AlacPoint::new(term.topmost_line(), Column(0));
    let end = AlacPoint::new(term.bottommost_line(), term.last_column());
    term.bounds_to_string(start, end)
}

pub(super) fn total_lines(term: &Term<BaymaxListener>) -> usize {
    term.total_lines()
}

pub(super) fn screen_lines(term: &Term<BaymaxListener>) -> usize {
    term.screen_lines()
}

pub(super) fn full_content_range(term: &Term<BaymaxListener>) -> Range {
    let start = AlacPoint::new(term.topmost_line(), Column(0));
    let end = AlacPoint::new(term.bottommost_line(), term.last_column());
    Range::from_alacritty(start..=end)
}

pub(super) fn last_non_empty_lines(term: &Term<BaymaxListener>, line_count: usize) -> Vec<String> {
    let grid = term.grid();
...
pub(super) fn update_vi_cursor_for_scroll(term: &mut Term<BaymaxListener>, scroll: Scroll) {
    match scroll {
        Scroll::Delta(delta) => {
...
pub(super) fn update_selection_to_vi_cursor(term: &mut Term<BaymaxListener>) -> Option<Point> {
...
/// The function achieves proper display and scrolling capabilities, at a cost of grid state not properly synchronibaymax.
...
pub(super) unsafe fn append_text_to_term(term: &mut Term<BaymaxListener>, text_lines: &[&str]) {
    term.newline();
