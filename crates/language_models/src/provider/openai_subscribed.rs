...
use language_model::{
    AuthenticateError, ConfigurationViewTargetAgent, FastModeConfirmation, IconOrSvg,
...
impl LanguageModelProvider for OpenAiSubscribedProvider {
    ...
    fn inline_title(&self, cx: &App) -> Option<SharedString> {
        (!self.state.read(cx).is_authenticated()).then(|| "Configure ChatGPT".into())
    }

    fn inline_description(&self, cx: &App) -> Option<InlineDescription> {
        (!self.state.read(cx).is_authenticated())
            .then(|| InlineDescription::Text(SUBSCRIPTION_DESCRIPTION.into()))
    }
