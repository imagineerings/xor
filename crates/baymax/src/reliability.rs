...
async fn upload_minidump(
    ...
    let mut form = Form::new()
        ...
        .text("sentry[tags][version]", metadata.init.baymax_version.clone())
    ...
    ::telemetry::event!(
        ...
        crashed_version = metadata.init.baymax_version.clone(),
    ...
    for (index, gpu) in metadata.gpus.iter().cloned().enumerate() {
        ...
        form = form
            .text(
                ...
                "A GPU found on the users system. May or may not be the GPU Baymax is running on",
            )
...
// users - except - we know internal users use Baymax.
