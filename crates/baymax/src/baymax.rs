...
pub fn init(cx: &mut App) {
    ...
    .on_action(|_: &baymax_actions::OpenLicenses, cx| {
        with_active_or_new_workspace(cx, |workspace, window, cx| {
            open_bundled_file(
                workspace,
    ...
    .on_action(|&baymax_actions::OpenKeymapFile, cx| {
    ...
    .on_action(|_: &baymax_actions::OpenDefaultKeymap, cx| {
        with_active_or_new_workspace(cx, |workspace, window, cx| {
            open_bundled_file(
                workspace,
...
pub fn build_window_options(display_uuid: Option<Uuid>, cx: &mut App) -> WindowOptions {
    ...
    let window_decorations = match std::env::var("BAYMAX_WINDOW_DECORATIONS") {
...
fn initialize_file_watcher(window: &mut Window, cx: &mut Context<Workspace>) {
    if let Err(e) = fs::fs_watcher::global(|_| {}) {
        let message = format!(
            ...
            This may be due to system-wide limits on inotify instances. For troubleshooting see: https://baymax.dev/docs/linux
            "#},
            e
        );
        ...
        cx.spawn(async move |_, cx| {
            if prompt.await == Ok(0) {
                cx.update(|cx| {
                    cx.open_url("https://baymax.dev/docs/linux#could-not-start-inotify");
                    cx.quit();
                });
            }
        })
...
fn initialize_file_watcher(window: &mut Window, cx: &mut Context<Workspace>) {
    if let Err(e) = fs::fs_watcher::global(|_| {}) {
        let message = format!(
            ...
            This may occur on network filesystems and WSL paths. For troubleshooting see: https://baymax.dev/docs/windows
            "#},
            e
        );
        let prompt = window.prompt(
        ...
        cx.spawn(async move |_, cx| {
            if prompt.await == Ok(0) {
                cx.update(|cx| {
                    cx.open_url("https://baymax.dev/docs/windows");
                    cx.quit()
                });
            }
        })
...
fn show_software_emulation_warning_if_needed(
    ...
    if specs.is_software_emulated && std::env::var("BAYMAX_ALLOW_EMULATED_GPU").is_err() {
        let (graphics_api, docs_url, open_url) = if cfg!(target_os = "windows") {
            (
                "DirectX",
                "https://baymax.dev/docs/windows",
                "https://baymax.dev/docs/windows",
            )
        } else {
            (
                ...
                "https://baymax.dev/docs/linux#baymax-fails-to-open-windows",
            )
        };
        let message = format!(
            db::indoc! {r#"
            Baymax uses {} for rendering and requires a compatible GPU.

            Currently you are using a software emulated GPU ({}) which
            will result in awful performance.

            For troubleshooting see: {}
            Set BAYMAX_ALLOW_EMULATED_GPU=1 env var to permanently override.
            "#},
...
fn register_actions(
    ...
    workspace
        ...
        .register_action(|_, action: &OpenBaymaxUrl, _, cx| {
        ...
        .register_action(|workspace, action: &baymax_actions::OpenRemote, window, cx| {
        ...
        .register_action({
            ...
            move |_, action: &baymax_actions::IncreaseUiFontSize, _window, cx| {
                if action.persist {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &baymax_actions::DecreaseUiFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, cx| {
                        let ui_font_size = ThemeSettings::get_global(cx).ui_font_size(cx) - px(1.0);
                        let _ = settings
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &baymax_actions::ResetUiFontSize, _window, cx| {
                if action.persist {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &baymax_actions::IncreaseBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, cx| {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &baymax_actions::DecreaseBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, cx| {
                        let buffer_font_size =
                            ThemeSettings::get_global(cx).buffer_font_size(cx) - px(1.0);
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &baymax_actions::ResetBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, _| {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &baymax_actions::ResetAllZoom, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, _| {
                        settings.theme.ui_font_size = None;
                        settings.theme.buffer_font_size = None;
        ...
        .register_action(|_, _: &install_cli::RegisterBaymaxScheme, window, cx| {
            cx.spawn_in(window, async move |workspace, cx| {
                install_cli::register_baymax_scheme(cx).await?;
                workspace.update_in(cx, |workspace, _, cx| {
                    struct RegisterBaymaxScheme;

                    workspace.show_toast(
                        Toast::new(
                            NotificationId::unique::<RegisterBaymaxScheme>(),
                            format!(
                                "baymax:// links will now open in {}.",
            ...
            .detach_and_prompt_err(
                "Error registering baymax:// scheme",
                window,
                cx,
                |_, _, _| None,
            );
        ...
        .register_action(
            |workspace: &mut Workspace,
             _: &baymax_actions::project_panel::ToggleFocus,
             window: &mut Window,
             cx: &mut Context<Workspace>| {
...
fn quit(_: &Quit, cx: &mut App) {
    ...
    cx.spawn(async move |cx| {
        ...
        for window in &workspace_windows {
            ...
            // actually had focused so it is the one serialibaymax (and restored on
...
/// Starts watching `~/.config/baymax/AGENTS.md` (or the platform equivalent) and
    ...
    show_app_notification(notification_id, cx, move |cx| {
        cx.new(|cx| {
            MessageNotification::new(message.clone(), cx)
                ...
                .primary_icon(IconName::Settings)
                .primary_on_click(|window, cx| {
                    window.dispatch_action(baymax_actions::OpenKeymapFile.boxed_clone(), cx);
                    cx.emit(DismissEvent);
                })
        })
    });
...
fn open_project_debug_tasks_file(
    workspace: &mut Workspace,
    _: &baymax_actions::OpenProjectDebugTasks,
    window: &mut Window,
    cx: &mut Context<Workspace>,
) {
    open_local_file(
...
mod tests {
    ...
    async fn test_window_edit_state_restoring_disabled(cx: &mut TestAppContext) {
        ...
        // Advance the clock to ensure that the item has been serialibaymax and dropped from the queue
    ...
    async fn test_window_edit_state_restoring_enabled(cx: &mut TestAppContext) {
        ...
        // Advance the clock to make sure the workspace is serialibaymax
        cx.executor().advance_clock(Duration::from_secs(1));
    ...
    async fn test_opening_project_settings_when_excluded(cx: &mut gpui::TestAppContext) {
        ...
        eprintln!("Created project with .baymax/settings.json containing UNIQUEVALUE");
        ...
        let original_settings = app_state
            .fs
            .load(Path::new("/root/.baymax/settings.json"))
            .await
            .unwrap();
        ...
        // 3. Add .baymax to file scan exclusions in user settings
        cx.update_global::<SettingsStore, _>(|store, cx| {
            store.update_user_settings(cx, |worktree_settings| {
                worktree_settings.project.worktree.file_scan_exclusions =
                    Some(vec![".baymax".to_string()]);
            });
        });

        eprintln!("Added .baymax to file_scan_exclusions in settings");
        ...
        // If .baymax is not excluded, the test will fail here
        assert!(
            !has_baymax_entry,
            "Test precondition failed: .baymax directory should be excluded but was found in worktree"
        );
