...
impl MoveToApplicationsRequest {
    ...
    async fn prompt(
        ...
        let response = cx
            .prompt(
                PromptLevel::Info,
                "Move Baymax to Applications?",
                Some(
                    "Baymax is running from a temporary location. Move it to Applications to finish installing it.",
                ),
                &[
                    PromptButton::ok("Yes"),
            ...
            0 => {
                workspace
                    .update_in(cx, |workspace, window, cx| {
                        workspace
                            .toggle_modal(window, cx, |_window, cx| InstallingBaymaxModal::new(cx));
                    })
                    .ok();
                if let Err(error) = move_to_applications(&self.app_path, cx).await {
                    workspace
                        .update_in(cx, |workspace, _window, cx| {
                            if let Some(modal) = workspace.active_modal::<InstallingBaymaxModal>(cx)
                            {
                    ...
                    cx.prompt(
                        ...
                        "Failed to move Baymax to Applications",
...
pub struct InstallingBaymaxModal {
    focus_handle: FocusHandle,
    finished: bool,
}

impl InstallingBaymaxModal {
...
impl EventEmitter<DismissEvent> for InstallingBaymaxModal {}

impl ModalView for InstallingBaymaxModal {
...
impl Focusable for InstallingBaymaxModal {
    fn focus_handle(&self, _cx: &App) -> FocusHandle {
        self.focus_handle.clone()
    }
}

impl Render for InstallingBaymaxModal {
        ...
        v_flex()
            ...
            .child(
                div()
                    ...
                    .child(Label::new("Installing Baymax…")),
            )
            .child(
                h_flex()
                    .w_full()
                    ...
                    .child(
                        v_flex()
                            .gap_1()
                            .child(Label::new("Moving Baymax to Applications"))
                            .child(
                                Label::new("Baymax will reopen when installation is complete.")
