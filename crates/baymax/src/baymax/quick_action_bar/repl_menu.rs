...
const BAYMAX_REPL_DOCUMENTATION: &str = "https://baymax.dev/docs/repl";
...
impl QuickActionBar {
    ...
    pub fn render_repl_setup(&self, language: &str, cx: &mut Context<Self>) -> Option<AnyElement> {
        let tooltip: SharedString = SharedString::from(format!("Setup Baymax REPL for {}", language));
        Some(
            h_flex()
                ...
                .child(
                    IconButton::new("toggle_repl_icon", IconName::ReplNeutral)
                        ...
                        .on_click(|_, _window, cx| {
                            cx.open_url(&format!("{}#installation", BAYMAX_REPL_DOCUMENTATION))
                        }),
                )
                .into_any_element(),
        )
