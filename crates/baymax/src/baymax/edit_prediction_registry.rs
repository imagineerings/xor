...
fn edit_prediction_provider_config_for_settings(cx: &App) -> Option<EditPredictionProviderConfig> {
    ...
    match provider {
        ...
        EditPredictionProvider::Mercury => Some(EditPredictionProviderConfig::Baymax(
...
impl EditPredictionProviderConfig {
    fn name(&self) -> &'static str {
        match self {
            ...
            EditPredictionProviderConfig::Baymax(model) => match model {
...
fn assign_edit_prediction_provider(
    ...
    match provider_config {
        None => {
            editor.set_edit_prediction_provider::<BaymaxEditPredictionDelegate>(None, window, cx);
        }
        ...
        Some(EditPredictionProviderConfig::Baymax(model)) => {
            let ep_store = edit_prediction::EditPredictionStore::global(client, &user_store, cx);
                ...
                user_store.read(cx).current_organization_configuration()
            {
                if !organization_configuration.edit_prediction.is_enabled {
                    editor.set_edit_prediction_provider::<BaymaxEditPredictionDelegate>(
                        None, window, cx,
                    );
            ...
            if let Some(project) = editor.project() {
                ...
                let provider = cx.new(|cx| {
                    BaymaxEditPredictionDelegate::new(
                        project.clone(),
                        singleton_buffer,
                        &client,
                        &user_store,
...
mod tests {
    ...
    async fn test_subscribe_uses_stale_provider_config_after_settings_change(
        ...
        // captures None at init time. (The test default is Baymax/Zeta1, which
