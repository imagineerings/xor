...
//! Visual Test Runner
//!
//! This binary runs visual regression tests for Baymax's UI. It captures screenshots
//! of real Baymax windows and compares them against baseline images.
//!
...
#[cfg(target_os = "macos")]
fn main() {
    // Set BAYMAX_STATELESS early to prevent file system access to real config directories
    // This must be done before any code accesses baymax_env_vars::BAYMAX_STATELESS
    // SAFETY: We're at the start of main(), before any threads are spawned
    unsafe {
        std::env::set_var("BAYMAX_STATELESS", "1");
    }
...
mod constants {
    ...
    pub const BASELINE_DIR: &str = "crates/baymax/test_fixtures/visual_tests";

    /// Embedded test image (Baymax app icon) for visual tests.
...
fn run_visual_tests(project_path: PathBuf, update_baseline: bool) -> Result<()> {
    ...
    // Test settings use Courier font, but we want the real Baymax fonts for visual tests
    cx.update(|cx| {
        settings::init(cx);
    });
    ...
    // Initialize all Baymax subsystems
    cx.update(|cx| {
        gpui_tokio::init(cx);
    ...
    // This prevents "root path could not be canonicalibaymax" errors when main() drops temp_dir
    workspace_window
        .update(&mut cx, |workspace, _window, cx| {
            let project = workspace.project().clone();
            project.update(cx, |project, cx| {
...
This is a test project for visual testing of Baymax.
...
fn run_agent_thread_view_test(
    ...
    let send_future = thread.update(cx, |thread, cx| {
        thread.send(vec!["Show me the Baymax logo".into()], cx)
    });
    ...
    // This prevents "root path could not be canonicalibaymax" errors when we clean up
    workspace_window
        .update(cx, |workspace, _window, cx| {
            let project = workspace.project().clone();
            project.update(cx, |project, cx| {
...
fn run_multi_workspace_sidebar_visual_tests(
    ...
    let workspace2_dir = canonical_temp.join("baymax");
    ...
    // Add worktree to workspace 2 (index 1) so it shows as "baymax"
                ...
                container().child(
                    ThreadItem::new("ti-linked-no-branch", "Review worktree cleanup")
                        ...
                        .worktrees(vec![ThreadItemWorktreeInfo {
                            ...
                            full_path: "/worktrees/focal-arrow/baymax".into(),
                            highlight_positions: Vec::new(),
                            kind: WorktreeKind::Linked,
                            branch_name: None,
                        }]),
                ...
                container().child(
                    ThreadItem::new("ti-same-name", "Implement feature")
                        ...
                        .worktrees(vec![ThreadItemWorktreeInfo {
                            ...
                            full_path: "/worktrees/stoic-reed/baymax".into(),
                            highlight_positions: Vec::new(),
                            kind: WorktreeKind::Linked,
...
fn run_sidebar_duplicate_project_names_visual_tests(
    ...
    // Create directory structure where every leaf directory is named "baymax" but
    ...
    //
    //   code/baymax/       — project1 (single worktree)
    //   code/foo/baymax/   — project2 (single worktree)
    //   code/bar/baymax/   — project3, first worktree
    //   code/baz/baymax/   — project3, second worktree
    //
    // No two projects share a worktree path, so ProjectGroupBuilder will
    // place each in its own group.
    let code_baymax = canonical_temp.join("code").join("baymax");
    let foo_baymax = canonical_temp.join("code").join("foo").join("baymax");
    let bar_baymax = canonical_temp.join("code").join("bar").join("baymax");
    let baz_baymax = canonical_temp.join("code").join("baz").join("baymax");
    std::fs::create_dir_all(&code_baymax)?;
    std::fs::create_dir_all(&foo_baymax)?;
    std::fs::create_dir_all(&bar_baymax)?;
    std::fs::create_dir_all(&baz_baymax)?;
    ...
    // Two single-worktree projects whose leaf name is "baymax"
    {
        let project1 = create_project_with_worktree(&code_baymax, &app_state, cx)?;
        let project2 = create_project_with_worktree(&foo_baymax, &app_state, cx)?;
    ...
    // Three projects, third has two worktrees (all leaf names "baymax")
    //
    // project1: code/baymax
    // project2: code/foo/baymax
    // project3: code/bar/baymax + code/baz/baymax
    //
    ...
    // separate groups. The sidebar must disambiguate all three.
    {
        ...
        let project3 = create_project_with_worktree(&bar_baymax, &app_state, cx)?;
        let add_second_worktree = cx.update(|cx| {
            project3.update(cx, |project, cx| {
                project.find_or_create_worktree(&baz_baymax, true, cx)
            })
        });
        cx.background_executor.allow_parking();
        cx.foreground_executor
