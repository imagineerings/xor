...
use client::{Client, ProxySettings, RefreshLlmTokenListener, UserStore, parse_baymax_link};
...
fn build_application() -> Application {
    ...
    if std::env::var("BAYMAX_EXPERIMENTAL_A11Y").as_deref() == Ok("1") {
...
fn files_not_created_on_launch(errors: HashMap<io::ErrorKind, Vec<&Path>>) {
    let message = "Baymax failed to launch";
...
fn fail_to_open_window(e: anyhow::Error, _cx: &mut App) {
    eprintln!(
        "Baymax failed to open a window: {e:?}. See https://baymax.dev/docs/linux for troubleshooting steps."
    );
    ...
    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
    {
        ...
        _cx.spawn(async move |_cx| {
            ...
            let notification_id = "dev.baymax.Oops";
            proxy
                .add_notification(
                    notification_id,
                    Notification::new("Baymax failed to launch")
                        .body(Some(
                            format!(
                                "{e:?}. See https://baymax.dev/docs/linux for troubleshooting steps."
                            )
                            .as_str(),
                        ))
...
fn main() {
    ...
    // `baymax --askpass` Makes baymax operate in nc/netcat mode for use with askpass
    ...
    // `baymax --crash-handler` Makes baymax operate in minidump crash handler mode
    ...
    if args.record_etw_trace {
        ...
        if let Err(error) =
            etw_tracing::record_etw_trace(baymax_pid, &output_path, etw_socket.as_str())
        {
    ...
    // `baymax --printenv` Outputs environment variables as JSON to stdout
    if args.printenv {
    ...
    #[cfg(target_os = "windows")]
    match util::get_baymax_cli_path() {
        Ok(path) => askpass::set_askpass_program(path),
        Err(err) => {
            eprintln!("Error: {}", err);
            if std::option_env!("BAYMAX_BUNDLE").is_some() {
                process::exit(1);
            }
        }
    }
    ...
    if args.system_specs {
        ...
        println!("Baymax System Specs (from CLI):\n{}", system_specs);
        return;
    }
    ...
    let failed_single_instance_check = if *baymax_env_vars::BAYMAX_STATELESS
        || *release_channel::RELEASE_CHANNEL == ReleaseChannel::Dev
    {
        ...
        #[cfg(any(target_os = "linux", target_os = "freebsd"))]
        {
            crate::baymax::listen_for_cli_connections(open_listener.clone()).is_err()
        }
        ...
        #[cfg(target_os = "macos")]
        {
            use baymax::mac_only_instance::*;
            ensure_only_instance() != IsOnlyInstance::Yes
        }
    };
    if failed_single_instance_check {
        println!("baymax is already running");
        return;
    }
        ...
        Some(
            app.background_executor().spawn(crashes::init(
                InitCrashHandler {
                    ...
                    baymax_version: semver::Version::new(
                        app_version.major,
                        app_version.minor,
                        app_version.patch,
                    )
                    .to_string(),
                    binary: "baymax".to_string(),
                ...
                |pid| paths::temp_dir().join(format!("baymax-crash-handler-{pid}")),
    ...
    app.run(move |cx| {
        ...
        baymax::watch_settings_files(fs.clone(), cx);
        ...
        baymax::init(cx);
        #[cfg(target_os = "macos")]
        baymax::move_to_applications::init(cx);
        ...
        acp_tools::init(cx);
        baymax::telemetry_log::init(cx);
        baymax::remote_debug::init(cx);
        ...
        baymax::watch_user_agents_md(app_state.fs.clone(), cx);
    ...
    if let Some(kind) = request.kind {
        match kind {
                    ...
                    multi_workspace.update(cx, |multi_workspace, window, cx| {
                        multi_workspace.workspace().update(cx, |workspace, cx| {
                            if let Some(panel) = workspace.focus_panel::<AgentPanel>(window, cx) {
                                ...
                                log::warn!(
                                    "baymax://agent received but the AgentPanel is not registered \
                ...
                workspace::with_active_or_new_workspace(cx, |_workspace, window, cx| {
                    cx.spawn_in(window, async move |workspace, cx| {
                        let res = async move {
                            ...
                            let uri = format!("baymax://schemas/{}", schema_path);
            ...
            OpenRequestKind::Setting { setting_path } => {
                // baymax://settings/languages/$(language)/tab_size  - DONT SUPPORT
                // baymax://settings/languages/Rust/tab_size  - SUPPORT
                ...
                cx.spawn(async move |cx| {
                    ...
                    workspace.update(cx, |_, window, cx| match setting_path {
                        None => window.dispatch_action(Box::new(baymax_actions::OpenSettings), cx),
            ...
            OpenRequestKind::GitCommit { sha } => {
                let base_open_options = baymax::open_options_for_request(
                    request.open_behavior,
                    &workspace::SerialibaymaxWorkspaceLocation::Local,
                    cx,
                );
                cx.spawn(async move |cx| {
...
pub(crate) async fn restore_or_create_workspace(
    ...
    if let Some(multi_workspaces) = restorable_workspaces(cx, &app_state).await {
        ...
        // check, Baymax would exit silently.
...
async fn restorable_workspaces(
    cx: &mut AsyncApp,
    app_state: &Arc<AppState>,
) -> Option<Vec<workspace::SerialibaymaxMultiWorkspace>> {
    let locations = restorable_workspace_locations(cx, app_state).await?;
    Some(cx.update(|cx| workspace::read_serialibaymax_multi_workspaces(locations, cx)))
}
...
#[derive(Parser, Debug)]
#[command(name = "baymax", disable_version_flag = true, max_term_width = 100)]
struct Args {
    ...
    ///
    /// URLs can either be `file://` or `baymax://` scheme, or relative to <https://baymax.dev>.
    ...
    /// On macOS, the default is `~/Library/Application Support/Baymax`.
    /// On Linux/FreeBSD, the default is `$XDG_DATA_HOME/baymax`.
    /// On Windows, the default is `%LOCALAPPDATA%\Baymax`.
    #[arg(long, value_name = "DIR", verbatim_doc_comment)]
    user_data_dir: Option<String>,

    /// The username and WSL distribution to use when opening paths. If not specified,
    /// Baymax will attempt to open the paths directly.
    ///
    ...
    /// Instructs baymax to run as a dev server on this machine. (not implemented)
    #[arg(long)]
    ...
    /// prevents Baymax from starting, so you can't run `baymax: copy system specs to
    /// clipboard`
    #[arg(long)]
    system_specs: bool,

    /// Used for recording minidumps on crashes by having Baymax run a separate
    /// process communicating over a socket.
    #[arg(long, hide = true)]
    crash_handler: Option<PathBuf>,

    /// Run baymax in the foreground, only used on Windows, to match the behavior on macOS.
    #[arg(long)]
    #[cfg(target_os = "windows")]
    #[arg(hide = true)]
    foreground: bool,
    ...
    /// by having Baymax act like netcat communicating over a Unix socket.
    #[arg(long)]
    #[cfg(not(target_os = "windows"))]
    #[arg(hide = true)]
    ...
    /// The PID of the Baymax process to trace for heap analysis.
    #[cfg(target_os = "windows")]
    #[arg(long, hide = true, allow_hyphen_values = true)]
    etw_baymax_pid: Option<i64>,
    ...
    /// Unix socket path for IPC with the parent Baymax process.
    #[cfg(target_os = "windows")]
    #[arg(long, hide = true)]
    ...
    match std::fs::canonicalize(Path::new(&arg)) {
        ...
        Err(_) => {
            if arg.starts_with("file://")
                || arg.starts_with("baymax://")
                || arg.starts_with("baymax-cli://")
                || arg.starts_with("ssh://")
                || parse_baymax_link(arg, cx).is_some()
            {
                arg.into()
            } else {
                format!("file://{arg}")
