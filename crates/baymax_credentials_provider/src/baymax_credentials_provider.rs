...
///
/// By default, running Baymax in development uses the development credentials
...
static BAYMAX_DEVELOPMENT_USE_KEYCHAIN: LazyLock<bool> = LazyLock::new(|| {
    std::env::var("BAYMAX_DEVELOPMENT_USE_KEYCHAIN").is_ok_and(|value| !value.is_empty())
});

pub struct BaymaxCredentialsProvider(pub Arc<dyn CredentialsProvider>);

impl Global for BaymaxCredentialsProvider {}
...
pub fn init_global(cx: &mut App) {
    ...
    let provider = new(cx);
    cx.set_global(BaymaxCredentialsProvider(provider));
}

pub fn global(cx: &App) -> Arc<dyn CredentialsProvider> {
    cx.try_global::<BaymaxCredentialsProvider>()
...
fn new(cx: &App) -> Arc<dyn CredentialsProvider> {
    let use_development_provider = match ReleaseChannel::try_global(cx) {
        Some(ReleaseChannel::Dev) => {
            ...
            //
            // However, if the `BAYMAX_DEVELOPMENT_USE_KEYCHAIN` environment
            // variable is set, we will use the actual keychain.
            !*BAYMAX_DEVELOPMENT_USE_KEYCHAIN
        }
...
/// re-allow access to the system keychain when developing Baymax.
