...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut another_family = parsed_facts("native", DType::F32, tiny_shape());
    another_family.formats[0]
        .metadata
        .insert("image_model".to_owned(), "flux2".to_owned());
    let another_probe = ModelProbe::from_parsed_facts(another_family)?;
    assert!(matches!(
        registry.detect(&another_probe),
...
fn parsed_facts(layout: &str, dtype: DType, shape: ErnieShape) -> ModelParsedFacts {
    ...
    ModelParsedFacts {
        ...
        formats: vec![ModelParsedFormatFact {
            ...
            metadata: BTreeMap::from([("image_model".to_owned(), "ernie".to_owned())]),
        }],
    }
}
...
fn write_safetensors(path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    header.insert(
        ...
        serde_json::json!({"image_model":"ernie"}),
    );
