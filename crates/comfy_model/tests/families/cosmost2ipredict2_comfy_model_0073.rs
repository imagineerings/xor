use comfy_model::{
    ArtifactIndex, ArtifactKey, ArtifactRoot, CosmosArchitecture, CosmosRatio,
    ...
    generated_cosmost2ipredict2_comfy_model_0073 as cosmos,
};
use comfy_tensor::{
...
const ARTIFACT_DIGEST: &str = "0073007300730073007300730073007300730073007300730073007300730073";
const RESOLVED_MEMORY_BYTES: u64 = 278_556;

static AMBIGUOUS_FAMILY: ModelFamilyDefinition = ModelFamilyDefinition {
    feature_id: "COMFY-MODEL-9073",
    identifier: "CosmosT2IPredict2AmbiguousFixture",
    ..cosmos::MODEL_FAMILY
};
...
#[test]
fn val_model_family_row_001_cosmost2ipredict2_source_profiles_provenance_and_ownership()
-> Result<(), Box<dyn std::error::Error>> {
    assert_eq!(cosmos::MODEL_FAMILY_IDENTIFIER, "CosmosT2IPredict2");
    assert_eq!(cosmos::MODEL_FAMILY_FEATURE_ID, "COMFY-MODEL-0073");
    assert_eq!(
        cosmos::MODEL_FAMILY_FIXTURE,
        "cosmost2ipredict2-comfy-model-0073"
    );
    assert_eq!(cosmos::MODEL_FAMILY_SOURCE_ORDINAL, 43);
    assert_eq!(cosmos::MODEL_FAMILY_REGISTRATION.source_ordinal, 43);
    assert_eq!(
        cosmos::MODEL_FAMILY_REGISTRATION.source_architecture,
        "model_base.CosmosPredict2"
    );
    assert!(cosmos::MODEL_FAMILY_REGISTRATION.source_configuration.is_empty());
    assert_eq!(cosmos::MODEL_FAMILY_DECLARED_MEMORY_USAGE_FACTOR, 1.0);
    assert_eq!(cosmos::MODEL_FAMILY_BASE_MEMORY_USAGE_FACTOR, 0.95);
    ...
    assert_eq!(descriptor.identifier, "CosmosT2IPredict2");
    assert_eq!(descriptor.family, "COMFY-MODEL-0073");
    assert_eq!(descriptor.architecture_version, "cosmos-predict2-t2i-v1");
    ...
    let two_b = ModelProbe::from_parsed_facts(parsed_facts(
        DType::F32,
        16,
        2_048,
        false,
    ...
    assert_eq!(
        ...
        cosmos::CosmosT2IPredict2ModelSize::Predict2TwoB
    );
    assert_eq!(
    ...
    assert_eq!(
        configuration.extra_extrapolation,
        Some([ratio(1, 1), ratio(1, 1), ratio(1, 1)])
    );
    assert_eq!(configuration.extra_per_block_absolute_position, Some(false));
    assert_eq!(configuration.memory_usage_factor, 0.95);

    let fourteen_b = ModelProbe::from_parsed_facts(parsed_facts(
        DType::F32,
        16,
        5_120,
        false,
    ...
    assert_eq!(
        configuration.model_size,
        cosmos::CosmosT2IPredict2ModelSize::FourteenB
    );
    assert_eq!(configuration.memory_usage_factor, 2.375);

    let mut misleading_facts =
        parsed_facts(DType::F32, 16, 2_048, false, false, false, "native");
    misleading_facts.formats[0].metadata.extend([
        ("image_model".to_owned(), "cosmos".to_owned()),
        ("in_channels".to_owned(), "17".to_owned()),
    ...
    assert_eq!(provenance["source_ordinal"], 43);
    let projection = provenance["source_projection"]
        .as_str()
        .ok_or("CosmosT2IPredict2 source projection must be a string")?;
    assert_eq!(
    ...
    for source in provenance["source_files"]
        .as_array()
        .ok_or("CosmosT2IPredict2 source_files must be an array")?
    {
    ...
    let row = catalog["models"]
        ...
        .ok_or("CosmosT2IPredict2 catalog row is missing")?;
    assert_eq!(row["source_ordinal"], 43);
    assert_eq!(
    ...
    let row_source = std::fs::read_to_string(
        Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("src/families/cosmost2ipredict2_comfy_model_0073.rs"),
    )?;
    ...
    assert!(row_source.contains("COSMOS_PREDICT2_STATE_PLAN_CASES"));
    ...
    super::write_model_family_row_artifact(
        ...
        "cosmost2ipredict2_comfy_model_0073",
        &[
            "source-provenance-catalog-and-canonical-ownership",
            "predict2-two-b-and-fourteen-b-profiles",
            "model-store-native-predict2-marker-detection",
            "transactional-predict2-component-mapping-and-checkpoints",
            "patch-order-memory-oom-dtype-device-cancellation",
            "i2v-anima-partial-ambiguous-and-layout-rejection",
        ],
    )?;
    Ok(())
}
...
#[test]
fn val_model_family_row_001_cosmost2ipredict2_model_store_mapping_forward_patch_and_memory()
-> Result<(), Box<dyn std::error::Error>> {
    let registry =
        ModelFamilyRegistry::checked_registrations(&[cosmos::MODEL_FAMILY_REGISTRATION])?;
    ...
    assert_eq!(resolved.source_ordinal(), 43);
    ...
    let source = source_tensors(&backend, &context, DType::F32, false, false)?;
    ...
    assert!(mapped.component("model").is_some_and(|model| {
        model.contains_key("native.blocks.0.mlp.layer1.weight")
            && model.contains_key("native.x_embedder.proj.1.weight")
    }));
    assert!(
        mapped
            .component("vae")
            .is_some_and(|vae| vae.contains_key("vae.decoder.weight"))
    );
    assert!(
        mapped.component("text_encoder").is_some_and(|encoder| {
            encoder.contains_key("text_encoder.t5xxl.transformer.weight")
        })
    );
    ...
    let patch = PatchGraph::checked(
        ARTIFACT_DIGEST,
        vec![PatchOperation {
            identifier: "cosmost2ipredict2-mlp-delta".to_owned(),
            kind: PatchKind::Lora,
            scale: 1.0,
...
#[test]
fn val_model_family_row_001_cosmost2ipredict2_dtype_and_typed_failures()
-> Result<(), Box<dyn std::error::Error>> {
    let registry =
        ModelFamilyRegistry::checked_registrations(&[cosmos::MODEL_FAMILY_REGISTRATION])?;
    let (backend, authority) = CpuWorkspaceAuthority::create_backend(8 * 1024 * 1024)?;
    ...
    for dtype in [DType::Bf16, DType::F16, DType::F32] {
        let probe = ModelProbe::from_parsed_facts(parsed_facts(
            dtype, 16, 2_048, false, false, false, "native",
        ))?;
        let resolved = registry.resolve(&probe)?;
        let source = source_tensors(&backend, &context, dtype, false, false)?;
        let weights = resolved.map_primary_weights(
            &ModelStateTransaction::new(&backend, &context),
            ARTIFACT_DIGEST,
            &source,
    ...
    let partial = source_tensors(&backend, &context, DType::F32, true, false)?;
    assert!(matches!(
    ...
    let i2v = ModelProbe::from_parsed_facts(parsed_facts(
        DType::F32,
        17,
        2_048,
        false,
    ...
    let mut mismatched_facts = parsed_facts(DType::F32, 17, 2_048, false, false, false, "native");
    mismatched_facts.formats[0]
        .metadata
        .insert("in_channels".to_owned(), "16".to_owned());
    ...
    let missing_marker = ModelProbe::from_parsed_facts(parsed_facts(
        DType::F32,
        16,
        2_048,
        false,
    ...
    assert!(matches!(
        registry.resolve(&missing_marker),
        Err(ModelFamilyError::NoDetectionMatch)
    ));
    let anima = ModelProbe::from_parsed_facts(parsed_facts(
        DType::F32,
        16,
        2_048,
        false,
    ...
    let mut unexpected_facts = parsed_facts(DType::F32, 16, 2_048, false, false, false, "native");
    ...
    let mut unexpected = source_tensors(&backend, &context, DType::F32, false, false)?;
    unexpected.insert(
        "unexpected.weight".to_owned(),
    ...
    let no_match = ModelProbe {
        ...
        metadata: BTreeMap::from([("image_model".to_owned(), "cosmos".to_owned())]),
    };
    assert!(matches!(
...
fn source_tensors(
    ...
    for (key, shape) in model_shapes(16, 2_048, omit_final, anima, false) {
...
fn probe_through_model_store() -> Result<ModelProbe, Box<dyn std::error::Error>> {
    ...
    let model_path = directory.path().join("cosmost2ipredict2.safetensors");
    write_safetensors(&model_path)?;
    let mut index = ArtifactIndex::default();
    index.add_root(ArtifactRoot::canonical(
        "cosmost2ipredict2-row",
        "checkpoints",
        directory.path(),
        ["safetensors"],
    )?)?;
    ...
    let key = ArtifactKey::new("cosmost2ipredict2-row", "cosmost2ipredict2.safetensors")?;
...
fn write_safetensors(path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut shapes = model_shapes(16, 2_048, false, false, false)
        .into_iter()
...
fn assert_checkpoint(
    ...
    let checkpoint = checkpoints
        ...
        .ok_or("CosmosT2IPredict2 checkpoint is missing")?;
...
fn ordered_patch_graph(replace_first: bool) -> Result<PatchGraph, Box<dyn std::error::Error>> {
    let replacement = PatchOperation {
        identifier: "cosmost2ipredict2-ordered-replacement".to_owned(),
    ...
    let addition = PatchOperation {
        identifier: "cosmost2ipredict2-ordered-addition".to_owned(),
        kind: PatchKind::Lora,
        scale: 1.0,
        targets: vec![PatchTarget {
            key: "native.blocks.0.mlp.layer1.weight".to_owned(),
