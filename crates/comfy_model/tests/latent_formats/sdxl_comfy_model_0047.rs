use comfy_model::{
    ...
    generated_sdxl_comfy_model_0047::{LATENT_FORMAT, LATENT_FORMAT_IDENTIFIER},
...
#[test]
fn val_latent_001_sdxl_exact_contract_selector_and_serialized_identity()
-> Result<(), Box<dyn std::error::Error>> {
    assert_eq!(LATENT_FORMAT_IDENTIFIER, "SDXL");
    assert_eq!(LATENT_FORMAT.feature_id, "COMFY-MODEL-0047");
    ...
    assert_eq!(LATENT_FORMAT.scale_factor.to_bits(), 0.13025_f32.to_bits());
    ...
    assert_eq!(LATENT_FORMAT.preview_bias, Some([0.1084, -0.0175, -0.0011]));
    ...
    assert_eq!(
        encoded,
        format!(
            r#"{{"schema_version":{LATENT_FORMAT_SCHEMA_VERSION},"feature_id":"COMFY-MODEL-0047","identifier":"SDXL"}}"#
        )
    );
    ...
    assert_eq!(
        ...
        Some("COMFY-MODEL-0047")
    );
    Ok(())
}

#[test]
fn sdxl_geometry_affine_round_trip_allocation_and_preview_are_source_exact()
-> Result<(), Box<dyn std::error::Error>> {
    let (backend, authority) = CpuWorkspaceAuthority::create_backend(1024 * 1024)?;
    let cancellation = CancellationToken::default();
    let stream = StreamId::new(47);
    let context =
    ...
    for (actual, source) in values(&model_facing)?.iter().zip(source) {
        assert!((actual - source * 0.13025_f32).abs() <= 1.0e-6);
    }
    ...
    let bias = LATENT_FORMAT
        .preview_bias
        .ok_or("SDXL preview bias is required")?;
    let preview_values = values(&preview)?;
    for color in 0..3 {
        for position in 0..2 {
            let expected = (0..4).fold(bias[color], |value, channel| {
...
#[test]
fn sdxl_rejects_invalid_extent_shape_dtype_stream_and_cancellation()
-> Result<(), Box<dyn std::error::Error>> {
    let (backend, authority) = CpuWorkspaceAuthority::create_backend(1024 * 1024)?;
    let cancellation = CancellationToken::default();
    ...
    let foreign_stream = StreamId::new(48);
