use crate::{
    ...
    generated_zimage_comfy_model_0153::{
        PREFIXED_CAPTION_WEIGHT, PREFIXED_FINAL_WEIGHT, PREFIXED_QK_NORM_WEIGHT,
        PREFIXED_X_WEIGHT, STANDALONE_CAPTION_WEIGHT, STANDALONE_FINAL_WEIGHT,
        STANDALONE_QK_NORM_WEIGHT, STANDALONE_X_WEIGHT,
    },
    lumina_zimage_family::{
        ...
        LUMINA_ZIMAGE_SUPPORTED_DTYPES, LuminaZImageConfiguration, LuminaZImageLayout,
        LuminaZImageVariant, ZIMAGE_CLIP_TARGET, ZIMAGE_PIXEL_MEMORY_USAGE_FACTOR,
...
pub const MODEL_FAMILY_IDENTIFIER: &str = "ZImagePixelSpace";
pub const MODEL_FAMILY_FEATURE_ID: &str = "COMFY-MODEL-0154";
pub const MODEL_FAMILY_FIXTURE: &str = "zimagepixelspace-comfy-model-0154";
pub const MODEL_FAMILY_SOURCE_ORDINAL: u16 = 45;
...
pub const MODEL_FAMILY_PROJECTION_SHA256: &str =
    "cc89a70053b966ae0a231754e9bd8d411bd97338dfd81370315459802028c992";
pub const MODEL_FAMILY_MEMORY_USAGE_FACTOR: f64 = ZIMAGE_PIXEL_MEMORY_USAGE_FACTOR;
pub const SOURCE_ARCHITECTURE: &str = "model_base.ZImagePixelSpace";

pub const PREFIXED_DECODER_CONDITION_WEIGHT: &str =
    "model.diffusion_model.dec_net.cond_embed.weight";
pub const STANDALONE_DECODER_CONDITION_WEIGHT: &str = "dec_net.cond_embed.weight";

const CAPTION_WEIGHT_KEYS: &[&str] = &[PREFIXED_CAPTION_WEIGHT, STANDALONE_CAPTION_WEIGHT];
const QK_NORM_WEIGHT_KEYS: &[&str] = &[PREFIXED_QK_NORM_WEIGHT, STANDALONE_QK_NORM_WEIGHT];
const X_WEIGHT_KEYS: &[&str] = &[PREFIXED_X_WEIGHT, STANDALONE_X_WEIGHT];
const FINAL_WEIGHT_KEYS: &[&str] = &[PREFIXED_FINAL_WEIGHT, STANDALONE_FINAL_WEIGHT];
const DECODER_WEIGHT_KEYS: &[&str] = &[
    PREFIXED_DECODER_CONDITION_WEIGHT,
    STANDALONE_DECODER_CONDITION_WEIGHT,
];
...
const REQUIRED_KEYS: &[&str] = &[
    ...
    "native.dec_net.final_layer.linear.weight",
    "native.dec_net.input_embedder.embedder.0.weight",
];
...
pub const MODEL_FAMILY: ModelFamilyDefinition = ModelFamilyDefinition {
    ...
    architecture_version: "zimage-pixel-space-nextdit-v1",
    latent_feature_id: "COMFY-MODEL-0055",
    latent_identifier: "ZImagePixelSpace",
...
pub fn configuration_for_probe(
    ...
    if configuration.variant != LuminaZImageVariant::ZImagePixelSpace {
        return Err(ModelFamilyError::InvalidSelectorOutput(format!(
            "ZImagePixelSpace registration cannot claim {:?}",
            configuration.variant
        )));
    }
    if !matches!(
        configuration.layout,
        LuminaZImageLayout::PrefixedNative | LuminaZImageLayout::StandaloneNative
    ) {
        return Err(ModelFamilyError::InvalidSelectorOutput(
            "ZImagePixelSpace admits only prefixed-native or standalone-native layouts"
                .to_owned(),
        ));
    }
    Ok(configuration)
