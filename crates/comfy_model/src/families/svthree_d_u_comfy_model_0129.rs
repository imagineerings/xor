...
pub const MODEL_FAMILY_IDENTIFIER: &str = "SV3D_u";
pub const MODEL_FAMILY_FEATURE_ID: &str = "COMFY-MODEL-0129";
pub const MODEL_FAMILY_FIXTURE: &str = "svthree-d-u-comfy-model-0129";
pub const MODEL_FAMILY_SOURCE_ORDINAL: u16 = 17;
...
pub const MODEL_FAMILY_PROJECTION_SHA256: &str =
    "7418426898eecfd48c2b5355d4d2093d97824189bf702167b9aa86f3d464a4e7";
...
pub const MODEL_FAMILY_ADM_IN_CHANNELS: u64 = 256;
...
const COMPONENTS: &[ModelFamilyComponent] = &[
    ModelFamilyComponent {
        identifier: "model",
        role: "unconditioned stable-view denoiser",
        required: true,
    },
    ModelFamilyComponent {
        identifier: "vae",
        role: "stable-view latent codec",
        required: false,
    },
    ModelFamilyComponent {
...
const FORWARD_PROGRAM: &[ModelForwardStep] = &[
    ModelForwardStep {
        checkpoint: "camera_conditioning_projection",
    ...
    ModelForwardStep {
        checkpoint: "view_output_projection",
        operation: ModelForwardOperation::Linear {
...
const NATIVE_STATE_PLAN: ModelStateTransformPlanDefinition = ModelStateTransformPlanDefinition {
    ...
    encoded_plan: r#"{
        "operations": [
            ...
            {"Move":{"selector":{"predicate":{"Prefix":"conditioner.embedders.1.encoder."},"minimum_matches":0,"maximum_matches":16384},"rewrite":{"Prefix":{"from":"conditioner.embedders.1.encoder.","to":"vae."}},"component":"vae"}},
...
pub fn configuration_for_probe(probe: &ModelProbe) -> Result<Sv3dUConfiguration, ModelFamilyError> {
    ...
    if configuration.kind() != ModelConfigurationKind::Native {
        return Err(ModelFamilyError::InvalidSelectorOutput(
            "SV3D_u is not present in the pinned source Diffusers detector table".to_owned(),
        ));
    }
        ...
        || !temporal_residual_blocks
    {
        return Err(ModelFamilyError::InvalidSelectorOutput(format!(
            "SV3D_u normalized configuration is incompatible: {:?}",
            configuration.facts()
        )));
    }
    Ok(Sv3dUConfiguration {
        model_channels,
        input_channels,
        context_dimension,
        adm_input_channels,
...
fn unsigned_fact(
    ...
    match configuration.fact(name) {
        ...
        value => Err(ModelFamilyError::InvalidSelectorOutput(format!(
            "SV3D_u requires unsigned {name}, found {value:?}"
        ))),
    }
}
...
fn boolean_fact(
    ...
    match configuration.fact(name) {
        ...
        value => Err(ModelFamilyError::InvalidSelectorOutput(format!(
            "SV3D_u requires boolean {name}, found {value:?}"
        ))),
    }
}
