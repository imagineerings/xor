use crate::{
    ...
    generated_wan21_t2v_comfy_model_0146::{
        CAUSAL_AUDIO_WEIGHT, FFN_WEIGHT, HEAD_MODULATION, HEAD_WEIGHT, PATCH_WEIGHT,
...
pub const MODEL_FAMILY_IDENTIFIER: &str = "WAN22_S2V";
pub const MODEL_FAMILY_FEATURE_ID: &str = "COMFY-MODEL-0150";
pub const MODEL_FAMILY_FIXTURE: &str = "wan22-s2v-comfy-model-0150";
pub const MODEL_FAMILY_SOURCE_ORDINAL: u16 = 58;
...
pub const MODEL_FAMILY_PROJECTION_SHA256: &str =
    "3b142063e696205547df0e3590a26a3152b3ebebcaf8bc3f72de26018a4a7d18";
pub const MODEL_FAMILY_SHIFT: f64 = 8.0;
pub const SOURCE_ARCHITECTURE: &str = "model_base.WAN22_S2V";

const COMPONENTS: &[ModelFamilyComponent] = &[
    ModelFamilyComponent { identifier: "model", role: "Wan 2.2 audio-driven S2V transformer", required: true },
    ModelFamilyComponent { identifier: "runtime_conditioning", role: "audio, reference latent/motion, control video, and sampling-shift contract", required: true },
...
const DETECTION_RULES: &[ModelDetectionRule] = &[
    ...
    ModelDetectionRule::KeyPresent { key: CAUSAL_AUDIO_WEIGHT, score: 600 },
];
const FORWARD_PROGRAM: &[ModelForwardStep] = &[
    ModelForwardStep { checkpoint: "s2v_timestep_embedding", operation: ModelForwardOperation::Linear { weight: "native.time_embedding.0.weight", bias: None, input_features: 2, output_features: 2 } },
    ModelForwardStep { checkpoint: "s2v_audio_reference_conditioning", operation: ModelForwardOperation::Silu },
    ModelForwardStep { checkpoint: "s2v_transformer_block", operation: ModelForwardOperation::Linear { weight: "native.blocks.0.ffn.2.weight", bias: None, input_features: 2, output_features: 2 } },
    ModelForwardStep { checkpoint: "s2v_block_normalization", operation: ModelForwardOperation::LayerNorm { normalized_shape: &[2], weight: None, bias: None, epsilon: 1.0e-6 } },
    ModelForwardStep { checkpoint: "s2v_head_projection", operation: ModelForwardOperation::Linear { weight: "native.time_projection.1.weight", bias: None, input_features: 2, output_features: 2 } },
    ModelForwardStep { checkpoint: "s2v_video_output", operation: ModelForwardOperation::Tanh },
];
pub const MODEL_FAMILY: ModelFamilyDefinition = ModelFamilyDefinition {
    feature_id: MODEL_FAMILY_FEATURE_ID, identifier: MODEL_FAMILY_IDENTIFIER,
    architecture_version: "wan22-s2v-v1", latent_feature_id: "COMFY-MODEL-0053",
...
const NATIVE_STATE_PLAN: ModelStateTransformPlanDefinition = ModelStateTransformPlanDefinition {
    ...
    encoded_plan: r#"{
        "operations": [
            ...
            {"Generate":{"shape":[{"Literal":1}],"fill":{"float":1.0},"dtype":"f32","output":{"component":"runtime_conditioning","key":"reference_latent"}}},
            {"Generate":{"shape":[{"Literal":1}],"fill":{"float":1.0},"dtype":"f32","output":{"component":"runtime_conditioning","key":"reference_motion"}}},
            {"Generate":{"shape":[{"Literal":1}],"fill":{"float":1.0},"dtype":"f32","output":{"component":"runtime_conditioning","key":"control_video"}}}
        ], "unmatched":"Reject"
    }"#,
};
...
const LAYOUT_SIGNATURES: &[ModelLayoutSignature] = &[ModelLayoutSignature { layout: ModelStateLayout::PrefixedNative, required_keys: &[HEAD_MODULATION, HEAD_WEIGHT, PATCH_WEIGHT, FFN_WEIGHT, CAUSAL_AUDIO_WEIGHT], required_prefixes: &[] }];
const COMPONENT_STATE_SCHEMAS: &[ModelFamilyComponentStateSchema] = &[
    ...
    ModelFamilyComponentStateSchema { component: "runtime_conditioning", required_keys: &["sampling_shift", "audio_embed", "reference_latent", "reference_motion", "control_video"], optional_keys: &[], allow_unexpected: false },
...
pub const MODEL_FAMILY_REGISTRATION: ModelFamilyRegistration = ModelFamilyRegistration {
    ...
    required_state_keys: &[CAUSAL_AUDIO_WEIGHT],
...
pub fn configuration_for_probe(probe: &ModelProbe) -> Result<Wan22BatchConfiguration, ModelFamilyError> {
    if probe.select_layout(LAYOUT_SIGNATURES)? != ModelStateLayout::PrefixedNative { return Err(ModelFamilyError::InvalidSelectorOutput("WAN22_S2V requires the source-native prefixed layout".to_owned())); }
    batch_configuration_for_probe(probe, Wan22BatchVariant::S2V)
}
