...
async fn test_editorconfig_support(cx: &mut gpui::TestAppContext) {
    ...
    // .editorconfig overrides .baymax/settings
    ...
    // When max_line_length is "off", default to .baymax/settings.json
...
async fn test_git_provider_project_setting(cx: &mut gpui::TestAppContext) {
    ...
    fs.atomic_write(
        Path::new(path!("/dir/.baymax/settings.json")).to_owned(),
        "{}".into(),
    )
    .await
    .unwrap();
...
async fn test_managing_project_specific_settings(cx: &mut gpui::TestAppContext) {
    ...
    let topmost_local_task_source_kind = TaskSourceKind::Worktree {
        id: worktree_id,
        directory_in_worktree: rel_path(".baymax").into(),
        id_base: "local worktree tasks from directory \".baymax\"".into(),
    };
        ...
        vec![
            (
                TaskSourceKind::Worktree {
                    id: worktree_id,
                    directory_in_worktree: rel_path("b/.baymax").into(),
                    id_base: "local worktree tasks from directory \"b/.baymax\"".into()
                },
...
async fn test_invalid_local_tasks_shows_toast_with_doc_link(cx: &mut gpui::TestAppContext) {
    ...
    // We need to start with a valid `.baymax/tasks.json` file as otherwise the
    ...
    // Update the `.baymax/tasks.json` file with an invalid variable, so we can
    // later assert that the `Event::Toast` even is emitted.
    fs.save(
        path!("/dir/.baymax/tasks.json").as_ref(),
        &r#"[{ "label": "test $BAYMAX_FOO", "command": "echo" }]"#.into(),
        Default::default(),
    )
    .await
    .unwrap();
    ...
    project.update(cx, |_, cx| {
        ...
        cx.subscribe(&project, move |_, _, event: &Event, _| match event {
            Event::Toast {
                ...
                assert!(message.contains("BAYMAX_FOO"));
                assert_eq!(*url, "https://baymax.dev/docs/tasks");
...
async fn test_fallback_to_single_worktree_tasks(cx: &mut gpui::TestAppContext) {
        ...
        json!({
            ".baymax": {
                "tasks.json": r#"[{
                    ...
                    "command": "echo $BAYMAX_WORKTREE_ROOT"
                }]"#,
            },
            "a": {
                "a.rs": "fn a() {\n    A\n}"
    ...
    assert!(
        ...
        "A task can not be resolved with context with no BAYMAX_WORKTREE_ROOT data"
    );
