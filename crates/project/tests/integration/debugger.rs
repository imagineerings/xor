mod go_locator {
    ...
    async fn test_create_scenario_for_go_build(_: &mut TestAppContext) {
        ...
        let task = TaskTemplate {
            ...
            env: Default::default(),
            cwd: Some("${BAYMAX_WORKTREE_ROOT}".into()),
...
mod python_locator {
    ...
    async fn test_python_locator() {
        ...
        let build_task = TaskTemplate {
            label: "run module '$BAYMAX_FILE'".into(),
            command: "$BAYMAX_CUSTOM_PYTHON_ACTIVE_BAYMAX_TOOLCHAIN".into(),
            args: vec!["-m".into(), "$BAYMAX_CUSTOM_PYTHON_MODULE_NAME".into()],
            env: Default::default(),
            cwd: Some("$BAYMAX_WORKTREE_ROOT".into()),
            use_new_terminal: false,
            allow_concurrent_runs: false,
        ...
        let expected_scenario = DebugScenario {
            ...
            config: json!({
                "request": "launch",
                "python": "$BAYMAX_CUSTOM_PYTHON_ACTIVE_BAYMAX_TOOLCHAIN",
                "args": [],
                "cwd": "$BAYMAX_WORKTREE_ROOT",
                "module": "$BAYMAX_CUSTOM_PYTHON_MODULE_NAME",
            }),
            tcp_connection: None,
        };
