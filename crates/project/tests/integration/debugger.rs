mod go_locator {
    ...
    async fn test_create_scenario_for_go_build(_: &mut TestAppContext) {
        ...
        let task = TaskTemplate {
            ...
            env: Default::default(),
            cwd: Some("${SIM_WORKTREE_ROOT}".into()),
...
mod python_locator {
    ...
    async fn test_python_locator() {
        ...
        let build_task = TaskTemplate {
            label: "run module '$SIM_FILE'".into(),
            command: "$SIM_CUSTOM_PYTHON_ACTIVE_SIM_TOOLCHAIN".into(),
            args: vec!["-m".into(), "$SIM_CUSTOM_PYTHON_MODULE_NAME".into()],
            env: Default::default(),
            cwd: Some("$SIM_WORKTREE_ROOT".into()),
            use_new_terminal: false,
            allow_concurrent_runs: false,
        ...
        let expected_scenario = DebugScenario {
            ...
            config: json!({
                "request": "launch",
                "python": "$SIM_CUSTOM_PYTHON_ACTIVE_SIM_TOOLCHAIN",
                "args": [],
                "cwd": "$SIM_WORKTREE_ROOT",
                "module": "$SIM_CUSTOM_PYTHON_MODULE_NAME",
            }),
            tcp_connection: None,
        };
