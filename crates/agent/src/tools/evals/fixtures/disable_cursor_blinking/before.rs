...
impl Editor {
    ...
    fn trigger_on_type_formatting(
        ...
        // OnTypeFormatting returns a list of edits, no need to pass them between Baymax instances,
        ...
        let result = h_flex()
            ...
            .when(!has_keybind, |el| {
                ...
                el.bg(status_colors.error_background)
                    .border_color(status_colors.error.opacity(0.6))
                    .pl_2()
                    .child(Icon::new(IconName::BaymaxPredictError).color(Color::Error))
                    .cursor_default()
                    .hoverable_tooltip(move |_window, cx| {
                        cx.new(|_| MissingEditPredictionKeybindingTooltip).into()
                    })
                        ...
                        h_flex()
                            ...
                            .child(div().px_1p5().child(match &prediction.completion {
                                InlineCompletion::Move { target, snapshot } => {
                                    ...
                                    {
                                        Icon::new(IconName::BaymaxPredictDown)
                                    } else {
                                        Icon::new(IconName::BaymaxPredictUp)
                                    }
                                }
                                InlineCompletion::Edit { .. } => Icon::new(IconName::BaymaxPredict),
                            }))
                            .child(
                                h_flex()
                                    .gap_1()
        ...
        match &completion.completion {
            ...
            InlineCompletion::Edit {
                ...
                let left = if first_edit_row != cursor_point.row {
                    ...
                    Icon::new(IconName::BaymaxPredict).into_any_element()
                };
                ...
                .child(
                    h_flex()
                        ...
                        .child(Button::new("open-keymap", "Assign Keybinding").size(ButtonSize::Compact).on_click(|_ev, window, cx| {
                            window.dispatch_action(baymax_actions::OpenKeymap.boxed_clone(), cx)
                        }))
                        .child(Button::new("see-docs", "See Docs").size(ButtonSize::Compact).on_click(|_ev, _window, cx| {
                            cx.open_url("https://baymax.dev/docs/completions#edit-predictions-missing-keybinding");
                        })),
                )
        })
    }
