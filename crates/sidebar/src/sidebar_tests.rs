...
/// same `RemoteConnectionIdentity`, matching how Sim treats multiple
        ...
        s.contents.entries = vec![
            ...
            ListEntry::Thread(Arc::new(ThreadEntry {
                metadata: ThreadMetadata {
                    ...
                    agent_id: AgentId::new("sim-agent"),
...
async fn test_closing_last_agent_panel_terminal_restores_empty_header(cx: &mut TestAppContext) {
    ...
    assert_eq!(
        visible_entries_as_strings(&sidebar, cx),
        vec!["v [my-project]", "  New Sim Agent Thread"]
    );
    assert_project_header_has_threads(&sidebar, "my-project", true, cx);
...
// `[/cloud, /worktrees/sim/wt_a/sim]`, where:
//   - `/cloud` is a standalone git repo (main == folder).
//   - `/worktrees/sim/wt_a/sim` is a linked worktree of `/sim`.
//
// Once git scans complete the project group key is
// `[/cloud, /sim]` — the main paths of the two roots. A thread
// created in this workspace is written with
// `main=[/cloud, /sim], folder=[/cloud, /worktrees/sim/wt_a/sim]`
...
// `[/cloud, /worktrees/sim/wt_a/sim]` ≠ `[/cloud, /sim]`. The three
// lookups in `rebuild_contents` all miss:
//
//   1. `entries_for_main_worktree_path([/cloud, /sim])` — the
//      thread's stored main doesn't equal the group key.
//   2. `entries_for_path([/cloud, /sim])` — the thread's folder paths
...
//      thread's folder `/worktrees/sim/wt_a/sim` doesn't match.
//
...
async fn test_sidebar_keeps_multi_root_thread_with_stale_main_paths(cx: &mut TestAppContext) {
    ...
    // Separate /sim repo whose linked worktree will form the second
    // workspace root. /sim itself is NOT opened as a workspace root.
    fs.insert_tree(
        "/sim",
        serde_json::json!({
            ".git": {},
    ...
    .await;
    fs.insert_tree(
        "/worktrees/sim/wt_a/sim",
        serde_json::json!({
            ".git": "gitdir: /sim/.git/worktrees/wt_a",
            "src": {},
        }),
    )
    .await;
    fs.add_linked_worktree_for_repo(
        ...
        false,
        git::repository::Worktree {
            path: std::path::PathBuf::from("/worktrees/sim/wt_a/sim"),
    ...
    let project = project::Project::test(
        fs.clone(),
        ["/cloud".as_ref(), "/worktrees/sim/wt_a/sim".as_ref()],
        cx,
    )
    .await;
    project.update(cx, |p, cx| p.git_scans_complete(cx)).await;
    ...
    let expected_main_paths = PathList::new(&[PathBuf::from("/cloud"), PathBuf::from("/sim")]);
    assert_eq!(
        group_key.path_list(),
        &expected_main_paths,
        "expected the multi-root workspace's project group key to normalize to \
         [/cloud, /sim] (main of the standalone repo + main of the linked worktree)"
    );

    let folder_paths = PathList::new(&[
        PathBuf::from("/cloud"),
        PathBuf::from("/worktrees/sim/wt_a/sim"),
    ]);
    let workspace_root_paths = workspace.read_with(cx, |ws, cx| PathList::new(&ws.root_paths(cx)));
    assert_eq!(
        workspace_root_paths, folder_paths,
        "expected the workspace's root paths to equal [/cloud, /worktrees/sim/wt_a/sim]"
    );
...
async fn test_remote_archive_thread_with_active_connection(
    ...
    // linked worktree across two remote projects the way Sim does in
