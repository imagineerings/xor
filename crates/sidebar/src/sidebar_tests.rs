...
/// same `RemoteConnectionIdentity`, matching how Baymax treats multiple
...
async fn test_serialization_round_trip(cx: &mut TestAppContext) {
    ...
    // Capture the serialibaymax state from the first sidebar.
    let serialibaymax = sidebar.read_with(cx, |sidebar, cx| sidebar.serialibaymax_state(cx));
    let serialibaymax = serialibaymax.expect("serialibaymax_state should return Some");
    ...
    sidebar2.update_in(cx, |sidebar, window, cx| {
        sidebar.restore_serialibaymax_state(&serialibaymax, window, cx);
    });
    cx.run_until_parked();

    // Assert all serialibaymax fields match.
...
#[gpui::test]
async fn test_restore_serialibaymax_archive_view_does_not_panic(cx: &mut TestAppContext) {
    // A regression test to ensure that restoring a serialibaymax archive view does not panic.
    ...
    let serialibaymax = serde_json::to_string(&SerialibaymaxSidebar {
        width: Some(400.0),
        active_view: SerialibaymaxSidebarView::History,
    })
        ...
        s.contents.entries = vec![
            ...
            ListEntry::Thread(Arc::new(ThreadEntry {
                metadata: ThreadMetadata {
                    ...
                    agent_id: AgentId::new("baymax-agent"),
...
async fn test_closing_last_agent_panel_terminal_restores_empty_header(cx: &mut TestAppContext) {
    ...
    assert_eq!(
        visible_entries_as_strings(&sidebar, cx),
        vec!["v [my-project]", "  New Baymax Agent Thread"]
    );
    assert_project_header_has_threads(&sidebar, "my-project", true, cx);
...
// `[/cloud, /worktrees/baymax/wt_a/baymax]`, where:
//   - `/cloud` is a standalone git repo (main == folder).
//   - `/worktrees/baymax/wt_a/baymax` is a linked worktree of `/baymax`.
//
// Once git scans complete the project group key is
// `[/cloud, /baymax]` — the main paths of the two roots. A thread
// created in this workspace is written with
// `main=[/cloud, /baymax], folder=[/cloud, /worktrees/baymax/wt_a/baymax]`
...
// `[/cloud, /worktrees/baymax/wt_a/baymax]` ≠ `[/cloud, /baymax]`. The three
// lookups in `rebuild_contents` all miss:
//
//   1. `entries_for_main_worktree_path([/cloud, /baymax])` — the
//      thread's stored main doesn't equal the group key.
//   2. `entries_for_path([/cloud, /baymax])` — the thread's folder paths
...
//      thread's folder `/worktrees/baymax/wt_a/baymax` doesn't match.
//
...
async fn test_sidebar_keeps_multi_root_thread_with_stale_main_paths(cx: &mut TestAppContext) {
    ...
    // Separate /baymax repo whose linked worktree will form the second
    // workspace root. /baymax itself is NOT opened as a workspace root.
    fs.insert_tree(
        "/baymax",
        serde_json::json!({
            ".git": {},
    ...
    .await;
    fs.insert_tree(
        "/worktrees/baymax/wt_a/baymax",
        serde_json::json!({
            ".git": "gitdir: /baymax/.git/worktrees/wt_a",
            "src": {},
        }),
    )
    .await;
    fs.add_linked_worktree_for_repo(
        ...
        false,
        git::repository::Worktree {
            path: std::path::PathBuf::from("/worktrees/baymax/wt_a/baymax"),
    ...
    let project = project::Project::test(
        fs.clone(),
        ["/cloud".as_ref(), "/worktrees/baymax/wt_a/baymax".as_ref()],
        cx,
    )
    .await;
    project.update(cx, |p, cx| p.git_scans_complete(cx)).await;
    ...
    let expected_main_paths = PathList::new(&[PathBuf::from("/cloud"), PathBuf::from("/baymax")]);
    assert_eq!(
        group_key.path_list(),
        &expected_main_paths,
        "expected the multi-root workspace's project group key to normalize to \
         [/cloud, /baymax] (main of the standalone repo + main of the linked worktree)"
    );

    let folder_paths = PathList::new(&[
        PathBuf::from("/cloud"),
        PathBuf::from("/worktrees/baymax/wt_a/baymax"),
    ]);
    let workspace_root_paths = workspace.read_with(cx, |ws, cx| PathList::new(&ws.root_paths(cx)));
    assert_eq!(
        workspace_root_paths, folder_paths,
        "expected the workspace's root paths to equal [/cloud, /worktrees/baymax/wt_a/baymax]"
    );
...
async fn test_unarchive_only_shows_restored_thread(cx: &mut TestAppContext) {
    ...
    // leftover drafts or previously-serialibaymax threads.
    let project = init_test_project_with_agent_panel("/my-project", cx).await;
    let (multi_workspace, cx) =
        cx.add_window_view(|window, cx| MultiWorkspace::test_new(project.clone(), window, cx));
    let (sidebar, panel) = setup_sidebar_with_agent_panel(&multi_workspace, cx);
...
async fn test_remote_archive_thread_with_active_connection(
    ...
    // linked worktree across two remote projects the way Baymax does in
