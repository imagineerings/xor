...
pub struct WorkspaceSettingsContent {
    ...
    /// Controls previous session restoration in freshly launched Baymax instance.
    ...
    /// When set to false, Baymax will use the built-in keyboard-first pickers.
    ///
    /// Default: true
    pub use_system_path_prompts: Option<bool>,
    /// Whether to use the system provided prompts.
    /// When set to false, Baymax will use the built-in prompts.
    /// Note that this setting has no effect on Linux, where Baymax will always
    ...
    /// What draws window decorations/titlebar, the client application (Baymax) or display server
    /// Default: client
...
pub enum WindowDecorations {
    /// Baymax draws its own window decorations/titlebar (client-side decoration)
    #[default]
    Client,
...
pub enum CliDefaultOpenBehavior {
    /// Open directories as a new workspace in the current Baymax window's sidebar.
    #[default]
...
pub enum DefaultOpenBehavior {
    /// Open projects in the current Baymax window.
    #[default]
    #[strum(serialize = "Add to Existing Window")]
    ExistingWindow,
...
pub enum RestoreOnStartupBehavior {
    ...
    LastWorkspace,
    /// Restore all workspaces that were open when quitting Baymax.
    #[default]
    LastSession,
