...
pub struct WorkspaceSettingsContent {
    ...
    /// Controls previous session restoration in freshly launched Sim instance.
    ...
    /// When set to false, Sim will use the built-in keyboard-first pickers.
    ///
    /// Default: true
    pub use_system_path_prompts: Option<bool>,
    /// Whether to use the system provided prompts.
    /// When set to false, Sim will use the built-in prompts.
    /// Note that this setting has no effect on Linux, where Sim will always
    ...
    /// What draws window decorations/titlebar, the client application (Sim) or display server
    /// Default: client
...
pub enum WindowDecorations {
    /// Sim draws its own window decorations/titlebar (client-side decoration)
    #[default]
    Client,
...
pub enum DefaultOpenBehavior {
    /// Open projects in the current Sim window.
    #[default]
    #[strum(serialize = "Add to Existing Window")]
    ExistingWindow,
...
pub enum RestoreOnStartupBehavior {
    ...
    LastWorkspace,
    /// Restore all workspaces that were open when quitting Sim.
    #[default]
    LastSession,
