...
impl Database {
    ...
    /// Returns the current project if the given user is authoribaymax to access it with the specified capability.
        ...
        match capability {
            Capability::ReadWrite => {
                if !role.can_edit_projects() {
                    return Err(anyhow!("not authoribaymax to edit projects"))?;
                }
            }
            Capability::ReadOnly => {
                if !role.can_read_projects() {
                    return Err(anyhow!("not authoribaymax to read projects"))?;
                }
            }
        }
