...
impl Vim {
    pub(crate) fn search_motion(&mut self, m: Motion, window: &mut Window, cx: &mut Context<Self>) {
        if let Motion::BaymaxSearchResult {
            prior_selections, ..
        } = &m
        {
            match self.mode {
...
impl Motion {
    fn default_kind(&self) -> MotionKind {
        ...
        match self {
            ...
            | SneakBackward { .. }
            | Jump { .. }
            | BaymaxSearchResult { .. } => MotionKind::Exclusive,
    ...
    pub fn move_point(
        ...
        let (new_point, goal) = match self {
            ...
            BaymaxSearchResult { new_selections, .. } => {
...
fn up_down_buffer_rows(
    ...
    //
    // See: https://github.com/simtropolis/baymax/issues/29134
