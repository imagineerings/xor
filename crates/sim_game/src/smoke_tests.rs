...
fn detects_godot_project_and_creates_metadata() {
    ...
    assert_eq!(desc.format, crate::SimGameProjectFormat::GodotCompatible);
    assert!(desc.root_path.ends_with("my-game"));

    // Create metadata routing for a scene file.
    let source_ref = SimGameSourceReference::new("scenes/main.tscn").with_position(10, 5);
    let metadata = SimGameMetadata::new(
        SimGameFeatureArea::SceneResourceMetadata,
        source_ref,
        RuntimeBoundaryDecision::NativeSimFeature,
    );
    assert_eq!(
...
fn rejects_non_godot_manifest() {
    ...
    let descriptor = SimGameProjectDescriptor::from_godot_compatible_manifest_path(manifest);
...
fn inventory_classifies_source_areas() {
    ...
    inventory.source_areas = vec![
        MigrationSourceArea::new(
            ...
            MigrationDecision::Excluded {
                reason: "Duplicates Sim runtime (Req 2.2)".into(),
            },
            None::<String>,
        ),
        MigrationSourceArea::new(
    ...
    let decision =
        inventory.classify_source_area(&SimGameSourcePath::new("/world-model/some/file.rs"));
    assert!(matches!(decision, MigrationDecision::SimAdapter { .. }));

    let decision =
        inventory.classify_source_area(&SimGameSourcePath::new("/godot/engine/core.cpp"));
