...
#[test]
fn classifies_baymax_owned_capability_as_native() {
...
fn classifies_world_generation_as_adapter_with_world_model_owner() {
    ...
    match decision {
        RuntimeBoundaryDecision::BaymaxAdapter { ref owner } => {
            assert_eq!(owner, "world_model");
        }
        other => panic!("Expected BaymaxAdapter, got: {other:?}"),
    }
}
...
#[test]
fn classifies_asset_library_as_adapter_with_baymax_game_owner() {
    let policy = DefaultBoundaryPolicy;
    let decision = policy.classify("assets", "Asset library and fixture management");
    match decision {
        RuntimeBoundaryDecision::BaymaxAdapter { ref owner } => {
            assert_eq!(owner, "baymax_game");
        }
        other => panic!("Expected BaymaxAdapter, got: {other:?}"),
    }
}
