...
///
/// Rules:
/// - If the feature scope names a capability Baymax already owns (UI,
///   rendering, platform, input, etc.), return `NativeBaymaxFeature`.
/// - If the feature scope names a Godot runtime subsystem that duplicates
///   Baymax runtime architecture, return `Excluded` or `ExternalCommand`.
/// - If the feature scope names an external tool or CLI, return `ExternalCommand`.
/// - Otherwise, return `BaymaxAdapter` as the default fallback.
...
impl RuntimeBoundaryPolicy for DefaultBoundaryPolicy {
    fn classify(&self, _name: &str, scope: &str) -> RuntimeBoundaryDecision {
        ...
        // Godot runtime subsystems that duplicate Baymax runtime (Req 2.2).
        // Check BEFORE Baymax-owned capabilities so that "Godot engine
        ...
        if contains_any(&lower, GODOT_RUNTIME_DUPLICATIONS) {
            return RuntimeBoundaryDecision::Excluded {
                reason: format!(
                    "Godot runtime subsystem '{}' duplicates Baymax runtime \
        ...
        // Baymax-owned capabilities (Req 2.1).
        if contains_any(&lower, BAYMAX_OWNED_CAPABILITIES) {
            return RuntimeBoundaryDecision::NativeBaymaxFeature;
        }
...
/// Feature areas Baymax already owns natively (Req 2.1).
const BAYMAX_OWNED_CAPABILITIES: &[&str] = &[
    "editor ui",
    "platform",
    "rendering",
    "input",
...
/// Godot runtime subsystems that duplicate Baymax runtime architecture (Req
/// 2.2).
...
fn infer_owner(scope: &str) -> String {
    ...
    if lower.contains("asset") || lower.contains("library") || lower.contains("fixture") {
        return "baymax_game".to_string();
    }
    "baymax_game".to_string()
}
