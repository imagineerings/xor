    ...
    let inventory = MigrationInventory::new(&spec_root)
        ...
        .with_source_areas(vec![
            MigrationSourceArea::new(
                ...
                MigrationDecision::NativeBaymaxFeature,
            ...
            MigrationSourceArea::new(
                ...
                MigrationDecision::Excluded {
                    reason: "Baymax already owns collaboration and RPC runtime infrastructure"
                        .to_string(),
                },
                None::<PathBuf>,
            ),
...
fn classifies_source_paths_by_most_specific_inventory_prefix() {
    ...
    assert_eq!(
        inventory.classify_source_area(&BaymaxGameSourcePath::new(
    ...
    assert_eq!(
        inventory.classify_source_area(&BaymaxGameSourcePath::new("projects/unknown")),
        MigrationDecision::Excluded {
            reason: "Source area is not listed in the Baymax game migration inventory".to_string(),
        }
    );
}
...
fn create_spec_root(name: &str) -> PathBuf {
    let timestamp = SystemTime::now()
        ...
        .as_nanos();
    let path = std::env::temp_dir().join(format!("baymax-game-inventory-{name}-{timestamp}"));
