//! Contains helper functions for constructing URLs to various Sim-related pages.
//!
...
/// Returns the URL to the account page on sim.dev.
pub fn account_url(cx: &App) -> String {
    format!("{server_url}/account", server_url = server_url(cx))
}

/// Returns the URL to the start trial page on sim.dev.
...
/// Returns the URL to the upgrade page on sim.dev.
pub fn upgrade_to_sim_pro_url(cx: &App) -> String {
    format!("{server_url}/account/upgrade", server_url = server_url(cx))
}

/// Returns the URL to Sim's terms of service.
pub fn terms_of_service(cx: &App) -> String {
    format!("{server_url}/terms-of-service", server_url = server_url(cx))
}

/// Returns the URL to Sim AI's privacy and security docs.
...
/// Returns the URL to Sim's edit prediction documentation.
...
pub fn shared_agent_thread_url(session_id: &str) -> String {
    format!("sim://agent/shared/{}", session_id)
}
