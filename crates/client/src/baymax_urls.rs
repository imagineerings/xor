//! Contains helper functions for constructing URLs to various Baymax-related pages.
//!
...
/// Returns the URL to the account page on baymax.dev.
pub fn account_url(cx: &App) -> String {
    format!("{server_url}/account", server_url = server_url(cx))
}

/// Returns the URL to the start trial page on baymax.dev.
...
/// Returns the URL to the upgrade page on baymax.dev.
pub fn upgrade_to_baymax_pro_url(cx: &App) -> String {
    format!("{server_url}/account/upgrade", server_url = server_url(cx))
}

/// Returns the URL to Baymax's terms of service.
pub fn terms_of_service(cx: &App) -> String {
    format!("{server_url}/terms-of-service", server_url = server_url(cx))
}

/// Returns the URL to Baymax AI's privacy and security docs.
...
/// Returns the URL to Baymax's edit prediction documentation.
...
/// Returns the URL to Baymax's ACP registry blog post.
...
pub fn shared_agent_thread_url(session_id: &str) -> String {
    format!("baymax://agent/shared/{}", session_id)
}
