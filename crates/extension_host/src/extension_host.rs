...
/// The current extension [`SchemaVersion`] supported by Baymax.
...
/// Returns the [`SchemaVersion`] range that is compatible with this version of Baymax.
pub fn schema_version_range() -> RangeInclusive<SchemaVersion> {
    SchemaVersion::ZERO..=CURRENT_SCHEMA_VERSION
}

/// Returns whether the given extension version is compatible with this version of Baymax.
...
impl ExtensionStore {
    ...
    /// Installs any extensions that should be included with Baymax by default.
    ///
    ...
    fn fetch_extensions_from_api(
        ...
        let url = self.http_client.build_baymax_api_url(path, query);
