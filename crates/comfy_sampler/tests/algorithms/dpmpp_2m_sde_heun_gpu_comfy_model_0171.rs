use comfy_sampler::{
    ...
    generated_dpmpp_2m_sde_gpu_comfy_model_0169::{
        DPMPP_2M_SDE_GPU_BROWNIAN_CONTRACT_ID, DPMPP_2M_SDE_GPU_SAMPLER_ID, Dpmpp2mSdeGpuOptions,
        Dpmpp2mSdeSolverType, sample_dpmpp_2m_sde_gpu,
    },
    generated_dpmpp_2m_sde_heun_gpu_comfy_model_0171::{
        DEFINITION, DPMPP_2M_SDE_HEUN_GPU_FEATURE_ID, DPMPP_2M_SDE_HEUN_GPU_SAMPLER_ID,
        DPMPP_2M_SDE_HEUN_GPU_SOURCE_ORDINAL, Dpmpp2mSdeHeunGpuError, sample_dpmpp_2m_sde_heun_gpu,
    },
};
use comfy_tensor::{
...
const FIXTURE_JSON: &str = include_str!(concat!(
    ...
    "/../comfy_test_support/fixtures/samplers/dpmpp_2m_sde_heun_gpu_comfy_model_0171/trajectory.json"
));
const IMPLEMENTATION: &str =
    include_str!("../../src/algorithms/dpmpp_2m_sde_heun_gpu_comfy_model_0171.rs");
...
fn profile() -> Result<DiscreteSamplingProfile, Box<dyn Error>> {
    Ok(DiscreteSamplingProfile::new(
        SamplingProfileIdentity::new("dpmpp-2m-sde-heun-gpu-row-v1")?,
...
#[test]
fn val_sampler_001_dpmpp_2m_sde_heun_gpu_provenance_registry_and_ownership_are_exact()
-> Result<(), Box<dyn Error>> {
    let fixture = fixture()?;
    assert_eq!(fixture.schema_version, 1);
    assert_eq!(fixture.identity, DPMPP_2M_SDE_HEUN_GPU_SAMPLER_ID);
    assert_eq!(fixture.feature_id, DPMPP_2M_SDE_HEUN_GPU_FEATURE_ID);
    assert_eq!(fixture.source_ordinal, DPMPP_2M_SDE_HEUN_GPU_SOURCE_ORDINAL);
    ...
    assert_eq!(
        ...
        "algorithms/dpmpp_2m_sde_heun_gpu_comfy_model_0171"
    );
    ...
    assert_eq!(
        registry.resolve(&SamplerIdentity::new(DPMPP_2M_SDE_HEUN_GPU_SAMPLER_ID)?)?,
        &DEFINITION
    );
    assert_ne!(
        DPMPP_2M_SDE_HEUN_GPU_SAMPLER_ID,
    ...
    assert!(
        samplers
            ...
            .is_some_and(|line| line.contains("\"dpmpp_2m_sde_heun_gpu\""))
    );
    ...
    assert!(
        catalog
            ...
            .is_some_and(|line| line.contains("sampler,dpmpp_2m_sde_heun_gpu,")
                && line.ends_with(",COMFY-MODEL-0171"))
    );

    assert!(IMPLEMENTATION.contains("sample_dpmpp_2m_sde_gpu("));
    assert!(IMPLEMENTATION.contains("Dpmpp2mSdeSolverType::Heun"));
    assert!(IMPLEMENTATION.contains("CompatibilityNoiseRequest"));
    for forbidden in [
        "struct Dpmpp2mSdeHeunGpuTrace",
        "struct Dpmpp2mSdeHeunGpuProgress",
        "struct Dpmpp2mSdeHeunGpuObservation",
        "struct Dpmpp2mSdeHeunGpuNoiseRequest",
...
#[test]
fn val_sampler_001_dpmpp_2m_sde_heun_gpu_matches_all_intermediates_callbacks_and_terminal()
-> Result<(), Box<dyn Error>> {
    let fixture = fixture()?;
    let profile = profile()?;
    ...
    let (trace, checkpoints) = sample_dpmpp_2m_sde_heun_gpu(
        &backend,
...
-> Result<(), Box<dyn Error>> {
    ...
    let (adapter, adapter_checkpoints) = sample_dpmpp_2m_sde_heun_gpu(
        &backend,
        plan(&fixture.identity, fixture.seed, fixture.steps.len())?,
        &profile,
        adapter_initial,
    ...
    let (family, family_checkpoints) = sample_dpmpp_2m_sde_gpu(
        &backend,
        plan(
            DPMPP_2M_SDE_GPU_SAMPLER_ID,
            fixture.seed,
...
-> Result<(), Box<dyn Error>> {
    ...
    let cancellation = CancellationToken::default();
    let context = execution_context(&backend, &authority, &cancellation)?;
    let (short, checkpoints) = sample_dpmpp_2m_sde_heun_gpu(
        &backend,
    ...
    let error = sample_dpmpp_2m_sde_heun_gpu(
        &backend,
        plan(
            DPMPP_2M_SDE_GPU_SAMPLER_ID,
            fixture.seed,
    ...
    assert!(matches!(error, Dpmpp2mSdeHeunGpuError::WrongSampler(_)));
    ...
    assert!(matches!(error, Dpmpp2mSdeHeunGpuError::EquationFamily(_)));
    ...
    assert!(matches!(error, Dpmpp2mSdeHeunGpuError::Tensor(_)));
    Ok(())
}
