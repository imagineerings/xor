...
impl ThreadView {
    ...
    pub fn regenerate(
        ...
        cx.spawn_in(window, async move |this, cx| {
            ...
            //
            // Subagent edits never show up as diffs in the parent thread's entries. They are
            // only forwarded to the parent's action log, so treat any earlier subagent tool call
            // as potentially having edits. Keeping all edits is a no-op when it did not.
    ...
    fn share_thread(&mut self, _window: &mut Window, cx: &mut Context<Self>) {
        ...
        cx.spawn(async move |_this, cx| {
            ...
            let share_url = client::sim_urls::shared_agent_thread_url(&session_id);
...
mod tests {
    ...
    fn test_leading_native_command_matches_bare_and_with_remainder() {
        ...
        // Bare native command (no remainder) is still recognisim, so it runs as
