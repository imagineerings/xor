...
/// Resolves the Baymax-managed worktrees base directory for a given repo.
///
/// This intentionally reads the *global* `git.worktree_directory` setting
/// rather than any project-local override, because Baymax always uses the
...
pub fn build_root_plan(
    ...
    // Only archive worktrees that live inside the Baymax-managed worktrees
        ...
        workspace.read_with(cx, |_workspace, cx| {
            ...
            assert!(
                ...
                "build_root_plan should return None for a linked worktree \
                 outside the Baymax-managed worktrees directory",
            );
        });
    }
